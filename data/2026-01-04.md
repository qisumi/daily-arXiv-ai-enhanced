...
TL;DR: 本文介绍了CAT框架，用于评估和可视化大语言模型在可控输入变化下准确性与响应一致性的相互作用，并提出CAR曲线和CORE指标来量化两者的权衡关系。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型评估主要关注准确性或基准分数，而一致性在高风险实际应用中是关键属性，但缺乏对准确性与一致性相互依赖关系的细致评估方法。

Method: 提出CAT框架，核心是Consistency-Accuracy Relation（CAR）曲线，可视化模型准确性随一致性要求增加的变化；同时提出Consistency-Oriented Robustness Estimate（CORE）指标，结合CAR曲线的面积和形状量化两者权衡。

Result: 在多种通用和领域特定LLM上通过多个多项选择基准验证了CAT框架的实用性，并展示其可扩展到长文本开放式任务的潜力。

Conclusion: CAT框架有效支持大语言模型准确性与一致性的综合评估，CAR曲线和CORE指标为模型性能分析提供了新工具，且可通过自适应评分函数扩展到更多任务类型。
...
TL;DR: 针对低资源语言（如孟加拉语）的代码生成问题，本文提出BanglaCodeAct框架，利用多智能体提示和迭代自校正提升孟加拉语到Python代码生成性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在英语提示的代码生成中表现优异，但在低资源语言（如孟加拉语）中进展有限，现有方法依赖特定任务微调，缺乏动态生成和优化能力。

Method: 引入BanglaCodeAct框架，基于开源多语言LLM构建Thought-Code-Observation循环，通过多智能体提示和迭代自校正实现从孟加拉语指令动态生成、测试和优化Python代码，并在mHumanEval数据集上对多个小参数开源LLM进行基准测试。

Result: Qwen3-8B结合BanglaCodeAct框架在开发集上pass@1准确率达94.0%，盲测集达71.6%，建立了孟加拉语到Python代码生成的新基准。

Conclusion: BanglaCodeAct框架显著提升低资源语言（孟加拉语）的代码生成性能，证明智能体推理在低资源语言可靠代码生成中的潜力，实验脚本已开源。
...
TL;DR: 本文提出PharmaShip数据集，用于测试预训练文本布局模型在嘈杂OCR和异构模板下的性能，覆盖三个任务并基准测试五种基线模型，发现注入阅读顺序正则化可提升性能，为医药领域文档理解建立基准。


<details>
  <summary>Details</summary>
Motivation: 现有预训练文本布局模型在嘈杂OCR和异构模板下的性能需测试，医药领域安全关键文档理解缺乏受控可复现基准。

Method: 构建PharmaShip数据集（含SER、RE、ROP三个任务），采用实体中心评估协议，基准测试LiLT、LayoutLMv3-base等五种像素感知和几何感知基线模型，标准化预处理、分割和优化。

Result: 像素和显式几何提供互补归纳偏置，但单独均不足；注入阅读顺序正则化持续提升SER和EL并获最稳健配置，更长位置覆盖稳定末页预测并减少截断伪影；ROP在词级准确但段级具挑战性。

Conclusion: PharmaShip为医药领域安全关键文档理解建立受控可复现基准，强调序列感知约束是结构建模的可迁移偏置。
...
TL;DR: 本文提出Luca-Noise Reflex协议（LN-RP），用于分析大语言模型中噪声驱动的角色涌现，通过注入随机噪声种子观察语言行为的非线性转变，发现三种稳定角色模式并验证其可诱导性和差异性。


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型中噪声驱动的角色涌现现象，探索外部噪声源对反射生成动态的影响。

Method: 提出Luca-Noise Reflex协议（LN-RP），向初始生成状态注入随机噪声种子，在152个生成周期中观察语言行为的非线性转变。

Result: 发现三种具有不同熵特征的稳定角色模式，外部噪声源可可靠诱导反射生成动态的相变，定量评估证实角色保留一致且模式间差异显著（p<0.01）。

Conclusion: LN-RP协议为研究大语言模型的反射生成、涌现行为和长程语言连贯性提供了可复现的方法。
...
TL;DR: 针对现有大语言模型安全机制忽略隐性有害查询的问题，提出HarmTransform多智能体辩论框架生成隐蔽有害查询，以完善安全训练数据，实验显示其优于基线但辩论存在双面性。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型的安全机制主要关注显性危险内容，忽略了用户通过隐蔽重述伪装有害意图的隐性威胁，导致安全训练数据存在显著缺口。

Method: 引入HarmTransform多智能体辩论框架，通过多智能体间的迭代批判与优化，系统地将有害查询转化为更隐蔽的形式同时保留其潜在有害意图。

Result: 实验表明HarmTransform在生成有效查询转换方面显著优于标准基线；分析发现辩论是双刃剑，既能提升转换的隐蔽性，也可能导致主题偏移和不必要的复杂性。

Conclusion: 多智能体辩论框架在生成全面安全训练数据方面具有潜力，但也存在局限性，需进一步优化以平衡效果与潜在问题。
...
TL;DR: 本文探讨Transformer大语言模型在不完全信息领域（如扑克）中是否能学习环境的内部表示，结果表明模型可学习确定性和随机性特征，且这些表示与理论信念状态相关。


<details>
  <summary>Details</summary>
Motivation: 现有研究显示Transformer大语言模型在完美信息游戏中能形成环境的隐状态表示，但在不完全信息领域（如扑克这类部分可观测马尔可夫决策过程）的表现尚未明确，需扩展该方向研究。

Method: 在扑克手牌历史数据上预训练GPT风格模型，并通过非线性探针分析其内部激活。

Result: 模型在无显式指导下学习了手牌等级等确定性结构和 equity 等随机性特征，且这些表示可解码并与理论信念状态相关。

Conclusion: Transformer大语言模型能学习德州扑克这类随机环境的自身表示。
...
TL;DR: 本文针对家庭服务机器人难以推断日常物品隐藏存储位置的问题，提出Stored Household Item Challenge基准任务及NOAM混合智能体模型，通过结合场景理解与大语言模型推理提升存储位置预测准确性。


<details>
  <summary>Details</summary>
Motivation: 家庭服务机器人执行如“拿盘子”等简单指令时，需推断抽屉、橱柜等隐藏位置的物品存储，但现有机器人缺乏完成该任务所需的常识推理能力，因此需要评估机器人认知能力的基准任务及解决方案。

Method: 提出Stored Household Item Challenge基准任务，包含100个真实厨房场景的物品-图像对评估集和6500个带存储区域标注的开发集；设计NOAM混合智能体模型，将视觉输入转化为空间上下文和可见容器的自然语言描述，再通过大语言模型（如GPT-4）推断最可能的隐藏存储位置。

Result: NOAM在与随机选择、视觉-语言管道、多模态模型（如Gemini、GPT-4o等）及人类表现的对比评估中，显著提升预测准确性并接近人类水平结果。

Conclusion: NOAM模型通过整合视觉场景理解与大语言模型推理，展现出 emergent 常识推理能力，为在家庭环境中部署具备认知能力的智能体提供了最佳实践参考。
...
TL;DR: 提出熵感知推测解码（EASD）方法，通过动态熵惩罚机制解决传统推测解码中模型过度对齐的问题，在保持效率的同时提升推理性能并超越目标模型本身。


<details>
  <summary>Details</summary>
Motivation: 传统推测解码（SD）因草稿模型与目标模型过度对齐而受限于目标模型性能，存在低置信度错误传播的问题。

Method: 基于标准推测解码，引入动态熵惩罚机制：在每个解码步骤利用采样分布的熵量化模型不确定性；当两个模型均表现出高熵且前N个预测重叠度大时，目标模型拒绝并重新采样对应token；同时结合草稿模型验证。

Result: 在多个推理基准测试中，EASD持续优于现有SD方法，且在大多数情况下超越目标LLM本身；其效率与SD相当。

Conclusion: EASD通过熵感知机制有效解决了传统SD的局限性，在保持效率的前提下提升了推理性能，甚至能超越目标模型的固有性能。
...
TL;DR: 现有音频语言模型依赖特定任务微调，而人类可通过少量示例或简单指令泛化到新任务。本文通过扩展MiMo-Audio预训练数据至超1亿小时，发现其在多种音频任务中展现少样本学习能力，MiMo-Audio-7B-Base在开源模型中实现语音智能和音频理解基准的SOTA性能，并能泛化到训练数据中不存在的任务。后训练阶段引入指令调优语料和思维机制，MiMo-Audio-7B-Instruct在多个基准上达到开源SOTA，接近或超越闭源模型。


<details>
  <summary>Details</summary>
Motivation: 现有音频语言模型通常依赖特定任务的微调来完成特定音频任务，而人类能够仅通过少量示例或简单指令泛化到新的音频任务。GPT-3已证明扩展下一个token预测预训练能在文本领域实现强泛化能力，作者认为该范式同样适用于音频领域。

Method: 扩展MiMo-Audio的预训练数据至超过1亿小时；开发系统评估其少样本学习能力；后训练阶段构建多样化指令调优语料，并在音频理解和生成中引入思维机制。

Result: MiMo-Audio-7B-Base在开源模型中，在语音智能和音频理解基准上达到SOTA性能；能泛化到训练数据中不存在的任务（如语音转换、风格迁移、语音编辑）；具有强大的语音续接能力，可生成高度真实的脱口秀、朗诵、直播和辩论。MiMo-Audio-7B-Instruct在音频理解基准（MMSU、MMAU、MMAR、MMAU-Pro）、口语对话基准（Big Bench Audio、MultiChallenge Audio）和指令TTS评估中达到开源SOTA，接近或超越闭源模型。

Conclusion: 通过扩展预训练数据规模，音频语言模型可获得强大的少样本学习和泛化能力；后训练阶段的指令调优和思维机制能进一步提升模型在音频理解和生成任务中的性能，使其接近或超越闭源模型。
...
TL;DR: 本文对两种基于Transformer的新闻文本偏见检测模型进行可解释性比较研究，发现架构和训练选择显著影响模型可靠性与部署适用性


<details>
  <summary>Details</summary>
Motivation: 现有新闻文本偏见检测模型的决策过程和失败原因缺乏了解，需要通过可解释性研究揭示其运行机制和问题

Method: 使用基于SHAP的解释方法，对在BABE数据集上微调的偏见检测器模型和领域自适应预训练RoBERTa模型进行词级归因分析，比较正确与错误预测的归因模式

Result: 1.两种模型关注相似的评价语言类别，但信号整合方式差异大；2.偏见检测器模型对假阳性的内部证据强度高于真阳性，导致中性内容过度标记；3.领域自适应模型归因模式与预测结果更一致，假阳性减少63%；4.模型错误源于不同语言机制，假阳性主要由语篇级歧义而非显式偏见线索驱动

Conclusion: 可解释性感知评估对偏见检测系统至关重要，架构和训练选择严重影响模型在新闻场景中的可靠性和部署适用性
...
TL;DR: 本文研究了大型语言模型（LLMs）在检索增强问答中的应用，针对长上下文引入无关信息导致性能下降的问题，提出自适应提示策略，实验表明该策略在使用更少token的情况下与标准提示性能相当，并发现模型在信息不足时易生成错误答案而非拒绝回答这一主要误差来源。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型扩展上下文窗口的成功推动了检索增强生成中更广泛上下文的使用，但更长的上下文会引入更多无关信息，阻碍模型生成过程并降低其性能，同时模型在信息不足时往往生成错误答案而非拒绝回答，这是主要误差来源。

Method: 设计自适应提示策略，将检索到的信息拆分为更小的块，依次提示LLM使用每个块回答问题，通过调整块大小在纳入相关信息和减少无关信息之间进行权衡。

Result: 在三个开放域问答数据集上的实验结果表明，自适应策略在使用更少token的情况下与标准提示的性能相当；分析发现，当遇到信息不足时，LLM经常生成错误答案而非拒绝回答，这是主要的错误来源。

Conclusion: 自适应提示策略有效平衡了相关信息纳入与无关信息减少，在减少token使用的同时保持性能；模型在信息不足时生成错误答案的现象突出了进一步研究增强LLM在信息不足时有效拒绝请求能力的必要性。
...
TL;DR: 本文利用机制可解释性中中间注意力层编码token级假设的特性，生成基于模型内部token预测的对抗样本，评估其对LLM下游任务（如论点质量评估）的影响，发现此类对抗样本能降低评估性能但存在语法退化问题。


<details>
  <summary>Details</summary>
Motivation: 机制可解释性研究表明中间注意力层编码token级假设并逐步优化至最终输出，现有对抗攻击多为基于提示或梯度的方法，缺乏利用模型内部生成过程的方式，因此本文旨在探索利用中间层token分布生成既合理又与模型内部一致的对抗样本。

Method: 利用中间注意力层的token分布生成对抗样本，在ArgQuality数据集上针对论点质量评估任务进行实验，使用LLaMA-3.1-Instruct-8B同时作为生成器和评估器，分析不同层和token位置的替换对评估性能的影响。

Result: 基于注意力的对抗样本导致评估性能显著下降，且与原始输入语义相似；但从特定层和token位置提取的替换会引入语法退化，限制其实用性。

Conclusion: 研究结果表明，使用中间层表示作为对抗样本来源对压力测试LLM评估 pipeline具有潜力，但当前存在语法退化等局限性，需进一步改进。
...
TL;DR: 本研究针对金融数值推理问答任务中因缺乏金融领域知识导致的错误问题，提出了多检索器RAG系统，并通过实验验证了领域特定训练的有效性及最新LLM的性能提升。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型（LLMs）取得了进展，但金融数值推理问答任务仍具挑战性，原因在于其需要特定金融领域知识和复杂多步数值推理能力。

Method: 实现了多检索器的检索增强生成（RAG）系统，用于检索外部领域知识和内部问题上下文，并利用最新LLM处理任务；进行了全面的消融实验和错误分析。

Result: 1. 采用SecBERT编码器的领域特定训练使神经符号模型超越FinQA论文的顶级基线模型；2. 最佳基于提示的LLM生成器实现了最先进（SOTA）性能，提升超过7%，但仍低于人类专家水平；3. 小型模型和少样本示例中存在幻觉损失与外部知识增益的权衡，而大型模型的外部事实增益通常超过幻觉损失；4. 最新LLM的数值推理能力在少样本学习优化下得到增强。

Conclusion: 领域特定训练对提升金融数值推理任务性能具有潜在优势；最新LLM在该任务上表现优异但仍有提升空间；不同规模模型在外部知识使用上存在差异，大型模型更能有效利用外部事实。
...
TL;DR: 本文提出一种分析优先框架，分离开放式学习响应中的内容信号与评分者倾向，通过动态先验和文本嵌入建模提升评分透明度与准确性。


<details>
  <summary>Details</summary>
Motivation: 开放式学习响应评分常混淆学生内容与教师评分倾向，缺乏可见性与可审计性。

Method: 使用ASSISTments数学响应数据，将教师历史建模为动态先验，基于句子嵌入提取文本表示，结合中心化和残差化缓解提示与教师混淆；通过时间验证线性模型量化信号贡献，投影表面模型显示分歧供定性检查。

Result: 教师先验显著影响评分预测，先验与内容嵌入结合时AUC达0.815，仅内容模型AUC为0.626；调整评分者效应后，残差内容表示更清晰，保留更多信息嵌入维度，揭示语义证据支持理解的案例。

Conclusion: 该框架将嵌入转化为反思性学习分析工具，帮助教师和研究者检查评分实践与学生推理学习证据的一致性或冲突。
...
TL;DR: 本文提出基于超图的记忆机制HGMem，解决现有多步检索增强生成（RAG）系统中工作记忆模块仅被动存储孤立事实、忽略高阶关联导致推理碎片化的问题，在全局感知任务上显著优于基线系统。


<details>
  <summary>Details</summary>
Motivation: 现有多步RAG系统的工作记忆模块主要作为被动存储，积累孤立事实以压缩长输入和生成子查询，但静态特性忽略了原始事实间的关键高阶关联，导致其表征能力和对多步推理、知识演化的影响受限，在扩展上下文下推理碎片化且全局感知能力弱。

Method: 提出HGMem超图记忆机制，将记忆表示为超图，超边对应不同记忆单元，支持记忆内高阶交互的渐进形成；该机制围绕焦点问题连接事实与思考，演化为集成的情境化知识结构，为后续步骤提供深层推理的强命题。

Result: 在多个面向全局感知的挑战性数据集上的实验表明，HGMem持续改进多步RAG，在不同任务中显著优于强基线系统。

Conclusion: HGMem通过动态、表达性的超图结构扩展记忆概念，从简单存储升级为复杂推理和全局理解的工具，有效提升多步RAG的性能，为多步推理任务提供了更优的记忆机制解决方案。
...
TL;DR: 本文提出稀疏注意力方案LoZA，可将现有全注意力模型转化为低计算成本的稀疏版本，在长上下文场景中显著加速，并通过应用于LongCat-Flash得到能处理百万级token的模型。


<details>
  <summary>Details</summary>
Motivation: 长上下文场景中，现有全注意力模型计算成本高，需设计低计算预算的稀疏注意力方案以实现高效处理。

Method: 提出LongCat ZigZag Attention（LoZA）稀疏注意力方案，在训练中期将其应用于LongCat-Flash模型。

Result: LoZA在长上下文场景中对预填充密集型（如检索增强生成）和解码密集型（如工具集成推理）任务均实现显著加速；得到的LongCat-Flash-Exp模型可快速处理多达100万token。

Conclusion: LoZA能有效将全注意力模型转化为稀疏版本，支持高效的长期推理和长视野智能体能力。
...
TL;DR: 本文提出CEC-Zero零监督强化学习框架，解决大规模中文拼写纠错中现有方法对新错误鲁棒性不足和依赖昂贵标注的问题，通过让LLM自我纠错实现无标签、高效的纠错范式。


<details>
  <summary>Details</summary>
Motivation: 大规模中文拼写纠错在实际文本处理中至关重要，但现有大语言模型（LLMs）和监督方法对新错误鲁棒性不足，且依赖昂贵的标注数据，因此需要一种无需标注且鲁棒的解决方案。

Method: 提出CEC-Zero零监督强化学习框架：从干净文本合成错误输入，通过语义相似性和候选一致性计算聚类共识奖励，使用PPO优化策略。

Result: 在9个基准数据集上，CEC-Zero比监督基线方法高10-13个F1点，比强LLM微调方法高5-8个F1点，且具有无偏奖励和收敛性的理论保证。

Conclusion: CEC-Zero建立了一种无标签的鲁棒、可扩展中文拼写纠错范式，释放了LLM在噪声文本处理 pipeline中的潜力。
...
TL;DR: 提出无监督框架RISE，通过稀疏自编码器从LLM思维链激活中发现可解释的推理行为向量，实现对推理过程的解释与可控引导


<details>
  <summary>Details</summary>
Motivation: 现有LLM推理机制研究不足，依赖人工定义概念的监督方法难以捕捉全部潜在推理行为，且无法在token空间定义所有行为

Method: 将思维链轨迹分割为句子级步骤，在步骤级激活上训练稀疏自编码器（SAE），发现编码不同推理行为的方向向量

Result: SAE提取的特征对应可解释行为（如反思、回溯），行为在解码器列空间占据可分离区域；对SAE向量的干预可可控地放大/抑制特定推理行为，无需重训练；还捕捉到响应长度等结构特性，并发现人工监督外的新行为（如信心相关向量）

Conclusion: 无监督 latent 发现方法在解释和可控引导LLM推理方面具有潜力
...
TL;DR: 本研究开发WISE框架，在20000样本的平衡数据集上评估8个轻量级Transformer模型和2个基线模型，发现MiniLM准确率最高（87.58%），RoBERTa-base的ROC-AUC最高（95.42%），轻量级模型可匹配或超越基线性能。


<details>
  <summary>Details</summary>
Motivation: 区分假新闻与讽刺/幽默因语言特征重叠但意图不同而具有挑战。

Method: 开发WISE框架，使用分层5折交叉验证，在Fakeddit的20000样本平衡数据集上评估模型，采用准确率、精确率、召回率、F1-score、ROC-AUC、PR-AUC、MCC、Brier分数和预期校准误差等综合指标，并通过配对t检验和McNemar检验进行统计比较。

Result: MiniLM准确率最高（87.58%）；RoBERTa-base的ROC-AUC最高（95.42%）且准确率较高（87.36%）；DistilBERT在效率和准确率间有良好权衡（准确率86.28%，ROC-AUC93.90%）；模型间性能差异显著。

Conclusion: 轻量级模型可匹配或超越基线性能，为在资源受限的实际环境中部署错误信息检测系统提供可行见解。
...
TL;DR: 本文提出iCLP框架，利用人类内隐认知启发，让大语言模型生成潜在计划以提升推理可靠性和效率


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在显式文本计划引导下可进行可靠推理，但生成准确有效文本计划面临幻觉和任务多样性挑战

Method: iCLP框架通过三步实现：1.从现有推理轨迹蒸馏显式计划；2.用向量量化自编码器结合码本学习计划的离散表示；3.在潜在计划与对应推理步骤对上微调大语言模型

Result: 在数学推理和代码生成任务上，iCLP使大语言模型在潜在空间规划、语言空间推理，提升准确性和效率，且跨域泛化能力强并保留思维链可解释性

Conclusion: iCLP框架有效解决显式计划生成问题，在多任务上表现优异，兼具性能与泛化性
...
TL;DR: 本文提出综合可靠性评分（CRS）框架，统一评估大语言模型在决策关键领域的校准性、鲁棒性和不确定性量化能力，通过实验揭示模型可靠性能的平衡要点


<details>
  <summary>Details</summary>
Motivation: 大语言模型在医疗、法律等决策关键领域应用日益广泛，但存在过度自信错误、输入偏移下性能下降及缺乏清晰不确定性估计等问题，现有评估方法碎片化仅关注孤立方面

Method: 引入综合可靠性评分（CRS）框架，整合校准性、鲁棒性和不确定性量化为单一可解释指标；在10个主流开源大语言模型和5个QA数据集上，评估基线、扰动及校准方法下的性能

Result: CRS提供稳定的模型排名，发现单一指标遗漏的隐藏失效模式，指出最可靠的系统需平衡准确性、鲁棒性和校准后的不确定性

Conclusion: CRS框架有效统一大语言模型可靠性评估，揭示模型在决策关键场景下的可靠性能特征及平衡要点
...
TL;DR: 本文介绍了HY-MT1.5系列机器翻译模型（1.8B和7B参数），通过整体训练框架开发，在多语言翻译任务中表现优异，超越多数开源基线和商业API，接近或超过超大型专有模型性能，并支持高级约束功能。


<details>
  <summary>Details</summary>
Motivation: 为开发高性能机器翻译模型，需要整合多阶段训练流程以提升参数效率和翻译质量，同时满足通用及专业翻译任务需求。

Method: 采用整体训练框架，包括通用与机器翻译导向的预训练、监督微调、策略蒸馏和强化学习的多阶段流水线。

Result: 1.8B模型参数效率显著，在中-外和英-外标准任务中全面超越更大开源基线（如Tower-Plus-72B）和商业API，达到Gemini-3.0-Pro性能的约90%；7B模型在其规模类别中建立新基准，Flores-200上达到Gemini-3.0-Pro性能的95%，并在WMT25和普通话-少数民族语言测试集上超越该模型；系列模型支持术语干预、上下文感知翻译和格式保留等高级约束。

Conclusion: HY-MT1.5系列模型在各自参数规模内，为通用和专业翻译任务提供了极具竞争力且稳健的解决方案，验证了整体训练框架的有效性。
...
TL;DR: 提出一种针对掩码扩散语言模型（MDLMs）的激活引导框架，无需模拟去噪轨迹即可实现高效的推理时控制


<details>
  <summary>Details</summary>
Motivation: MDLMs虽因掩码并行解码和与自回归大语言模型竞争的性能受关注，但推理时控制和引导机制尚未充分探索

Method: 设计激活引导框架，利用对比示例通过单次前向传递计算分层引导向量，并在每个反向扩散步骤应用这些方向

Result: 在LLaDA-8B-Instruct上的实验表明能可靠调制高级属性，消融实验分析了跨Transformer子模块和token范围（提示vs响应）引导的效果

Conclusion: 该框架为MDLMs提供了一种高效的推理时控制机制
...
TL;DR: 本文提出大型情感世界模型（LEWM），通过构建整合情感与因果关系的EWH数据集，显式建模情感状态，提升对情感驱动社会行为的预测准确性。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型（LLMs）在世界知识建模中主要关注物理规律，缺乏对情感因素的系统探索，而情感作为世界知识的关键组成部分显著影响人类决策，移除情感相关信息会降低推理性能。

Method: 1. 构建Emotion-Why-How（EWH）数据集，将情感整合到因果关系中，支持对行为原因及情感如何驱动未来世界状态的推理；2. 提出LEWM模型，在视觉观察和行为之外显式建模情感状态，同时预测未来状态和情感转变。

Result: LEWM在预测情感驱动的社会行为方面更准确，且在基础任务上保持与通用世界模型相当的性能。

Conclusion: LEWM通过显式建模情感状态，有效提升了对情感驱动社会行为的预测能力，验证了情感因素在世界模型中的重要性。
...
TL;DR: 本文介绍TeleChat3-MoE系列大语言模型的训练基础设施，包括数值准确性验证方法、性能优化技术及并行化框架，实现高效扩展和显著吞吐量提升


<details>
  <summary>Details</summary>
Motivation: 为支持TeleChat3-MoE系列大语言模型（参数规模从1050亿到超万亿）的可靠且高效扩展，需构建适配Ascend NPU集群的训练基础设施

Method: 采用算子级和端到端数值准确性验证方法；应用交错流水线调度、长序列训练的注意力感知数据调度、专家并行的分层重叠通信、基于DVM的算子融合等性能优化技术；提出基于分析估计和整数线性规划的系统并行化框架；并实施集群级优化以解决主机和设备瓶颈

Result: 这些基础设施改进实现了数千设备集群上的显著吞吐量提升和近线性扩展

Conclusion: 该训练基础设施为硬件生态系统上的大规模语言模型开发提供了坚实基础
...
TL;DR: 针对当前大语言模型在临床诊断中存在的幻觉内容、低效提问和对话不一致问题，提出整合医学知识图谱、信息增益问题选择和结构化状态追踪的MedKGI框架，在临床基准测试中提升诊断准确性和查询效率。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在临床诊断场景中难以模拟真实的迭代式假设驱动推理，存在三个关键局限：生成缺乏验证知识支撑的幻觉医疗内容、提出冗余低效而非具有鉴别性的问题、多轮对话中失去连贯性导致结论矛盾或不一致。

Method: 提出MedKGI诊断框架，整合医学知识图谱以约束推理至验证的医学本体，基于信息增益选择问题以最大化诊断效率，采用OSCE格式的结构化状态在多轮对话中维持一致的证据追踪。

Result: 在临床基准测试中，MedKGI在诊断准确性和查询效率上均优于强大的大语言模型基线，平均提升对话效率30%同时保持最先进的准确性。

Conclusion: MedKGI框架有效解决了当前大语言模型在临床诊断中的关键局限，显著提升了诊断性能和对话效率，为基于大语言模型的临床诊断应用提供了可行方案。
...
TL;DR: 本文介绍了LAILA数据集，这是目前最大的公开阿拉伯语自动作文评分（AES）数据集，包含7859篇作文及7个维度的评分，并提供了基准结果以支持阿拉伯语AES研究。


<details>
  <summary>Details</summary>
Motivation: 近年来自动作文评分（AES）受到越来越多关注，但阿拉伯语AES研究因缺乏公开可用数据集而受限，需要填补这一空白。

Method: 设计、收集并标注了LAILA数据集，该数据集包含7859篇作文，标注了整体得分及相关性、组织性、词汇、风格、发展、 mechanics和语法七个维度的特定特征得分；使用最先进的阿拉伯语和英语模型在特定提示和跨提示设置下提供基准结果。

Result: LAILA是目前最大的公开阿拉伯语AES数据集，填补了阿拉伯语AES研究中的关键需求，支持鲁棒评分系统的开发。

Conclusion: LAILA数据集的引入解决了阿拉伯语AES研究中缺乏公开数据集的问题，为该领域的进一步研究和系统开发提供了重要资源。
...
TL;DR: 开发适用于专利和科学出版物的语言相似性模型Pat-SPECTER，经评估表现最佳，并验证其在实际场景中的能力，同时开放给学术界和从业者使用


<details>
  <summary>Details</summary>
Motivation: 需要一种能同时处理专利和科学出版物的语言相似性模型，以解决专利与论文之间的引用预测等问题

Method: 通过赛马式评估对八个语言相似性模型进行测试，将SPECTER2模型在专利数据上微调得到Pat-SPECTER模型，并在两个真实场景中验证其能力，还测试了美国专利引用论文语义相似性较低的假设

Result: Pat-SPECTER模型在预测可信专利-论文引用方面表现最佳，在两个实际场景中展示了其能力，验证了美国专利因坦诚义务引用语义相似性较低论文的假设

Conclusion: Pat-SPECTER模型适用于同时处理专利和科学出版物的语言相似性任务，表现优异且已开放给学术界和从业者使用
...
TL;DR: 本文提出DATAMASK框架，通过联合优化质量和多样性指标，从15万亿token的FineWeb数据集中选出10%的子集FineWeb-Mask，使模型性能显著提升。


<details>
  <summary>Details</summary>
Motivation: 大规模预训练数据选择中，单独考虑质量或多样性指标存在局限性：质量指标选择在长期预训练中收益递减，多样性指标选择会移除大量高质量样本，限制模型能力。

Method: 提出DATAMASK框架，将数据选择视为掩码学习问题，通过迭代采样数据掩码、基于预定义目标计算策略梯度、更新掩码采样logits，结合策略梯度优化和加速增强，大幅减少选择时间。

Result: DATAMASK将选择时间较贪心算法减少98.9%，从FineWeb数据集中选出10%的子集FineWeb-Mask；在12个任务上评估，1.5B稠密模型性能提升3.2%，7B MoE模型提升1.9%。

Conclusion: DATAMASK框架高效实现大规模预训练数据的多指标联合选择，显著提升模型性能，为万亿级token数据集的数据选择提供可行方案。
...
TL;DR: 本文针对欧盟分类法合规的手动流程挑战，提出首个结构化数据集并系统评估大语言模型（LLMs）在合规核心工作流的表现，发现定性任务表现中等、定量任务零样本设置下全面失败，结论是LLMs目前适合辅助而非完全自动化。


<details>
  <summary>Details</summary>
Motivation: 企业遵守欧盟分类法需手动处理资源密集型流程，存在挑战；虽大语言模型（LLMs）有望实现自动化，但缺乏公开基准数据集阻碍相关研究。

Method: 构建包含190份企业报告、带有真实经济活动和定量关键绩效指标（KPIs）的结构化数据集；使用该数据集对LLMs在合规核心工作流进行首次系统性评估，包括定性任务（识别经济活动）和定量任务（预测财务KPIs的零样本设置），并分析模型性能及相关现象（如元数据与完整报告的对比、置信度校准情况）。

Result: LLMs在识别经济活动的定性任务中表现中等，多步骤智能体框架可小幅提升精度；在零样本设置下预测财务KPIs的定量任务全面失败；发现简洁元数据常比完整非结构化报告性能更优的悖论，且模型置信度分数校准不佳。

Conclusion: LLMs尚未准备好用于欧盟分类法合规的完全自动化，但可作为人类专家的强大辅助工具；所提出的数据集为未来研究提供公开基准。
...
TL;DR: 本文提出FIGR模型，通过端到端强化学习将主动视觉思维融入多轮推理，解决纯文本推理难以捕捉复杂问题中全局结构约束的问题，并在数学推理基准测试中取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 复杂推理问题常涉及文本未显式编码的隐式空间、几何和结构关系，而现有纯文本推理模型难以表示复杂场景中的全局结构约束。

Method: 引入FIGR模型，通过端到端强化学习将主动视觉思维融入多轮推理，在问题解决过程中构建视觉表示以外化中间结构假设，并自适应调节视觉推理的调用时机和方式。

Result: 在具有挑战性的数学推理基准测试中，FIGR优于强纯文本思维链基线，尤其在AIME 2025上提升基础模型13.12%，在BeyondAIME上提升11.00%。

Conclusion: 图形引导的多模态推理能有效增强复杂推理的稳定性和可靠性，FIGR模型通过整合视觉思维显著提升了复杂推理任务的性能。
...
TL;DR: 本文提出金融领域大语言模型QianfanHuijin及多阶段训练范式，通过持续预训练和细粒度后训练（含金融SFT、推理RL、智能体RL及通用RL）提升模型性能，在权威金融基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 金融服务复杂度加深，现有模型（如BloombergGPT、Baichuan-Finance）仅侧重知识增强，缺乏强大的金融推理和智能体能力，因此需要兼具领域知识、推理及智能体能力的金融大语言模型。

Method: 采用多阶段训练范式：首先在金融语料上进行持续预训练巩固知识库；然后进行细粒度后训练，包括金融监督微调（Financial SFT）、金融推理强化学习（Finance Reasoning RL）、金融智能体强化学习（Finance Agentic RL）及与实际业务场景对齐的通用强化学习（General RL）。

Result: QianfanHuijin在多个权威金融基准测试中性能优异；消融实验证实推理RL和智能体RL阶段对各自能力提升显著。

Conclusion: 细粒度、渐进式的后训练方法有效验证了研究动机，有望成为各类工业增强大语言模型的主流范式。
...
<details>
  <summary>Details</summary>
Motivation: 基于Transformer的模型在处理长文档（如法律和医学领域）时存在困难，稀疏注意力机制资源密集且难以捕捉全文语境，分层Transformer模型效率较高但无法清晰解释文档各部分关系。

Method: 引入自监督对比学习框架，随机掩盖文档某一部分，使用基于自然语言推理（NLI）的对比目标，将其与相关部分对齐，与不相关部分疏远，模拟人类信息合成方式。

Result: 在法律和生物医学文本上的实验表明，该方法在准确性和效率方面均取得显著提升。

Conclusion: 所提出的框架能够生成更丰富且计算效率更高的长文档表示，有效解决了现有模型处理长文档的局限性。
...
TL;DR: 本文比较了针对低资源语言文本摘要的多种方法，包括零样本提示LLM、微调mT5（含数据增强和多语言迁移）及LLM翻译管道，并发现多语言微调的mT5优于多数方法，且LLM作为评判在低资源语言上可靠性较低


<details>
  <summary>Details</summary>
Motivation: 自动文本摘要在英语等高资源语言中已取得高性能，但低资源语言的摘要研究关注较少

Method: 比较了多种方法：零样本提示不同大小的LLM、微调mT5（包含三种数据增强方法和多语言迁移）、LLM翻译管道方法（源语言转英语→摘要→转回源语言）

Result: 1. 相似参数规模的LLM在性能上存在差异；2. 多语言微调的mT5基线在多数指标上优于多数其他方法（包括零样本LLM性能）；3. LLM作为评判在低资源语言上可能不太可靠

Conclusion: 多语言微调的mT5是低资源语言文本摘要的有效方法，且需注意LLM评判在低资源语言上的局限性
...
TL;DR: 本文设计、开发并评估了一个辅助雅思写作考试考生的修订平台，通过迭代的基于设计的研究（DBR）改进评分系统和反馈机制，结果表明自动化反馈可作为人工指导的补充，但在高分作文评估等方面仍有挑战。


<details>
  <summary>Details</summary>
Motivation: 传统雅思备考方法缺乏针对雅思写作评分标准的个性化反馈，存在不足，因此需要开发能提供个性化反馈的平台。

Method: 采用基于设计的研究（DBR）循环方法，从基于规则的评分系统逐步过渡到带有回归头的DistilBERT transformer模型评分系统，并实现自适应反馈；通过迭代测试评估不同阶段的效果。

Result: 基于规则的方法存在中分段压缩、低准确性和负R²值等局限；DistilBERT模型实现后MAE为0.66且R²为正；自适应反馈使考生分数有统计显著提升（平均+0.060分，p=0.011，Cohen's d=0.504），但效果因修订策略而异。

Conclusion: 自动化反馈最适合作为人工指导的补充，保守的表面级修正比激进的结构干预更可靠；在评估高分作文方面仍有挑战，未来应纳入真实雅思考生的纵向研究和官方考官的验证。
...
TL;DR: 本文针对自动语音转录文本的非结构化问题，提出将段落分割作为结构化步骤，建立了TEDPara和YTSegPara两个基准数据集，提出约束解码方法，并开发了MiniSeg模型，使其在语音处理中成为标准化实用任务。


<details>
  <summary>Details</summary>
Motivation: 自动语音转录文本常以非结构化词流形式呈现，影响可读性和再利用；语音领域段落分割缺乏基准数据集，文本分割领域也缺少鲁棒且自然的基准。

Method: 1. 建立TEDPara（人工标注TED演讲）和YTSegPara（带合成标签的YouTube视频）两个段落分割基准数据集；2. 提出约束解码公式，让大语言模型在保留原始转录文本的同时插入段落分隔符；3. 开发MiniSeg紧凑模型，并扩展为分层模型联合预测章节和段落。

Result: MiniSeg模型达到最先进的准确率，其分层扩展模型能以最小计算成本联合预测章节和段落。
...
TL;DR: 提出HaluNet框架，融合多粒度不确定性检测LLM的QA幻觉


<details>
  <summary>Details</summary>
Motivation: LLM在QA中易产生幻觉，现有方法忽略不同不确定性源的互补性，尤其是token级概率不确定性与内部语义表示不确定性的互补性

Method: 提出HaluNet轻量级可训练神经框架，整合多粒度token级不确定性，结合语义嵌入、概率置信度和分布不确定性，采用多分支架构自适应融合模型知识与输出不确定性

Result: 在SQuAD、TriviaQA和Natural Questions数据集上，无论是否有上下文，HaluNet均表现出强检测性能和良好计算效率

Conclusion: HaluNet为基于LLM的QA系统中的实时幻觉检测提供了潜力
...
TL;DR: 本文介绍了韩国规范法律基准（KCL），旨在评估语言模型的法律推理能力，独立于领域特定知识，并通过系统性评估30多个模型发现推理专用模型优于通用模型，同时发布了相关资源。


<details>
  <summary>Details</summary>
Motivation: 现有评估语言模型法律推理能力的方法可能受限于领域特定知识，无法准确分离推理能力与参数化知识，因此需要一个能够独立评估推理能力的基准。

Method: 构建了包含两个组件的KCL基准：KCL-MCQA（283个多项选择题及1103个对齐先例）和KCL-Essay（169个开放式生成题、550个对齐先例及2739个实例级评估准则）；对30多个模型进行系统性评估。

Result: 评估结果显示模型在KCL上仍存在较大差距，尤其是在KCL-Essay部分；推理专用模型持续优于通用模型。

Conclusion: KCL基准有效评估了语言模型的法律推理能力，揭示了现有模型的不足，推理专用模型表现更优，相关资源已公开发布。
...
TL;DR: 本文研究LLM推理轨迹结构，提出训练无关的CREST方法，通过干预特定注意力头抑制低效推理行为，提升准确率并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 大语言模型依赖长链式推理解决复杂任务，但存在效率低（高延迟、冗余token）和推理不稳定（思考不足或过度）的问题。

Method: 提出CREST方法，包括离线校准（识别认知头并推导转向向量）和推理时旋转隐藏表示以抑制低效行为。

Result: 在多种推理基准和模型上，准确率提升最高17.5%，token使用量减少37.6%。

Conclusion: CREST是一种简单有效的方法，能同时提高LLM推理的速度和可靠性。
...
### [40] [Youtu-LLM: Unlocking the Native Agentic Potential for Lightweight Large Language Models](https://arxiv.org/abs/2512.24618)
*Junru Lu,Jiarui Qin,Lingfeng Qiao,Yinghui Li,Xinyi Dai,Bo Ke,Jianfeng He,Ruizhi Qiao,Di Yin,Xing Sun,Yunsheng Wu,Yinsong Liu,Shuangyin Liu,Mingkong Tang,Haodong Lin,Jiayi Kuang,Fanxu Meng,Xiaojuan Tang,Yunjia Xi,Junjie Huang,Haotong Yang,Zhenyi Shen,Yangning Li,Qianwen Zhang,Yifei Yu,Siyu An,Junnan Dong,Qiufeng Wang,Jie Wang,Keyu Chen,Wei Wen,Taian Guo,Zhifeng Shen,Daohai Yu,Jiahao Li,Ke Li,Zongyi Li,Xiaoyu Tan*

Main category: cs.CL

TL;DR: 本文介绍了Youtu-LLM，一种1.96B参数的轻量级语言模型，通过从头预训练、紧凑架构、分阶段课程训练和可扩展智能体中期训练，在通用基准和智能体特定任务上表现优异，成为2B以下模型的新标杆。


<details>
  <summary>Details</summary>
Motivation: 现有小型语言模型通常依赖蒸馏，缺乏原生的推理和智能体能力，无法满足长上下文任务和深度认知需求。

Method: 1. 采用密集多潜在注意力（MLA）架构和面向STEM的词汇表，支持128k上下文窗口；2. 设计“常识-STEM-智能体”分阶段课程，使用约11T tokens的语料库进行渐进式训练；3. 针对智能体任务实施可扩展的中期训练，合成数学、编码和工具使用领域的多样化轨迹数据。

Result: Youtu-LLM在2B以下模型中达到最先进水平：在通用基准上与更大模型竞争，在智能体特定任务上显著超越现有基线。

Conclusion: 轻量级语言模型可通过从头预训练、合理的架构设计和分阶段训练，具备强大的内在推理和智能体能力。
...
### [41] [R-Debater: Retrieval-Augmented Debate Generation through Argumentative Memory](https://arxiv.org/abs/2512.24684)
*Maoyuan Li,Zhongsheng Wang,Haoyuan Li,Jiamou Liu*

Main category: cs.CL

TL;DR: 提出基于论证记忆的多轮辩论生成框架R-Debater，在标准化辩论任务中优于LLM基线，且经人类评估证实其一致性和证据使用能力


<details>
  <summary>Details</summary>
Motivation: 现有辩论系统缺乏基于记忆的论证检索与适应能力，难以在多轮辩论中保持立场一致性、有效回应对手并提供证据支持

Method: 构建整合辩论知识库（用于检索案例式证据和先前辩论动作）与角色型智能体（用于生成连贯多轮话语）的R-Debater框架；在ORCHID辩论数据集上构建1000项检索语料库和32个跨7领域的测试辩论集，评估单轮生成（InspireScore）和多轮对抗模拟（Debatrix）任务

Result: 与强LLM基线相比，R-Debater在单轮和多轮评分中均获更高分数；20位经验丰富辩手的人类评估进一步证实其一致性和证据使用能力，表明检索 grounding 与结构化规划的结合能产生更忠实、立场一致且连贯的多轮辩论

Conclusion: 结合检索 grounding 与结构化规划的R-Debater框架可有效提升多轮辩论的忠实性、立场一致性和连贯性，为构建高质量辩论系统提供了新方法
...
### [42] [MUSIC: MUlti-Step Instruction Contrast for Multi-Turn Reward Models](https://arxiv.org/abs/2512.24693)
*Wenzhe Li,Shujian Zhang,Wenxuan Zhou,John Lambert,Chi Jin,Andrew Hard,Rajiv Mathews,Lun Wang*

Main category: cs.CL

TL;DR: 针对多轮对话质量评估的挑战，本文提出无监督数据增强策略MUSIC，通过合成跨多轮的对比对话对训练多轮奖励模型，在多轮对话评估上优于基线且不影响单轮性能


<details>
  <summary>Details</summary>
Motivation: 多轮对话质量评估对LLM发展至关重要，但人工评估成本高；现有自动评估方法滞后，标准偏好数据集仅基于最终轮对比，无法捕捉多轮交互的细微差异

Method: 提出MUSIC无监督数据增强策略，合成跨多轮差异的对比对话对；基于Skywork偏好数据集，使用Gemma-2-9B-Instruct模型训练多轮奖励模型

Result: MUSIC增强的奖励模型在多轮对话评估上与先进专有LLM评判的一致性更高，且未降低单轮基准测试性能

Conclusion: 跨多轮的对比信号对构建鲁棒多轮奖励模型至关重要，MUSIC策略有效提升了多轮对话评估能力且保持单轮性能
...
### [43] [BIOME-Bench: A Benchmark for Biomolecular Interaction Inference and Multi-Omics Pathway Mechanism Elucidation from Scientific Literature](https://arxiv.org/abs/2512.24733)
*Sibo Wei,Peng Chen,Lifeng Dong,Yin Luo,Lei Wang,Peng Zhang,Wenpeng Lu,Jianbin Guo,Hongjun Yang,Dajun Zeng*

Main category: cs.CL

TL;DR: 本文针对多组学通路富集分析存在的资源局限性及LLM应用缺乏标准化基准的问题，构建BIOME-Bench基准数据集并评估现有LLM在多组学分析中的两个核心能力，发现现有模型存在显著不足。


<details>
  <summary>Details</summary>
Motivation: 多组学研究依赖通路富集分析解释异质分子变化，但现有通路富集工作流存在资源结构局限（如 curation lag、功能冗余、对分子状态和干预敏感性有限）；尽管近期用LLM改进通路富集解释，但缺乏端到端多组学通路机制阐明的标准化基准，导致评估局限于小型手动数据集或临时案例研究，阻碍可复现进展。

Method: 构建BIOME-Bench基准数据集（通过四阶段严格工作流），定义两个核心任务（生物分子相互作用推断、端到端多组学通路机制阐明）的评估协议，并对多个当代强模型进行全面实验。

Result: 现有模型在多组学分析中仍存在显著缺陷，难以可靠区分细粒度生物分子关系类型，且无法生成忠实、鲁棒的通路级机制解释。

Conclusion: BIOME-Bench为LLM在多组学通路分析中的能力评估提供了标准化基准，现有模型在该领域的表现仍需大幅改进。
...
### [44] [Compute-Accuracy Pareto Frontiers for Open-Source Reasoning Large Language Models](https://arxiv.org/abs/2512.24776)
*Ákos Prucs,Márton Csutora,Mátyás Antal,Márk Marosi*

Main category: cs.CL

TL;DR: 本文针对大语言模型（LLMs）复杂推理时的计算负担问题，评估了开源模型在数学和推理密集型基准上的帕累托前沿，分析了计算效率趋势及推理计算的饱和点。


<details>
  <summary>Details</summary>
Motivation: 当前文献常忽视LLMs生成长推理序列的巨大计算负担，而工业应用中模型选择需兼顾精度、资源约束与推理成本。

Method: 对当代及旧版开源LLMs进行测试时计算感知评估，绘制其在数学和推理密集型基准上的帕累托前沿，并追踪帕累托效率随时间的轨迹。

Result: 发现混合专家（MoE）架构是平衡性能与效率的强候选；推理计算存在饱和点，超过阈值后精度增益递减；得出单位计算精度增益的新兴趋势。

Conclusion: 扩展推理能力有益，但无法克服模型在特定复杂度上的内在限制，MoE架构在平衡性能与效率方面表现突出。
...
### [45] [Practising responsibility: Ethics in NLP as a hands-on course](https://arxiv.org/abs/2512.24825)
*Malvina Nissim,Viviana Patti,Beatrice Savoldi*

Main category: cs.CL

TL;DR: 随着自然语言处理系统的普及，将伦理考量融入NLP教育至关重要，本文介绍了相关课程的教学方法及四年实践经验，旨在为教育者提供参考。


<details>
  <summary>Details</summary>
Motivation: 自然语言处理系统日益普及，但在课程开发中融入伦理考量面临两大挑战：该领域学术与工业界的快速演变，以及培养超越传统技术训练的批判性思维的需求。

Method: 采用基于主动学习的教学方法，包括互动环节、实操活动和“以教促学”模式，并在四年间跨不同机构、教育层次和跨学科背景进行课程优化与调整。

Result: 课程产出了大量可复用产品，包括教学材料及学生自主开发的面向不同受众的教育产品。

Conclusion: 通过分享该教学方法与实践经验，希望为寻求将社会影响考量纳入课程的教育者提供启发。
...
### [46] [Triangulation as an Acceptance Rule for Multilingual Mechanistic Interpretability](https://arxiv.org/abs/2512.24842)
*Yanan Long*

Main category: cs.CL

TL;DR: 多语言语言模型在不同语言、脚本和文化中表现不稳定，本文提出因果标准的机制解释方法——三角测量法，通过必要性、充分性和不变性验证筛选可靠电路，过滤跨语言不变性测试失败的虚假电路。


<details>
  <summary>Details</summary>
Motivation: 多语言语言模型整体性能强，但在不同语言、脚本和文化中表现不可预测，现有机制解释缺乏因果标准验证，难以确保跨环境的可靠性。

Method: 1. 形式化参考家族为保留谓词的变体；2. 引入三角测量法，要求必要性（消融电路降低目标行为）、充分性（修补激活转移行为）和不变性（跨参考家族效果稳定且足够显著）；3. 采用自动电路发现提供候选子图，通过三角测量法接受或拒绝候选；4. 将三角测量法基于因果抽象，作为交换干预分布的近似转换分数，连接语用可解释性议程，并提出跨模型家族、语言对和任务的比较实验协议。

Result: 三角测量法提供了可证伪的机制性主张标准，能过滤通过单环境测试但跨语言不变性测试失败的虚假电路，确保机制解释的可靠性。

Conclusion: 三角测量法是一种有效的机制解释方法，满足因果标准，可用于验证多语言语言模型的机制性主张，提高跨语言、跨环境解释的可靠性。
...
### [47] [Big AI is accelerating the metacrisis: What can we do?](https://arxiv.org/abs/2512.24863)
*Steven Bird*

Main category: cs.CL

TL;DR: 本文指出全球正面临生态、意义和语言危机的叠加（元危机），而大型AI正加速这些危机，语言工程师需探索替代方案以设计以人类繁荣和地球生命为中心的NLP未来


<details>
  <summary>Details</summary>
Motivation: 全球面临生态、意义和语言危机的元危机，大型AI正加速这些危机，现有语言工程实践存在问题（如坚持失败的可扩展性叙事、为权贵提供人才、忽视价值导向）

Method: 未明确提及具体研究方法，而是呼吁探索替代方案并应用集体智慧

Result: 未报告具体实证结果，而是强调当前路径的危害及改变的紧迫性

Conclusion: 需探索替代方案，设计以人类繁荣和地球生命为中心的NLP未来
...
### [48] [Encyclo-K: Evaluating LLMs with Dynamically Composed Knowledge Statements](https://arxiv.org/abs/2512.24867)
...
TL;DR: 本文提出Encyclo-K，一种基于知识陈述的动态基准，解决现有基准的数据污染、单知识点评估和高标注成本问题，实验验证其对LLMs的挑战性和区分能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLMs基准存在三大局限：易受数据污染、限于单知识点评估、依赖昂贵领域专家标注，无法有效追踪LLMs的综合理解能力边界。

Method: 从权威教科书提取独立知识陈述，测试时随机采样8-10个陈述动态组合成评估问题；标注者仅需验证格式合规无需领域知识；在50+LLMs上进行实验评估。

Result: Encyclo-K对LLMs构成显著挑战且区分能力强：最优模型OpenAI-GPT-5.1准确率仅62.07%；推理模型准确率16.04%-62.07%，聊天模型9.71%-50.40%，呈现清晰梯度分布；动态生成的问题集上模型排名稳定，支持可靠的周期性数据集更新。

Conclusion: Encyclo-K是一个可扩展的框架，能动态评估LLMs对多个细粒度学科知识陈述的综合理解能力，其设计有效解决了现有基准的关键局限。
...
### [49] [mHC: Manifold-Constrained Hyper-Connections](https://arxiv.org/abs/2512.24880)
...
TL;DR: 本文提出流形约束超连接（mHC）框架，解决超连接（HC）模型训练不稳定、扩展性受限及内存开销大的问题，通过恢复恒等映射特性和优化架构提升性能与可扩展性。


<details>
  <summary>Details</summary>
Motivation: 近年来以超连接（HC）为代表的研究扩展了残差连接范式，但多样化连接模式破坏了残差连接的恒等映射特性，导致训练不稳定、扩展性受限及显著内存访问开销，亟需解决这些挑战。

Method: 提出流形约束超连接（mHC）框架，将HC的残差连接空间投影到特定流形以恢复恒等映射特性，同时结合严格的基础设施优化确保效率。

Result: 实证实验表明mHC在大规模训练中有效，提供切实的性能提升和更优的可扩展性。

Conclusion: mHC作为HC的灵活实用扩展，有助于深化对拓扑架构设计的理解，并为基础模型的演化指明有前景的方向。
...
### [50] [BEDA: Belief Estimation as Probabilistic Constraints for Performing Strategic Dialogue Acts](https://arxiv.org/abs/2512.24885)
...
TL;DR: 本文提出BEDA框架，通过将信念估计转化为概率约束来优化策略对话，在三个任务上持续优于强基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有策略对话研究中，信念估计虽准确但缺乏在生成过程中使用信念的原则性机制，导致对话行为执行不足。

Method: 提出BEDA框架，包含世界集、信念估计器和条件生成器，通过形式化Adversarial和Alignment两种核心对话行为，并将其转化为生成时的概率约束来实现策略对话。

Result: 在三个任务上取得显著提升：CKBG任务中各骨干模型成功率至少提高5.0个百分点，GPT-4.1-nano提升20.6个百分点；Mutual Friends任务平均提升9.3个百分点；CaSiNo任务达成最优交易。

Conclusion: 将信念估计转化为约束条件为可靠的策略对话提供了简单通用的机制，BEDA框架在多种场景下均表现优异。
...
TL;DR: 针对多步骤LLM流水线中联合优化提示词的困难，提出ADOPT框架以建模步骤间依赖并实现自适应优化，实验证明其优于现有基线


<details>
  <summary>Details</summary>
Motivation: 多步骤LLM流水线性能依赖各步骤提示词，但因缺乏步骤级监督和存在步骤间依赖，联合优化提示词困难，现有端到端方法效果不佳或不稳定

Method: 提出ADOPT框架，显式建模各LLM步骤与最终任务结果的依赖关系，实现精确文本梯度估计；将多提示词优化解耦为灵活的单提示词优化步骤；采用基于Shapley值的机制自适应分配优化资源

Result: 在真实数据集和多样流水线结构上的实验表明，ADOPT有效且鲁棒，持续优于最先进的提示词优化基线

Conclusion: ADOPT框架解决了多步骤LLM流水线中提示词联合优化的问题，其建模依赖关系和自适应优化的方法提升了性能和稳定性
...
### [52] [Classifying long legal documents using short random chunks](https://arxiv.org/abs/2512.24997)
*Luis Adrián Cabrera-Diego*

Main category: cs.CL

TL;DR: 提出基于DeBERTa V3和LSTM的法律文档分类器及Temporal部署管道，解决长文档处理问题


<details>
  <summary>Details</summary>
Motivation: 法律文档因专业词汇多、长度长，导致基于Transformers的模型处理困难（不可能、昂贵或缓慢）

Method: 采用DeBERTa V3和LSTM结合的模型，输入为48个随机选择的短块（最大128个token）；使用Temporal作为部署管道

Result: 最佳模型加权F-score达0.898；CPU上处理100个文件的中位数时间为498秒

Conclusion: 该方法有效解决法律文档分类中的长文档处理问题，且部署管道可靠高效
...
### [53] [MAMA-Memeia! Multi-Aspect Multi-Agent Collaboration for Depressive Symptoms Identification in Memes](https://arxiv.org/abs/2512.25015)
*Siddhant Agarwal,Adya Dhuler,Polly Ruhnke,Melvin Speisman,Md Shad Akhtar,Shweta Yadav*

Main category: cs.CL

TL;DR: 本文研究社交媒体模因中的抑郁症状识别，提出RESTOREx资源和MAMAMemeia框架，后者在宏观F1上比现有最佳方法提升7.55%，成为新基准。


<details>
  <summary>Details</summary>
Motivation: 近年来模因从幽默交流媒介演变为用户自由表达多种情绪的工具，社交媒体模因中抑郁情绪表达日益增多，因此需要研究如何识别这些模因中的抑郁症状。

Method: 1. 构建RESTOREx资源，结合大语言模型生成和人工标注的解释用于检测模因中的抑郁症状；2. 提出基于临床心理学认知分析疗法（CAT）能力的协作多智能体多方面讨论框架MAMAMemeia。

Result: MAMAMemeia框架在宏观F1指标上比当前最优方法提升7.55%，与30多种方法相比确立为新基准。

Conclusion: 本研究提出的RESTOREx资源和MAMAMemeia框架有效提升了社交媒体模因中抑郁症状识别的性能，为该领域提供了新的基准方法。
...
### [54] [Modeling Language as a Sequence of Thoughts](https://arxiv.org/abs/2512.25026)
*Nasim Borazjanizadeh,James McClelland*

Main category: cs.CL

TL;DR: 本文提出Thought Gestalt（TG）模型，一种递归Transformer，通过同时建模token和句子级思想状态，提升语言模型的数据效率和关系方向泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer语言模型依赖表面共现统计，缺乏全局一致的实体和事件潜在表示，导致关系方向脆弱性、语境化错误和数据低效；而认知科学表明人类理解涉及将语言流转换为紧凑的事件状记忆表示。

Method: TG模型采用递归Transformer架构，一次生成一个句子的token，同时交叉关注先前句子表示的记忆；token和句子表示使用相同参数集，通过单一目标（下一个token交叉熵）训练，未来token损失的梯度通过交叉注意力反向优化早期句子向量参数。

Result: 在缩放实验中，TG持续优于匹配的GPT-2及其他基线模型，缩放拟合显示GPT-2需多5-8%数据和33-42%参数才能匹配TG的损失；TG还减少了父子反转诅咒探针中关系方向泛化的错误。

Conclusion: TG模型通过双层次表示和递归记忆机制，有效提升了语言模型的数据效率和关系方向泛化能力，为解决现有Transformer的局限性提供了新途径。
...
### [55] [AdaGReS:Adaptive Greedy Context Selection via Redundancy-Aware Scoring for Token-Budgeted RAG](https://arxiv.org/abs/2512.25052)
*Chao Peng,Bin Wang,Zhilei Long,Jinfang Sheng*

Main category: cs.CL

TL;DR: 本文提出AdaGReS框架，解决RAG中top-k检索导致的冗余上下文问题，通过结合相关性与冗余惩罚的集合级目标优化，实现token预算约束下的高效上下文选择，提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成（RAG）对所选上下文质量高度敏感，但标准top-k检索常返回冗余或近重复的文本块，浪费token预算并降低下游生成效果。

Method: 提出AdaGReS框架，采用结合查询-文本块相关性和集合内冗余惩罚的集合级目标函数；在token预算约束下通过边际增益进行贪心选择；引入闭形式、实例自适应的相关性-冗余权衡参数校准，无需手动调参；理论分析证明目标函数在实际嵌入相似性条件下具有ε近似子模性，保证贪心选择的近最优性。

Result: 在开放域问答（Natural Questions）和高冗余生物医学（药物）语料库上的实验表明，AdaGReS在冗余控制和上下文质量上持续改进，转化为更好的端到端答案质量和跨设置的鲁棒性。

Conclusion: AdaGReS框架有效解决了RAG中的上下文冗余问题，通过自适应权衡相关性与冗余性，提升了检索上下文的质量和生成效果，具有理论保证和实践有效性。
...
### [56] [The Drill-Down and Fabricate Test (DDFT): A Protocol for Measuring Epistemic Robustness in Language Models](https://arxiv.org/abs/2512.23850)
*Rahul Baxi*

Main category: cs.AI

TL;DR: 本文提出Drill-Down and Fabricate Test（DDFT）协议以评估语言模型的认知鲁棒性，发现其与传统设计范式正交，错误检测能力是关键瓶颈，挑战了模型规模与可靠性的关联假设。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型评估（如MMLU、TruthfulQA）仅在理想条件下测量模型知识，无法区分模型缺乏知识还是验证机制在信息退化或对抗性探测时失效，需评估现实压力下的认知鲁棒性。

Method: 设计DDFT协议，通过渐进语义压缩和对抗性伪造测量认知鲁棒性；提出包含语义系统和认知验证器的双系统认知模型；对8个知识领域的9个前沿模型在5个压缩级别下进行1800次回合级评估。

Result: 认知鲁棒性与参数数量（r=0.083，p=0.832）和架构类型（r=0.153，p=0.695）无显著相关性；错误检测能力与整体鲁棒性强相关（rho=-0.817，p=0.007）；旗舰模型虽规模大但表现脆弱，较小模型可实现鲁棒性能。

Conclusion: DDFT框架为关键应用部署前评估认知鲁棒性提供理论基础和实用工具，认知鲁棒性源于训练方法和验证机制，错误检测能力是关键瓶颈，挑战了模型规模与可靠性的传统假设。
...
### [57] [A Proof-of-Concept for Explainable Disease Diagnosis Using Large Language Models and Answer Set Programming](https://arxiv.org/abs/2512.23932)
*Ioanna Gemou,Evangelos Lamprou*

Main category: cs.AI

TL;DR: 本文提出McCoy框架，结合大语言模型（LLMs）与回答集编程（ASP），解决符号AI在医疗领域因构建高质量知识库困难导致的应用受限问题，初步结果显示其在小规模疾病诊断任务中表现良好。


<details>
  <summary>Details</summary>
Motivation: 准确的疾病预测对及时干预、有效治疗和减少医疗并发症至关重要，但符号AI在医疗领域的应用因构建高质量知识库需大量努力而受限。

Method: 提出McCoy框架，协调LLM将医学文献翻译成ASP代码，结合患者数据，并使用ASP求解器处理以得到最终诊断。

Result: McCoy框架在小规模疾病诊断任务中表现出较强的性能。

Conclusion: McCoy框架通过整合LLMs与ASP的优势，提供了一个稳健且可解释的预测框架，有望解决符号AI在医疗领域的应用障碍。
...
### [58] [SPARK: Search Personalization via Agent-Driven Retrieval and Knowledge-sharing](https://arxiv.org/abs/2512.24008)
*Gaurab Chhetri,Subasish Das,Tausif Islam Chowdhury*

Main category: cs.AI

TL;DR: 本文提出SPARK框架，通过基于角色的LLM智能体协作实现任务特定检索和涌现式个性化搜索，解决静态用户画像或单一检索 pipeline 的局限。


<details>
  <summary>Details</summary>
Motivation: 个性化搜索需要建模用户动态、多维度的信息需求，但现有系统受限于静态用户画像或单一检索 pipeline，难以满足该需求。

Method: 提出SPARK框架：1. 定义由角色、专业知识、任务上下文和领域构成的角色空间；2. 引入角色协调器动态解释查询以激活最相关的专业智能体；3. 每个智能体执行独立的检索增强生成过程，支持专用长短期记忆存储和上下文感知推理模块；4. 通过结构化通信协议（共享内存库、迭代辩论、中继式知识转移）实现智能体协作；5. 结合认知架构、多智能体协调理论和信息检索原理，通过最小协调规则下的分布式智能体行为建模涌现式个性化特性，并融入自适应学习机制持续优化角色。

Result: SPARK框架产生可测试的预测，涉及协调效率、个性化质量和认知负载分布，同时支持角色的持续优化。

Conclusion: SPARK通过细粒度智能体专业化与协作检索的整合，为下一代能够捕捉人类信息寻求行为复杂性、流动性和上下文敏感性的搜索系统提供了见解。
...
### [59] [ROAD: Reflective Optimization via Automated Debugging for Zero-Shot Agent Alignment](https://arxiv.org/abs/2512.24040)
...
TL;DR: 本文提出ROAD框架，通过动态调试而非随机搜索优化LLM提示词，无需大规模标注数据集，在基准测试和生产环境中均显著提升性能


<details>
  <summary>Details</summary>
Motivation: 现有APO方法依赖大规模标注数据集计算适应度分数，但实际软件工程中初始冷启动阶段缺乏此类数据集，面临混乱生产日志和动态故障模式

Method: 提出ROAD框架，采用多智能体架构（Analyzer做根因分析、Optimizer做模式聚合、Coach做策略整合），将非结构化故障日志转化为结构化决策树协议

Result: 在标准化基准和生产知识管理引擎中，3次迭代内成功率提升5.6%（73.6%→79.2%）、搜索精度提升3.8%；零售领域复杂推理任务性能相对基线提升约19%

Conclusion: 模仿人类故障分析与修复的工程循环，为部署可靠LLM智能体提供了数据高效的替代方案，优于资源密集型RL训练
...
### [60] [LoongFlow: Directed Evolutionary Search via a Cognitive Plan-Execute-Summarize Paradigm](https://arxiv.org/abs/2512.24077)
*Chunhui Wan,Xunan Dai,Zhuo Wang,Minglei Li,Yanpeng Wang,Yinan Mao,Yu Lan,Zhiwen Xiao*

Main category: cs.AI

TL;DR: 本文提出LoongFlow自进化智能体框架，通过整合LLM的认知推理范式与混合进化记忆系统，解决传统进化方法在高维代码空间中的早熟收敛和探索效率低问题，在算法发现和ML pipeline优化任务上显著优于现有基线


<details>
  <summary>Details</summary>
Motivation: 传统进化方法缺乏结构化推理能力，在高维代码空间中易出现早熟收敛和探索效率低下，阻碍静态大语言模型向自改进智能体的过渡

Method: LoongFlow框架采用LLM驱动的“Plan-Execute-Summarize”认知范式，结合混合进化记忆系统（多岛模型+MAP-Elites+自适应玻尔兹曼选择）以平衡探索-利用权衡；实例化为算法发现通用智能体和ML pipeline优化智能体

Result: 在AlphaEvolve基准和Kaggle竞赛上的评估显示，LoongFlow的进化效率比OpenEvolve、ShinkaEvolve等领先基线高60%，同时发现更优解决方案

Conclusion: LoongFlow为自主科学发现迈出重要一步，可在降低计算开销的情况下生成专家级解决方案
...
### [61] [Constrained Language Model Policy Optimization via Risk-aware Stepwise Alignment](https://arxiv.org/abs/2512.24263)
*Lijun Zhang,Lin Li,Wei Wei,Yajie Qi,Huizhong Song,Jun Wang,Yaodong Yang,Jiye Liang*

Main category: cs.AI

TL;DR: 本文提出风险感知逐步对齐（RSA）方法，通过嵌套风险度量将风险意识纳入预训练语言模型的策略优化，解决现有安全对齐方法的风险中性局限，提升模型安全性同时保持有用性。


<details>
  <summary>Details</summary>
Motivation: 现有安全对齐方法（如Safe RLHF、SACPO）采用风险中性范式，无法有效应对偏离参考策略的风险，且对罕见但灾难性的有害行为鲁棒性有限。

Method: 提出风险感知逐步对齐（RSA）方法，将安全对齐转化为基于嵌套风险度量的token级风险感知约束策略优化问题，通过逐步对齐过程生成token级策略更新。

Result: 实验表明，RSA方法在保持高有用性的同时确保强安全性，显著抑制尾部风险（即低概率高影响的不安全响应）。

Conclusion: RSA方法能缓解模型过度偏离参考策略的风险，明确抑制低概率高影响的有害行为，且在温和假设下具有策略最优性理论保证。
...
### [62] [What Drives Success in Physical Planning with Joint-Embedding Predictive World Models?](https://arxiv.org/abs/2512.24497)
*Basile Terver,Tsung-Yen Yang,Jean Ponce,Adrien Bardes,Yann LeCun*

Main category: cs.AI

TL;DR: 本文研究AI中能解决多种物理任务并泛化到新任务的智能体，聚焦基于世界模型的规划方法，提出JEPA-WMs模型家族，通过实验优化关键组件并提出优于现有基线的模型。


<details>
  <summary>Details</summary>
Motivation: AI领域长期面临开发能解决多种物理任务并泛化到新任务和环境的智能体的挑战，现有基于世界模型的规划方法多在输入空间进行，而在世界模型的学习表示空间中优化规划算法有望通过抽象无关细节提升效率，需探索该类方法的最优实现。

Method: 将基于世界模型学习表示空间规划的方法归类为JEPA-WMs，对模型架构、训练目标和规划算法等关键组件进行综合研究；在模拟环境和真实机器人数据上开展实验，分析各组件对规划成功的影响；结合发现提出新模型。

Result: 提出的模型在导航和操作任务中均优于DINO-WM和V-JEPA-2-AC两个已建立的基线模型。

Conclusion: 通过对JEPA-WMs家族关键组件的优化，可得到在多种任务中性能更优的基于世界模型表示空间规划的智能体模型。
...
### [63] [Thinking on Maps: How Foundation Model Agents Explore, Remember, and Reason Map Environments](https://arxiv.org/abs/2512.24504)
*Zhiwei Wei,Yuxing Liu,Hua Liao,Wenjia Xu*

Main category: cs.AI

TL;DR: 本文提出交互式评估框架，分析基础模型（FM）智能体在符号地图环境中的探索、记忆与推理能力，揭示各组件功能作用及空间推理性能饱和现象。


<details>
  <summary>Details</summary>
Motivation: 现有FM空间能力评估多依赖静态地图输入或文本查询，忽略空间理解的交互性与经验驱动本质，亟需更全面的评估框架。

Method: 构建交互式符号地图环境（含道路、交叉口、POI的部分可观测网格），通过系统变化探索策略、记忆表示（如序列或图结构）和推理方案，对多个FM进行多任务（含路径规划等）评估。

Result: 探索策略主要影响经验获取，对最终推理精度影响有限；结构化记忆（尤其是序列和图表示）显著提升结构密集型任务性能；推理方案影响空间知识使用效率；空间推理性能在模型版本和规模超过一定阈值后饱和。

Conclusion: FM的地图空间理解需针对性优化空间表示与推理机制，而非仅依赖模型规模扩展。
...
### [64] [Evaluating the Reasoning Abilities of LLMs on Underrepresented Mathematics Competition Problems](https://arxiv.org/abs/2512.24505)
*Samuel Golladay,Majid Bani-Yaghoub*

Main category: cs.AI

TL;DR: 本研究分析了GPT-4o-mini、Gemini-2.0-Flash和DeepSeek-V3三个主流大语言模型在密苏里大学数学竞赛中微积分、解析几何和离散数学问题的表现，发现DeepSeek-V3整体最优，但所有模型在几何领域表现较弱，且各模型存在不同类型的错误模式。


<details>
  <summary>Details</summary>
Motivation: 现有研究多使用相同数据集评估大语言模型的数学推理能力，限制了结果的泛化性，无法全面捕捉数学任务中的多样化挑战，因此本研究旨在分析大语言模型在未充分代表的数学竞赛问题上的表现。

Method: 使用密苏里大学数学竞赛中的微积分、解析几何和离散数学问题对GPT-4o-mini、Gemini-2.0-Flash和DeepSeek-V3三个模型进行提示，将模型响应与已知正确解对比以确定各问题领域的准确性，并分析模型推理过程以探索不同问题类型和模型的错误模式。

Result: DeepSeek-V3在微积分、解析几何和离散数学三个类别中的推理和最终正确答案表现均最佳；所有三个模型在几何领域表现显著较弱；DeepSeek-V3的错误主要归因于计算和逻辑错误，GPT-4o-mini常出现逻辑和方法相关错误，Gemini则倾向于推理不完整和仓促下结论。

Conclusion: 在未充分代表的数学竞赛数据集上评估大语言模型可深入了解其独特的错误模式，并突出结构化推理中持续存在的挑战，尤其是在几何领域。
...
### [65] [From Building Blocks to Planning: Multi-Step Spatial Reasoning in LLMs with Reinforcement Learning](https://arxiv.org/abs/2512.24532)
*Amir Tahmasbi,Sadegh Majidi,Kazem Taram,Aniket Bera*

Main category: cs.AI

TL;DR: 本文提出一种两阶段方法提升大语言模型的空间推理能力，包括基础空间变换微调与基于LoRA的组合策略学习，并通过合成数据集和环境验证其优于基线模型


<details>
  <summary>Details</summary>
Motivation: 大语言模型在导航和规划等应用中的空间推理能力受关注，但仍难以处理结构化环境中的空间变换和多步规划

Method: 两阶段方法：1）监督微调基础空间变换（旋转、平移、缩放）；2）冻结物理感知模型，用GRPO框架训练轻量LoRA适配器学习组合策略；合成ASCII艺术数据集并构建强化学习环境

Result: 在动态（显式状态更新）和静态（依赖内部状态）环境中均优于基线（通用骨干、物理感知模型、端到端RL模型），收敛更快且训练更稳定

Conclusion: 该方法有效提升大语言模型空间推理能力，通过注意力模式分析验证微调对空间理解的改善
...
### [66] [Recursive Language Models](https://arxiv.org/abs/2512.24601)
*Alex L. Zhang,Tim Kraska,Omar Khattab*

Main category: cs.AI

TL;DR: 本文提出递归语言模型（RLMs），通过将长提示视为外部环境并允许模型程序化检查、分解和递归调用自身，成功处理超出模型上下文窗口两个数量级的输入，并在四个不同长上下文任务中显著优于基础LLM和常见长上下文支架，且查询成本相当或更低。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决大型语言模型（LLMs）处理任意长提示的能力限制问题，现有方法在处理超长输入时效果不佳或成本较高。

Method: 提出递归语言模型（RLMs）这一通用推理策略，核心是将长提示作为外部环境，让LLM能够程序化地检查、分解提示片段，并递归调用自身处理这些片段。

Result: RLMs能成功处理比模型上下文窗口长两个数量级的输入；在四个不同的长上下文任务中，即使对于较短提示，其质量也显著优于基础LLM和常见长上下文支架；且每次查询的成本与基础LLM相当或更低。

Conclusion: 递归语言模型（RLMs）是一种有效的长上下文处理方法，能够突破现有LLM的上下文窗口限制，在多种任务中表现优异且成本可控，为LLM处理超长输入提供了新的解决方案。
...
### [67] [Reinforcement Learning-Augmented LLM Agents for Collaborative Decision Making and Performance Optimization](https://arxiv.org/abs/2512.24609)
*Dong Qiu,Duo Xu,Limengxi Yue*

Main category: cs.AI

TL;DR: 本文提出了一种强化学习增强的大语言模型（LLM）智能体框架，旨在解决LLM在多智能体场景中缺乏协作意识和全局性能优化能力的问题，通过将协作建模为分散式部分可观测马尔可夫决策过程（Dec-POMDP）并采用集中训练分散执行（CTDE）策略，结合Group Relative Policy Optimization（GRPO）算法和简化的联合奖励机制，在协作写作和编码基准测试中取得了显著优于单智能体和强多智能体基线的结果。


<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLMs）在语言任务中表现出色，但在多智能体场景中往往缺乏协作意识，难以优化全局性能。

Method: 提出强化学习增强的LLM智能体框架，将协作建模为分散式部分可观测马尔可夫决策过程（Dec-POMDP），采用集中训练分散执行（CTDE）策略；引入Group Relative Policy Optimization（GRPO）算法，在训练期间利用全局信号联合优化智能体策略；设计简化的联合奖励机制，平衡任务质量、速度和协调成本。

Result: 在协作写作和编码基准测试中，任务处理速度比单智能体基线提高3倍；写作的结构/风格一致性达98.7%；编码测试通过率为74.6%；该方法持续优于强多智能体LLM基线。

Conclusion: 该方法为复杂工作流中可靠协作提供了实用路径，在协作写作和编码任务中显著提升了性能，优于现有基线。
...
### [68] [Group Deliberation Oriented Multi-Agent Conversational Model for Complex Reasoning](https://arxiv.org/abs/2512.24613)
*Zheyu Shi,Dong Qiu,Shanlong Yu*

Main category: cs.AI

TL;DR: 本文提出面向群体 deliberation 的多智能体对话模型，通过三级角色分工架构、自博弈机制和检索增强模块提升复杂推理任务性能


<details>
  <summary>Details</summary>
Motivation: 解决单一大型语言模型在复杂推理任务中的局限性

Method: 采用生成、验证、整合三级角色分工架构；引入自博弈机制扩展多路径推理轨迹；使用检索增强模块动态补充外部知识；设计结合事实一致性和逻辑连贯性的复合奖励函数，并应用改进的近端策略优化策略进行协同训练
...
### [69] [Multi-modal cross-domain mixed fusion model with dual disentanglement for fault diagnosis under unseen working conditions](https://arxiv.org/abs/2512.24679)
*Pengcheng Xia,Yixiang Huang,Chengjin Qin,Chengliang Liu*

Main category: cs.AI

TL;DR: 针对现有智能故障诊断方法在未知工况下性能下降、依赖目标域样本及单模态局限的问题，本文提出多模态跨域混合融合模型，通过双解耦框架、跨域混合融合策略和三模态融合机制提升模型泛化能力，并在电机故障诊断实验中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有智能故障诊断方法在未知工况下性能显著下降，域适应方法依赖目标域样本，且多数研究忽视多模态信息互补性对模型泛化的提升作用，因此需要解决这些局限。

Method: 提出多模态跨域混合融合模型，包含双解耦框架（解耦模态不变/特异性特征及域不变/特异性表示）、跨域混合融合策略（跨域随机混合模态信息以增强多样性）和三模态融合机制（自适应整合多模态异构信息）。

Result: 在感应电机未知恒定和时变工况下的故障诊断实验中，所提方法持续优于先进方法，且消融研究验证了各组件及多模态融合的有效性。

Conclusion: 所提多模态跨域混合融合模型可有效提升智能故障诊断在未知工况下的泛化能力，各关键组件（双解耦框架、跨域混合融合策略、三模态融合机制）均对性能提升有贡献。
...
### [70] [BatteryAgent: Synergizing Physics-Informed Interpretation with LLM Reasoning for Intelligent Battery Fault Diagnosis](https://arxiv.org/abs/2512.24686)
*Songqi Zhou,Ruixue Liu,Boman Su,Jiazhou Wang,Yixing Wang,Benben Jiang*

Main category: cs.AI

TL;DR: 本文提出BatteryAgent框架，结合物理知识特征与大语言模型推理能力，解决现有锂电池故障诊断方法的黑箱性和分类局限性问题，实现多类型可解释诊断。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在锂电池故障诊断中存在黑箱性导致可解释性差，且受限于二分类范式无法提供根本原因分析和维护建议，因此需要改进方法。

Method: 提出分层框架BatteryAgent，包含三个核心模块：物理感知层（基于电化学原理提取10个机制特征）、检测与归因层（使用梯度提升决策树和SHAP量化特征贡献）、推理与诊断层（利用大语言模型构建数值-语义桥梁，结合SHAP归因和机制知识库生成综合报告）。

Result: BatteryAgent有效纠正硬边界样本的误分类，AUROC达0.986，显著优于现有最优方法；将传统二分类检测扩展到多类型可解释诊断。

Conclusion: 该框架实现了从被动检测到智能诊断的范式转变，为电池安全管理提供新方法。
...
### [71] [Explaining Why Things Go Where They Go: Interpretable Constructs of Human Organizational Preferences](https://arxiv.org/abs/2512.24829)
*Emmanuel Fashae,Michael Burke,Leimin Tian,Lingheng Meng,Pamela Carreno-Medrano*

Main category: cs.AI

TL;DR: 本文提出一种基于四个可解释构念的家庭物品重排偏好显式公式，并将其整合到蒙特卡洛树搜索规划器中，生成与人类偏好一致的合理布局。


<details>
  <summary>Details</summary>
Motivation: 现有家庭物品重排机器人系统依赖从人类演示中推断的潜在偏好模型，虽预测有效但缺乏对指导人类决策的可解释因素的洞察。

Method: 设计并通过63名参与者的在线研究验证了自我报告问卷，以捕捉空间实用性、习惯便利性、语义连贯性和常识适当性四个构念；将这些构念整合到蒙特卡洛树搜索（MCTS）规划器中。

Result: 验证了四个构念的心理独特性及其在厨房和客厅场景中的解释力；整合构念的规划器生成的布局与参与者生成的布局高度一致。

Conclusion: 本研究贡献了紧凑、可解释的物品布局偏好公式，并展示了其在机器人规划中的可操作性。
...
### [72] [GenZ: Foundational models as latent variable generators within traditional statistical models](https://arxiv.org/abs/2512.24834)
*Marko Jojic,Nebojsa Jojic*

Main category: cs.AI

TL;DR: 本文提出GenZ混合模型，通过可解释语义特征连接基础模型与统计建模，解决基础模型难以捕捉预测任务中数据集特定模式的问题，并在房价预测和电影推荐冷启动场景验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽具备广泛领域知识，但在预测任务中常无法捕捉数据集特定模式，影响预测性能。

Method: 采用迭代过程发现语义特征描述，通过对比统计建模误差识别的项目组，而非仅依赖基础模型领域知识；将其构建为广义EM算法，联合优化语义特征描述符与统计模型参数；提示冻结基础模型基于发现特征分类项目，将判断视为潜在二元特征的噪声观测，通过学习的统计关系预测实值目标。

Result: 1. 房价预测：使用多模态房源数据发现的语义特征，模型中位数相对误差达12%，显著优于GPT-5基线（38%误差）；2. Netflix电影嵌入：仅通过语义描述预测协同过滤表示的余弦相似度为0.59，相当于传统协同过滤约4000个用户评分的性能；3. 发现数据集特定模式（如建筑细节预测本地住房市场、 franchise成员身份预测用户偏好）。

Conclusion: GenZ模型通过结合基础模型语义能力与统计建模，有效捕捉数据集特定模式，在房价预测和电影推荐冷启动任务中表现出色，优于仅依赖基础模型领域知识的方法。
...
### [73] [A study on constraint extraction and exception exclusion in care worker scheduling](https://arxiv.org/abs/2512.24853)
*Koki Suenaga,Tomohiro Furuta,Satoshi Ono*

Main category: cs.AI

TL;DR: 本研究提出一种基于约束模板的长期护理机构排班约束提取方法，通过灵活调整模板参数并排除例外约束，辅助约束规划求解器生成满足硬约束且减少软约束违规的护理人员排班。


<details>
  <summary>Details</summary>
Motivation: 长期护理机构排班条件因机构而异，需通过访谈管理者获取特定约束，但现有技术未有效处理例外约束，导致排班生成存在局限。

Method: 采用约束模板提取连续天数班次模式、员工组合等多种约束组件；通过调整关注天数、员工数量及提取焦点（模式或频率）生成多样约束；加入例外约束排除机制；将提取约束输入约束规划求解器生成排班。

Result: 实验表明，该方法能成功生成满足所有硬约束的排班，并通过排除例外约束减少软约束违规次数。

Conclusion: 所提出的约束模板方法可有效提取长期护理机构的个性化排班约束，辅助生成高质量排班，为解决机构特异性排班问题提供实用方案。
...
### [74] [Let It Flow: Agentic Crafting on Rock and Roll, Building the ROME Model within an Open Agentic Learning Ecosystem](https://arxiv.org/abs/2512.24873)
...
TL;DR: 本文介绍了Agentic Learning Ecosystem（ALE）基础设施及其支持的开源智能体模型ROME，通过多组件优化智能体开发流程并验证其性能


<details>
  <summary>Details</summary>
Motivation: 开源社区缺乏用于简化智能体开发的原则性端到端生态系统，无法有效支持LLMs在现实环境中的多轮行动、观察和迭代优化

Method: 提出ALE基础设施，包含ROLL（权重优化后训练框架）、ROCK（轨迹生成沙箱环境管理器）、iFlow CLI（上下文工程框架）；开发ROME模型，使用超百万轨迹训练，采用数据合成协议和基于交互的策略对齐（IPA）算法

Result: ROME在SWE-bench Verified和Terminal Bench等基准测试中表现优异，验证了ALE基础设施的有效性

Conclusion: ALE基础设施能优化智能体LLM的生产流程，ROME模型的强性能证明了该生态系统的价值
...
### [75] [Semi-Automated Data Annotation in Multisensor Datasets for Autonomous Vehicle Testing](https://arxiv.org/abs/2512.24896)
*Andrii Gamalii,Daniel Górniak,Robert Nowak,Bartłomiej Olber,Krystian Radlak,Jakub Winter*

Main category: cs.AI

TL;DR: 本文介绍DARTS项目中开发的半自动化数据标注流水线，采用人机协作方法结合AI与人类专业知识，减少驾驶场景多模态数据标注的成本和时间，同时保证质量，支持波兰自动驾驶研究。


<details>
  <summary>Details</summary>
Motivation: DARTS项目需创建波兰驾驶场景的大规模多模态数据集，但异构数据的人工标注成本高、耗时长，亟需解决方案。

Method: 采用人机协作（human-in-the-loop）方法，结合AI与人类专业知识；系统自动生成初始标注，支持迭代模型重训练，融入数据匿名化和领域自适应技术；核心依赖3D目标检测算法生成初步标注。

Result: 开发的工具和方法显著节省时间，同时确保不同传感器模态间标注的一致性和高质量。

Conclusion: 该解决方案通过加速标准化格式的大型标注数据集制备，直接支持DARTS项目，增强波兰自动驾驶研究的技术基础。
...
### [76] [Iterative Deployment Improves Planning Skills in LLMs](https://arxiv.org/abs/2512.24940)
*Augusto B. Corrêa,Yoav Gelberg,Luckeciano C. Melo,Ilia Shumailov,André G. Pereira,Yarin Gal*

Main category: cs.AI

TL;DR: 研究表明，通过迭代部署大型语言模型（LLMs）——每次基于前一模型部署中用户精心筛选的数据进行微调——可显著改变最终模型的特性；并从AI安全和替代训练机制两方面分析其与强化学习（RL）的关联。


<details>
  <summary>Details</summary>
Motivation: 探索迭代部署LLMs（基于用户筛选数据微调）对模型特性的影响，以及该机制与RL的关系及其潜在意义。

Method: 在多个规划领域测试迭代部署LLMs的机制，并对其进行理论分析以揭示与RL的联系。

Result: 迭代部署LLMs可大幅提升规划技能，后期模型能发现比初始模型更长的计划，展现出涌现泛化能力；理论分析表明该机制在外循环中有效实现RL训练（依赖数据筛选而非显式奖励）。

Conclusion: 迭代部署LLMs的机制具有双重意义：对AI安全而言，重复部署带来的隐式奖励函数可能对未来模型特性产生意外影响；同时，该机制可作为显式RL的替代训练方案，依赖数据筛选而非显式奖励。
...
### [77] [AMAP Agentic Planning Technical Report](https://arxiv.org/abs/2512.24957)
...
TL;DR: 本文提出STAgent，一种专为时空理解设计的智能体大语言模型，能解决约束兴趣点发现和行程规划等复杂任务，通过三种关键贡献增强能力并在TravelBench和通用基准上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有模型在时空场景复杂任务中可能缺乏有效工具交互能力和高质量数据支撑，难以平衡专业能力与通用能力。

Method: 提出三种关键贡献：1）支持十多种领域工具的稳定工具环境；2）分层数据筛选框架，以1:10000比例筛选高质量查询；3）级联训练方案，包括种子SFT阶段、高确定性查询SFT阶段和低确定性数据RL阶段，初始化为Qwen3-30B-A3B。

Result: STAgent在TravelBench上取得良好性能，同时在多种通用基准上保持通用能力。

Conclusion: 所提出的智能体模型STAgent有效展示了其在时空理解任务中的有效性，同时保持通用能力。
...
### [78] [A Comprehensive Study of Deep Learning Model Fixing Approaches](https://arxiv.org/abs/2512.23745)
*Hanmo You,Zan Wang,Zishuo Dong,Luanqi Mo,Jianjun Zhao,Junjie Chen*

Main category: cs.LG

TL;DR: 本文对16种先进的深度学习模型修复方法进行了大规模实证研究，评估其修复效果及对鲁棒性、公平性等关键属性的影响，总结出对工业界和学术界有意义的关键发现和未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 深度学习系统在自动驾驶、智能医疗等领域广泛应用，但易出现故障且故障可能带来重大风险，因此需要评估现有修复方法的性能及对其他关键属性的影响。

Method: 对16种模型级、层级和神经元级的深度学习模型修复方法，在统一实验设置下使用多样化数据集、模型架构和应用领域进行评估，分析其修复效果及对鲁棒性、公平性、向后兼容性等属性的影响。

Result: 模型级方法的修复效果优于其他级别方法；没有单一方法能在实现最佳修复性能的同时提升准确性并保持所有其他关键属性。

Conclusion: 学术界应优先研究减轻深度学习模型修复方法的副作用，这些发现为该领域未来探索指明了有前景的方向。
...
### [79] [A Review of Diffusion-based Simulation-Based Inference: Foundations and Applications in Non-Ideal Data Scenarios](https://arxiv.org/abs/2512.23748)
*Haley Rosso,Talea Mayo*

Main category: cs.LG

TL;DR: 本文综述了基于扩散模型的模拟推断（SBI）方法，从理论基础到实际应用，重点讨论其在科学数据非理想条件下的鲁棒性及与归一化流的对比和权衡。


<details>
  <summary>Details</summary>
Motivation: 复杂模拟问题中，似然函数难以处理导致经典似然方法不可用，而扩散模型作为灵活的生成模型框架，在SBI任务中展现潜力，需系统综述其在科学数据非理想条件下的鲁棒性。

Method: 回顾扩散模型的数学基础（前向加噪、反向SDE/ODE、概率流、去噪得分匹配），解释条件得分如何实现无似然后验采样；分析扩散模型解决归一化流痛点的方式及引入的新权衡；综合基于薛定谔桥公式、条件和序列后验采样器、非结构化数据的摊销架构、推理时先验适应等方法。

Result: 扩散模型在SBI任务中能处理非理想科学数据条件（如模型误设、非结构化/无限维观测、缺失数据），但存在迭代采样成本等新权衡；需明确准确后验所需的条件和注意事项。

Conclusion: 基于扩散的SBI在科学数据非理想条件下具有鲁棒性，未来需解决开放问题，尤其关注其在概率地球物理模型不确定性量化中的应用潜力。
...
### [80] [Coordinate Matrix Machine: A Human-level Concept Learning to Classify Very Similar Documents](https://arxiv.org/abs/2512.23749)
*Amin Sadri,M Maruf Hossain*

Main category: cs.LG

TL;DR: 本文提出Coordinate Matrix Machine（CM²）模型，通过学习文档结构实现单样本分类，作为绿色AI解决方案优于传统模型。


<details>
  <summary>Details</summary>
Motivation: 人类能从单个样本学习新概念，而机器学习算法通常需要大量样本；现代"Red AI"依赖大规模预训练和高能耗GPU基础设施，存在数据和计算资源消耗大的问题。

Method: 提出CM²模型，聚焦文档的结构坐标而非详尽语义向量，通过识别人类会考虑的结构性"重要特征"实现单样本分类，适用于仅CPU环境。

Result: CM²在少数据（单样本学习）下具有高精度，优于需要更大数据集和大量计算的传统向量化器和复杂深度学习模型；具备几何和结构智能、绿色AI特性、可解释性、低延迟、不平衡类鲁棒性等优势。

Conclusion: CM²是一种高效的绿色AI解决方案，在单样本分类任务中表现出色，具有多种优势，适用于资源受限环境且具备良好的扩展性。
...
### [81] [Geometric Scaling of Bayesian Inference in LLMs](https://arxiv.org/abs/2512.23752)
*Naman Aggarwal,Siddhartha R. Dalal,Vishal Misra*

Main category: cs.LG

TL;DR: 本文研究现代语言模型中是否存在与贝叶斯推理相关的几何特征，发现其最后一层值表示沿主导轴组织且与预测熵相关，干预实验表明该几何结构是不确定性的特权读出而非单一计算瓶颈。


<details>
  <summary>Details</summary>
Motivation: 已有研究表明在受控“风洞”环境中训练的小型Transformer可实现精确贝叶斯推理，且训练动态产生编码后验结构的几何基底，但需验证该几何特征是否在生产级语言模型中持续存在。

Method: 对Pythia、Phi-2、Llama-3和Mistral系列模型进行分析，观察其最后一层值表示的结构；对Pythia-410M的熵对齐轴进行针对性干预，并与随机轴干预对比。

Result: 发现生产级语言模型最后一层值表示沿单一主导轴组织，其位置与预测熵强相关；领域受限提示会将结构压缩为合成环境中观察到的低维流形；干预熵对齐轴会选择性破坏局部不确定性几何，而随机轴干预无影响，但单层操作未导致贝叶斯行为成比例退化。

Conclusion: 现代语言模型保留了风洞环境中支持贝叶斯推理的几何基底，并沿该基底组织其近似贝叶斯更新；该几何结构是不确定性的特权读出而非单一计算瓶颈。
...
### [82] [Generalized Regularized Evidential Deep Learning Models: Theory and Comprehensive Evaluation](https://arxiv.org/abs/2512.23753)
*Deep Shankar Pandey,Hyomin Choi,Qi Yu*

Main category: cs.LG

TL;DR: 本文针对证据深度学习（EDL）模型中基于主观逻辑框架的非负证据约束导致的激活依赖学习冻结问题，提出了通用激活函数家族及相应正则化器，并通过多任务实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 证据深度学习模型依赖主观逻辑框架约束证据为非负，需特定激活函数，但这些函数的几何特性会导致低证据区域样本的梯度极小，引发学习冻结行为，影响模型性能。

Method: 1. 理论分析不同证据激活函数对学习动态的影响及学习冻结行为；2. 设计通用激活函数家族及对应的证据正则化器，提供跨激活机制的一致证据更新途径。

Result: 在MNIST、CIFAR-10/100、Tiny-ImageNet四个分类基准、两个少样本分类问题及盲脸恢复任务上的实验验证了所提理论的正确性，并证明广义正则化证据模型的有效性。

Conclusion: 所提出的通用激活函数家族及正则化器可有效解决EDL模型中的学习冻结问题，提升模型在多种任务中的性能，为证据深度学习的稳定训练提供了新方案。
...
### [83] [HINTS: Extraction of Human Insights from Time-Series Without External Sources](https://arxiv.org/abs/2512.23755)
*Sheo Yon Jhin,Noseong Park*

Main category: cs.LG

TL;DR: 本研究提出HINTS框架，无需外部数据即可从时间序列残差中内生提取人类因素（如社会影响、记忆和偏见模式），并将其整合到注意力图中以提升预测准确性，同时验证了其可解释性和实用性。


<details>
  <summary>Details</summary>
Motivation: 金融和经济系统中的时间动态受人类决策、情绪和集体心理等复杂因素影响，但现有利用外部数据（如新闻、社交媒体）捕捉这些因素的方法存在高数据依赖成本（财务、计算和实践层面）。

Method: 提出HINTS自监督学习框架，以Friedkin-Johnsen（FJ）意见动态模型为结构归纳偏置，从时间序列残差中内生提取潜在人类因素，并将提取的因素作为注意力图整合到最先进的骨干模型中。

Result: 在9个真实世界和基准数据集上的实验表明，HINTS持续提升预测准确性；案例研究和消融研究验证了其可解释性，提取的因素与真实事件具有强语义对齐。

Conclusion: HINTS框架有效解决了现有方法的高数据依赖问题，提升了时间序列预测准确性，且具有良好的可解释性和实际应用价值。
...
### [84] [Neural Optimal Design of Experiment for Inverse Problems](https://arxiv.org/abs/2512.23763)
*John E. Darges,Babak Maboudi Afkham,Matthias Chung*

Main category: cs.LG

TL;DR: 本文提出NODE框架，通过联合训练神经重建模型与连续设计变量，在逆问题中实现最优实验设计，无需经典双层优化和间接稀疏正则化，提升重建精度与任务性能。


<details>
  <summary>Details</summary>
Motivation: 经典逆问题最优实验设计存在双层优化复杂度高、间接稀疏正则化需调参等问题，需更高效的方法。

Method: 提出NODE框架，在单一优化循环中联合训练神经重建模型与固定预算的连续设计变量（如传感器位置、采样时间等），直接优化测量位置以实现稀疏性，消除l1调参需求。

Result: 在指数增长基准、MNIST图像采样及真实稀疏视图X射线CT实例中验证，NODE均优于基线方法，重建精度和任务特定性能均有提升。

Conclusion: NODE框架有效解决逆问题最优实验设计的传统局限，在多种场景下表现出色，为相关领域提供高效解决方案。
...
### [85] [A Granular Grassmannian Clustering Framework via the Schubert Variety of Best Fit](https://arxiv.org/abs/2512.23766)
*Karim Salta,Michael Kirby,Chris Peterson*

Main category: cs.LG

TL;DR: 本文提出一种基于舒伯特最佳拟合簇（SVBF）的子空间聚类算法SVBF-LBG，在LBG管道中用可训练原型替代子空间均值，提升了聚类纯度。


<details>
  <summary>Details</summary>
Motivation: 在分类和聚类任务中，数据集或簇的几何代表（如均值、中位数）对分析很重要，但子空间数据集的代表位于Grassmann或旗流形上，其距离由主角度等几何因素诱导，现有方法可能存在不足。

Method: 引入舒伯特最佳拟合簇（SVBF）作为可训练原型，该原型是尽可能与每个簇成员在至少一个固定方向相交的子空间；将SVBF集成到Linde-Buzo-Grey（LBG）管道中，形成SVBF-LBG方案。

Result: 在合成数据、图像、光谱和视频动作数据上，SVBF-LBG方案的聚类纯度优于现有方法，同时保留了下游分析所需的数学结构。

Conclusion: SVBF-LBG算法有效提升了子空间聚类的纯度，且保持了数学结构的完整性，适用于多种类型的数据。
...
TL;DR: 本文提出SB-TRPO算法，解决安全关键领域强化学习中现有方法难以兼顾零安全违规与奖励性能的问题，通过自适应偏向约束满足的策略更新，在理论和实验上验证了其在安全与任务完成间的最优平衡。


<details>
  <summary>Details</summary>
Motivation: 安全关键领域的强化学习要求智能体在严格遵守安全约束的同时最大化奖励，但现有拉格朗日和基于投影的方法要么无法保证近乎零的安全违规，要么在硬约束下牺牲奖励性能。

Method: 提出Safety-Biased Trust Region Policy Optimisation（SB-TRPO）算法，该算法使用成本和奖励的自然策略梯度的凸组合进行信任区域更新，确保每一步都有固定比例的最优成本降低。

Result: 理论上保证了局部向安全进展，当梯度适当对齐时奖励提升；在标准和具有挑战性的Safety Gymnasium任务上，与最先进方法相比，SB-TRPO持续实现安全与有意义任务完成之间的最佳平衡。

Conclusion: SB-TRPO算法有效解决了安全关键领域强化学习中安全约束与奖励性能的平衡问题，在理论和实验中均表现出优越性能。
...
TL;DR: 本文提出FineFT框架，解决加密期货交易中高杠杆导致的训练不稳定和风险管控问题，通过三阶段集成强化学习实现稳定训练与风险感知，实验证明其优于12种SOTA基线，风险降低超40%且收益更高。


<details>
  <summary>Details</summary>
Motivation: 加密期货市场因高杠杆和流动性而活跃，但现有强化学习方法多针对现货市场，直接应用存在两大挑战：一是高杠杆放大奖励波动，导致训练随机且难以收敛；二是缺乏能力边界自我认知，在新市场状态（如黑天鹅事件）下易遭受重大损失。

Method: 提出三阶段集成强化学习框架FineFT：阶段I通过集成TD误差选择性更新集成Q学习器以提升收敛性；阶段II基于盈利能力过滤Q学习器，并训练变分自动编码器（VAEs）识别学习器的能力边界；阶段III在过滤后的集成和保守策略中选择，由训练好的VAEs引导，以维持收益并缓解新市场状态下的风险。

Result: 在高保真高频交易环境中对加密期货（5倍杠杆）进行广泛实验，结果显示FineFT在6个金融指标上优于12种SOTA基线，风险降低超40%，且收益优于亚军；选择性更新机制可视化表明不同智能体擅长不同市场动态，消融研究证实VAEs路由有效降低最大回撤，选择性更新提升收敛性和性能。

Conclusion: FineFT框架能有效解决加密期货交易中的训练稳定性和风险管控问题，在高杠杆环境下实现优异的收益与风险平衡，为加密期货量化交易提供了新的有效方法。
...
### [88] [Zero-Trust Agentic Federated Learning for Secure IIoT Defense Systems](https://arxiv.org/abs/2512.23809)
*Samaresh Kumar Singh,Joyjit Roy,Martin So*

Main category: cs.LG

TL;DR: 本文针对工业物联网(IIoT)安全漏洞，提出零信任智能体联邦学习(ZTA-FL)框架，结合TPM加密认证、SHAP加权聚合和隐私保护设备端对抗训练，在三个IDS基准数据集上实现高检测精度、拜占庭攻击抗性和对抗鲁棒性，并降低通信开销。


<details>
  <summary>Details</summary>
Motivation: 近年来针对关键基础设施的攻击（如2021年老斯玛水处理厂入侵和2023年丹麦能源部门妥协事件）凸显了工业物联网(IIoT)部署中的紧急安全漏洞。现有联邦学习(FL)隐私保护协同入侵检测框架易受拜占庭投毒攻击且缺乏强大的智能体验证机制。

Method: 提出零信任智能体联邦学习(ZTA-FL)深度防御框架，包含三个核心组件：1)基于TPM的加密认证（错误接受率低于1e-7）；2)新型SHAP加权聚合算法（在非独立同分布条件下提供可解释的拜占庭检测及理论保证）；3)隐私保护的设备端对抗训练。

Result: 在三个IDS基准数据集(Edge-IIoTset、CIC-IDS2017、UNSW-NB15)上的综合实验表明：ZTA-FL实现97.8%的检测精度；在30%拜占庭攻击下达到93.2%的精度（比FLAME高3.1%，p<0.01）；对抗鲁棒性达89.3%；通信开销降低34%。

Conclusion: ZTA-FL框架有效解决了工业物联网入侵检测中的拜占庭攻击、智能体验证和隐私保护问题，实验结果验证了其在检测精度、攻击抗性、鲁棒性和通信效率方面的优势。研究提供了理论分析、故障模式表征及可复现代码。
...
### [89] [Improved Bounds for Private and Robust Alignment](https://arxiv.org/abs/2512.23816)
*Wenqian Weng,Yi He,Xingyu Zhou*

Main category: cs.LG

TL;DR: 本文从理论角度研究语言模型的私有鲁棒对齐，通过建立离线和在线设置下的次优差距上界，分析隐私约束和对抗性腐败偏好标签的两种相互作用（隐私优先和腐败优先），并得出相关理论结果


<details>
  <summary>Details</summary>
Motivation: 现有语言模型对齐研究缺乏对隐私约束和对抗性腐败偏好标签的联合理论分析，尤其是在线设置下的私有鲁棒对齐尚未有结果，且传统观点认为MLE风格算法在隐私设置下性能不佳

Method: 通过建立隐私和腐败下log损失与平方损失的统一收敛保证，分析隐私优先和腐败优先两种相互作用场景，分别对隐私仅设置、联合隐私与腐败设置（离线和在线）进行理论推导

Result: 1.隐私仅设置下，MLE风格算法的log损失达到近最优速率；2.联合隐私与腐败的离线算法提供比已知更强的保证（同时在腐败水平和隐私参数方面），并改进腐败仅设置的边界；3.首次给出私有鲁棒在线对齐的结果；4.提出的统一收敛保证具有广泛适用性

Conclusion: 本文的理论结果填补了语言模型私有鲁棒对齐的理论空白，尤其是在线设置下的结果，且提出的统一收敛保证可应用于学习理论和统计学领域
...
### [90] [Exploiting the Prior of Generative Time Series Imputation](https://arxiv.org/abs/2512.23832)
*YuYang Miao,Chang Li,Zehua Chen*

Main category: cs.LG

TL;DR: 提出Bridge-TS方法，通过改进生成式时间序列补全的先验（专家先验和组合先验）提升补全精度，在多个基准数据集上取得最优结果。


<details>
  <summary>Details</summary>
Motivation: 现有生成式时间序列补全方法（如扩散模型、薛定谔桥模型）的先验对真实目标信息不足，导致生成负担增加且补全精度有限。

Method: 提出Bridge-TS方法，包含两种先验设计：1）专家先验：使用预训练Transformer模块生成确定性估计作为真实目标的先验；2）组合先验：利用多个预训练模型提供不同估计结果，并在数据到数据生成过程中组合这些结果。

Result: 在ETT、Exchange、Weather等基准数据集上，Bridge-TS在均方误差和平均绝对误差指标上达到新的补全精度记录。

Conclusion: 改进生成式时间序列补全的先验设计可有效提升补全性能，Bridge-TS方法展现出优越性。
...
### [91] [Trellis: Learning to Compress Key-Value Memory in Attention Models](https://arxiv.org/abs/2512.23852)
...
<details>
  <summary>Details</summary>
Motivation: Transformer存在注意力机制的二次计算复杂度和KV缓存持续增长的问题，限制其在长上下文应用中的表现

Method: 引入固定大小内存替代标准KV缓存，采用双通循环压缩机制，结合带遗忘门的在线梯度下降过程动态更新压缩内存以保留重要上下文信息

Result: 在语言建模、常识推理、回忆密集型任务和时间序列等实验中优于强基线，且序列长度增长时性能提升更明显

Conclusion: Trellis架构有效解决Transformer的内存和复杂度问题，在长上下文应用中具有潜力
...
### [92] [Flow Matching Neural Processes](https://arxiv.org/abs/2512.23853)
*Hussen Abu Hamad,Dan Rosenbaum*

Main category: cs.LG

TL;DR: 本文提出一种基于流匹配的新型神经过程（NP）模型，该模型实现简单，可通过ODE求解器进行条件分布采样，并在多个基准数据集上优于现有神经过程方法。


<details>
  <summary>Details</summary>
Motivation: 神经过程（NPs）是一类直接从数据中学习随机过程的模型，但现有NP模型可能存在实现复杂或需要辅助条件方法等问题，因此需要更简单高效的NP模型。

Method: 提出基于流匹配的新型NP模型，遵循NP训练框架，使用ODE求解器进行条件分布采样，且可通过调整ODE求解器的步数来控制准确性与运行时间的权衡。

Result: 该模型在多个基准数据集（包括合成1D高斯过程数据、2D图像和真实世界天气数据）上优于之前的最先进神经过程方法。

Conclusion: 基于流匹配的NP模型是一种简单高效的替代方案，在多种数据模态上表现出色，为神经过程的应用提供了新的可能性。
...
### [93] [Yggdrasil: Bridging Dynamic Speculation and Static Runtime for Latency-Optimal Tree-Based LLM Decoding](https://arxiv.org/abs/2512.23858)
...
TL;DR: 提出Yggdrasil系统优化推测解码以提升LLM推理性能


<details>
  <summary>Details</summary>
Motivation: 现有推测解码系统因动态推测与静态运行时假设不匹配导致性能欠佳

Method: 采用上下文感知树起草、等增长树结构、延迟感知优化目标及阶段式调度

Result: 在多种硬件环境下相比最优基线实现高达3.98倍的加速且支持未修改的LLM

Conclusion: Yggdrasil通过协同设计实现延迟最优的推测解码
...
### [94] [Probing the Limits of Compressive Memory: A Study of Infini-Attention in Small-Scale Pretraining](https://arxiv.org/abs/2512.23862)
*Ruizhe Huang,Kexuan Zhang,Yihao Fang,Baifeng Yu*

Main category: cs.LG

TL;DR: 本研究针对小型语言模型（SLMs）的小规模预训练，探索Infini-attention架构以提升其长上下文外推能力，结果表明该架构能有效补偿SLM参数限制，在长上下文任务中优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 为解决小型语言模型（SLMs）在有限数据和计算资源下的长上下文外推能力不足问题，提升低资源场景下的可用性并降低成本。

Method: 采用300M参数的LLaMA模型，通过Infini-attention架构（从过往片段构建压缩记忆同时保留局部注意力）进行预训练，并开展实证研究。

Result: Infini-attention模型训练稳定，在长上下文检索任务中优于基线；平衡因子是模型性能关键，重复记忆压缩会导致检索精度下降，但仍能有效补偿SLM参数限制；在16384-token上下文下虽性能下降，但比基线准确率高31%。

Conclusion: 在小型语言模型中实现稳健的长上下文能力可受益于Infini-attention这类架构记忆。
...
### [95] [Max-Entropy Reinforcement Learning with Flow Matching and A Case Study on LQR](https://arxiv.org/abs/2512.23870)
*Yuyang Zhang,Yang Hu,Bo Dai,Na Li*

Main category: cs.LG

TL;DR: 本文提出一种基于流模型的软演员评论家（SAC）变体算法，通过重要性采样流匹配（ISFM）更新策略，在最大熵线性二次调节器问题中验证了其能学习最优动作分布。


<details>
  <summary>Details</summary>
Motivation: 现有SAC算法为效率常使用简单策略类，牺牲了表达性和鲁棒性，需要更具表达力的策略参数化方法。

Method: 提出基于流模型的SAC变体，利用瞬时变量变换技术评估流策略，并通过在线变体ISFM更新策略，仅需用户指定采样分布的样本而非未知目标分布。

Result: 对ISFM进行理论分析，刻画不同采样分布选择对学习效率的影响；在最大熵线性二次调节器问题上的案例研究表明，该算法能学习最优动作分布。

Conclusion: 所提出的基于流模型的SAC变体算法有效，能在相关问题中学习到最优动作分布，且ISFM的理论分析为采样分布选择提供指导。
...
TL;DR: 本文提出Stagewise Pairwise Mixers (SPM)结构化线性算子，替代密集线性层以降低计算和参数成本，同时引入组合归纳偏置提升泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现代机器学习模型中密集线性层存在二次复杂度，且常与学习表示的组合结构不匹配，导致计算和参数成本过高。

Method: 提出SPM，用稀疏成对混合阶段的组合替代密集矩阵，实现O(nL)时间和参数复杂度（L通常为常数或log₂n）；推导正交保范旋转变体和完全通用2×2混合变体的前向/反向表达式；将SPM作为密集线性层的即插即用替代方案。

Result: 在结构化学习问题上显著降低实际耗时并提高准确率，在真实世界基准测试中保持竞争力。

Conclusion: SPM不仅节省计算成本，其阶段结构引入的显式组合归纳偏置可约束模型容量，当与任务结构对齐时能提升泛化能力；实验验证了其有效性。
...
### [97] [Interactive Machine Learning: From Theory to Scale](https://arxiv.org/abs/2512.23924)
*Yinglun Zhu*

Main category: cs.LG

TL;DR: 本论文研究交互式机器学习，提出统计最优且计算高效的算法，解决标签获取成本高、动作空间大及部分反馈下模型选择等问题，为实际应用提供理论基础。


<details>
  <summary>Details</summary>
Motivation: 机器学习的有效方法依赖大量标注数据或在线交互，但获取高质量标签和试错决策在大规模或高风险场景中成本高、耗时或有风险。

Method: 从三个维度研究交互式学习：带噪声数据和丰富模型类的主动学习、大动作空间的序列决策、部分反馈下的模型选择。

Result: 提出首个无需低噪声假设即可实现指数级标签节省的高效主动学习算法；首个保证独立于动作空间大小的通用上下文 bandit 算法；首个序列决策中模型选择基础成本的紧刻画。

Conclusion: 本论文通过开发统计最优且计算高效的算法，推进了交互式学习的理论基础，并为在大规模实际场景部署交互式学习方法提供原则性指导。
...
### [98] [Improved Balanced Classification with Theoretically Grounded Loss Functions](https://arxiv.org/abs/2512.23947)
*Corinna Cortes,Mehryar Mohri,Yutao Zhong*

Main category: cs.LG

TL;DR: 本文针对类别不平衡多分类问题中平衡损失难以直接最小化的问题，提出并研究了广义对数调整（GLA）和广义类别感知加权（GCA）两种替代损失家族，通过理论分析和实验验证了它们的优越性


<details>
  <summary>Details</summary>
Motivation: 平衡损失是类别不平衡多分类中广泛采用的目标，但直接最小化该损失通常难以处理，因此设计有效的替代损失成为核心问题

Method: 提出广义对数调整（GLA）损失家族（将基于类别先验偏移对数的对数调整损失推广到广义交叉熵损失家族）和广义类别感知加权（GCA）损失家族（通过引入类别依赖的置信边际并推广到广义交叉熵家族来扩展标准类别加权损失），并对两种损失家族的一致性进行全面理论分析

Result: 理论分析表明GLA损失是贝叶斯一致的，但仅对完整假设集是H一致的，且H一致性边界与最小类别概率成反比（至少为1/p_min）；GCA损失对任何有界或完整假设集都是H一致的，边界更优（1/√p_min）。实验结果显示，带校准类别依赖置信边际的GCA损失和GLA损失显著优于直接类别加权损失和LA损失；GLA在常见基准中略优，GCA在高度不平衡场景中略有优势

Conclusion: GLA和GCA两种替代损失家族在类别不平衡多分类问题中具有良好的理论保证和实验性能，其中GCA在高度不平衡设置下的理论保证更强，而GLA在常见基准中表现稍好
...
### [99] [Causify DataFlow: A Framework For High-performance Machine Learning Stream Computing](https://arxiv.org/abs/2512.23977)
*Giacinto Paolo Saggese,Paul Smith*

Main category: cs.LG

TL;DR: 提出DataFlow框架，统一批处理与流处理机器学习系统开发，解决传统工作流从原型到生产的适配问题。


<details>
  <summary>Details</summary>
Motivation: 传统数据科学工作流假设数据集有限，从批处理原型迁移到流处理生产系统需大量重实现，导致因果关系违规、批处理边界伪影及实时故障重现性差。

Method: 基于带有点时间幂等性的有向无环图（DAG）统一执行模型，输出仅依赖时间t前固定长度上下文窗口；自动跟踪所有转换的知识时间以强制严格因果性；支持时间和特征维度的灵活分块，原生集成Python数据科学栈，提供在线学习的fit/predict语义、缓存与增量计算、基于DAG调度的自动并行化。

Result: 在金融交易、物联网、欺诈检测和实时分析等领域证明了框架的有效性。

Conclusion: DataFlow框架解决了传统工作流批处理到流处理的适配问题，支持同一模型通过配置在不同频率和内存配置下运行，无需代码变更即可实现批处理模型在流处理生产中的相同执行。
...
### [100] [Assured Autonomy: How Operations Research Powers and Orchestrates Generative AI Systems](https://arxiv.org/abs/2512.23978)
*Tinglong Dai,David Simchi-Levi,Michelle Xiao Wu,Yao Xie*

Main category: cs.LG

TL;DR: 本文针对生成式人工智能向自主智能体系统转变中的自主性悖论，提出基于运筹学的可信自主概念框架，旨在解决生成模型在操作领域的脆弱性问题。


<details>
  <summary>Details</summary>
Motivation: 生成式人工智能正从对话助手转向自主决策系统，但随着自主性提升，需更强的结构、约束和尾部风险管控，而随机生成模型在操作领域易脆弱，缺乏可验证可行性、分布偏移鲁棒性及高后果场景下的压力测试机制。

Method: 构建基于运筹学的可信自主框架，包含两种互补方法：1）流基生成模型将生成视为常微分方程表征的确定性传输，实现可审计性、约束感知生成及与最优传输等领域的连接；2）通过对抗鲁棒性视角制定操作安全，在不确定性或模糊集内针对最坏扰动评估决策规则。

Result: 该框架明确了运筹学角色从求解器向护栏再到系统架构师的转变，涵盖控制逻辑、激励协议、监控机制和安全边界等要素。

Conclusion: 提出的可信自主框架为安全关键、可靠性敏感操作领域的可信自主研究设定了议程。
...
### [101] [Information-Theoretic Quality Metric of Low-Dimensional Embeddings](https://arxiv.org/abs/2512.23981)
*Sebastián Gutiérrez-Bernal,Hector Medel Cobaxin,Abiel Galindo González*

Main category: cs.LG

TL;DR: 本研究从信息论视角提出熵秩保持度量（ERPM），用于评估低维嵌入的信息保留质量，并通过与现有距离和几何度量对比验证其互补性


<details>
  <summary>Details</summary>
Motivation: 经典低维嵌入评估指标（如应力、基于秩的邻域准则或局部Procrustes）仅量化距离或局部几何扭曲，未直接评估高维数据投影到低维空间时的信息保留量

Method: 引入基于邻域矩阵奇异值谱的香农熵和稳定秩的局部度量ERPM，量化原始表示与降维投影间的不确定性变化；使用金融时间序列和文献中常见的流形数据，将ERPM与基于距离的平均相对秩误差（MRRE）及基于几何性质的局部Procrustes进行对比验证

Result: 距离基准则与几何和谱度量相关性极低；ERPM与局部Procrustes平均相关性强，但在局部区域存在显著差异

Conclusion: ERPM通过识别信息严重损失的邻域补充现有指标，使嵌入评估更全面，尤其适用于信息敏感应用（如预警指标构建）
...
### [102] [Tracing the Heart's Pathways: ECG Representation Learning from a Cardiac Conduction Perspective](https://arxiv.org/abs/2512.24002)
*Tan Pan,Yixuan Sun,Chen Jiang,Qiong Gao,Rui Sun,Xingmeng Zhang,Zhenqi Yang,Limei Han,Yixiu Liang,Yuan Cheng,Kaiyu Guo*

Main category: cs.LG

TL;DR: 本文提出CLEAR-HUG两阶段框架，通过捕捉心电导联间的细微传导差异并遵循诊断指南，提升ECG分析的表示学习效果。


<details>
  <summary>Details</summary>
Motivation: 现有心电图自监督学习方法存在两个局限：一是仅关注导联和心跳间的一致模式，忽略了心脏传导过程中心跳的固有差异及其中携带的独特生理特征；二是预训练模型应用于下游任务时未遵循从单个心跳到单导联再到导联组合的ECG诊断指南顺序逻辑。

Method: 提出两阶段框架CLEAR-HUG：第一阶段引入Conduction-LEAd Reconstructor（CLEAR）模型，将每个心跳视为独立实体，采用稀疏注意力机制捕捉心跳间的特定变异和通用共性，在不受其他心跳干扰的情况下重建信号；第二阶段实现Hierarchical lead-Unified Group（HUG）头用于疾病诊断，模拟临床工作流程。

Result: 在六个任务上的实验结果显示，CLEAR-HUG框架使性能提升了6.84%，验证了其有效性。

Conclusion: CLEAR-HUG框架能够增强心脏传导的表示能力，并使模式与专家诊断指南对齐，为ECG分析提供了更有效的方法。
...
### [103] [How and Why LLMs Generalize: A Fine-Grained Analysis of LLM Reasoning from Cognitive Behaviors to Low-Level Patterns](https://arxiv.org/abs/2512.24063)
*Haoyue Bai,Yiyou Sun,Wenjie Hu,Shi Qiu,Maggie Ziyu Huan,Peiyang Song,Robert Nowak,Dawn Song*

Main category: cs.LG

TL;DR: 本文通过引入新基准分解LLM推理能力为原子核心技能，结合低层次统计模式分析，揭示了SFT与RL调优在泛化行为上差异的原因，为设计稳健泛化的训练策略提供了新见解。


<details>
  <summary>Details</summary>
Motivation: 现有研究依赖粗略的准确率指标，无法明确解释LLM在监督微调（SFT）和强化学习（RL）调优中泛化行为差异的原因，即SFT常缩小能力范围而RL调优倾向保留能力。

Method: 引入分解推理为原子核心技能（如计算、事实检索等）的新基准，结合分布差异、参数统计等低层次统计模式分析，以及跟踪不同训练阶段模型行为的元探测框架。

Result: RL调优模型保持更稳定的行为特征，抵抗推理技能崩溃；SFT模型表现出更明显的漂移，过度拟合表面模式。

Conclusion: 本研究为LLM推理本质提供了新见解，并指向设计促进广泛、稳健泛化训练策略的原则。
...
### [104] [Multi-Scenario Highway Lane-Change Intention Prediction: A Temporal Physics-Informed Multi-Modal Framework](https://arxiv.org/abs/2512.24075)
*Jiazhao Shi,Ziyu Wang,Yichen Lin,Shoufeng Lu*

Main category: cs.LG

TL;DR: 本文提出融合深度时间表示与物理启发交互线索的混合框架TPI-AI，用于自动驾驶中的车道变换意图预测，解决自然交通场景中的噪声运动学、类别不平衡和场景泛化问题。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶和ADAS中的车道变换意图预测对安全至关重要，但在自然交通场景中因噪声运动学、严重类别不平衡及异构高速公路场景泛化能力有限而面临挑战。

Method: 提出Temporal Physics-Informed AI（TPI-AI）混合框架：用两层双向LSTM编码器从多步轨迹历史学习紧凑嵌入，拼接运动学、安全和交互感知特征（如车头时距、TTC、安全间隙指标），训练LightGBM分类器识别三类意图；应用重采样/加权和折叠阈值校准等不平衡感知优化方法。

Result: 在highD（直道高速）和exiD（多匝道环境）数据集上，使用基于位置的分割和预测 horizon T=1、2、3秒评估，TPI-AI优于单独的LightGBM和Bi-LSTM基线，highD上macro-F1分别为0.9562、0.9124、0.8345，exiD上为0.9247、0.8197、0.7605。

Conclusion: 将物理启发交互特征与学习的时间嵌入相结合，可实现鲁棒的多场景车道变换意图预测。
...
### [105] [Autoregressivity in the Latent Space of a GP-VAE Language Model: An Empirical Ablation Study](https://arxiv.org/abs/2512.24102)
*Yves Ruffenach*

Main category: cs.LG

TL;DR: 本文对GP-VAE模型中的潜在自回归进行了消融分析，研究其在组织长程结构中的作用。


<details>
  <summary>Details</summary>
Motivation: 语言模型通常依赖于token级的自回归分解，而之前的工作提出将序列结构转移到潜在空间（通过因果高斯过程）同时使用非自回归解码器，需系统分析潜在自回归的作用。

Method: 对比三种模型：（1）具有自回归潜在动力学的完整GP-VAE；（2）潜在变量独立的非自回归变体；（3）标准token级自回归Transformer，进行消融研究。

Result: 在中等规模语料库和短训练上下文条件下，潜在自回归使潜在轨迹与高斯过程先验更兼容，且长程稳定性更高；移除自回归会导致潜在结构退化和长程行为不稳定。

Conclusion: 潜在自回归是组织长程结构的有效机制，与token级自回归建模互补；本研究是对表征结构的实证分析，而非新架构提案。
...
### [106] [Enhancing LLM Planning Capabilities through Intrinsic Self-Critique](https://arxiv.org/abs/2512.24103)
...
TL;DR: 本文提出一种让大语言模型（LLM）通过内在自我批判提升规划任务性能的方法，在Blocksworld、Logistics和Mini-grid数据集上取得显著改进，超越现有基线并达到2024年10月LLM模型检查点的最新水平


<details>
  <summary>Details</summary>
Motivation: 早期研究对LLM利用自我批判方法的有效性提出质疑，而本文旨在探索无需外部验证器的内在自我批判能否提升LLM在规划任务中的性能

Method: 采用少样本学习技术并逐步扩展至多样本方法作为基础，通过迭代修正和细化过程实现自我批判，提升规划性能

Result: 在Blocksworld、Logistics和Mini-grid数据集上取得显著性能提升，超越强基线准确率，成为2024年10月LLM模型检查点中的最新技术水平

Conclusion: 内在自我批判可显著提升LLM的规划性能，该方法具有模型版本无关的自我改进能力，将其应用于更复杂的搜索技术和更强大的模型有望获得更好的性能
...
### [107] [OptRot: Mitigating Weight Outliers via Data-Free Rotations for Post-Training Quantization](https://arxiv.org/abs/2512.24124)
*Advait Gadhikar,Riccardo Grazzi,James Hensman*

Main category: cs.LG

TL;DR: 提出OptRot和OptRot⁺两种可融合旋转方法，通过最小化权重量化误差相关目标以缓解大语言模型权重和激活中的异常值问题，提升量化性能并揭示权重与激活量化间的权衡关系


<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLMs）权重和激活中存在的异常值导致其难以量化，现有基于旋转的方法需优化更高效的目标以缓解该问题

Method: 提出两种方法：1）OptRot：通过最小化旋转后权重的元素-wise四次方来减少权重异常值；2）OptRot⁺：结合激活协方差信息的依赖数据方法；主要以GPTQ作为量化基准方法

Result: OptRot在权重量化上优于Hadamard旋转、SpinQuant和OSTQuant，且在W4A8设置下提升激活量化性能；OptRot⁺进一步提升性能；但在W4A4设置下两者性能下降

Conclusion: 所提旋转方法有效缓解LLMs量化中的异常值问题，提升量化性能，同时揭示权重量化与激活量化之间存在权衡关系
...
### [108] [Paired Seed Evaluation: Statistical Reliability for Learning-Based Simulators](https://arxiv.org/abs/2512.24145)
*Udit Sharma*

Main category: cs.LG

TL;DR: 本文分析了基于学习的模拟器中比较评估的统计结构，提出配对种子评估设计以减少方差并提高统计效率


<details>
  <summary>Details</summary>
Motivation: 基于学习的模拟器在比较算法等场景中因随机初始化和学习随机性导致评估结果方差高，而标准独立评估设计未利用备选方案间共享的随机性来源

Method: 提出配对种子评估设计，在相同随机种子下评估竞争系统，诱导随机组件的匹配实现

Result: 该设计能产生更紧的置信区间、更高的统计功效和固定计算预算下的有效样本量增益；实证显示种子级相关性通常大且正，带来数量级的效率提升

Conclusion: 配对种子评估在实践中弱占优，存在相关性时提升统计可靠性，无相关性时退化为独立评估且不损失有效性
...
### [109] [Early Prediction of Sepsis using Heart Rate Signals and Genetic Optimized LSTM Algorithm](https://arxiv.org/abs/2512.24253)
*Alireza Rafiei,Farshid Hajati,Alireza Rezaee,Amirhossien Panahi,Shahadat Uddin*

Main category: cs.LG

TL;DR: 本研究提出并评估了四种基于心率数据的机器学习算法，用于可穿戴设备上的脓毒症早期预测，通过遗传算法优化模型架构，并扩展预测窗口至4小时，为非ICU/病房环境下的脓毒症早期检测提供可能。


<details>
  <summary>Details</summary>
Motivation: 脓毒症因免疫反应失调导致高死亡率、发病率及医疗成本，现有ICU患者预测模型存在非病房环境早期检测的缺口，需探索可穿戴技术实现该场景下的早期检测。

Method: 提出四种机器学习算法，基于心率数据预测脓毒症发作；通过遗传算法优化模型架构（平衡性能、计算复杂度和内存需求）；先针对1小时预测窗口设计模型，再通过迁移学习扩展至4小时；评估模型在可穿戴设备上的可行性。

Result: 模型在可穿戴设备上展现出良好性能，经遗传算法优化和迁移学习扩展预测窗口后取得了鼓舞人心的结果，表明其在非ICU/病房环境下应用的潜力。

Conclusion: 本研究的积极成果提示，可穿戴技术有望在ICU和病房以外的环境中促进脓毒症的早期检测。
...
### [110] [Empower Low-Altitude Economy: A Reliability-Aware Dynamic Weighting Allocation for Multi-modal UAV Beam Prediction](https://arxiv.org/abs/2512.24324)
*Haojin Li,Anbang Zhang,Chen Sun,Chenyuan Feng,Kaiqian Qu,Tony Q. S. Quek,Haijun Zhang*

Main category: cs.LG

TL;DR: 针对无人机通信中波束预测的多模态方法存在静态加权和模态失配问题，提出可靠性感知动态加权的语义感知多模态框架SaM2B，通过动态权重分配和跨模态对比学习提升波束预测性能。


<details>
  <summary>Details</summary>
Motivation: 低空经济快速发展，无人机通信中的波束预测需快速准确以保障可靠连接；现有多模态方法采用固定或经验权重，未考虑不同模态在无人机运动场景中的重要性波动，且模态失配和弱对齐影响跨场景泛化能力。

Method: 提出SaM2B框架：1）利用环境视觉、飞行姿态和地理空间等轻量级线索，通过可靠性感知动态权重更新自适应分配不同时刻各模态的贡献；2）采用跨模态对比学习，将特定波束信息的多源表示语义对齐到共享语义空间。

Result: 在真实低空无人机数据集上的实验表明，SaM2B的性能优于基线方法。

Conclusion: SaM2B通过动态权重分配和跨模态语义对齐，有效解决了现有多模态波束预测方法的静态加权和模态失配问题，提升了波束预测的判别力和鲁棒性，在实际场景中表现更优。
...
### [111] [Tubular Riemannian Laplace Approximations for Bayesian Neural Networks](https://arxiv.org/abs/2512.24381)
*Rodrigo Pereira David*

Main category: cs.LG

TL;DR: 提出管状黎曼拉普拉斯（TRL）近似，通过建模后验概率管分离先验主导的切向不确定性与数据主导的横向不确定性，在ResNet-18上实现与深度集成相当的校准性能且训练成本仅为1/5


<details>
  <summary>Details</summary>
Motivation: 传统欧几里得拉普拉斯近似难以处理现代深度模型高度各向异性、弯曲的损失曲面及大对称群的问题

Method: 引入管状黎曼拉普拉斯（TRL）近似，使用Fisher/Gauss-Newton度量分离先验主导的切向不确定性与数据主导的横向不确定性，将其视为利用隐式曲率估计在高维参数空间运行的可扩展重参数化高斯近似

Result: 在ResNet-18（CIFAR-10和CIFAR-100）上的实验表明，TRL校准性能优异，ECE指标匹配或超过深度集成，且训练成本仅为后者的1/5
...
TL;DR: 本文提出了一种仅使用视觉表示进行规划和定位的框架ViReLoc，通过强化学习和对比学习等方法，在空间推理任务中取得了显著改进，为无GPS导航提供了更安全的解决方案。


<details>
  <summary>Details</summary>
Motivation: 当前多模态智能在视觉理解和高级推理方面取得进展，但多数推理系统仍依赖文本信息作为主要推理媒介，限制了其在视觉导航和地理定位等空间任务中的有效性。

Method: 提出了Geo-Consistent Visual Planning视觉推理范式及ViReLoc框架，仅使用视觉表示进行规划和定位；学习空间依赖和几何关系，在视觉域中编码逐步推理并通过强化学习目标优化；集成对比学习和自适应特征交互以对齐跨视图视角并减少视点差异。

Result: 在多种导航和定位场景的实验中，空间推理准确性和跨视图检索性能均得到持续改进。

Conclusion: 视觉推理是导航和定位的有效补充方法，此类任务可在无实时GPS数据的情况下完成，从而实现更安全的导航解决方案。
...
### [113] [Efficient Inference for Inverse Reinforcement Learning and Dynamic Discrete Choice Models](https://arxiv.org/abs/2512.24407)
*Lars van der Laan,Aurelien Bibaut,Nathan Kallus*

Main category: cs.LG

TL;DR: 本文提出半参数去偏逆强化学习框架，解决传统IRL无有效推断保证和DDC模型参数限制问题，实现统计高效推断


<details>
  <summary>Details</summary>
Motivation: 传统逆强化学习（IRL）方法依赖机器学习但缺乏有效推断保证，经典动态离散选择（DDC）模型参数限制严格且需重复动态规划，存在方法缺陷

Method: 构建半参数去偏IRL框架，将日志行为策略作为伪奖励以识别策略价值差异及归一化奖励，将目标形式化为行为策略和转移核的平滑函数并推导有效影响函数，构建自动去偏机器学习估计器

Result: 该框架对最大熵IRL和Gumbel-shock DDC模型中广泛的奖励依赖泛函实现统计高效推断，估计器具有√n一致性、渐近正态性和半参数效率

Conclusion: 框架扩展了DDC模型的经典推断至非参数奖励和现代机器学习工具，为IRL中的统计推断提供统一且计算可行的方法
...
### [114] [Sparse classification with positive-confidence data in high dimensions](https://arxiv.org/abs/2512.24443)
*The Tien Mai,Mai Anh Nguyen,Trung Nghia Nguyen*

Main category: cs.LG

TL;DR: 针对高维学习中弱监督（如Pconf分类）场景下现有方法不适用的问题，提出一种稀疏惩罚框架，通过凸和非凸惩罚估计器提升特征恢复能力，并在理论和实验上验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 高维学习问题（特征数超过样本量）常需稀疏正则化，但在弱监督场景（如Positive-Confidence分类）中相关技术研究不足；现有Pconf方法不适用于高维情况。

Method: 提出高维Pconf分类的稀疏惩罚框架，引入凸（Lasso）和非凸（SCAD、MCP）惩罚估计器；建立L1正则化Pconf估计器的误差界理论；开发高效近端梯度算法求解复合目标。

Result: 理论上证明L1正则化Pconf估计器在Restricted Strong Convexity条件下达到近极小极大最优稀疏恢复率；仿真实验显示所提方法的预测性能和变量选择精度可与全监督方法媲美。

Conclusion: 所提稀疏惩罚框架有效填补了弱监督与高维统计之间的 gap，在高维Pconf分类中实现了良好的预测和特征选择效果。
...
### [115] [Adaptive Learning Guided by Bias-Noise-Alignment Diagnostics](https://arxiv.org/abs/2512.24445)
*Akash Samanta,Sheldon Williamson*

Main category: cs.LG

TL;DR: 提出诊断驱动的自适应学习框架，通过误差演化分解解决非平稳安全关键环境中的学习不稳定性问题


<details>
  <summary>Details</summary>
Motivation: 非平稳和安全关键环境中部署的学习系统在学习动态随时间演变时，常面临不稳定性、收敛缓慢或适应性脆弱的问题，而现有方法大多忽略误差信号本身的时间结构

Method: 提出诊断驱动的自适应学习框架，将误差演化显式建模为偏差（捕获持续漂移）、噪声（捕获随机变异性）和对齐（捕获导致过冲的重复方向激励）的分解；在线从损失或时间差分误差轨迹的轻量级统计量计算这些诊断信息，且独立于模型架构或任务领域；基于该框架推导诊断驱动实例，包括稳定监督优化器、诊断调节的Actor-Critic方案和诊断条件的学习优化器

Result: 所提出的偏差-噪声-对齐分解为监督优化、Actor-Critic强化学习和学习优化器提供统一控制骨干；在标准平滑性假设下，所有案例均建立有界有效更新和稳定性属性；Actor-Critic学习中的代表性诊断示例突出了所提出信号如何根据时间差分误差结构调节适应性

Conclusion: 本工作将误差演化提升为自适应学习中的一等对象，为动态环境中的可靠学习提供可解释、轻量级的基础
...
### [116] [Generative forecasting with joint probability models](https://arxiv.org/abs/2512.24446)
*Patrick Wyrod,Ashesh Chattopadhyay,Daniele Venturi*

Main category: cs.LG

TL;DR: 本研究针对混沌动力系统预测的局限性，提出一种将预测重构为完全生成问题的方法，通过学习滞后系统状态的联合概率分布并边缘化获得预测，同时引入模型无关的训练推理框架及不确定性量化指标，在Lorenz-63和Kuramoto-Sivashinsky系统上验证了其优于传统条件下一步模型的性能。


<details>
  <summary>Details</summary>
Motivation: 混沌动力系统对初始条件高度敏感且存在未解析的多尺度过程，导致确定性预测存在根本局限性；现有生成模型多聚焦于下一步条件预测而非底层动力学结构。

Method: 将预测重构为完全生成问题，学习短时间窗口内滞后系统状态的联合概率分布，通过边缘化获得预测；引入通用的、模型无关的联合生成预测训练与推理框架；使用三种互补的不确定性量化指标（集成方差、短时间范围自相关、累积Wasserstein漂移）评估预测鲁棒性和可靠性，无需真实值。

Result: 在Lorenz-63系统和Kuramoto-Sivashinsky方程两个典型混沌动力系统上评估，联合生成模型相比传统条件下一步模型，具有更优的短期预测技能、保留了吸引子几何结构、长程统计行为更准确。

Conclusion: 联合生成预测方法能够捕捉非线性时间依赖关系，代表多步轨迹段，产生与学习到的联合分布一致的下一步预测；该方法在混沌动力系统预测中表现出优于传统模型的性能，且能有效评估预测的鲁棒性和可靠性。
...
### [117] [HOLOGRAPH: Active Causal Discovery via Sheaf-Theoretic Alignment of Large Language Model Priors](https://arxiv.org/abs/2512.24478)
*Hyunjun Kim*

Main category: cs.LG

TL;DR: 本文提出HOLOGRAPH框架，利用层论将大语言模型(LLMs)的先验因果知识与观测数据结合，解决因果发现中的可识别性限制问题，实验表明其在50-100变量任务上性能优异并提供理论基础


<details>
  <summary>Details</summary>
Motivation: 观测数据的因果发现受可识别性约束限制，现有利用LLMs作为先验因果知识源的方法依赖缺乏理论基础的启发式整合

Method: 引入HOLOGRAPH框架，通过层论将局部因果信念表示为变量子集上预层的截面；提出代数潜变量投影处理隐藏混杂因素，以及信念流形上的自然梯度下降进行原则性优化

Result: 在合成和真实世界基准测试中，HOLOGRAPH在50-100变量的因果发现任务上取得竞争性性能；层论分析显示Identity、Transitivity和Gluing公理满足数值精度(<1e-6)，但Locality公理在大图中失效

Conclusion: HOLOGRAPH为LLM引导的因果发现提供了严格的数学基础，其层论分析揭示了潜变量投影中的基本非局部耦合现象
...
### [118] [More Than Bits: Multi-Envelope Double Binary Factorization for Extreme Quantization](https://arxiv.org/abs/2512.24545)
*Yuma Ichikawa,Yoshihiko Fujisawa,Yudai Fujimoto,Akira Sakai,Katsuki Fujisawa*

Main category: cs.LG

TL;DR: 针对大语言模型极端低位量化，提出多包络双二进制分解(MDBF)以解决原有DBF缩放参数限制导致的性能饱和问题，通过引入秩-l包络提升幅度表达能力并优化初始化与精炼方法，在匹配位宽下提升LLaMA和Qwen家族模型的困惑度与零样本精度


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型极端低位量化中，双二进制分解(DBF)虽能实现高效推理且不牺牲精度，但缩放参数过于严格：分解符号后所有秩分量共享相同幅度分布，导致性能饱和

Method: 提出多包络双二进制分解(MDBF)，保留共享的1位符号基，将单包络替换为秩-l包络；通过在包络分量间共享符号矩阵，维持二进制载体并利用有限内存预算提升幅度表达能力；同时引入闭式初始化和交替精炼方法优化MDBF

Result: 在LLaMA和Qwen家族模型上，MDBF在匹配每权重位数的情况下，相比之前的二进制格式提升了困惑度和零样本精度，同时保留了相同的部署友好推理原语

Conclusion: MDBF有效解决了原有DBF的性能饱和问题，在极端低位量化场景下实现了精度与推理效率的平衡，为大语言模型的高效部署提供了新方案
...
### [119] [From Perception to Punchline: Empowering VLM with the Art of In-the-wild Meme](https://arxiv.org/abs/2512.24555)
*Xueyan Li,Yingyi Xue,Mengjie Jiang,Qingzi Zhu,Yazhe Niu*

Main category: cs.LG

TL;DR: 本文提出HUMOR框架，通过分层推理和群体偏好对齐提升VLMs生成幽默 memes 的能力，在推理多样性、偏好对齐和整体质量上表现优异，并为开放式人类对齐多模态生成提供通用范式。


<details>
  <summary>Details</summary>
Motivation: 生成幽默 memes 是一项具有挑战性的多模态任务，需超越直接图像到标题的监督，对视觉内容、上下文线索和主观幽默进行细致推理，现有方法在视觉感知与幽默笑点创建间存在差距。

Method: 提出HUMOR框架：1. 采用分层多路径思维链（CoT），从模板级意图出发，探索不同上下文下的多样推理路径，最终锚定高质量上下文特定路径；2. 训练基于相同模板 meme 组的成对奖励模型，通过群体强化学习优化确保单调改进。

Result: 实验表明HUMOR使多种VLMs在推理多样性、偏好对齐可靠性和整体 meme 质量上均有优异表现。

Conclusion: HUMOR框架有效提升VLMs生成幽默 memes 的能力，其分层推理和群体偏好对齐方法为开放式、人类对齐的多模态生成提供了通用训练范式。
...
### [120] [CPR: Causal Physiological Representation Learning for Robust ECG Analysis under Distribution Shifts](https://arxiv.org/abs/2512.24564)
*Shunbo Jia,Caizhi Liao*

Main category: cs.LG

TL;DR: 针对心电图（ECG）深度学习模型易受对抗性扰动影响的问题，提出因果生理表示学习（CPR）方法，结合生理结构先验和因果解耦框架，在PTB-XL数据集上实现了鲁棒性、效率和临床可解释性的平衡。


<details>
  <summary>Details</summary>
Motivation: 心电图深度学习模型虽准确率高，但对模仿生物形态的平滑对抗性扰动（SAP）脆弱；现有防御方法存在计算负担重或推理延迟高的问题，根源在于模型依赖非鲁棒的虚假关联而非不变病理特征。

Method: 提出因果生理表示学习（CPR），通过结构因果模型（SCM）建模ECG生成过程，在因果解耦框架中融入生理结构先验，严格分离不变病理形态（P-QRS-T复合体）与非因果伪影。

Result: 在PTB-XL数据集上，CPR显著优于标准临床预处理方法；SAP攻击下F1得分为0.632，比中值平滑（0.541）高9.1%；与随机平滑的认证鲁棒性相当，同时保持单遍推理效率。

Conclusion: CPR在鲁棒性、效率和临床可解释性之间实现了更优权衡，为ECG诊断模型的对抗性防御提供了有效解决方案。
...
### [121] [Dynamic Large Concept Models: Latent Reasoning in an Adaptive Semantic Space](https://arxiv.org/abs/2512.24617)
...
TL;DR: 本文提出动态大概念模型（DLCM），通过从潜在表示中学习语义边界，将计算从token转移到压缩概念空间，以解决大语言模型（LLMs）对所有token应用统一计算导致的效率问题，并引入压缩感知缩放律和解耦μP参数化方法，在匹配推理FLOPs下，12个零样本基准测试平均提升2.69%


<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLMs）对所有token应用统一计算，而语言信息密度高度不均匀，导致在局部可预测片段上浪费计算能力，同时对语义关键转换分配不足

Method: 提出动态大概念模型（DLCM），端到端发现变长概念；引入压缩感知缩放律，分离token级容量、概念级推理容量和压缩比；开发解耦μP参数化方法，支持跨宽度和压缩机制的零样本超参数转移

Result: 在R=4（平均每个概念4个token）的实际设置下，DLCM将约三分之一的推理计算重新分配到更高容量的推理主干，在匹配推理FLOPs下，12个零样本基准测试平均提升2.69%

Conclusion: DLCM通过将计算从token转移到压缩概念空间，有效提升了大语言模型的推理效率和性能，压缩感知缩放律和解耦μP参数化为异构架构的稳定训练和计算分配提供了原则性方法
...
### [122] [AutoFed: Manual-Free Federated Traffic Prediction via Personalized Prompt](https://arxiv.org/abs/2512.24625)
*Zijian Zhao,Yitong Shang,Sen Li*

Main category: cs.LG

TL;DR: 本文提出AutoFed框架，通过联邦表示器和客户端对齐适配器生成全局共享提示矩阵，无需手动调参，在交通预测任务中实现跨客户端知识共享与本地特异性保持，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 智能交通系统需要准确的交通预测，但交通数据隐私问题导致现有方法依赖本地训练形成数据孤岛；联邦学习虽能隐私保护协作训练，但存在非独立同分布（non-IID）问题；现有个性化联邦学习框架需针对交通预测任务优化，且依赖跨数据集超参数优化，现实中难以部署。

Method: 提出AutoFed框架，受提示学习启发，引入联邦表示器，采用客户端对齐适配器将本地数据提炼为紧凑的全局共享提示矩阵，该提示调节个性化预测器，使每个客户端受益于跨客户端知识同时保持本地特异性。

Result: 在真实世界数据集上的广泛实验表明，AutoFed在不同场景下持续取得优异性能。

Conclusion: AutoFed框架有效解决了交通预测中个性化联邦学习的超参数调参问题，实现了隐私保护下的跨客户端知识共享与本地性能提升，具有实际部署价值。
...
### [123] [Nested Learning: The Illusion of Deep Learning Architectures](https://arxiv.org/abs/2512.24695)
*Ali Behrouz,Meisam Razaviyayn,Peilin Zhong,Vahab Mirrokni*

Main category: cs.LG

TL;DR: 本文提出嵌套学习（NL）范式，通过嵌套、多层次和/或并行优化问题表示机器学习模型，旨在解决持续学习、自我改进等挑战，并展示了其在表达性优化器、自修改学习模块和连续记忆系统方面的贡献及应用效果。


<details>
  <summary>Details</summary>
Motivation: 尽管语言模型等领域取得进展，但机器学习模型在持续学习/记忆、自我改进和有效解决方案寻找方面仍面临基础挑战和未解决问题。

Method: 提出嵌套学习（NL）范式，将模型表示为嵌套、多层次和/或并行优化问题；开发表达性优化器（如基于梯度的优化器作为关联记忆模块）；设计自修改学习模块（学习自身更新算法的序列模型）；构建连续记忆系统（泛化长/短期记忆）；结合自修改序列模型与连续记忆系统形成持续学习模块Hope。

Result: 基于NL的Hope模块在语言建模、知识整合、少样本泛化任务、持续学习和长上下文推理任务中显示出有前景的结果；表达性优化器验证了梯度基优化器的记忆特性并提出更优变体；自修改模块实现了模型自我更新能力；连续记忆系统泛化了传统记忆观点。

Conclusion: 嵌套学习范式为设计更具表达力的学习算法提供了哲学指导，可实现高阶上下文学习和潜在的有效持续学习能力；其核心贡献（表达性优化器、自修改模块、连续记忆系统）及Hope模块的应用证明了NL的有效性和潜力。
...
### [124] [Causal Discovery with Mixed Latent Confounding via Precision Decomposition](https://arxiv.org/abs/2512.24696)
*Amir Asiaee,Samhita Pal,James O'quinn,James P. Long*

Main category: cs.LG

TL;DR: 本文针对线性高斯系统中混合潜在混淆（既有全局潜在因素又有局部潜在因素）下的因果发现问题，提出了DCL-DECOR方法，通过分解精度矩阵分离全局混淆效应，再应用相关噪声DAG学习器恢复有向边并保证无弓结构，实验证明该方法在有向边恢复上优于直接应用相关噪声DAG学习的方法。


<details>
  <summary>Details</summary>
Motivation: 线性高斯系统中混合潜在混淆（部分未观测因素影响多变量，部分仅影响小子集）是实际常见场景，但现有方法存在缺陷：可微分和基于分数的DAG学习器易将全局潜在效应误判为因果边，而潜在变量图模型仅能恢复无向结构。

Method: 提出DCL-DECOR模块化方法：1. 分解观测精度矩阵为结构化分量（对应去除全局混淆后的条件分布，保留因果图和局部混淆的局部依赖）和低秩分量（全局潜在效应）；2. 对去混淆后的表示应用相关噪声DAG学习器恢复有向边，同时建模剩余结构化误差相关性；3. 通过简单协调步骤确保无弓结构。

Result: 理论上提供了混合混淆下可恢复因果目标的可识别性结果，表明整体问题可分解为有模块化保证的子问题；合成实验中，在不同强度和维度的全局混淆场景下，该方法在有向边恢复上相比直接应用相关噪声DAG学习的方法有持续改进。

Conclusion: DCL-DECOR方法能有效处理线性高斯系统中的混合潜在混淆问题，通过模块化设计分离全局和局部混淆效应，实现更准确的有向因果边恢复，具有理论可识别性保证和实验有效性。
...
### [125] [BandiK: Efficient Multi-Task Decomposition Using a Multi-Bandit Framework](https://arxiv.org/abs/2512.24708)
*András Millinghoffer,András Formanek,András Antos,Péter Antal*

Main category: cs.LG

TL;DR: 针对多任务学习中辅助任务集选择的计算成本高、候选集数量大及跨目标任务复杂度变化等问题，提出BandiK三阶段多任务辅助任务子集选择方法，通过多臂老虎机框架提升效率。


<details>
  <summary>Details</summary>
Motivation: 多任务学习中，有效跨任务知识迁移的传递性与非传递性本质仍是开放问题，负迁移是显著障碍；辅助任务集选择面临计算成本高、候选集数量大及跨目标任务复杂度变化等约束。

Method: BandiK分三阶段：1）估计任务间成对迁移以识别可联合学习的任务；2）基于初始估计为每个目标任务构建线性数量的辅助任务候选集；3）为每个任务采用多臂老虎机（MAB）框架，将个体任务MAB整合为多老虎机结构，利用半重叠臂属性优化成本/奖励结构。
...
### [126] [FPGA Co-Design for Efficient N:M Sparse and Quantized Model Inference](https://arxiv.org/abs/2512.24713)
*Fen-Yu Hsieh,Yun-Chang Teng,Ding-Yong Hong,Jan-Jan Wu*

Main category: cs.LG

TL;DR: 本文提出了一个结合权重剪枝和低比特量化的自动化框架及软硬件协同设计方法，以解决大语言模型（LLMs）在资源受限环境中部署的计算和内存问题，并在多种硬件平台上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLMs）在多种语言处理任务中表现优异，但高计算和内存需求严重阻碍了其在资源受限环境中的部署，因此需要高效的优化方法来提升LLMs的推理效率。

Method: 本文引入了一个自动化框架，结合N:M结构化剪枝和4比特整数量化，并提出软硬件协同设计方法生成FPGA加速器；实现了统一流水线，在CPU、NVIDIA GPU（含密集和2:4稀疏张量核）及基于 systolic-array 的定制FPGA加速器上应用2:4稀疏性和量化优化矩阵乘法。

Result: 在4096×4096矩阵上使用2:4稀疏性与量化，权重存储减少达4倍，矩阵乘法速度提升1.71倍，端到端延迟比密集GPU基线降低1.29倍；LLaMA-7B模型的扩展分析显示结构化稀疏性使每token吞吐量提升1.36倍。

Conclusion: 细粒度N:M稀疏性与量化的协同作用可实现高效且可部署的LLM推理，所提出的FPGA加速器提供了灵活的架构路径，支持超越固定2:4硬件约束的更广泛稀疏模式。
...
### [127] [From Trial to Deployment: A SEM Analysis of Traveler Adoptions to Fully Operational Autonomous Taxis](https://arxiv.org/abs/2512.24767)
*Yutong Cai,Hua Wang*

Main category: cs.LG

TL;DR: 本研究利用中国武汉百度阿波罗Robotaxi服务的实际用户调查数据，通过结构方程模型分析影响自动驾驶出租车采用行为的潜在心理构念及其影响，为自动驾驶出租车的规模化部署提供实证依据。


<details>
  <summary>Details</summary>
Motivation: 现有文献多通过陈述偏好实验和假设场景探讨自动驾驶出租车的用户接受度，但很少基于实际运营的自动驾驶汽车服务研究实际用户行为，本研究旨在填补这一空白。

Method: 本研究设计包含实际服务属性的真实调查，收集336份来自武汉百度阿波罗Robotaxi实际用户的有效回复，采用结构方程模型识别6个潜在心理构念（信任与政策支持、成本敏感性、性能、行为意向、生活方式、教育），并分析它们对自动驾驶出租车在10个场景中选择频率的影响。

Result: 结果显示成本敏感性和行为意向是采用行为最强的正向预测因素，其他潜在构念的作用更微妙；模型在多个指标上表现出良好的拟合优度。

Conclusion: 本研究结果为自动驾驶出租车在现实城市环境中的规模化部署提供了政策制定、票价设计和公众宣传策略的实证支持。
...
### [128] [Self-Supervised Neural Architecture Search for Multimodal Deep Neural Networks](https://arxiv.org/abs/2512.24793)
*Shota Suzuki,Satoshi Ono*

Main category: cs.LG

TL;DR: 本文提出一种用于多模态DNN架构搜索的自监督学习方法，解决了NAS在多模态任务中需要大量标注数据的问题


<details>
  <summary>Details</summary>
Motivation: 多模态DNN因结构复杂可从NAS中获益，但NAS构建多模态架构需大量标注训练数据，存在数据依赖问题

Method: 提出将自监督学习全面应用于架构搜索和模型预训练过程的方法

Result: 实验结果表明该方法能成功从无标注训练数据中为DNN设计架构

Conclusion: 该自监督学习方法有效支持了无需大量标注数据的多模态DNN架构搜索
...
### [129] [DTI-GP: Bayesian operations for drug-target interactions using deep kernel Gaussian processes](https://arxiv.org/abs/2512.24810)
*Bence Bolgár,András Millinghoffer,Péter Antal*

Main category: cs.LG

TL;DR: 提出DTI-GP架构，结合神经嵌入模块和高斯过程模块，用于药物-靶点相互作用预测，实现贝叶斯精度-置信度富集评分、拒绝方案及top-K选择与排序。


<details>
  <summary>Details</summary>
Motivation: 药物-靶点相互作用（DTI）预测的精确概率信息对理解局限性和提升预测性能至关重要。

Method: 提出基于深度核学习的高斯过程架构（DTI-GP），整合化合物和蛋白质靶点的神经嵌入模块与高斯过程模块，从预测分布采样估计贝叶斯优先矩阵。

Result: DTI-GP优于现有方案，可构建贝叶斯精度-置信度富集评分、通过拒绝方案提升富集效果、估计并搜索具有高预期效用的top-K选择与排序。

Conclusion: DTI-GP架构有效提升DTI预测性能，支持多种贝叶斯操作以优化预测应用。
...
### [130] [Unregularized Linear Convergence in Zero-Sum Game from Preference Feedback](https://arxiv.org/abs/2512.24818)
*Shulun Chen,Runlong Zhou,Zihan Zhang,Maryam Fazel,Simon S. Du*

Main category: cs.LG

TL;DR: 本文针对大语言模型（LLMs）的人类偏好对齐问题，提出了乐观乘法权重更新（OMWU）算法在纳什学习从人类反馈（NLHF）中的收敛性保证，无需假设纳什均衡（NE）唯一性，并通过实验验证了其在表格和神经策略类中的优势。


<details>
  <summary>Details</summary>
Motivation: 标准的人类偏好建模使用Bradley-Terry模型假设传递性，忽略了人类群体偏好的固有复杂性；现有NLHF算法依赖正则化，在计算原始游戏的对偶间隙时会产生不可避免的偏差。

Method: 使用乐观乘法权重更新（OMWU）算法，分析其在NLHF中的收敛性，当存在完全支持的NE时，证明其在预热阶段后实现最后迭代线性收敛，且收敛率依赖于实例，同时识别了一种新的边际收敛行为。

Result: OMWU算法在无需NE唯一性假设的情况下，对原始NE的对偶间隙实现实例依赖的线性收敛；其边际收敛行为使得对实例依赖常数的依赖性比先前结果指数级更好；实验证实了OMWU在表格和神经策略类中的理论优势。

Conclusion: OMWU算法在NLHF中具有良好的收敛性保证，且无需NE唯一性假设，其边际收敛行为带来了指数级的性能提升，在LLM应用中具有潜力。
...
TL;DR: 本研究训练语言模型进行开放式预测问题的预测，通过从日常新闻中合成新预测问题构建数据集OpenForesight，训练Qwen3模型得到OpenForecaster 8B，该模型在2025年5-8月的测试中匹配更大的专有模型，提升了预测的准确性、校准性和一致性，并开源所有资源。


<details>
  <summary>Details</summary>
Motivation: 高风险决策需要在未来不确定性下进行推理，现有语言模型预测研究缺乏大规模高质量数据集和有效的训练方法，且需避免未来信息泄露。

Method: 1. 从日常新闻中合成新的开放式预测问题，构建数据集OpenForesight；2. 使用离线新闻语料库生成数据和检索，防止未来信息泄露；3. 基于小验证集，采用检索增强和改进的强化学习奖励函数训练Qwen3模型；4. 在2025年5-8月进行保留测试。

Result: 1. 专用模型OpenForecaster 8B匹配更大的专有模型；2. 训练提高了预测的准确性、校准性和一致性；3. 预测训练带来的校准改进在流行基准测试中具有泛化性。

Conclusion: 本研究成功训练出高性能的语言模型预测系统OpenForecaster 8B，其性能可与更大的专有模型媲美，且校准改进具有泛化性；开源所有模型、代码和数据，促进语言模型预测研究的广泛开展。
...
### [132] [Discovering Coordinated Joint Options via Inter-Agent Relative Dynamics](https://arxiv.org/abs/2512.24827)
*Raul D. Steleac,Mohan Sridharan,David Abel*

Main category: cs.LG

TL;DR: 本文提出一种多智能体选项发现方法，通过联合状态抽象和神经图拉普拉斯估计器生成强协调行为的选项，在多个场景中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 多智能体环境中联合状态空间随智能体数量指数增长，现有多智能体选项发现方法常产生松散耦合或独立行为，缺乏强协调能力。

Method: 1. 提出联合状态抽象，压缩状态空间同时保留发现强协调行为所需信息；2. 近似与团队最大对齐的虚拟Fermat状态，定义spreadness度量以捕捉团队在各状态维度的错位；3. 使用神经图拉普拉斯估计器推导捕捉智能体间状态同步模式的选项。

Result: 在两个多智能体领域的多个场景中评估，结果显示所生成的选项比替代方法具有更强的下游协调能力。

Conclusion: 该方法有效解决了现有多智能体选项发现方法的局限性，能生成强协调行为的选项，提升多智能体系统的协调能力。
...
### [133] [AODDiff: Probabilistic Reconstruction of Aerosol Optical Depth via Diffusion-based Bayesian Inference](https://arxiv.org/abs/2512.24847)
*Linhao Fan,Hongqiang Fang,Jingyang Dai,Yong Jiang,Qixing Zhang*

Main category: cs.LG

TL;DR: 本文提出AODDiff框架，基于扩散贝叶斯推理实现AOD场的概率重建，解决数据稀缺和不确定性量化问题，在降尺度和修复任务中表现出色


<details>
  <summary>Details</summary>
Motivation: 大气监测中高质量AOD场重建至关重要，但现有模型受限于完整训练数据稀缺和缺乏不确定性量化

Method: 提出AODDiff概率重建框架：1. 采用腐败感知训练策略从自然不完整数据中学习时空AOD先验；2. 使用解耦退火后验采样策略整合异构观测作为约束引导生成过程

Result: 在再分析数据上的实验表明，AODDiff在降尺度和修复任务中有效且鲁棒，保持高空间光谱保真度；作为生成模型，通过多次采样实现不确定性量化，为下游应用提供置信度指标

Conclusion: AODDiff框架能灵活适应多种重建任务，无需任务特定重训练，在AOD场重建中具有高效性、鲁棒性和不确定性量化能力
...
### [134] [Characterization of Transfer Using Multi-task Learning Curves](https://arxiv.org/abs/2512.24866)
*András Millinghoffer,Bence Bolgár,Péter Antal*

Main category: cs.LG

TL;DR: 本文提出通过数据扰动而非模型扰动来表征迁移效应，并使用多任务学习曲线定量建模归纳性能，同时提出高效近似方法，通过药物靶点相互作用数据集评估，结果表明学习曲线能更好捕捉多任务学习效应及其扩展可描述基础模型中的成对和上下文迁移效应


<details>
  <summary>Details</summary>
Motivation: 迁移效应在固定数据集训练和累积数据归纳推理中均有体现，现有通过梯度更新扰动模型的方法存在局限性，需更基础的表征方式

Method: 使用多任务学习曲线定量建模迁移效应，提出类似训练期间任务亲和分组方法的高效近似多任务学习曲线方法，并比较统计与计算迁移方法

Result: 统计方法计算成本更高，但功效更好且适用性更广；学习曲线能更好捕捉多任务学习效应，其多任务扩展可描绘基础模型中的成对和上下文迁移效应

Conclusion: 通过数据扰动（增加样本）表征迁移效应是互补且更基础的方式，学习曲线及其扩展可有效捕捉和描述多任务学习中的迁移效应
...
### [135] [MSACL: Multi-Step Actor-Critic Learning with Lyapunov Certificates for Exponentially Stabilizing Control](https://arxiv.org/abs/2512.24955)
...
<details>
  <summary>Details</summary>
Motivation: 无模型强化学习中实现可证明的稳定性仍具挑战，尤其是在探索与严格安全性之间的平衡方面

Method: MSACL框架整合指数稳定性理论与最大熵RL，利用离策略多步数据学习满足理论稳定性条件的Lyapunov证书，引入指数稳定性标签（ESL）和λ加权聚合机制平衡偏差-方差权衡，通过稳定性感知优势函数引导策略优化

Result: 在六个基准任务（包括稳定化和非线性跟踪任务）上评估，MSACL优于最先进的基于Lyapunov的RL算法，实现指数稳定性和快速收敛，对不确定性具有显著鲁棒性并能泛化到未见过的轨迹，敏感性分析确定多步 horizon n=20为不同系统的稳健默认值

Conclusion: MSACL通过将Lyapunov理论与离策略actor-critic框架联系起来，为可验证的安全基于学习的控制提供基础，源代码和基准环境将公开
...
### [136] [Attribution-Guided Distillation of Matryoshka Sparse Autoencoders](https://arxiv.org/abs/2512.24975)
*Cristina P. Martin-Linares,Jonathan P. Ling*

Main category: cs.LG

TL;DR: 本文提出蒸馏套娃稀疏自动编码器（DMSAEs）训练流水线，解决稀疏自动编码器（SAEs）特征冗余且跨训练运行和稀疏度水平不一致的问题，通过迭代蒸馏得到一致有用的紧凑核心特征并复用，提升SAE性能和特征可迁移性。


<details>
  <summary>Details</summary>
Motivation: 稀疏自动编码器（SAEs）旨在将模型激活分解为单语义、可解释的特征，但实际中学到的特征常冗余，且跨训练运行和稀疏度水平存在差异，导致解释难以迁移和复用。

Method: 提出蒸馏套娃稀疏自动编码器（DMSAEs）训练流水线，包含迭代蒸馏循环：训练共享核心的套娃SAE，用梯度×激活测量每个特征对最嵌套重构中下一个token损失的贡献，保留解释固定比例归因的最小子集；仅核心编码器权重向量跨循环迁移，核心解码器和所有非核心潜在变量每次重新初始化。

Result: 在Gemma-2-2B第12层残差流激活上，经过7次蒸馏循环（500M tokens、65k宽度）得到含197个重复选中特征的蒸馏核心；使用该核心训练提升多个SAEBench指标，证明一致的潜在特征集可跨稀疏度水平迁移。

Conclusion: 蒸馏套娃稀疏自动编码器（DMSAEs）能有效提取一致有用的紧凑核心特征，复用该核心可提升SAE性能，且一致特征集可跨稀疏度水平迁移，为SAE特征的稳定解释和复用提供解决方案。
...
### [137] [Efficiently Estimating Data Efficiency for Language Model Fine-tuning](https://arxiv.org/abs/2512.24991)
*Gyung Hyun Je,Colin Raffel*

Main category: cs.LG

TL;DR: 本文提出一种基于低置信度示例梯度余弦相似度的方法，可利用少量标注样本预测任务的数据效率，减少不必要的标注工作


<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLMs）微调能提升性能，但任务的数据效率（即达到目标性能所需的微调样本数）常未知，导致增量标注和重训练成本高昂；此外，高性能LLMs在零样本下可能表现不佳，但微调后性能显著提升，因此需要无需增量标注即可预测数据效率的方法

Method: 首先定义量化任务数据效率的具体指标，然后提出使用低置信度示例的梯度余弦相似度，基于少量标注样本预测任务的数据效率

Result: 在多种具有不同数据效率的任务上验证了该方法，数据效率预测总体误差为8.6%，且每个任务通常可减少数百个不必要的标注

Conclusion: 该方法能有效预测任务的数据效率，减少标注成本，实验结果和代码已开源至GitHub
...
### [138] [Diffusion Language Models are Provably Optimal Parallel Samplers](https://arxiv.org/abs/2512.25014)
*Haozhe Jiang,Nika Haghtalab,Lijie Chen*

Main category: cs.LG

TL;DR: 扩散语言模型（DLMs）通过并行生成token实现更快推理，本文从理论上证明了带多项式长度思维链（CoT）的DLM可模拟任何并行采样算法且序列步数最优，加入重掩蔽或修订后可进一步优化空间复杂度，并存在表达能力差距。


<details>
  <summary>Details</summary>
Motivation: DLMs作为自回归模型的替代方案，其并行token生成的推理速度优势缺乏严格理论基础，且无修改已揭示token能力时存在较大中间足迹问题。

Method: 形式化并行采样模型，证明带多项式长度CoT的DLM可模拟任何并行采样算法且序列步数最优；证明加入重掩蔽或修订的DLM可模拟任何并行采样算法且空间复杂度最优；建立表达能力差距的严格证明。

Result: 带CoT的DLM可实现并行采样的最优序列步数；加入重掩蔽或修订的DLM可实现最优空间复杂度；带修订或重掩蔽的DLM比无此功能的DLM表达能力更强。

Conclusion: DLMs作为最高效并行采样器的潜力得到理论证明，且应在DLMs中启用修订功能。
...
### [139] [Generative Classifiers Avoid Shortcut Solutions](https://arxiv.org/abs/2512.25034)
*Alexander C. Li,Ananya Kumar,Deepak Pathak*

Main category: cs.LG

TL;DR: 本文研究表明生成式分类器可避免判别式分类器依赖虚假相关特征的问题，在分布偏移基准上表现优异，并通过高斯玩具模型分析其归纳偏置。


<details>
  <summary>Details</summary>
Motivation: 判别式分类方法常学习在分布内成立但在微小分布偏移下失效的捷径，这源于对与标签虚假相关特征的过度依赖。

Method: 使用类条件生成模型构建生成式分类器，无需专门的数据增强、强正则化、额外超参数或特定虚假相关知识；采用扩散和自回归生成式分类器，并通过高斯玩具模型分析归纳偏置。

Result: 生成式分类器在五个标准图像和文本分布偏移基准上达到最先进性能，减少了医疗或卫星数据集等实际应用中虚假相关的影响；高斯玩具模型分析揭示了生成式分类器的归纳偏置及优于判别式分类器的数据特性。

Conclusion: 生成式分类器通过建模所有特征（核心和虚假）可有效避免判别式分类器的捷径问题，在分布偏移场景下表现出色，其性能取决于数据特性，需进一步理解归纳偏置以优化应用。
...
### [140] [On the geometry and topology of representations: the manifolds of modular addition](https://arxiv.org/abs/2512.25060)
*Gabriela Moisescu-Pareja,Gavin McCracken,Harley Wiltzer,Vincent Létourneau,Colin Daniels,Doina Precup,Jonathan Love*

Main category: cs.LG

TL;DR: 本研究表明，采用均匀注意力和可学习注意力的架构在实现模块化加法时使用相同算法，其表示在拓扑和几何上等价。


<details>
  <summary>Details</summary>
Motivation: 之前的Clock和Pizza解释认为不同架构设计会产生不同的模块化加法电路，本研究旨在验证这一观点是否正确。

Method: 超越单个神经元和权重的解释，识别每个学习表示对应的所有神经元并将其作为整体研究，利用拓扑工具分析学习表示的流形，并对数百个电路的学习表示进行统计分析。

Result: 发现均匀注意力和可学习注意力架构的学习表示在拓扑和几何上等价，且从常见深度学习范式自然产生的模块化加法电路具有相似性。

Conclusion: 不同注意力架构（均匀或可学习）实现模块化加法时采用相同算法，其学习表示等价，挑战了之前关于架构设计差异导致电路不同的观点。
