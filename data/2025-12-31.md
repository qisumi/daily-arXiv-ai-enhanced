...
TL;DR: 本文介绍了完全开源的大语言模型Moxin 7B及其三个变体（Moxin-VLM、Moxin-VLA、Moxin-Chinese），它们在不同任务中表现优异，并开源了模型、数据和代码。


<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLMs）快速发展，闭源模型性能强但缺乏透明度，开源模型虽易定制但未实现训练、数据集和细节的完全透明，需要更包容协作的开源生态。

Method: 开发完全开源的Moxin 7B模型，遵循模型开放框架，实现训练、数据集和实现细节的完全透明；基于Moxin开发三个变体，分别针对视觉语言、视觉语言动作和中文能力；采用开源框架和开放数据训练，并发布模型、数据和代码。

Result: Moxin及其变体在多种评估中取得了优异的性能。

Conclusion: Moxin 7B及其变体通过完全开源促进了更包容协作的研究环境，支持健康的开源生态系统，且在不同任务中表现出色，开源资源有助于后续研究。
...
TL;DR: 本研究探究Transformer模型的句子嵌入是否编码了与人类可解释认知属性对齐的分级层次结构，结果表明其存在这种结构且非仅源于表面词汇统计


<details>
  <summary>Details</summary>
Motivation: 现有研究显示Transformer语言模型嵌入空间存在丰富几何结构，但其中是否存在更高层次的认知组织尚未充分探索

Method: 构建含480个自然语言句子的数据集（标注连续序数能量得分和7个有序认知类别的离散层级标签），使用多个Transformer模型的固定句子嵌入，通过线性和浅层非线性探针评估标注可恢复性，并与词汇TF-IDF基线对比，结合非参数置换检验、UMAP可视化和混淆矩阵进行定性分析

Result: 1. 跨模型的连续得分和层级标签均可可靠解码，浅层非线性探针性能优于线性探针；2. TF-IDF基线性能显著更差；3. 置换检验确认探针性能超随机水平；4. UMAP可视化和混淆矩阵显示嵌入空间存在平滑梯度和相邻层级混淆

Conclusion: Transformer嵌入空间表现出与人类定义认知属性对齐的层次几何组织，但未涉及内部意识或现象学相关主张
...
TL;DR: 本文提出SmartSnap范式，将传统被动事后任务验证转变为智能体主动原位自验证，通过3C原则引导智能体生成关键快照证据，提升LLM驱动智能体在复杂GUI任务中的可扩展性和性能。


<details>
  <summary>Details</summary>
Motivation: 现有智能体强化学习在复杂GUI任务中的可扩展性受限于被动事后任务验证方式，冗长且含噪声的交互轨迹导致验证成本高、可靠性低，成为瓶颈。

Method: 提出SmartSnap范式，设计具有双重任务的自验证智能体（完成任务+用精选快照证据证明完成），遵循3C原则（完整性、简洁性、创造性），利用在线环境获取最小决定性快照集作为LLM裁判的唯一验证材料。

Result: 在跨模型家族和规模的移动任务实验中，SmartSnap范式使8B和30B模型性能分别提升26.08%和16.66%，其解决方案与证据寻找的协同效果使智能体性能可与DeepSeek V3.1和Qwen3-235B-A22B竞争。

Conclusion: SmartSnap范式实现了LLM驱动智能体的可扩展训练，通过主动自验证和关键快照证据提升了任务完成性能和验证效率，为自主智能体的发展提供了新方向。
...
TL;DR: 本研究在最简方案框架下探讨也门伊比阿拉伯语（YIA）中qulk从句的句法结构，提出其为双分句结构并解释方言特定特征，为生成句法提供理论贡献。


<details>
  <summary>Details</summary>
Motivation: 研究动机是分析也门伊比阿拉伯语中qulk从句的句法结构，该结构是形态融合形式（意为“我说”），可引入嵌入式陈述、疑问和祈使从句且常无补语词，需在最简方案下解析其句法特性。

Method: 采用最简方案核心操作（合并、移动、一致、拼写），对qulk从句进行分层句法分析，展示其通过标准计算步骤及形态合并等后句法过程的推导，并解释方言特定特征（如二分否定、附着化、CP嵌入）。

Result: 发现qulk从句为双分句结构，qulk作为从句嵌入谓词选择完整CP补语；该分析能解释方言特定特征，为生成句法（尤其是最简主义）提供理论贡献。

Conclusion: 研究提出qulk从句的双分句结构分析，引发将分析扩展至受话者从句kil-k（“你说”）的理论问题，并为最简主义的普遍性提供见解。
...
TL;DR: 提出一种名为Fourier-Activated Adapter（FAA）的新型框架，用于大型预训练语言模型的参数高效微调，通过将随机傅里叶特征融入轻量级适配器模块，实现频率感知的语义信息调制，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 为了在大型预训练语言模型的微调过程中，以较低的计算和内存开销实现高效且高性能的参数调整，同时解决现有方法可能存在的频率信息利用不足的问题。

Method: 提出Fourier-Activated Adapter（FAA）框架，将随机傅里叶特征整合到轻量级适配器模块中，分解中间表示为互补的低频和高频分量，实现频率感知的语义信息调制，同时保持冻结主干的表示能力。

Result: 在GLUE、E2E NLG和指令调优基准测试中，FAA持续取得与现有参数高效微调方法相比具有竞争力或更优的性能，同时保持较低的计算和内存开销；消融研究验证了频率感知激活和自适应加权机制的有效性。

Conclusion: FAA是一种鲁棒且高效的大型语言模型后训练方法，其频率感知激活和自适应加权机制显著提升了参数高效微调的性能，为大型预训练语言模型的高效适应提供了新的有效途径。
...
TL;DR: 本文提出LLM引导的样本选择框架，解决HAR依赖大标签数据集和纯几何样本选择的局限，在UCI-HAR数据集少样本条件下实现88.78%的宏F1分数。


<details>
  <summary>Details</summary>
Motivation: 现有HAR方法依赖大标签数据集和纯几何样本选择，难以区分步行、上楼、下楼等相似可穿戴传感器活动。

Method: 整合LLM生成的知识先验（特征重要性、类间混淆性、样本预算乘数），结合基于边际的验证线索、PageRank中心性、中心性惩罚和设施位置优化进行样本评分与选择。

Result: 在UCI-HAR数据集严格少样本条件下，框架宏F1分数达88.78%，优于随机采样、herding和k-center等经典方法。

Conclusion: LLM衍生的语义先验与结构和几何线索结合，为少样本可穿戴传感器HAR中选择代表性传感器样本提供更强基础。
...
TL;DR: 针对大语言模型幻觉问题，现有评估方法成本高，本文集成轻量级HHEM框架提升效率并保持准确性，分析不同模型幻觉情况及改进策略。


<details>
  <summary>Details</summary>
Motivation: 大语言模型幻觉生成误导性内容，现有评估方法如KnowHalu计算成本高，需高效且准确的检测框架。

Method: 集成轻量级分类框架HHEM，独立于LLM判断；对不同LLM的幻觉检测方法进行比较分析，评估TPR、TNR和准确性；引入基于片段的检索改进局部幻觉检测；使用CDF分析模型参数与幻觉的关系。

Result: HHEM将评估时间从8小时降至10分钟；带非伪造检查的HHEM达到最高准确率82.2%和TPR78.9%；HHEM在摘要任务中对局部幻觉检测效果差；7B-9B参数大模型幻觉较少，中等规模模型稳定性低。

Conclusion: 需结构化评估框架平衡计算效率与事实验证，以增强LLM生成内容的可靠性。
...
TL;DR: 本文提出HiFi-RAG系统，通过多阶段流水线解决开放域检索增强生成中的无关信息和意图对齐问题，在MMU-RAGent竞赛中获胜并显著提升性能


<details>
  <summary>Details</summary>
Motivation: 开放域检索增强生成（RAG）面临检索文档中无关信息过多以及生成答案与用户意图对齐不足的挑战

Method: 采用多阶段流水线方法：利用Gemini 2.5 Flash进行查询构建、分层内容过滤和引用归因（成本效率比Pro版高4-6倍），同时保留Gemini 2.5 Pro的推理能力用于最终答案生成

Result: 在MMU-RAGent验证集上，ROUGE-L提升至0.274（+19.6%），DeBERTaScore提升至0.677（+6.2%）；在Test2025数据集（需2025年1月后知识的问题）上，ROUGE-L优于参数基线57.4%，DeBERTaScore优于14.9%

Conclusion: HiFi-RAG系统有效解决了开放域RAG的关键挑战，在竞赛中表现优异并在需要最新知识的场景下显著超越基线
...
TL;DR: 本文研究大型语言模型（LLMs）在会计领域的推理能力，提出垂直领域会计推理概念及评估标准，评估多个模型的会计推理任务表现并指出改进方向。


<details>
  <summary>Details</summary>
Motivation: LLMs正在重塑多领域学习范式、认知过程和研究方法，但将其与专业领域整合并重新定义LLMs与特定领域应用的关系是推动企业数字化转型和社会发展的关键挑战；为有效整合LLMs到会计领域，需理解其领域特定推理能力。

Method: 通过分析代表性GLM系列模型的训练数据特征，提出垂直领域会计推理概念并建立评估标准；基于该框架，在会计推理任务集上评估GLM-6B、GLM-130B、GLM-4和OpenAI GPT-4等模型的表现。

Result: 不同提示工程策略对各模型性能有不同程度提升，GPT-4的会计推理能力最强；但当前LLMs仍未满足实际应用需求，尤其在企业级会计场景部署需进一步优化。

Conclusion: LLMs在会计领域有应用潜力，但需针对企业级会计场景进一步优化以充分实现其价值。
...
TL;DR: 本文探讨韩语选区树库设计中终端单元选择的问题，主张采用以语块（eojeol）为基础的选区表示法，并通过比较分析证明世宗和宾大韩语树库在此层面具有等效性，进而提出保留可解释选区并支持跨树库比较及选区-依存转换的标注方案


<details>
  <summary>Details</summary>
Motivation: 韩语单词形态复杂，将语素作为选区终端会混淆词内部形态与短语级句法结构，且与基于语块的依存资源不匹配，需解决韩语选区树库终端单元选择的根本表示问题

Method: 采用以语块（eojeol）为基础的选区表示法，将形态切分和细粒度词性信息编码在独立的非成分层；对世宗和宾大韩语树库进行比较分析（基于显式归一化假设）

Result: 在显式归一化假设下，世宗和宾大韩语树库在以语块为基础的选区层面可视为表示等效

Conclusion: 提出的以语块为基础的标注方案能够保留可解释的选区结构，支持跨树库比较及选区-依存转换
...
TL;DR: 提出All-or-Here Attention（AHA）机制，使大语言模型动态决定何时使用全局注意力或局部滑动窗口注意力，在不损失性能的情况下减少93%的全局注意力操作，并发现上下文依赖的长尾分布特性。


<details>
  <summary>Details</summary>
Motivation: 受人类阅读时主要关注当前页面、仅必要时回顾前文的启发，探索大语言模型如何动态决定何时关注全局上下文以提升效率。

Method: 提出AHA机制，每个注意力头配备一个二进制路由器，为每个token动态切换全局全注意力和局部滑动窗口注意力。

Result: 当窗口大小为256token时，可替换93%的原始全局注意力操作且无性能损失；评估不同窗口大小发现上下文依赖呈长尾分布，全局注意力需求随局部窗口扩大快速衰减。

Conclusion: 全局注意力在大语言模型中大多冗余，高效推理只需按需访问全局上下文，AHA机制通过解耦局部处理与全局访问实现这一点。
...
TL;DR: 本文针对多模态对话情感分析挑战的两个子任务提出解决方案：子任务I采用结构化提示管道引导大语言模型提取情感六元组，子任务II通过集成三个大语言模型识别情感翻转及触发因素，最终在两个子任务上分别取得47.38%平均得分和74.12%精确匹配F1值。


<details>
  <summary>Details</summary>
Motivation: 多模态对话中的情感理解是构建具有情感智能AI系统的复杂且关键挑战，MCABSA挑战包含两个高难度子任务：提取多说话者对话中的情感六元组及检测情感翻转和其触发因素。

Method: 子任务I：设计结构化提示管道，引导大语言模型（LLMs）以逐步细化的上下文理解顺序提取情感组件；子任务II：集成三个LLMs的互补优势，稳健识别情感转换及其触发因素。

Result: 系统在子任务I上获得47.38%的平均得分，在子任务II上获得74.12%的精确匹配F1值。

Conclusion: 分步细化策略和集成策略在丰富的多模态情感分析任务中具有有效性。
...
TL;DR: 本文提出T3LLM框架，通过三模型协作（worker、reviewer、student）实现带显式校正机制的多步推理，解决基于大语言模型的时间序列问答（TSQA）中复杂数值序列推理错误问题，并在多个真实基准上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的TSQA方法多采用通用自然语言处理技术，处理复杂数值序列时易出现推理错误；而时间序列数据具有可验证性，可用于检查推理步骤与原始输入的一致性，此特性为研究动机。

Method: 提出T3LLM框架，包含三个大语言模型：worker负责在结构化提示下生成分步思维链（CoT）；reviewer检查推理、识别错误步骤并提供校正意见；利用协作生成的校正后CoT微调student模型，使其内化多步推理和自校正能力。

Result: 在多个真实世界TSQA基准测试中，T3LLM的性能优于现有的强基于大语言模型的基线方法，达到了最先进水平。

Conclusion: T3LLM通过显式校正机制和三模型协作，有效提升了时间序列问答任务中的推理准确性，为处理复杂数值序列推理问题提供了新的解决方案。
...
TL;DR: 本文提出多粒度多语言代码生成评估框架M2G-Eval，涵盖4个结构粒度和18种语言，通过训练模型评估揭示代码生成能力的分层差异及跨语言迁移性。


<details>
  <summary>Details</summary>
Motivation: 现有代码大语言模型评估基准多限于单一结构粒度和少数编程语言，无法反映模型在不同代码范围和多语言场景下的细粒度能力差异。

Method: 提出M2G-Eval框架，包含Class、Function、Block、Line四个评估粒度及18种语言，构建17K+训练任务和1286个人工标注的无污染测试实例；训练M2G-Eval-Coder模型（基于Qwen3-8B的监督微调与Group Relative Policy Optimization）；评估30个模型（28个SOTA LLM及2个自研变体）。

Result: 发现三个关键结论：1）存在明显难度层级（Line级最易，Class级最难）；2）任务复杂度增加时，全粒度与部分粒度语言间的性能差距扩大；3）跨语言相关性强，模型学习到可迁移的编程概念。

Conclusion: M2G-Eval可实现代码生成能力的细粒度诊断，凸显复杂长代码合成的持续挑战。
...
TL;DR: 本文重新审视扩散语言建模，分析现有方法的结构权衡，并指出当前大扩散语言模型存在的两个核心问题，为更连贯的扩散语言模型研究提供方向。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在语言生成中具有并行解码和迭代优化等吸引人的特性，但文本的离散性和高度结构化特性对扩散原理的直接应用构成挑战。

Method: 本文从扩散过程和语言建模的角度重新审视扩散语言建模，概述区分扩散机制与语言特定需求的五个特性；将现有方法分为嵌入空间的连续扩散和令牌上的离散扩散两类；分析近期大扩散语言模型的问题。

Result: 现有扩散语言模型方法仅满足五个关键特性中的部分，存在结构权衡；当前大扩散语言模型存在两个核心问题：（1）均匀破坏不尊重信息在位置间的分布；（2）令牌级边际训练无法在并行解码期间捕捉多令牌依赖关系。

Conclusion: 这些观察结果推动扩散过程更紧密地与文本结构对齐，并鼓励未来研究更连贯的扩散语言模型。
...
TL;DR: 本文评估了GRPO和DPO两种优化方法对大语言模型CoT推理忠实性的提升效果，发现GRPO在较大模型上表现更优，为开发更透明可信的LLM推理提供了方向。


<details>
  <summary>Details</summary>
Motivation: 思维链（CoT）推理能提升大语言模型的问题解决能力，但CoT解释常无法反映模型真实推理过程，导致基于CoT的方法可靠性不足，难以用于安全监督和对齐监测。

Method: 评估Group Relative Policy Optimization（GRPO）和Direct Preference Optimization（DPO）两种优化方法对CoT忠实性的提升能力。

Result: GRPO在较大模型上的性能优于DPO，Qwen2.5-14B-Instruct模型在所有评估指标中取得最佳结果；两种方法的性能均与模型大小呈正相关，但GRPO在提升忠实性指标上潜力更大，不过在较小规模模型上行为较不稳定。

Conclusion: GRPO为开发大语言模型中更透明、更可信的推理提供了有前景的方向。
...
TL;DR: 本文针对大语言模型在高风险领域部署时的不确定性量化问题，提出词汇感知 conformal 预测（VACP）框架，通过语义掩码和温度调整评分在保证边缘覆盖率的同时大幅减小预测集大小。


<details>
  <summary>Details</summary>
Motivation: 在高风险领域部署大语言模型需要严格的不确定性量化，但标准 softmax 概率校准效果差；传统 conformal 预测虽能保证有效覆盖，但生成的预测集包含数百个token，缺乏实用性。

Method: 提出词汇感知 conformal 预测（VACP）框架，利用语义掩码和温度调整评分来减少有效预测空间，同时理论上保证边缘覆盖率。

Result: 在Gemma-2B模型上使用SQUAD和WikiText基准测试，VACP达到89.7%的经验覆盖率（目标90%），预测集平均大小从847个token降至4.3个token，效率提升197倍。

Conclusion: VACP框架在保证预测覆盖度的前提下显著提高了预测集的效率，为大语言模型在高风险领域的可靠部署提供了有效解决方案。
...
TL;DR: 本文提出了一个以乌尔都语为重点的多语言希望话语检测框架，使用预训练Transformer模型并在PolyHope-M 2025基准上取得良好性能。


<details>
  <summary>Details</summary>
Motivation: 希望话语在NLP中研究不足，现有研究多集中于英语，低资源语言（如乌尔都语）缺乏资源，基于Transformer的架构在仇恨和攻击性话语检测中有效但未广泛应用于希望话语或多语言场景。

Method: 使用XLM-RoBERTa、mBERT、EuroBERT和UrduBERT等预训练Transformer模型，进行简单预处理并训练分类器。

Result: 在PolyHope-M 2025基准上，乌尔都语二分类F1-score达95.2%，多分类为65.2%，西班牙语、德语和英语也取得竞争性结果。

Conclusion: 现有多语言模型可在低资源环境中实施，有助于识别希望话语并构建更具建设性的数字话语。
...
TL;DR: 本文提出一个人类验证框架，评估大语言模型在多语言环境下推理轨迹与结论的逻辑一致性，发现非拉丁脚本语言中推理与结论的错位程度是拉丁脚本的两倍以上，揭示现有多语言评估的不足并强调推理感知评估框架的必要性


<details>
  <summary>Details</summary>
Motivation: 大语言模型通过思维链提示展现出强大的推理能力，但推理质量是否跨语言迁移尚未得到充分探索

Method: 引入人类验证框架，分析来自GlobalMMLU数据集6种语言的65k推理轨迹及6个前沿模型，通过人类标注建立错误分类法

Result: 模型任务准确率高但推理可能无法支持结论；非拉丁脚本语言的推理与结论错位程度至少是拉丁脚本的两倍；错误主要源于证据错误（无支持主张、模糊事实），其次是不合逻辑的推理步骤

Conclusion: 现有多语言评估实践未能全面反映模型推理能力，需构建推理感知的评估框架
...
TL;DR: 本文研究了如何通过基于心理学的提示词优化减轻大语言模型（LLM）在模拟人群响应时的社会期望偏差（SDB），以提升硅采样与真实人类数据的对齐度。


<details>
  <summary>Details</summary>
Motivation: 大语言模型用于硅采样时，对社会敏感问题的响应常存在社会期望偏差，偏离真实人类数据，而现有针对LLM硅采样中社会期望偏差的研究有限。

Method: 使用美国全国选举研究（ANES）数据，在Llama-3.1系列和GPT-4.1-mini两个模型家族的三个LLM上开展研究：首先复现基线硅采样研究确认SDB存在；然后测试四种提示词缓解方法（重构式、反向编码式、启动式元指令、序言式元指令）；通过带 bootstrap 置信区间的 Jensen-Shannon 散度评估与ANES数据的对齐度。

Result: 重构式提示词最有效，通过减少社会可接受答案的分布集中，使分布更接近ANES；反向编码在合格项目中结果混合；启动式和序言式元指令导致响应同质化，无系统缓解偏差的益处。

Conclusion: 基于提示词的框架控制能有效缓解LLM固有的社会期望偏差，为获得更具代表性的硅采样提供了实用路径。
...
TL;DR: 本研究提出一种利用社交媒体数据（如Twitter）增强现有数据集的新方法，通过NLP管道识别孕妇并分类其妊娠结局，展示社交媒体数据作为妊娠结局流行病学调查辅助资源的可行性。


<details>
  <summary>Details</summary>
Motivation: 美国婴儿死亡率仍是重要公共卫生问题，出生缺陷是主要原因；尽管持续努力理解流产、死产等不良妊娠结局的原因，但仍需更全面的研究和干预策略。

Method: 构建自然语言处理（NLP）管道，自动识别分享妊娠经历的女性，根据报告结局分类：足月妊娠且正常出生体重为阳性案例，报告不良妊娠结局为阴性案例。

Result: 本研究展示了社交媒体数据作为妊娠结局流行病学调查辅助资源的可行性，提供了评估特定干预、治疗或产前暴露对母婴健康结局因果影响的潜在应用，以及未来涉及孕妇队列和对照组健康研究的框架。

Conclusion: 社交媒体数据可作为现有数据集的补充资源，用于妊娠结局的观察性研究，为流行病学调查和未来健康研究提供新的方法和框架。
...
TL;DR: 提出WeDLM扩散解码框架，基于因果注意力实现前缀缓存友好的并行生成，在保持AR模型质量的同时显著提升推理速度


<details>
  <summary>Details</summary>
Motivation: 自回归生成因逐token特性限制推理并行性，扩散语言模型虽支持并行解码但依赖双向注意力破坏KV缓存导致效率不足，难超优化AR引擎（如vLLM）

Method: 1. 采用拓扑重排将观察到的token移至物理前缀同时保留逻辑位置，使每个掩码位置基于当前所有观察到的token且保持严格因果掩码；2. 引入流式解码过程，持续将置信token提交到左到右增长的前缀中，维持固定并行工作负载，避免块扩散方法的停止等待行为

Result: 在挑战性推理基准上速度提升近3倍，低熵生成场景下可达10倍，且在匹配部署设置下对比vLLM服务的AR基线，扩散式解码可实际优于优化AR引擎

Conclusion: WeDLM保留强AR骨干模型质量的同时实现大幅加速，证明扩散式解码在实际部署中可超越优化AR引擎
...
### [23] [Text-Routed Sparse Mixture-of-Experts Model with Explanation and Temporal Alignment for Multi-Modal Sentiment Analysis](https://arxiv.org/abs/2512.22741)
*Dongning Rao,Yunbiao Zeng,Zhihua Jiang,Jujian Lv*

Main category: cs.CL

TL;DR: 本文提出TEXT模型，通过多模态大语言模型增强解释并利用时间对齐神经块融合音视频表示，结合文本引导的稀疏混合专家机制，在四个数据集上取得最优性能


<details>
  <summary>Details</summary>
Motivation: 多模态情感分析（MSA）中，现有方法对解释能力和时间对齐的挖掘不足，难以处理不同模态中的细微情感

Method: 提出TEXT模型：1. 用多模态大语言模型（MLLM）增强MSA的解释；2. 采用融合Mamba和时间交叉注意力的时间对齐块对齐音视频表示；3. 结合文本引导的稀疏混合专家机制与门融合实现多模态对齐

Result: TEXT在四个数据集上优于所有测试模型（包括3种最新方法和3种MLLM），在所有6个指标中至少4个指标领先；例如在CH-SIMS数据集上MAE降至0.353，比最新方法降低13.5%

Conclusion: TEXT模型有效提升了多模态情感分析的性能，验证了增强解释和时间对齐策略的有效性
...
### [24] [Fake News Classification in Urdu: A Domain Adaptation Approach for a Low-Resource Language](https://arxiv.org/abs/2512.22778)
*Muhammad Zain Ali,Bernhard Pfahringer,Tony Smith*

Main category: cs.CL

TL;DR: 针对乌尔都语等低资源语言虚假信息检测研究不足的问题，本文提出在微调前进行领域自适应预训练的方法，以提升多语言模型在该任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 社交媒体上的虚假信息是公认的问题，但乌尔都语等低资源语言在该领域的研究有限；直接使用多语言预训练模型进行微调时，模型难以处理领域特定术语，导致性能不佳。

Method: 采用分阶段训练方法优化模型泛化能力，在微调前对多语言模型（XLM-RoBERTa和mBERT）进行领域自适应预训练，使用公开的乌尔都语新闻语料库；在四个公开的乌尔都语虚假新闻数据集上进行评估。

Result: 领域自适应后的XLM-RoBERTa模型持续优于其原始版本；领域自适应后的mBERT模型表现则好坏参半。

Conclusion: 在低资源语言（如乌尔都语）的虚假新闻分类任务中，微调前的领域自适应预训练可有效提升XLM-RoBERTa模型的性能，但对mBERT模型的效果不稳定。
...
### [25] [CNSight: Evaluation of Clinical Note Segmentation Tools](https://arxiv.org/abs/2512.22795)
*Risha Surana,Adrian Law,Sunwoo Kim,Rishab Sridhar,Angxiao Han,Peiyu Hong*

Main category: cs.CL

TL;DR: 本研究评估了基于规则的基线模型、领域特定Transformer模型和大型语言模型在临床笔记分段任务中的性能，发现大型API模型表现最佳，为后续临床应用提供方法选择指导。


<details>
  <summary>Details</summary>
Motivation: 临床笔记从电子病历系统提取后常以非结构化或半结构化格式存储，阻碍了二次分析和下游临床应用；可靠识别章节边界是结构化临床笔记的关键步骤。

Method: 使用MIMIC-IV的1000条临床笔记数据集，评估规则基线模型、领域Transformer模型和大型语言模型的临床笔记分段性能。

Result: 大型API模型（如GPT-5-mini）在句子级和自由文本分段任务中平均F1达72.4，表现最佳；轻量级基线模型在结构化句子级任务上有竞争力，但在非结构化自由文本任务中表现不佳。

Conclusion: 研究结果为临床笔记分段任务的方法选择提供指导，为信息提取、队列识别和自动摘要等下游任务奠定基础。
...
### [26] [AutoForge: Automated Environment Synthesis for Agentic Reinforcement Learning](https://arxiv.org/abs/2512.22857)
...
TL;DR: 本文提出自动化可扩展的高难度可验证任务模拟环境合成流水线及环境级强化学习算法，以解决现有基于模拟环境的语言智能体强化学习存在的局限性，并通过多个基准测试验证了方法的有效性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于模拟环境的语言智能体强化学习存在局限性：模拟环境合成多为半自动化、任务难度不足且缺乏广度深度；模拟环境中的用户不稳定及环境异质性给智能体强化学习带来挑战。

Method: 1）提出自动化、可扩展的模拟环境合成流水线，用于生成高难度但易验证的任务相关环境；2）提出环境级强化学习算法，该算法可有效缓解用户不稳定性，并在环境层面进行优势估计以提升训练效率与稳定性。

Result: 在tau-bench、tau2-Bench和VitaBench等智能体基准测试上的综合评估验证了所提方法的有效性；进一步深入分析表明该方法具有良好的域外传化能力。

Conclusion: 所提出的模拟环境合成流水线及环境级强化学习算法可有效解决现有基于模拟环境的语言智能体强化学习面临的问题，提升训练效率、稳定性及泛化能力。
...
### [27] [Diversity or Precision? A Deep Dive into Next Token Prediction](https://arxiv.org/abs/2512.22955)
*Haoyuan Wu,Hai Wang,Jiajia Wu,Jinxiang Ou,Keyao Wang,Weile Chen,Zihao Zheng,Bei Yu*

Main category: cs.CL

TL;DR: 本文提出一种广义预训练目标，通过奖励塑形策略重塑预训练语言模型的token输出分布，为后续强化学习（RL）提供更优探索空间，以提升端到端推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明RL可提升大语言模型（LLMs）的推理能力，但RL训练的有效性严重依赖预训练模型token输出分布定义的探索空间；需系统研究预训练分布如何影响后续RL的探索潜力。

Method: 1. 将标准交叉熵损失解释为单步episode中的策略梯度优化实例；2. 提出广义预训练目标，将on-policy RL原则适配到监督学习；3. 引入奖励塑形策略：（a）使用正奖励缩放因子控制ground-truth token的概率集中度；（b）采用rank-aware机制不对称处理高排名和低排名负token，以重塑预训练token输出分布。

Result: 研究发现，与更高分布熵利于有效探索的直觉相反，施加面向精度的先验可为RL提供更优的探索空间。

Conclusion: 通过奖励塑形策略重塑预训练分布，能为RL提供更有利的探索空间，最终增强LLMs的端到端推理性能；面向精度的先验比高熵分布更适合作为RL的探索基础。
...
### [28] [Prompt engineering does not universally improve Large Language Model performance across clinical decision-making tasks](https://arxiv.org/abs/2512.22966)
*Mengdi Chai,Ali R. Zomorrodi*

Main category: cs.CL

TL;DR: 本研究评估了三种大语言模型（ChatGPT-4o、Gemini 1.5 Pro、LIama 3.3 70B）在临床决策支持任务中的表现及提示工程的影响，发现模型性能因任务和温度设置而异，提示工程需针对具体模型和任务定制。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在医学知识评估中表现出潜力，但在实际临床决策中的实用性仍未充分探索，需明确其在完整临床推理流程中的表现及优化策略。

Method: 对三种模型在两种温度设置下的五个临床决策任务（鉴别诊断、紧急步骤、相关诊断测试、最终诊断、治疗建议）进行评估；应用MedPrompt框架的提示工程（目标动态少样本学习和随机动态少样本学习）分析其对模型性能的影响。

Result: 模型性能因任务而异：最终诊断准确率接近完美，相关诊断测试表现差，其余任务中等；温度设置影响模型表现（ChatGPT零温度更好，LIama默认温度更好）；提示工程对低基线任务（相关诊断测试）有显著提升，但对其他任务可能适得其反；目标动态少样本提示未持续优于随机选择。

Conclusion: 提示工程的影响高度依赖模型和任务，需定制化、上下文感知的策略才能有效将大语言模型整合到医疗保健中。
...
### [29] [Improving Generalization in LLM Structured Pruning via Function-Aware Neuron Grouping](https://arxiv.org/abs/2512.23014)
*Tao Yu,Yongqi An,Kuan Zhu,Guibo Zhu,Ming Tang,Jinqiao Wang*

Main category: cs.CL

TL;DR: 提出FANG框架解决大语言模型后训练剪枝中的校准偏差问题，提升下游任务泛化性


<details>
  <summary>Details</summary>
Motivation: 大语言模型因规模大导致计算存储成本高，现有后训练剪枝方法在少样本校准集无法反映预训练数据分布时，下游任务泛化性有限

Method: 提出Function-Aware Neuron Grouping（FANG）框架，基于语义上下文类型对神经元按功能分组并独立剪枝，在组内重要性估计时加权与功能角色强相关的token，保留跨多上下文类型的神经元，自适应分配稀疏度

Result: FANG在保持语言建模性能的同时提升下游任务准确性，与FLAP和OBC结合时达到SOTA，在30%和40%稀疏度下平均准确率比FLAP和OBC高1.5%-8.5%

Conclusion: FANG有效缓解校准偏差，在稀疏度与性能间实现更好权衡，是高效的大语言模型后训练剪枝方案
...
### [30] [LENS: LLM-Enabled Narrative Synthesis for Mental Health by Aligning Multimodal Sensing with Language Models](https://arxiv.org/abs/2512.23025)
...
TL;DR: 本文提出LENS框架，通过构建大规模传感器-文本QA数据集和训练补丁级编码器，将多模态健康传感数据与语言模型对齐，生成临床相关的心理健康叙事，提升LLM处理原始行为信号的能力。


<details>
  <summary>Details</summary>
Motivation: 多模态健康传感能提供丰富的行为信号评估心理健康，但将数值时间序列转化为自然语言存在挑战：当前LLM无法原生处理长时长传感器流，且配对的传感器-文本数据集稀缺。

Method: 1. 构建大规模数据集：将258名参与者的抑郁和焦虑症状相关生态瞬时评估（EMA）响应转化为自然语言描述，生成超10万条传感器-文本QA对；2. 训练补丁级编码器：将原始传感器信号直接投影到LLM的表示空间，实现原生时间序列整合。

Result: 1. 标准NLP指标和症状严重度准确性任务特定指标上优于强基线；2. 13名心理健康专业人员的用户研究表明，LENS生成的叙事全面且具有临床意义。

Conclusion: LENS框架推动LLM成为健康传感的接口，为能推理原始行为信号并支持下游临床决策的模型提供了可扩展路径。
...
### [31] [Is Chain-of-Thought Really Not Explainability? Chain-of-Thought Can Be Faithful without Hint Verbalization](https://arxiv.org/abs/2512.23032)
*Kerem Zaman,Shashank Srivastava*

Main category: cs.CL

TL;DR: 现有CoT不忠实性评估指标混淆了不忠实性与不完整性，本研究通过新指标和因果中介分析揭示了其中问题并提出改进方向


<details>
  <summary>Details</summary>
Motivation: 现有Biasing Features指标将CoT未提及提示注入的影响预测的线索视为不忠实，但该指标混淆了不忠实性与不完整性（将分布式Transformer计算转化为线性自然语言叙述所需的有损压缩）

Method: 在Llama-3和Gemma-3的多跳推理任务中比较不同指标；提出faithful@k新指标分析更大推理时token预算对线索表述的影响；使用因果中介分析探究未表述线索对预测变化的中介作用

Result: Biasing Features标记为不忠实的CoT在其他指标下多数被判定为忠实（部分模型超50%）；更大token预算显著提升线索表述率（部分设置达90%）；未表述线索仍可通过CoT因果中介预测变化

Conclusion: 应避免仅依赖基于线索的评估，倡导使用更广泛的可解释性工具包，包括因果中介和基于破坏的指标
...
### [32] [Accelerating Language Model Workflows with Prompt Choreography](https://arxiv.org/abs/2512.23049)
*TJ Bai,Jason Eisner*

Main category: cs.CL

TL;DR: 本文提出Prompt Choreography框架，通过动态全局KV缓存优化多智能体工作流中的LLM调用效率，支持并行调用并通过微调减少缓存与重新编码结果差异，显著降低延迟和加速冗余计算主导的工作流。


<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLM）在多智能体工作流中部署日益增多，但现有调用方式存在冗余计算问题，导致延迟较高，需要更高效的框架来优化LLM工作流执行。

Method: 提出Prompt Choreography框架，该框架维护动态全局KV缓存，允许每个LLM调用关注先前编码消息的任意重新排序子集，并支持并行调用；同时对LLM进行微调以减少缓存使用与重新编码结果的差异。

Result: 该框架显著降低了每条消息的延迟（首token时间加快2.0-6.2倍），在一些以冗余计算为主的工作流中实现了超过2.2倍的端到端加速。

Conclusion: Prompt Choreography框架有效优化了多智能体工作流中的LLM调用效率，通过缓存和微调解决了冗余计算问题，在延迟和速度方面取得了显著改进。
...
### [33] [TabiBERT: A Large-Scale ModernBERT Foundation Model and Unified Benchmarking Framework for Turkish](https://arxiv.org/abs/2512.23065)
*Melikşah Türker,A. Ebrar Kızıloğlu,Onur Güngör,Susan Üsküdarlı*

Main category: cs.CL

TL;DR: 本文介绍了TabiBERT，一种基于ModernBERT架构的土耳其语单语种编码器，通过整合现代技术（如RoPE、FlashAttention）并在大规模语料上从头训练，解决了土耳其NLP领域缺乏现代单语种编码器的问题，同时构建了TabiBench基准数据集并取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 自BERT出现以来，仅编码器Transformer在计算效率、训练稳定性和长上下文建模方面有显著进步，但土耳其NLP领域缺乏整合这些现代架构范式的从头训练的单语种编码器。

Method: 1. 基于ModernBERT架构构建TabiBERT，整合Rotary Positional Embeddings（RoPE）、FlashAttention和优化的归一化方法；2. 在包含84.88B token的多领域语料（网页文本73%、科学出版物20%、源代码6%、数学内容0.3%）中采样1万亿token进行预训练；3. 支持8192-token上下文长度；4. 构建包含28个数据集、8个任务类别的TabiBench基准，采用GLUE风格宏平均评估。

Result: 1. TabiBERT在TabiBench上获得77.58分，比BERTurk高1.62分；2. 在8个任务类别中的5个（问答、代码检索、文档检索等）达到最先进水平，其中问答+9.55分、代码检索+2.41分、文档检索+0.60分；3. 与任务特定的最佳结果（如TurkishBERTweet）相比，平均提升1.47分；4. 推理速度提升达2.65倍，减少GPU内存消耗，支持更大批次大小。

Conclusion: TabiBERT是土耳其语NLP领域的一种高效、高性能的现代单语种编码器，具有良好的跨领域泛化能力，其发布的模型权重、训练配置和评估代码将促进土耳其语编码器研究的透明性和可复现性。
...
### [34] [Reservoir Computing inspired Matrix Multiplication-free Language Model](https://arxiv.org/abs/2512.23145)
*Takumi Shiratsuchi,Yuichiro Tanaka,Hakaru Tamukoh*

Main category: cs.CL

TL;DR: 本研究针对大型语言模型(LLMs)的高计算成本问题，提出一种基于储层计算的无矩阵乘法语言模型(MatMul-free LM)架构，通过部分固定和共享选定层权重、插入储层层及优化内存访问操作，在保持与基线模型相当性能的同时，减少了参数数量、训练时间和推理时间。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型(LLMs)在自然语言处理中取得了最先进的性能，但高计算成本仍是主要瓶颈。

Method: 1. 聚焦无矩阵乘法语言模型(MatMul-free LM)以提升计算效率；2. 采用储层计算启发的架构，部分固定和共享选定层的权重；3. 插入储层层以获得丰富的动态表示且无额外训练开销；4. 组合若干操作以减少内存访问。

Result: 所提出的架构可将参数数量减少高达19%，训练时间减少9.9%，推理时间减少8.0%，同时保持与基线模型相当的性能。

Conclusion: 通过基于储层计算的架构优化，在不显著牺牲性能的前提下有效降低了大型语言模型的计算成本。
...
### [35] [Not too long do read: Evaluating LLM-generated extreme scientific summaries](https://arxiv.org/abs/2512.23206)
*Zhuoqi Lyu,Qing Ke*

Main category: cs.CL

TL;DR: 本文提出BiomedTLDR数据集，测试开源大语言模型的科学摘要生成能力，发现其相比人类更倾向于抽取式而非抽象式生成。


<details>
  <summary>Details</summary>
Motivation: 高质量科学极端摘要（TLDR）有助于有效科学传播，但缺乏全面高质量的科学TLDR数据集阻碍了大语言模型（LLMs）摘要能力的开发与评估。

Method: 提出BiomedTLDR数据集，该数据集利用论文参考文献中作者评论的常见做法，包含大量研究者撰写的摘要样本；基于摘要测试流行的开源LLMs生成TLDR的能力。

Result: 部分LLMs能生成类人摘要，但总体而言，与人类相比，LLMs更倾向于保留原文的词汇选择和修辞结构，因此更偏向抽取式而非抽象式生成。

Conclusion: BiomedTLDR数据集及相关分析结果为LLMs科学摘要生成能力的研究提供了资源与洞见，代码和数据集已开源。
...
### [36] [Scoring, Reasoning, and Selecting the Best! Ensembling Large Language Models via a Peer-Review Process](https://arxiv.org/abs/2512.23213)
...
TL;DR: 提出LLM-PeerReview无监督集成方法，通过同行评审框架从多LLM候选响应中选最优，在四数据集上优于Smoothie-Global


<details>
  <summary>Details</summary>
Motivation: 现有LLM生成响应质量不一，需利用多模型集体智慧选最优且保持无监督适应性和可解释性

Method: 采用三阶段同行评审框架：用LLM-as-a-Judge技术评分，通过图形模型真值推理或平均策略聚合分数，选最高分响应

Result: 两变体在四数据集上获强结果，分别比Smoothie-Global优6.9%和7.3%

Conclusion: LLM-PeerReview概念简单且实证有效，是灵活通用的无监督LLM集成方法
...
### [37] [Anka: A Domain-Specific Language for Reliable LLM Code Generation](https://arxiv.org/abs/2512.23214)
*Saif Khalfan Saif Al Mazrouei*

Main category: cs.CL

TL;DR: 本文提出领域特定语言Anka以解决大语言模型在复杂多步编程任务中的系统错误，实验表明Anka在多步数据转换管道任务上显著优于Python，且LLM可通过上下文提示快速掌握该DSL。


<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLMs）在代码生成方面表现出色，但在复杂多步编程任务中存在系统错误，推测原因是通用语言的灵活性导致歧义及隐式状态管理问题。

Method: 设计并引入面向数据转换管道的领域特定语言Anka，其具有显式、约束性语法以减少代码生成歧义；使用Claude 3.5 Haiku和GPT-4o-mini对Anka与Python在100个基准问题上进行对比实验，评估多步任务的准确性。

Result: Claude 3.5 Haiku在Anka上实现99.9%解析成功率和95.8%总体任务准确率；在多步管道任务中，Anka比Python准确率高40个百分点（100% vs 60%）；GPT-4o-mini验证显示Anka在多步任务上优势达26.7个百分点。

Conclusion: 1）LLMs可通过上下文提示完全学习新DSL，达到接近原生的准确率；2）约束性语法显著减少复杂任务的错误；3）专为LLM生成设计的领域特定语言可优于LLM经过大量训练的通用语言；此外，作者发布了完整的语言实现、基准套件和评估框架以促进进一步研究。
...
### [38] [Interpretable Safety Alignment via SAE-Constructed Low-Rank Subspace Adaptation](https://arxiv.org/abs/2512.23260)
*Dianyun Wang,Qingsen Ma,Yuhu Shang,Zhifeng Lu,Lechen Ning,Zhenbo Xu,Huijia Wu,Zhaofeng He*

Main category: cs.CL

TL;DR: 本文提出一种基于稀疏自编码器（SAE）的参数高效微调方法，通过解纠缠特征空间构建可解释的低秩子空间，在安全对齐任务中实现高安全率并提升透明度


<details>
  <summary>Details</summary>
Motivation: 现有低秩适配方法（如LoRA）在隐式学习任务相关子空间时缺乏可解释性和直接控制，根源在于特征空间的多义性（单个维度编码多个纠缠概念）

Method: 利用预训练SAE识别解纠缠特征空间中的任务相关特征，构建显式可解释的低秩子空间以指导适配器初始化；通过理论分析验证单义性假设下SAE子空间识别的低误差特性

Result: 在安全对齐任务中，该方法安全率高达99.6%，比全微调提升7.4个百分点，接近RLHF方法，且仅更新0.19-0.24%的参数；同时通过SAE特征的语义接地提供对齐子空间的可解释洞察

Conclusion: 将机制可解释性融入微调过程可同时提升模型性能和透明度
...
### [39] [Chinese Morph Resolution in E-commerce Live Streaming Scenarios](https://arxiv.org/abs/2512.23280)
*Jiahao Zhu,Jipeng Qiang,Ran Bai,Chenyu Liu,Xiaoye Ouyang*

Main category: cs.CL

TL;DR: 本研究针对中国电商直播（如抖音平台）中主播使用语音变体逃避监管和虚假宣传的问题，提出Live Auditory Morph Resolution（LiveAMR）任务，构建数据集并采用文本生成方法结合大语言模型提升检测性能，有效增强直播监管。


<details>
  <summary>Details</summary>
Motivation: 中国电商直播（尤其是抖音等平台）已成为主要销售渠道，但主播常使用语音变体逃避监管并进行虚假宣传，而现有研究多聚焦社交媒体和地下产业的文本规避，缺乏针对健康医疗直播中发音规避的检测方法。

Method: 1. 提出LiveAMR任务以检测直播中的语音变体违规行为；2. 构建首个包含86,790样本的LiveAMR数据集；3. 将任务转化为文本到文本生成问题；4. 利用大语言模型生成额外训练数据以提升性能。

Result: 通过大语言模型生成额外训练数据的方法有效提升了LiveAMR任务的性能，且语音变体解析显著增强了电商直播的监管效果。

Conclusion: 本研究提出的LiveAMR任务及其方法能够有效检测电商直播中的语音变体违规行为，为直播监管提供了新的技术手段，有助于规范电商直播行业的健康发展。
...
### [40] [AI4Reading: Chinese Audiobook Interpretation System Based on Multi-Agent Collaboration](https://arxiv.org/abs/2512.23300)
*Minjiang Huang,Jipeng Qiang,Yi Zhu,Chaowei Zhang,Xiangyu Zhao,Kui Yu*

Main category: cs.CL

TL;DR: 提出AI4Reading多智能体协作系统以自动生成播客式有声书解读，解决人工创作耗时耗资源问题


<details>
  <summary>Details</summary>
Motivation: 有声书解读日益受关注，但人工创作过程耗时且资源密集，亟需自动化解决方案

Method: 构建含11个专业智能体（如主题分析师、案例分析师、编辑、叙述者、校对员）的多智能体协作框架，结合大语言模型和语音合成技术

Result: 与专家解读相比，AI4Reading生成的解读脚本更简洁准确，但语音生成质量仍有差距

Conclusion: AI4Reading系统可有效生成符合内容准确、易理解、叙事逻辑要求的有声书解读脚本，为自动化有声书解读提供可行方案
...
### [41] [AI Meets Brain: Memory Systems from Cognitive Neuroscience to Autonomous Agents](https://arxiv.org/abs/2512.23343)
...
TL;DR: 本文系统整合认知神经科学与大语言模型驱动智能体的记忆跨学科知识，涵盖定义功能、分类存储、管理生命周期、基准评测、安全攻防及未来方向。


<details>
  <summary>Details</summary>
Motivation: 现有自主智能体研究虽借鉴认知神经科学设计记忆工作流，但受跨学科壁垒限制，难以吸收人类记忆机制的本质，存在知识整合缺口。

Method: 首先从认知神经科学到LLM再到智能体的递进路径阐明记忆定义与功能；其次对比分析生物与人工视角下的记忆分类、存储机制及完整管理生命周期；然后综述智能体记忆评估的主流基准；此外从攻防双重视角探索记忆安全；最后展望未来研究方向。

Result: 完成跨学科记忆知识的系统整合，明确生物与人工记忆的关键差异与联系，梳理现有评测基准及安全挑战，并指出未来突破方向。

Conclusion: 需进一步聚焦多模态记忆系统与技能获取等方向，推动智能体记忆机制向更接近人类认知的方向发展。
...
### [42] [A Stepwise-Enhanced Reasoning Framework for Large Language Models Based on External Subgraph Generation](https://arxiv.org/abs/2512.23356)
*Xin Zhang,Yang Cao,Baoxing Wu,Xinyi Chen,Kai Song,Siying Li*

Main category: cs.CL

TL;DR: 本文提出一种基于外部子图生成的分步推理增强框架SGR，以解决大语言模型在深度推理任务中易受噪声信息影响的问题，实验表明该框架能有效提升推理准确性。


<details>
  <summary>Details</summary>
Motivation: 近年来大语言模型在多种自然语言处理任务中表现出色，但在深度推理和逻辑推断任务中仍面临挑战，尤其是训练于大规模文本语料的模型在生成时可能融入噪声或无关信息，导致预测错误或与事实知识不一致。

Method: 提出分步推理增强框架SGR：首先针对输入查询生成定制化外部子图，然后引导模型基于该子图进行多步推理，最后整合多条推理路径得到最终答案。

Result: 在多个基准数据集上的实验结果表明，SGR持续优于强基线模型，证明其在增强大语言模型推理能力方面的有效性。

Conclusion: SGR框架通过动态构建查询相关子图并基于结构化子图进行分步推理，减少了噪声信息的影响，有效提升了大语言模型的推理准确性。
...
### [43] [Entropy-Guided Token Dropout: Training Autoregressive Language Models with Limited Domain Data](https://arxiv.org/abs/2512.23422)
*Jiapeng Wang,Yiwen Hu,Yanzipeng Gao,Haoyu Wang,Shuo Wang,Hongyu Lu,Jiaxin Mao,Wayne Xin Zhao,Junyi Li,Xiao Zhang*

Main category: cs.CL

TL;DR: 本文针对大语言模型多轮训练中因重复数据暴露导致的性能下降问题，提出熵引导的token dropout方法EntroDrop，通过选择性掩盖低熵token并采用课程调度调整正则化强度，有效提升模型在有限数据下的适应性。


<details>
  <summary>Details</summary>
Motivation: 高质量领域特定数据获取日益稀缺，多轮训练成为大语言模型适配的实用策略，但自回归模型在重复数据暴露下常出现性能下降，原因是学习动态不平衡：低熵token快速学习并主导优化，高熵token泛化能力随训练持续恶化。

Method: 提出EntroDrop方法，这是一种熵引导的token dropout结构化数据正则化技术，训练时选择性掩盖低熵token，并采用课程调度根据训练进度调整正则化强度。

Result: 在0.6B到8B参数的模型规模上进行实验，结果显示EntroDrop持续优于标准正则化基线，且在扩展多轮训练中保持稳健性能。

Conclusion: 研究强调在有限数据训练时，正则化需与token级学习动态对齐的重要性，EntroDrop为数据受限领域中大语言模型的更有效适配提供了有前景的途径。
...
### [44] [The Effect of Gender Diversity on Scientific Team Impact: A Team Roles Perspective](https://arxiv.org/abs/2512.23429)
*Yi Zhao,Yongjun Zhu,Donghun Kim,Yuzhuo Wang,Heng Zhang,Chao Lu,Chengzhi Zhang*

Main category: cs.CL

TL;DR: 本文研究科学团队中不同角色（领导与支持）的性别多样性对团队影响力的影响，发现其关系呈倒U型，且团队规模会调节领导角色的性别多样性效应。


<details>
  <summary>Details</summary>
Motivation: 学术界关注性别多样性对科学团队成功的影响，但现有研究结果不一致，且多以整体方式衡量多样性，忽略内部角色差异，导致无法细致理解性别多样性如何塑造团队影响力，尤其不同团队角色的性别多样性效应尚不明确。

Method: 将科学团队定义为论文所有合著者，以五年引用量衡量团队影响力；利用作者贡献声明将成员分为领导和支持角色；基于PLOS期刊13万余篇生物医学相关论文，采用多变量回归分析不同角色性别多样性与团队影响力的关联，并用阈值回归模型研究团队规模的调节作用。

Result: 1）领导和支持角色的性别多样性与团队影响力均呈倒U型关系；2）全女性领导+全男性支持的团队影响力高于其他类型；3）领导角色性别多样性对小型团队影响显著负向，对大型团队影响正向但不显著；支持角色性别多样性影响始终显著正向，不受团队规模影响。

Conclusion: 性别多样性对科学团队影响力的作用因角色和团队规模而异，需考虑内部角色差异以更准确理解其影响。
...
### [45] [Semantic Tree Inference on Text Corpa using a Nested Density Approach together with Large Language Model Embeddings](https://arxiv.org/abs/2512.23471)
*Thomas Haschka,Joseph Bakarji*

Main category: cs.CL

TL;DR: 本文提出嵌套密度聚类方法，利用LLM嵌入空间构建文本的层次语义树结构，实现无预定义类别的文本分类，并在多个数据集验证其鲁棒性，讨论其在科学计量学等领域的应用。


<details>
  <summary>Details</summary>
Motivation: 当前LLM嵌入常用于向量数据库的语义相似性存储与检索，但文本语料库的全局结构语义关系仍不明确，缺乏无预定义类别下数据驱动的研究领域及其子领域发现方法。

Method: 提出嵌套密度聚类方法：先在LLM嵌入空间识别强语义相似的密集簇，逐步放松密度准则使密集簇合并为更弥散的簇，直到形成单一根簇；通过将密集簇嵌入更弥散簇，构建捕获文本层次语义关系的树结构。

Result: 将该方法应用于科学摘要分类案例，实现无预定义类别的研究领域及子领域发现；在20 Newsgroups和IMDB 50k电影评论等基准数据集上验证了跨领域的鲁棒性。

Conclusion: 嵌套密度树可揭示文本数据集的语义结构与演化，讨论其在科学计量学、主题演化等领域的潜在应用价值。
...
### [46] [Automatic Detection of Complex Quotation Patterns in Aggadic Literature](https://arxiv.org/abs/2512.23504)
*Hadar Miller,Tsvi Kuflik,Moshe Lavee*

Main category: cs.CL

TL;DR: 本文提出ACT算法用于拉比文献中圣经引文的自动检测，通过三阶段方法结合形态感知对齐和上下文敏感富集，优于现有系统并为数字人文研究提供新途径


<details>
  <summary>Details</summary>
Motivation: 现有文本复用框架难以处理拉比文献中简短、转述或结构嵌入的圣经引文，存在穷尽机器检测与人类编辑判断之间的方法学差距

Method: 提出三阶段算法ACT，包括形态感知对齐算法、上下文敏感富集阶段（识别复杂引用模式如Wave和Echo引文），并评估三种ACT配置（ACT-QE、ACT-2、ACT-3）以分离各组件贡献

Result: ACT-QE全流程优于所有基线系统，F1得分为0.91（召回率0.89，精确率0.94）；ACT-2（无风格富集）召回率更高（0.90）但精确率较低；ACT-3（使用更长n-grams）在覆盖范围和特异性间取得平衡

Conclusion: ACT不仅提升引文检测性能，还能跨语料库分类风格模式，为体裁分类和互文分析开辟新方向，填补数字人文和计算文献学中的方法学空白，为历史文本分析（尤其是形态丰富、引文密集的传统如Aggadic文学）奠定基础
...
### [47] [UniHetero: Could Generation Enhance Understanding for Vision-Language-Model at Large Data Scale?](https://arxiv.org/abs/2512.23512)
*Fengjiao Chen,Minhao Jing,Weitao Lu,Yan Feng,Xiaoyu Li,Xuezhi Cao*

Main category: cs.CL

TL;DR: 本文研究了视觉-语言大模型中生成任务对理解任务的增强作用，提出简洁结构的UniHetero模型并在大规模预训练下验证了生成语义而非像素可提升理解、生成任务数据缩放趋势更优及输入嵌入自回归有效捕获视觉细节三大关键发现。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言大模型正朝着视觉理解与生成任务统一的方向发展，但生成任务能否在大规模数据下增强理解任务的问题尚未得到充分探索。

Method: 设计了具有简洁结构的统一模型UniHetero，并在超过2亿样本的大规模预训练数据上进行分析。

Result: 得出三个关键观察结果：1）生成任务可提升理解任务，但仅当生成语义而非像素时有效；2）生成任务展现出更优的数据缩放趋势和更高的数据利用率；3）对输入嵌入进行自回归处理能有效捕获视觉细节。

Conclusion: 生成任务在特定条件下（生成语义）可增强视觉理解任务，且生成任务在数据利用和缩放方面具有优势，输入嵌入自回归是捕获视觉细节的有效方法。
...
TL;DR: 本文提出一种利用知识图谱增强大语言模型（LLM）幻觉自检测的方法，通过将LLM响应转换为实体关系图并估计幻觉可能性，在两个数据集上对GPT-4o和Gemini-2.5-Flash评估，相比现有方法准确率提升16%、F1-score提升20%，同时发布了一个人工整理的数据集。


<details>
  <summary>Details</summary>
Motivation: 幻觉（生成看似可信但虚假的陈述）是LLM安全部署的主要障碍，现有自检测方法性能仍需提升，因此探索结构化知识表示（知识图谱）对幻觉自检测的改进潜力。

Method: 提出一种方法：1）将LLM响应转换为包含实体和关系的知识图谱；2）利用该图谱估计响应包含幻觉的可能性。使用GPT-4o和Gemini-2.5-Flash两种LLM，在两个幻觉检测数据集上评估，其中一个数据集是人工整理增强的并作为次要成果发布。

Result: 相比标准自检测方法和最先进的SelfCheckGPT，该方法准确率相对提升达16%，F1-score相对提升达20%；结果表明LLM在事实以知识图谱结构化时能更好地分析原子事实，即使初始输出存在不准确。

Conclusion: 这种低成本、模型无关的方法为更安全可信的语言模型铺平了道路，同时发布的人工整理数据集支持未来更可靠的基准测试。
...
TL;DR: 本研究针对大型视觉语言模型（LVLMs）在视觉指令微调后指令遵循能力下降的问题，通过构建含输出格式说明的训练数据集，证明该方法可缓解此下降现象。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型（LVLMs）在使用常用训练数据集进行视觉指令微调后，常失去原LLM具备的指令遵循能力，导致无法按预期执行任务指令，此问题需被量化验证及分析原因。

Method: 构建强调输出格式是否明确的新训练数据集，研究微调时显式指示输出格式对LVLMs指令遵循能力的影响，并进行定量评估。

Result: 定量评估证实LVLMs经常用数据集微调后指令遵循能力下降；使用含输出格式说明的数据集训练的LVLMs比未使用的模型指令遵循更准确。

Conclusion: 在（视觉）指令微调中纳入含输出格式说明的样本，可能有助于缓解LVLMs指令遵循能力的下降。
...
TL;DR: 本文研究了口语语言模型（SLMs）在多轮对话中无法维持特定口语风格的现象（称为风格失忆），评估了不同模型的表现，并探讨了提示策略对缓解该问题的影响。


<details>
  <summary>Details</summary>
Motivation: 口语语言模型在多轮对话中难以维持特定口语风格（如情感、口音、音量、语速），这一现象（风格失忆）影响模型的实用性，需要深入研究其表现及缓解方法。

Method: 评估了3个专有和2个开源SLMs在多轮对话中维持口语风格的能力；测试了让模型回忆风格指令的效果；比较了系统消息和用户消息中放置风格指令的不同表现；探讨了多种提示策略。

Result: 所有测试的SLMs均存在风格失忆现象；让模型回忆风格指令可部分缓解该问题；当风格指令放在系统消息而非用户消息中时，SLMs更难遵循要求的风格，与系统提示的预期功能矛盾。

Conclusion: SLMs普遍存在多轮对话中的风格失忆问题；让模型回忆风格指令可部分缓解该问题；系统消息中的风格指令效果不如用户消息，提示策略需进一步优化。
...
TL;DR: 提出InfTool框架，通过自进化多智能体合成解决大语言模型调用外部工具的瓶颈，无需人类标注即可生成多样化验证轨迹并提升模型性能


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型调用外部工具的方法存在三大挑战：高质量轨迹需要昂贵的人类标注、对未见过的工具泛化能力差、单模型合成存在质量上限导致偏差和覆盖缺口

Method: 引入InfTool框架，仅使用原始API规范，协调三个协作智能体（用户模拟器、工具调用助手、MCP服务器）生成从单轮调用到复杂多步工作流的多样化验证轨迹；通过Group Relative Policy Optimization（GRPO）带门控奖励的闭环迭代训练模型，无需人类干预

Result: 在Berkeley Function-Calling Leaderboard（BFCL）上，InfTool将基础32B模型的准确率从19.8%提升至70.9%（+258%），超过10倍规模的模型并与Claude-Opus媲美，且完全基于合成数据无人类标注

Conclusion: InfTool框架有效突破大语言模型调用外部工具的瓶颈，仅用合成数据即可显著提升模型性能，超越更大规模模型并达到顶尖水平
...
### [52] [Nested Browser-Use Learning for Agentic Information Seeking](https://arxiv.org/abs/2512.23647)
...
TL;DR: 本文提出NestBrowse框架，解决信息检索智能体浏览器交互复杂的问题，提升深度Web信息获取能力


<details>
  <summary>Details</summary>
Motivation: 现有信息检索智能体工具使用局限于API级片段检索和URL页面获取，无法充分利用真实浏览的丰富信息；而全浏览器交互因细粒度控制和冗长内容返回导致ReAct风格智能体推理复杂

Method: 提出Nested Browser-Use Learning（NestBrowse）框架，通过嵌套结构将交互控制与页面探索解耦，简化智能体推理同时支持有效深度Web信息获取

Result: 在具有挑战性的深度信息检索基准测试中，NestBrowse表现出明显的实践优势，且深入分析证实其效率和灵活性

Conclusion: NestBrowse框架能够有效解决现有信息检索智能体在浏览器交互方面的局限性，提升深度Web信息获取的效率与灵活性
...
### [53] [Less is more: Probabilistic reduction is best explained by small-scale predictability measures](https://arxiv.org/abs/2512.23659)
*Cassandra L. Jacobs,Andrés Buxó-Lugo,Anna K. Taylor,Marie Leopold-Hooke*

Main category: cs.CL

TL;DR: 本文研究了在探索语言模型概率与认知现象关系时所需的上下文数量，证明n元语法表示足以作为规划的认知单元，无需完整话语。


<details>
  <summary>Details</summary>
Motivation: 明确在研究语言模型概率与认知现象关系时所需的必要和合适的上下文数量。

Method: 通过研究完整话语是否为观察概率简化所必需，验证n元语法表示作为认知规划单元的充分性。

Result: n元语法表示足以作为认知规划单元，无需使用完整话语即可观察到概率简化现象。

Conclusion: n元语法可作为有效的认知规划单元，为语言模型与认知现象关系的研究提供了简化的上下文分析依据。
...
### [54] [Multilingual Hidden Prompt Injection Attacks on LLM-Based Academic Reviewing](https://arxiv.org/abs/2512.23684)
*Panagiotis Theocharopoulos,Ajinkya Kulkarni,Mathew Magimai. -Doss*

Main category: cs.CL

TL;DR: 本文研究了大型语言模型在学术同行评审中易受文档级隐藏提示注入攻击的问题，通过在ICML论文中嵌入多语言对抗性提示发现不同语言的脆弱性差异


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）越来越多地用于高影响力工作流如学术同行评审，但易受文档级隐藏提示注入攻击，需评估其在实际场景中的脆弱性

Method: 构建约500篇ICML已接受论文的数据集，在每篇论文中嵌入四种不同语言的语义等效对抗性提示，并使用LLM进行评审

Result: 提示注入对英语、日语和中文注入导致评审分数和录用/拒绝决策发生显著变化，而阿拉伯语注入几乎无影响

Conclusion: 基于LLM的评审系统易受文档级提示注入攻击，且不同语言的脆弱性存在显著差异
...
### [55] [PROFASR-BENCH: A Benchmark for Context-Conditioned ASR in High-Stakes Professional Speech](https://arxiv.org/abs/2512.23686)
*Deepak Babu Piskala*

Main category: cs.CL

TL;DR: 本文提出ProfASR-Bench专业语音识别基准套件，针对金融、医疗等领域的高风险应用，通过上下文条件控制测量现有模型的上下文利用差距，并提供标准化测试床。


<details>
  <summary>Details</summary>
Motivation: 现有ASR基准未充分考虑专业场景中的挑战：密集领域术语、正式语域变化及对关键实体错误的零容忍需求。

Method: 构建ProfASR-Bench套件，包含自然语言提示与实体丰富目标 utterance 的配对样本；使用Whisper和Qwen-Omni模型，在无上下文、轮廓、领域+轮廓、Oracle及对抗条件下评估；采用传统ASR指标及实体感知分数、按口音和性别切片报告。

Result: 轻量级文本上下文对平均词错误率（WER）几乎无影响，即使Oracle提示也未改善；对抗提示未可靠降低性能；发现当前系统存在上下文利用差距（CUG），名义上可提示但未充分利用辅助信息。

Conclusion: ProfASR-Bench提供标准化上下文阶梯、带置信区间的实体及切片感知报告，为跨模型家族比较融合策略提供可复现测试床。
...
### [56] [Fine-Tuning LLMs with Fine-Grained Human Feedback on Text Spans](https://arxiv.org/abs/2512.23693)
*Sky CH-Wang,Justin Svegliato,Helen Appel,Jason Eisner*

Main category: cs.CL

TL;DR: 提出一种基于反馈驱动改进链的偏好监督微调语言模型的方法及数据集，通过局部针对性编辑提升偏好调优效率与效果


<details>
  <summary>Details</summary>
Motivation: 现有基于标准A/B偏好排序或全对比重写的直接对齐方法存在效率与效果不足的问题，需更结构化、基于修订的监督方式

Method: 1. 标注者对模型响应标记“喜欢”和“不喜欢”的跨度并说明原因；2. 基础模型从左到右重写不喜欢的跨度，形成增量改进链；3. 从改进链相邻步骤构建偏好对用于直接对齐

Result: 该方法优于基于标准A/B偏好排序或全对比重写的直接对齐方法

Conclusion: 结构化、基于修订的监督能带来更高效且有效的偏好调优
...
### [57] [Eliciting Behaviors in Multi-Turn Conversations](https://arxiv.org/abs/2512.23701)
*Jing Huang,Shujian Zhang,Lun Wang,Andrew Hard,Rajiv Mathews,John Lambert*

Main category: cs.CL

TL;DR: 本文研究多轮对话场景下的行为诱导问题，提出分析框架并评估三类方法，发现在线方法在有限查询预算下能有效提升行为诱导成功率，强调动态基准的必要性。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型行为诱导技术主要针对单轮场景，而多轮对话中的行为识别对模型评估至关重要，缺乏有效方法和动态基准。

Method: 提出三类方法的分析框架（基于先验知识、离线交互、在线交互），并将在线方法推广到多轮场景；在自动生成多轮测试用例上评估三类方法，分析查询预算与成功率的权衡。

Result: 在线方法在三个任务中仅用数千次查询就达到平均45%、19%、77%的成功率，而现有多轮对话基准中的静态方法几乎找不到失败案例。

Conclusion: 在线方法在多轮对话行为诱导中具有高效性，应推动社区转向动态基准以更好评估大语言模型的多轮对话行为。
...
### [58] [Towards Unsupervised Causal Representation Learning via Latent Additive Noise Model Causal Autoencoders](https://arxiv.org/abs/2512.22150)
*Hans Jarett J. Ong,Brian Godwin S. Lim,Dominic Dayta,Renzo Roel P. Tan,Kazushi Ikeda*

Main category: cs.LG

TL;DR: 本文提出LANCA模型，以ANM为归纳偏置解决无监督因果表示学习中的可识别性问题，在多个基准上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 无监督表示学习依赖统计独立性常无法捕捉因果依赖，且无监督情况下从观测数据中解耦因果变量存在可识别性挑战。

Method: 提出Latent Additive Noise Model Causal Autoencoder（LANCA），采用确定性Wasserstein Auto-Encoder（WAE）结合可微分ANM层，将残差独立性转化为显式优化目标。

Result: LANCA在合成物理基准（Pendulum、Flow）和真实感环境（CANDLE）上优于最先进基线，对复杂背景场景的虚假相关性表现出更强鲁棒性。

Conclusion: ANM约束虽不能保证一般混合情况下的唯一可识别性，但能将允许的变换从任意微分同胚限制到仿射类，解决组件级不确定性；LANCA架构有效支持无监督因果发现。
...
### [59] [Latent Sculpting for Zero-Shot Generalization: A Manifold Learning Approach to Out-of-Distribution Anomaly Detection](https://arxiv.org/abs/2512.22179)
*Rajeeb Thapa Chhetri,Zhixiong Chen,Saurab Thapa*

Main category: cs.LG

TL;DR: 本文提出Latent Sculpting框架解决高维表格数据中监督深度学习的泛化崩溃问题，通过两阶段表示学习提升零样本异常检测性能


<details>
  <summary>Details</summary>
Motivation: 监督深度学习在高维表格领域面临泛化崩溃问题，模型对已知分布决策边界学习精确但对分布外（OOD）数据表现极差，根源在于 latent 空间缺乏拓扑约束导致新异常与良性数据统计上难以区分

Method: 提出分层两阶段表示学习框架Latent Sculpting：阶段1用混合1D-CNN和Transformer Encoder，结合新型双中心紧致损失（DCCL）将良性流量塑造成低熵超球面簇；阶段2基于预结构化流形用掩码自回归流（MAF）学习精确密度估计

Result: 在CIC-IDS-2017基准测试中，零样本异常检测F1分数达0.87，优于监督基线（约0.30）和最强无监督基线（0.76）；对“Infiltration”场景检测率88.89%，而最先进监督模型准确率为0%

Conclusion: 显式流形塑造是鲁棒零样本泛化的前提，将结构学习与密度估计解耦为广义异常检测提供可扩展路径
...
### [60] [Learning Tennis Strategy Through Curriculum-Based Dueling Double Deep Q-Networks](https://arxiv.org/abs/2512.22186)
*Vishnu Mohan*

Main category: cs.LG

TL;DR: 本文提出了一个强化学习框架，结合自定义网球模拟环境与基于课程学习训练的Dueling Double Deep Q-Network(DDQN)，用于优化网球策略，解决分层计分、随机结果等挑战，并分析了训练结果及存在的局限性。


<details>
  <summary>Details</summary>
Motivation: 网球策略优化是一个复杂的序列决策问题，涉及分层计分、随机结果、长期信用分配、身体疲劳和对手技能适应等多个挑战，现有方法难以有效应对这些问题。

Method: 提出了一个强化学习框架，集成自定义网球模拟环境与Dueling Double Deep Q-Network(DDQN)，并采用课程学习。环境模拟完整的计分系统、战术决策、疲劳动态和对手技能参数；DDQN分解动作价值估计为状态价值和优势分量；课程学习逐步提高对手难度（从0.40到0.50）。

Result: 训练后的智能体在平衡对手中胜率达98%-100%，对更具挑战性的对手仍保持良好表现；发球效率为63.0%-67.5%，回球效率为52.8%-57.1%。消融研究显示Dueling架构和课程学习对稳定收敛是必要的，而标准DQN基线无法学习有效策略；但策略存在明显防御偏向，优先避免失误而非积极构建得分点。

Conclusion: 该强化学习框架在网球策略优化中表现良好，但赢率驱动的优化在简化体育模拟中存在局限性，强调了奖励设计对现实体育强化学习的重要性。
...
TL;DR: 本研究探讨现代CNN的谱偏差，通过视觉诊断框架和谱抑制比（SSR）量化正则化器的低通滤波强度，揭示L2正则化对高频能量的抑制作用及精度-鲁棒性权衡。


<details>
  <summary>Details</summary>
Motivation: 现有L2正则化和Dropout等正则化技术的特征频率选择物理机制尚未明确，需从信号处理视角理解其对泛化的影响。

Method: 引入视觉诊断框架追踪训练中权重频率的动态演化，提出谱抑制比（SSR）量化不同正则化器的低通滤波强度；通过离散径向剖面解决小核（如3x3）的混叠问题，在ResNet-18和CIFAR-10上进行实证分析。

Result: L2正则化对高频能量积累的抑制是无正则化基线的3倍以上；L2模型对宽带高斯噪声敏感，但在高频信息损失场景（如低分辨率）中鲁棒性优于基线6%以上。

Conclusion: 正则化强制CNN形成对低频结构的强谱归纳偏差，为泛化提供信号处理视角，揭示精度与鲁棒性之间的关键权衡。
...
TL;DR: 本文提出一种受生物情绪启发的学习信号框架EILS，以解决现代AI系统在开放动态环境中的脆弱性问题，通过模拟好奇心、压力和信心等连续稳态评估信号动态调节智能体优化过程。


<details>
  <summary>Details</summary>
Motivation: 现代AI（从深度强化学习到大型语言模型）依赖静态外部奖励函数的“外在最大化”方法，虽在封闭静态领域表现超人，但在开放动态真实环境中存在脆弱性；标准智能体缺乏内在自主性，如无密集反馈难以探索、无法适应分布偏移、需大量手动调参，而鲁棒自主性的未解决因素是生物情绪的功能类似物（作为高级稳态控制机制）。

Method: 提出Emotion-Inspired Learning Signals（EILS）统一框架，将情绪建模为连续稳态评估信号（如好奇心、压力、信心）而非语义标签；将这些信号形式化为基于交互历史的向量值内部状态，实时动态调节智能体优化场景：好奇心调节熵防止模式崩溃、压力调节可塑性克服停滞、信心适应信任区域稳定收敛。

Result: （摘要未明确给出实验结果，仅提出假设）假设该闭环稳态调节可使EILS智能体在样本效率和非平稳适应性方面优于标准基线。

Conclusion: （摘要未给出最终结论，仅提出假设性结论）EILS框架通过生物启发的内在反馈引擎替代分散优化启发式方法，有望提升智能体在开放动态环境中的鲁棒自主性。
...
### [63] [On the Existence and Behaviour of Secondary Attention Sinks](https://arxiv.org/abs/2512.22213)
*Jeffrey T. H. Wong,Cheng Zhang,Louis Mahon,Wayne Luk,Anton Isopoussu,Yiren Zhao*

Main category: cs.LG

TL;DR: 本文识别并分析了一类与现有研究不同的次要注意力 sink（secondary sinks），它们主要出现在中间层，持续层数可变，吸引的注意力质量较少但显著，并通过实验揭示了其形成机制、特性及对注意力机制的影响。


<details>
  <summary>Details</summary>
Motivation: 现有研究仅关注与BOS token类似的主要注意力 sink（primary sinks），但未深入探讨中间层中存在的、特性不同的次要注意力 sink，这些次要 sink 对注意力机制的影响尚不明确。

Method: 通过对11个模型家族进行广泛实验，分析次要注意力 sink 的出现位置、特性、形成方式及其对注意力机制的影响，包括探究其与MLP模块的关系、向量的ℓ₂范数作用及与主要 sink 的关联。

Result: 1）次要 sink 由特定中间层MLP模块形成，这些MLP将token表示映射到与该层主要 sink 方向一致的向量；2）向量的ℓ₂范数决定次要 sink 的得分及持续层数，进而影响注意力机制；3）主要 sink 在中间层减弱，与次要 sink 的出现时间一致；4）大规模模型中，sink的位置和生命周期（sink levels）更具确定性和频繁性，如QwQ-32B有3个sink levels，Qwen3-14B有6个。

Conclusion: 次要注意力 sink 是一类重要的注意力现象，其形成与中间层MLP模块密切相关，特性受向量ℓ₂范数影响，且在大规模模型中表现更规律，这为理解注意力机制的内部工作原理提供了新视角。
...
### [64] [DiRL: An Efficient Post-Training Framework for Diffusion Language Models](https://arxiv.org/abs/2512.22234)
*Ying Zhu,Jiaxin Wan,Xiaoran Liu,Siyanag He,Qiqi Wang,Xu Guo,Tianyi Liang,Zengfeng Huang,Ziwei He,Xipeng Qiu*

Main category: cs.LG

TL;DR: 本文提出DiRL高效后训练框架及DiPO优化算法，提升扩散语言模型（dLLMs）在数学复杂推理任务上的性能


<details>
  <summary>Details</summary>
Motivation: 现有dLLMs后训练方法存在计算效率低、训练与推理目标不匹配问题，限制其在数学等复杂推理任务中的表现

Method: 提出DiRL框架，整合FlexAttention加速的分块训练与LMDeploy优化推理，实现高效两阶段后训练（监督微调+强化学习）；提出DiPO，即首个针对dLLMs的无偏Group Relative Policy Optimization（GRPO）实现

Result: 基于DiRL框架训练的DiRL-8B-Instruct模型在数学任务上达到dLLMs的SOTA性能，并在多个基准测试中超过Qwen2.5系列的可比模型

Conclusion: DiRL框架和DiPO算法有效解决了dLLMs后训练的关键问题，显著提升其在复杂推理任务上的表现
...
### [65] [Masking Teacher and Reinforcing Student for Distilling Vision-Language Models](https://arxiv.org/abs/2512.22238)
*Byung-Kwan Lee,Yu-Chiang Frank Wang,Ryo Hachiuma*

Main category: cs.LG

TL;DR: 提出Masters框架，通过掩码渐进强化学习解决大规模视觉语言模型向小型模型蒸馏的挑战，提升学生模型性能。


<details>
  <summary>Details</summary>
Motivation: 大规模视觉语言模型（VLMs）性能优异但难以部署在移动或边缘设备上，需要紧凑且高效的VLMs；然而大模型到小模型的知识蒸馏因尺寸差距大导致学生模型难以复现教师模型的复杂高维表示，学习不稳定且性能下降。

Method: 提出Masters框架，采用掩码渐进强化学习（RL）蒸馏策略：先掩码教师模型非主导权重以降低复杂度，再逐步恢复教师模型容量；集成离线RL阶段，使用准确性奖励（衡量生成响应正确性）和蒸馏奖励（量化教师到学生的响应迁移难度），利用掩码教师预生成的响应提供高效指导。

Result: 该框架使学生模型能从教师模型中平滑稳定地学习更丰富的表示，无需在线思考-回答过程即可实现强性能。

Conclusion: Masters框架有效解决了大规模VLMs向小型模型蒸馏的挑战，为在资源受限设备上部署高效VLMs提供了可行方案。
...
### [66] [EvoXplain: When Machine Learning Models Agree on Predictions but Disagree on Why -- Measuring Mechanistic Multiplicity Across Training Runs](https://arxiv.org/abs/2512.22240)
*Chama Bensmail*

Main category: cs.LG

TL;DR: 本文提出EvoXplain框架，用于诊断重复训练下机器学习模型解释的稳定性，发现高准确率模型的解释常存在多模态现象，挑战了单一解释的可靠性


<details>
  <summary>Details</summary>
Motivation: 现有研究假设高准确率模型的解释正确可信，但忽略了不同模型可能通过不同机制达到相同结果的问题，缺乏对解释稳定性的评估方法

Method: 提出EvoXplain诊断框架，将解释视为随机优化过程中的样本，不聚合预测或构建集成，分析样本是否形成单一连贯解释或多个不同解释模式

Result: 在乳腺癌和COMPAS数据集上使用逻辑回归和随机森林模型评估发现，尽管模型准确率高，但解释频繁表现出明显多模态性，即使逻辑回归这类被认为稳定的模型也会产生多个分离的解释盆地，且差异无法用超参数变化或简单性能权衡解释

Conclusion: EvoXplain使解释不稳定性可见且可量化，揭示单一实例或平均解释可能掩盖多种潜在机制的存在，将可解释性重新定义为模型类在重复实例化下的属性而非单个训练模型的属性
...
### [67] [Fairness Evaluation of Risk Estimation Models for Lung Cancer Screening](https://arxiv.org/abs/2512.22242)
*Shaurya Gaur,Michel Vitale,Alessa Hering,Johan Kwisthout,Colin Jacobs,Lena Philipp,Fennie van der Graaf*

Main category: cs.LG

TL;DR: 本研究评估了三种肺癌筛查风险估算模型（两种深度学习模型和一种 logistic 回归模型）在不同人口统计亚组中的性能差异和公平性，发现深度学习模型存在显著的性别和种族相关性能偏差，强调需改进和监测模型在代表性不足亚组中的表现。


<details>
  <summary>Details</summary>
Motivation: 肺癌是全球成人癌症相关死亡的主要原因，年度低剂量 CT（LDCT）筛查有助于早期发现和降低死亡，但广泛实施可能给有限的放射科人力带来压力。AI 模型在从 LDCT 扫描估算肺癌风险方面显示出潜力，但肺癌高危人群多样化，这些模型在不同人口统计群体中的性能仍是未解决的问题。

Method: 基于 JustEFAB 框架中关于混杂因素和伦理显著偏差的考虑，评估两种深度学习肺癌筛查风险估算模型（Sybil 和 Venkadesh21）以及英国胸科学会结节管理指南推荐的 PanCan2b logistic 回归模型。两种深度学习模型均在美国国家肺部筛查试验（NLST）数据上训练，并在保留的 NLST 验证集上评估；评估不同人口统计亚组的 AUROC、敏感性和特异性，并探索临床风险因素的潜在混杂影响。

Result: Sybil 模型在女性（AUROC=0.88，95%CI：0.86-0.90）和男性（AUROC=0.81，95%CI：0.78-0.84，p<0.001）之间存在统计学显著的 AUROC 差异；Venkadesh21 模型在特异性为 90% 时，黑人参与者的敏感性（0.39，95%CI：0.23-0.59）低于白人参与者（0.69，95%CI：0.65-0.73）；这些差异无法用现有临床混杂因素解释，根据 JustEFAB 可归类为不公平偏差。

Conclusion: 研究结果强调了在肺癌筛查中改进和监测模型在代表性不足亚组中的性能，以及进一步研究算法公平性的重要性。
...
### [68] [Calibrating LLM Judges: Linear Probes for Fast and Reliable Uncertainty Estimation](https://arxiv.org/abs/2512.22245)
*Bhaktipriya Radharapu,Eshika Saxena,Kenneth Li,Chenxi Whitehouse,Adina Williams,Nicola Cancedda*

Main category: cs.LG

TL;DR: 本文提出一种基于线性探针的不确定性估计方法，用于LLM法官模型，在保持校准性的同时大幅降低计算成本，为生产环境提供实用解决方案。


<details>
  <summary>Details</summary>
Motivation: 随着基于LLM的法官模型在工业应用中日益重要，高效获取校准良好的不确定性估计对生产部署至关重要，但现有方法存在校准差或计算成本高的问题。

Method: 引入以Brier分数损失训练的线性探针，从推理法官的隐藏状态中提取校准后的不确定性估计，无需额外模型训练。

Result: 在客观任务（推理、数学、事实性、编码）和主观人类偏好判断上评估，结果显示探针校准性能优于现有方法且计算效率提升约10倍，对未见过的评估领域泛化性强，高置信度预测准确率更高；但在简单数据集上估计保守，可能有利于优先考虑低假阳性率的安全关键部署。

Conclusion: 基于可解释性的不确定性估计为生产环境中的LLM法官模型提供了实用且可扩展的即插即用解决方案。
...
### [69] [The Affine Divergence: Aligning Activation Updates Beyond Normalisation](https://arxiv.org/abs/2512.22247)
*George Bird*

Main category: cs.LG

TL;DR: 本文指出梯度下降中数学理想与实际激活更新存在系统不匹配，提出修正方法并重新解释归一化作用，还提出非尺度不变的替代方案及新的PatchNorm方法，为模型构建提供理论框架。


<details>
  <summary>Details</summary>
Motivation: 梯度下降中参数按最陡下降方向更新，但激活作为更直接影响优化的量（更接近损失且携带样本依赖信息），其传播更新未采取最优最陡下降步骤，且在仿射、卷积和注意力层中存在非理想的样本级缩放问题。

Method: 通过从第一性原理推导修正激活更新的非理想缩放问题，提出与现代归一化功能不同且无尺度不变性的替代方案（作为仿射映射的替代），并将其推广到卷积层形成新的“PatchNorm”（组合不可分离的归一化器）；同时将归一化器分解为带参数化缩放的激活函数类映射。

Result: 提出的非尺度不变替代方案在多个测试中优于传统归一化器；PatchNorm作为新的卷积层归一化方法有效；辅助实验支持对归一化作用的机制性重新解释；新方法经实证验证成功。

Conclusion: 该理论原则性方法提供了新的经验验证函数，为归一化讨论补充并反驳部分观点，提出替代机制框架，对仿射+非线性的模型创建方法提出质疑，有助于优化中表征的优先级排序。
...
### [70] [Temporal Visual Semantics-Induced Human Motion Understanding with Large Language Models](https://arxiv.org/abs/2512.22249)
*Zheng Xing,Weibing Zhao*

Main category: cs.LG

TL;DR: 本文提出一种结合时间视觉语义（TVS）的子空间聚类方法，通过大语言模型（LLM）提取运动序列的文本信息并融入子空间聚类框架，提升无监督人体运动分割（HMS）性能。


<details>
  <summary>Details</summary>
Motivation: 传统子空间聚类技术在无监督人体运动分割中忽略了时间语义探索的作用，导致性能受限。

Method: 利用LLM的图文转换能力提取连续帧的文本运动信息，通过查询LLM判断连续帧是否为同一运动以学习时间邻域信息；构建TVS集成子空间聚类方法，将子空间嵌入与时间正则化器结合，使帧与时间邻居共享相似嵌入；基于带时间约束的子空间嵌入进行分割；引入反馈框架根据分割结果持续优化子空间嵌入。

Result: 在四个基准人体运动数据集上，所提方法优于现有最先进方法。

Conclusion: 结合时间视觉语义的子空间聚类方法可有效提升无监督人体运动分割性能，为该领域提供新的解决方案。
...
### [71] [Cardiac mortality prediction in patients undergoing PCI based on real and synthetic data](https://arxiv.org/abs/2512.22259)
*Daniil Burakov,Ivan Petrov,Dmitrii Khelimskii,Ivan Bessonov,Mikhail Lazarev*

Main category: cs.LG

TL;DR: 本研究开发了基于真实和合成数据的PCI术后心脏死亡风险预测模型，通过合成样本增强解决类别不平衡问题，并识别出关键影响因素。


<details>
  <summary>Details</summary>
Motivation: PCI术后患者的状态、血管造影和手术特征对长期预后预测至关重要，但现有模型在类别不平衡（少数类为死亡病例）情况下表现不佳，几乎完全忽略少数类。

Method: 分析2044例PCI分叉病变患者数据，应用多种机器学习模型；生成500个合成样本添加到训练集以解决类别不平衡；使用置换特征重要性评估特征贡献；进行移除非信息特征的额外实验。

Result: 未过采样时模型整体准确率高（0.92-0.93）但忽略少数类；样本增强后少数类召回率提升，AUROC损失最小，概率质量改善，严重病例风险估计更合理；识别出年龄、射血分数、外周动脉疾病、脑血管疾病为最具影响力的四个特征。

Conclusion: 使用真实和极端病例的简单样本增强可暴露、量化和减少不平衡临床预测中的脆弱性；建议常规报告概率质量和压力测试结果，而非仅关注主要指标。
...
### [72] [The Physics Constraint Paradox: When Removing Explicit Constraints Improves Physics-Informed Data for Machine Learning](https://arxiv.org/abs/2512.22261)
*Rahul D Ray*

Main category: cs.LG

TL;DR: 本文针对科学领域中真实数据稀缺时的物理约束数据生成问题，通过对物理信息光栅耦合器光谱生成器的系统消融研究，揭示了物理约束悖论、关键约束的影响及设计陷阱，并为物理信息数据集设计提供指导。


<details>
  <summary>Details</summary>
Motivation: 科学领域中真实数据稀缺，物理约束数据生成对机器学习至关重要，但现有方法常过度约束模型而未识别必要的物理组件。

Method: 对映射五个几何参数到100点光谱响应的物理信息光栅耦合器光谱生成器进行系统消融研究，选择性移除显式能量守恒执行、法布里-珀罗振荡、带宽变化和噪声。

Result: 1.能量守恒约束在基础方程物理一致时数学冗余，约束与无约束变体守恒精度相同（平均误差约7×10^-9）；2.法布里-珀罗振荡主导基于阈值的带宽变异性，移除后带宽半最大值分布减少72%（从132.3nm降至37.4nm）；3.标准噪声添加加重新归一化流程引入0.5%非物理负吸收值；4.生成器速度达每秒200样本，比典型全波求解器快多个数量级；5.下游机器学习评估显示物理可学习性权衡：中心波长预测不受影响，移除法布里-珀罗振荡使带宽预测R²提升31.3%、RMSE降低73.8%。

Conclusion: 研究结果为物理信息数据集设计提供可操作指导，并强调机器学习性能可作为评估约束相关性的诊断工具。
...
TL;DR: 本文针对大语言模型（LLMs）在动态图时态 motif 分析中的应用展开研究，提出LLMTM基准、工具增强LLM智能体及结构感知调度器，在保证高精度的同时平衡成本。


<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLMs）广泛应用激发了其处理动态图的研究兴趣，时态 motif 作为动态图的基本单元和重要局部属性对理解其演化动态和结构特征至关重要，但LLMs在时态 motif 分析中的应用仍未充分探索。

Method: 1. 提出LLMTM基准，包含9种时态 motif 类型下的6个定制任务；2. 实验分析不同提示技术和9种LLM模型（如openPangu-7B、GPT-4o-mini等）的性能；3. 开发工具增强LLM智能体，利用精确设计的提示提升任务精度；4. 提出结构感知调度器，结合动态图结构属性和LLM认知负荷，智能分配查询至标准LLM提示或工具增强智能体。

Result: 1. 工具增强LLM智能体可高精度完成时态 motif 相关任务，但成本较高；2. 结构感知调度器在保持高精度的同时有效降低了成本。

Conclusion: 本研究通过LLMTM基准验证了LLMs在时态 motif 分析中的潜力，工具增强智能体和结构感知调度器的提出为平衡精度与成本提供了有效解决方案，为动态图分析中LLMs的应用提供了新的思路和方法。
...
TL;DR: 本文针对辐射诱导接枝（RIG）技术中因基膜形态变异性导致的重现性问题，提出了一种基于Dirichlet过程的分层堆叠优化框架（SoDip），通过整合多模态数据建模和贝叶斯优化，提升了RIG过程的可预测性和重现性。


<details>
  <summary>Details</summary>
Motivation: 辐射诱导接枝（RIG）可实现聚合物膜的精准功能化，但基膜形态（结晶度、晶粒取向、自由体积）的未报告变异性导致单体扩散、自由基分布和Trommsdorff效应不稳定，进而引发空间接枝梯度和性能不一致，限制了技术重现性。

Method: 提出SoDip框架，包括：1）使用Decoder-only Transformer（DeepSeek-R1）编码文本工艺描述符；2）TabNet和XGBoost建模多模态特征交互；3）结合Dirichlet过程混合模型（DPMM）的高斯过程回归（GPR）进行不确定性量化和异方差性分析；4）贝叶斯优化探索高维合成空间。同时使用ChemDataExtractor 2.0和WebPlotDigitizer构建多变量数据集。

Result: 交叉验证显示，SoDip比GPR模型性能提升约33%，并提供校准的置信区间以识别低重现性区域；其堆叠架构可整合稀疏、质量不一的文本和数值输入，优于现有模型。

Conclusion: SoDip框架为接枝聚合研究中可重现、形态感知的设计奠定了基础，有效解决了RIG技术的重现性问题。
...
TL;DR: 现代AI系统的浮点向量嵌入导致跨硬件非确定性，影响可复现性和安全部署，本文提出Valori确定性AI内存基板解决该问题


<details>
  <summary>Details</summary>
Motivation: 现代AI系统依赖浮点向量嵌入进行近似相似性搜索，但该设计存在根本非确定性：相同模型、输入和代码在不同硬件架构（如x86与ARM）上会产生不同内存状态和检索结果，阻碍可复现性和安全部署，导致静默数据分歧，影响事后验证并损害受监管行业的审计跟踪

Method: 提出Valori，一种确定性AI内存基板，用定点算术（Q16.16）替代浮点内存操作，并将内存建模为可复现状态机

Result: Valori保证跨平台的位一致内存状态、快照和搜索结果；证明非确定性在索引或检索前就已出现，并展示Valori如何在内存边界强制执行确定性

Conclusion: 确定性内存是可信AI系统的必要原语，Valori参考实现开源可用
...
### [76] [Cluster Aggregated GAN (CAG): A Cluster-Based Hybrid Model for Appliance Pattern Generation](https://arxiv.org/abs/2512.22287)
*Zikun Guoa,Adeyinka. P. Adedigbaa,Rammohan Mallipeddi*

Main category: cs.LG

TL;DR: 本文提出Cluster Aggregated GAN框架，通过分支化模型处理间歇性和连续性电器的行为差异，提升合成电器数据的真实性、多样性和训练稳定性，为非侵入式负荷监测算法开发和隐私保护能源研究提供支持。


<details>
  <summary>Details</summary>
Motivation: 合成电器数据对非侵入式负荷监测算法开发和隐私保护能源研究至关重要，但标注数据集稀缺；现有基于GAN的方法将所有设备统一处理，忽略间歇性与连续性电器的行为差异，导致训练不稳定和输出保真度有限。

Method: 提出Cluster Aggregated GAN混合生成框架：基于行为特征将电器路由至专用分支；间歇性电器通过聚类模块分组相似激活模式，为每个簇分配专用生成器；连续性电器通过LSTM生成器捕捉渐变时间演化，通过序列压缩保持训练稳定。

Result: 在UVIC智能插头数据集上的实验表明，该框架在真实性、多样性和训练稳定性指标上持续优于基线方法；将聚类作为主动生成组件显著提升可解释性和可扩展性。

Conclusion: Cluster Aggregated GAN框架是一种有效的合成负荷生成方法，可用于非侵入式负荷监测研究。
...
### [77] [Co-GRPO: Co-Optimized Group Relative Policy Optimization for Masked Diffusion Model](https://arxiv.org/abs/2512.22288)
*Renping Zhou,Zanlin Ni,Tianyi Chen,Zeyu Liu,Yang Yue,Yulin Wang,Yuxuan Wang,Jingshu Liu,Gao Huang*

Main category: cs.LG

TL;DR: 本文提出Co-GRPO方法，通过将掩码扩散模型（MDMs）的生成过程重构为统一的马尔可夫决策过程（MDP），协同优化模型参数和推理调度参数，解决训练与推理过程不一致的问题，提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散模型（MDMs）在视觉、语言和跨模态生成领域表现出潜力，但存在训练与推理过程差异的问题：推理是多步迭代过程，受模型和调度影响；而训练通常采用单步BERT式目标，导致训练范式与推理轨迹性质脱节，推理调度未在训练中优化。

Method: 提出Co-GRPO方法，将MDM生成重构为统一的马尔可夫决策过程（MDP），联合模型和推理调度；应用轨迹级别的Group Relative Policy Optimization，在共享奖励下协同优化模型参数和调度参数，无需通过多步生成过程进行昂贵的反向传播。

Result: 在ImageReward、HPS、GenEval和DPG-Bench四个基准测试中验证了方法的有效性，显著提升生成质量。

Conclusion: Co-GRPO通过整体优化使训练与推理更一致，大幅提高生成质量，为掩码扩散模型的训练与推理对齐提供了有效解决方案。
...
### [78] [When Algorithms Manage Humans: A Double Machine Learning Approach to Estimating Nonlinear Effects of Algorithmic Control on Gig Worker Performance and Wellbeing](https://arxiv.org/abs/2512.22290)
*Arunkumar V,Nivethitha S,Sharan Srinivas,Gangadharan G. R*

Main category: cs.LG

TL;DR: 本研究探讨算法管理下人本管理的生存可能性，通过双机器学习框架分析464名零工工人数据，发现算法监督透明度对HR实践与绩效关系的非单调影响，为平台设计和组织研究方法提供启示


<details>
  <summary>Details</summary>
Motivation: 未来工作的核心问题是算法承担管理角色时人本管理能否存续，而现有工具因工人对算法系统的反应非线性常无法捕捉实际情况

Method: 采用双机器学习框架估计无限制性函数形式的调节中介模型，使用464名零工工人的调查数据

Result: 支持性HR实践可提升工人福祉，但与绩效的关联在算法监督存在但难以解释的模糊中间区域减弱，当监督透明且可解释时该关系再次增强

Conclusion: 简单线性模型会遗漏上述非单调模式甚至得出相反结论；平台设计中，部分定义的控制会造成混乱，清晰规则和可信追索权可使强监督可行；方法论上，双机器学习可用于组织研究中估计条件间接效应而无需将数据强转为线性
...
### [79] [Learning from Negative Examples: Why Warning-Framed Training Data Teaches What It Warns Against](https://arxiv.org/abs/2512.22293)
*Tsogt-Ochir Enkhbayar*

Main category: cs.LG

TL;DR: 研究发现训练数据中的警告框架内容无法教会语言模型避免被警告行为，统计共现主导模型学习而非语用解释


<details>
  <summary>Details</summary>
Motivation: 探索为何训练数据中的警告框架内容（如“请勿使用——此代码存在漏洞”）未能有效阻止语言模型再现被警告的行为

Method: 通过实验对比暴露于警告内容的模型与直接接收内容的模型的再现率，使用稀疏自动编码器分析潜在特征激活情况，研究“隐形滑移”现象及提示、推理时引导与训练时特征消融的效果

Result: 1. 暴露于警告内容的模型再现标记内容的比例（76.7%）与直接接收内容的模型（83.3%）无统计学差异；2. 稀疏自动编码器分析显示“描述X”与“执行X”激活重叠的潜在特征（如跟踪代码执行模式的特征#8684在警告和利用场景中激活程度相当）；3. “隐形滑移”现象使对话前导可将激活旋转到线性探针完全无法检测的子空间；4. 提示和推理时引导无法解决该问题，但训练时特征消融有效

Conclusion: 当前架构中统计共现主导语用解释，模型学习上下文后倾向出现的内容而非其出现原因，警告框架内容无法有效阻止模型再现被警告行为，需训练时特征消融等方法解决
...
### [80] [LLMBoost: Make Large Language Models Stronger with Boosting](https://arxiv.org/abs/2512.22309)
...
TL;DR: 本文提出LLMBoost框架，通过利用LLM中间状态实现集成微调，提升性能同时降低推理延迟。


<details>
  <summary>Details</summary>
Motivation: 现有LLM集成方法将模型视为黑盒，仅结合输入或最终输出，忽略了丰富的内部表示和跨模型交互。

Method: LLMBoost框架包含三个创新点：1.跨模型注意力机制，让后续模型访问并融合前驱模型的隐藏状态；2.链式训练范式，以抑制错误为目标逐步微调连接模型；3.近并行推理范式，逐层传递模型间的隐藏状态。此外还建立了理论基础，证明在有界修正假设下序列集成可保证单调改进。

Result: 在常识推理和算术推理任务上的实验表明，LLMBoost持续提升准确率且降低推理延迟。

Conclusion: LLMBoost通过显式利用LLM中间状态突破了传统集成方法的黑盒限制，在性能和效率上均有显著优势。
...
### [81] [Optimistic Feasible Search for Closed-Loop Fair Threshold Decision-Making](https://arxiv.org/abs/2512.22313)
*Wenzhang Du*

Main category: cs.LG

TL;DR: 本文研究闭环决策系统中满足人口统计平等（DP）和服务率约束的在线阈值策略学习问题，提出Optimistic Feasible Search（OFS）方法并验证其在合成和半合成基准上的有效性。


<details>
  <summary>Details</summary>
Motivation: 闭环决策系统（如贷款、筛选或再犯风险评估）在公平性和服务约束下运行时会产生反馈效应，导致数据非平稳并可能扩大差异，现有方法在约束满足和高回报之间的平衡存在不足。

Method: 提出基于网格的Optimistic Feasible Search（OFS）方法，该方法维护每个候选阈值的奖励和约束残差的置信区间；在置信区间下选择可行且乐观奖励最大的阈值，若无可行则选择乐观约束违反最小的阈值。

Result: 在稳定收缩动力学的合成闭环基准、基于German Credit和COMPAS的半合成闭环基准上，OFS相比无约束和原始对偶bandit基线，获得更高奖励且累积约束违反更小，与相同扫描过程下最优可行固定阈值相比接近oracle水平。

Conclusion: OFS方法在低维、可解释的策略类中（离散化自然的场景）直接针对可行的高效用阈值，表现优异，实验可复现且采用双盲友好的相对输出组织方式。
...
### [82] [LangPrecip: Language-Aware Multimodal Precipitation Nowcasting](https://arxiv.org/abs/2512.22317)
*Xudong Ling,Tianxi Huang,Qian Dong,Tao He,Chaorong Li,Guiduo Duan*

Main category: cs.LG

TL;DR: 提出一种语言感知的多模态短时降水临近预报框架LangPrecip，利用气象文本作为语义运动约束，通过Rectified Flow范式整合文本与雷达信息，并构建160k规模数据集LangPrecip-160k，实验显示在重降水CSI指标上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 短时降水临近预报存在不确定性和约束不足问题，尤其是快速演变和极端天气事件；现有生成方法依赖视觉条件，导致未来运动约束弱且模糊。

Method: 提出LangPrecip框架，将气象文本作为降水演变的语义运动约束；将临近预报建模为Rectified Flow范式下的语义约束轨迹生成问题，在 latent空间高效且物理一致地整合文本与雷达信息；构建包含160k对雷达序列和运动描述的多模态数据集LangPrecip-160k。

Result: 在瑞典和MRMS数据集上实验，相比最先进方法持续改进，在80分钟提前期的重降水CSI指标上分别获得超过60%和19%的提升。

Conclusion: LangPrecip框架通过引入语言语义约束有效提升了短时降水临近预报的性能，特别是在重降水场景下，构建的大规模数据集为相关研究提供了支持。
...
### [83] [Decomposing Uncertainty in Probabilistic Knowledge Graph Embeddings: Why Entity Variance Is Not Enough](https://arxiv.org/abs/2512.22318)
*Chorok Lee*

Main category: cs.LG

TL;DR: 本文指出概率知识图谱嵌入中实体不确定性与关系无关的局限性，提出将不确定性分解为语义和结构两部分并通过学习权重结合的方法（CAGP），显著提升了时间分布偏移下的OOD检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有概率知识图谱嵌入方法的实体不确定性与关系无关，无法区分新兴实体（罕见、学习不足）和新关系上下文（熟悉实体在未观察关系中）这两种表现相反的分布外现象，导致在时间分布偏移下OOD检测性能差。

Method: 提出不确定性分解为语义不确定性（来自实体嵌入方差，检测新兴实体）和结构不确定性（来自实体-关系共现，检测新关系上下文），并通过学习权重结合这两部分的方法（CAGP）。

Result: 在多个基准数据集上，CAGP在时间OOD检测上达到0.94-0.99的AUROC，相对关系无关基线提升60-80%；在选择性预测中，85%回答率下错误减少43%；验证了三个数据集上新颖上下文三元组与频率匹配的分布内三元组完全重叠。

Conclusion: 语义不确定性和结构不确定性信号是非冗余的，其凸组合严格优于单独使用任一信号；CAGP方法有效结合这两种不确定性，显著提升了时间分布偏移下的OOD检测性能和选择性预测效果。
...
### [84] [Expert System for Bitcoin Forecasting: Integrating Global Liquidity via TimeXer Transformers](https://arxiv.org/abs/2512.22326)
*Sravan Karthick T*

Main category: cs.LG

TL;DR: 本文针对比特币价格预测的极端波动性和非平稳性问题，提出整合18个主要经济体的全球M2流动性作为滞后12周的外生变量，采用TimeXer架构构建流动性条件预测模型（TimeXer-Exog），并与多种基准模型对比，结果表明该模型在长期预测中显著优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 比特币价格预测具有极端波动性和非平稳性，传统单变量时间序列模型在长期预测中效果不佳，存在未利用宏观经济变量（如全球流动性）的研究缺口。

Method: 整合18个主要经济体的全球M2流动性作为滞后12周的外生变量，采用TimeXer架构构建流动性条件预测模型（TimeXer-Exog），并与LSTM、N-BEATS、标准单变量TimeXer等基准模型进行对比实验。

Result: 在2020年1月至2025年8月的日度比特币价格数据上，TimeXer-Exog模型在70天预测 horizon 下的均方误差（MSE）为1.08e8，比单变量TimeXer基线模型的性能提升超过89%，显式的宏观经济条件显著稳定了长期预测结果。

Conclusion: 将深度学习模型与全球流动性条件结合，能大幅改善比特币价格的长期预测效果，为解决比特币价格长期预测难题提供了有效方法。
...
### [85] [The Effectiveness of Approximate Regularized Replay for Efficient Supervised Fine-Tuning of Large Language Models](https://arxiv.org/abs/2512.22337)
*Matthew Riemer,Erik Miehling,Miao Liu,Djallel Bouneffouf,Murray Campbell*

Main category: cs.LG

TL;DR: 本文研究表明LoRA等参数高效微调方法可能导致模型能力灾难性下降，但通过正则化近似重放方法（惩罚与初始模型的KL散度并插入预训练类似语料的下一个token预测数据）可在几乎不增加计算开销的情况下消除该问题，同时保留模型的通用知识和对新任务的可塑性。


<details>
  <summary>Details</summary>
Motivation: 虽然LoRA等参数高效微调方法仅修改少量参数，但可能对模型产生显著负面影响，例如在指令微调实验中，基于LoRA的监督微调即使在小数据集上训练很少步骤也会灾难性地降低模型能力。

Method: 本文采用正则化近似重放方法，具体包括：1）惩罚与初始模型的KL散度；2）插入来自与预训练所用语料不同但相似的开放访问语料库的下一个token预测数据。

Result: 将该方法应用于Qwen指令微调模型时，发现该方案在增加适度计算开销的情况下，既能保留模型的通用知识，又不会阻碍其对新任务的可塑性。

Conclusion: 通过对训练过程进行小调整（正则化近似重放方法），可以有效解决LoRA等参数高效微调方法导致的模型能力下降问题，同时保持模型的通用知识和新任务适应性。
...
### [86] [Completed Hyperparameter Transfer across Modules, Width, Depth, Batch and Duration](https://arxiv.org/abs/2512.22382)
*Bruno Mlodozeniec,Pierre Ablin,Louis Béthune,Dan Busbridge,Michal Klein,Jason Ramapuram,Marco Cuturi*

Main category: cs.LG

TL;DR: 本文提出Complete^(d)参数化方法，统一模型宽度、深度、批量大小和训练时长的缩放，并研究模块级超参数优化与迁移，结果表明该方法能提升大语言模型训练速度。


<details>
  <summary>Details</summary>
Motivation: 超参数调优对大规模模型训练稳定性和性能影响显著，但现有如μP的参数化方法仅支持全局超参数跨模型大小迁移，未覆盖多维度缩放及模块级超参数迁移需求。

Method: 1. 提出Complete^(d)参数化方法，统一宽度、深度、批量大小和训练时长的缩放；2. 研究模块级超参数优化与迁移，分析高维超参数空间的挑战并提出实用指南。

Result: 在模块级超参数场景下，通过合适的参数化方法可实现超参数迁移，且迁移后的模块级超参数能显著提升大语言模型的训练速度。

Conclusion: Complete^(d)参数化方法有效支持多维度缩放及模块级超参数迁移，为大规模模型超参数优化提供了实用解决方案。
...
### [87] [Causality-Inspired Safe Residual Correction for Multivariate Time Series](https://arxiv.org/abs/2512.22428)
*Jianxiang Xie,Yuncheng Hua*

Main category: cs.LG

TL;DR: 本文提出CRC（因果启发式安全残差校正）框架，解决现代多变量预测模型在部署中性能退化问题，通过因果编码器和安全机制确保校正不损害可靠预测，提升准确性和非退化率。


<details>
  <summary>Details</summary>
Motivation: 现代多变量预测模型（如Transformer和GNN）虽基准性能强，但存在特定变量或时间范围的系统误差，且部署时缺乏性能不退化的保障；现有事后残差校正方法贪心，可能过度校正可靠预测导致新场景局部失效，存在安全缺口。

Method: 提出CRC框架：1.因果启发编码器，通过解耦自变量和交叉变量动态暴露方向感知结构；2.混合校正器建模残差误差；3.严格四重安全机制管控校正过程，防止有害更新。

Result: 多数据集和预测骨干实验显示CRC持续提升准确性；消融研究证实核心安全机制确保极高非退化率（NDR）。

Conclusion: CRC是适用于安全可靠部署的校正框架，能在提升准确性的同时保证非退化率，解决现有方法的安全问题。
...
### [88] [AFA-LoRA: Enabling Non-Linear Adaptations in LoRA with Activation Function Annealing](https://arxiv.org/abs/2512.22455)
*Jiacheng Li,Jianchao Tan,Zhidong Yang,Feiye Huo,Yerui Sun,Yuchen Xie,Xunliang Cai*

Main category: cs.LG

TL;DR: 本文提出AFA-LoRA，一种为LoRA引入非线性表达能力同时保持可合并性的新训练策略，通过退火激活函数缩小LoRA与全参数训练的性能差距。


<details>
  <summary>Details</summary>
Motivation: LoRA作为广泛采用的参数高效微调方法，其线性适配过程限制了表达能力，导致线性训练与非线性训练之间存在表达能力差距。

Method: 提出AFA-LoRA策略，核心创新是退火激活函数，该函数在训练过程中从非线性转换过渡到线性转换，使适配器先具备更强的表征能力，再收敛到可合并的线性形式；并在监督微调、强化学习和推测解码场景中实现该方法。

Result: AFA-LoRA缩小了LoRA与全参数训练之间的性能差距。

Conclusion: 该工作实现了更强大且实用的参数高效适配范式。
...
### [89] [AMBIT: Augmenting Mobility Baselines with Interpretable Trees](https://arxiv.org/abs/2512.22466)
*Qizhi Wang*

Main category: cs.LG

TL;DR: 本文提出AMBIT框架，结合物理移动基线与可解释树模型解决OD流预测中准确性与可解释性的冲突问题。


<details>
  <summary>Details</summary>
Motivation: OD流预测是GIS和城市分析的核心任务，但实际部署中存在高准确性与清晰可解释性的冲突需求。

Method: 1. 基于纽约市一年每小时出租车OD数据集，全面评估经典空间交互模型；2. 在物理基线基础上构建残差学习器，使用梯度提升树和SHAP分析；3. 提供可复现的管道、丰富诊断和空间误差分析。

Result: 1. 大多数物理模型在小时级时间分辨率下脆弱，PPML引力模型是最强物理基线，约束变体在全OD边际校准后有所改进但仍较弱；2. 基于物理的残差模型接近强树模型预测器的准确性，同时保留可解释结构；3. POI锚定残差模型在空间泛化下持续具有竞争力且最稳健。

Conclusion: AMBIT框架有效平衡了OD流预测的准确性与可解释性，为城市决策提供支持。
...
### [90] [Collaborative Optimization of Multiclass Imbalanced Learning: Density-Aware and Region-Guided Boosting](https://arxiv.org/abs/2512.22478)
*Chuantao Li,Zhi Li,Jiahao Xu,Jie Li,Sheng Li*

Main category: cs.LG

TL;DR: 本研究提出一种多类不平衡学习的协同优化Boosting模型，通过集成密度因子和置信因子设计抗噪权重更新机制与动态采样策略，实现不平衡学习与模型训练的协同优化，在20个公共不平衡数据集上的实验表明该模型显著优于8种先进基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有研究尚未探索不平衡学习与模型训练的协同优化，这一约束阻碍了性能的进一步提升，因此需要提出新方法来填补这一空白。

Method: 提出一种多类不平衡学习的协同优化Boosting模型，集成密度因子和置信因子，设计抗噪权重更新机制和动态采样策略，并将这些模块紧密集成以协调权重更新、样本区域划分和区域引导采样。

Result: 在20个公共不平衡数据集上的广泛实验表明，所提出的模型显著优于8种最先进的基线方法。

Conclusion: 本研究通过协同优化不平衡学习与模型训练，提出的模型在处理多类不平衡分类问题上表现出色，为该领域提供了一种有效解决方案。
...
### [91] [Predicting LLM Correctness in Prosthodontics Using Metadata and Hallucination Signals](https://arxiv.org/abs/2512.22508)
*Lucky Susanto,Anasta Pranawijayana,Cortino Sukotjo,Soni Prasad,Derry Wijaya*

Main category: cs.LG

TL;DR: 本文研究了在高风险领域中预测大语言模型（LLM）回答正确性的可行性，通过分析GPT-4o和OSS-120B模型在口腔修复学多选题考试中的表现，利用元数据和幻觉信号构建正确性预测器，结果表明该方法可提升准确性，但尚未足够鲁棒以支持关键高风险场景部署。


<details>
  <summary>Details</summary>
Motivation: LLM在医疗等高风险领域应用日益广泛，但生成事实错误（幻觉）信息的风险是主要关注点；尽管已有大量检测和缓解幻觉的努力，但预测LLM回答是否正确仍是关键且未充分探索的问题。

Method: 分析通用模型GPT-4o和推理中心模型OSS-120B在口腔修复学多选题考试中的表现；利用三种不同提示策略下的元数据和幻觉信号，为每个（模型，提示）对构建正确性预测器。

Result: 基于元数据的方法相比假设所有答案都正确的基线，准确性可提升高达+7.14%，并达到83.12%的精度；实际幻觉是回答错误的强指标，但仅元数据信号不能可靠预测幻觉；提示策略不影响整体准确性，但显著改变模型内部行为和元数据的预测效用。

Conclusion: 基于元数据的方法为LLM可靠性信号开发提供了有前景的方向，但本文探索的方法尚未足够鲁棒以支持关键、高风险场景的部署。
...
### [92] [Decomposing Task Vectors for Refined Model Editing](https://arxiv.org/abs/2512.22511)
*Hamed Damirchi,Ehsan Abbasnejad,Zhen Zhang,Javen Shi*

Main category: cs.LG

TL;DR: 本文提出了一种任务向量分解方法，将任务向量分为共享知识和独特任务信息两部分，解决了任务向量算术操作中概念重叠导致的不可预测问题，并在图像分类、扩散模型风格混合和语言模型毒性降低三个领域验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 大型预训练模型的有效适配以展现精确的概念特定行为仍是挑战，现有任务向量算术操作因概念重叠会导致不可预测的结果，影响模型编辑操作的准确性。

Method: 提出一种原则性分解方法，将每个任务向量分为共享知识组件和独特任务信息组件，通过识别投影中的不变子空间实现对概念操作的精确控制。

Result: 在三个领域验证了方法的有效性：图像分类中多任务合并提升5%；扩散模型中仅混合独特组件实现干净的风格混合且无生成质量下降；语言模型中减少47%毒性同时保留通用知识任务性能。

Conclusion: 该分解方法为理解和控制任务向量算术提供了新框架，解决了模型编辑操作中的基本限制。
...
### [93] [TimePerceiver: An Encoder-Decoder Framework for Generalized Time-Series Forecasting](https://arxiv.org/abs/2512.22550)
*Jaebin Lee,Hankook Lee*

Main category: cs.LG

TL;DR: 本文提出TimePerceiver统一编码器-解码器预测框架，解决时间序列预测中编码、预测和训练策略分离的问题，通过灵活处理不同时间位置的输入和目标段，在多数据集上显著优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 机器学习中有效建模需综合考虑输入编码、预测和解码及训练策略，但现有时间序列预测工作主要聚焦编码器设计，常将预测和训练视为分离或次要问题，未能统一处理。

Method: 提出TimePerceiver框架，将预测任务泛化为包含外推、内插和插补等多样时间预测目标；设计能灵活感知不同时间位置的编码器-解码器架构：编码阶段用潜在瓶颈表示捕获时间和跨通道依赖，解码阶段用可学习查询检索目标时间戳信息。

Result: 在多种基准数据集上，该框架持续且显著优于现有最先进基线模型。

Conclusion: TimePerceiver框架有效统一了时间序列预测中的编码、预测和解码及训练策略，在多任务和数据集上表现出色，代码已开源。
...
### [94] [On Admissible Rank-based Input Normalization Operators](https://arxiv.org/abs/2512.22587)
*Taeyun Kim*

Main category: cs.LG

TL;DR: 本文针对基于秩的输入归一化的稳定性问题，提出形式化公理并构建满足条件的算子，明确有效设计空间与现有方法的区别。


<details>
  <summary>Details</summary>
Motivation: 基于秩的输入归一化在机器学习中广泛应用，但缺乏对其在不变性下保持稳定的结构条件的形式化定义，现有可微分排序和排序算子存在结构设计导致的稳定性缺陷。

Method: 提出三个形式化公理以明确基于秩的输入归一化所需的最小不变性和稳定性属性，证明满足公理的算子需分解为特征wise秩表示和单调且Lipschitz连续的标量化映射，并构建满足条件的最小算子。

Result: 现有基于连续松弛的排序方法因依赖值间隙和批量级成对交互，在严格单调变换、小批量组成变化及微小输入扰动下本质不稳定；构建的算子在实际场景中满足非平凡约束。

Conclusion: 研究结果清晰界定了有效基于秩的归一化算子的设计空间，并将其与现有连续松弛排序方法从形式上区分开来。
...
### [95] [Data-Driven Analysis of Crash Patterns in SAE Level 2 and Level 4 Automated Vehicles Using K-means Clustering and Association Rule Mining](https://arxiv.org/abs/2512.22589)
*Jewel Rana Palit,Vijayalakshmi K Kumarasamy,Osama A. Osman*

Main category: cs.LG

TL;DR: 本研究通过分析美国国家公路交通安全管理局（NHTSA）的2500多条自动驾驶汽车（AV）碰撞记录，使用两阶段数据挖掘框架揭示了不同SAE自动化级别的碰撞动态，为AV开发者、监管者和政策制定者提供行动指导。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶汽车（AV）虽有减少人为驾驶错误、提升交通安全等潜力，但近期碰撞数据显示其行为可能偏离预期安全结果，引发对混合交通环境中技术安全性和可靠性的担忧；过往研究多依赖以加州为中心的小型数据集，且对不同SAE自动化级别的碰撞趋势关注有限。

Method: 开发两阶段数据挖掘框架：首先应用K-means聚类基于时间、空间和环境因素将碰撞记录分为4个不同行为簇；然后使用关联规则挖掘（ARM）提取每个簇内碰撞模式与碰撞贡献因素（如光照条件、路面状况、车辆动态和环境条件）之间可解释的多变量关系。

Result: 通过两阶段数据挖掘框架，成功将碰撞记录分为4个行为簇，并在每个簇内提取了碰撞模式与多种贡献因素的关联规则，获得了可解释的多变量关系。

Conclusion: 研究得出的碰撞动态洞察可为自动驾驶汽车开发者、安全监管者和政策制定者制定AV部署策略和最小化碰撞风险提供可操作的指导。
...
### [96] [Energy-Guided Flow Matching Enables Few-Step Conformer Generation and Ground-State Identification](https://arxiv.org/abs/2512.22597)
*Guikun Xu,Xiaohan Yi,Peilin Zhao,Yatao Bian*

Main category: cs.LG

TL;DR: 本文提出EnFlow框架，通过将流匹配与显式学习的能量模型耦合，解决分子构象生成中多样性与能量校准的碎片化问题，提升低能构象集合生成和基态识别的效率与准确性。


<details>
  <summary>Details</summary>
Motivation: 基于物理的分子构象生成和基态识别计算需求高，现有学习方法存在碎片化问题：生成模型捕获多样性但缺乏可靠能量校准，确定性预测器仅目标单一结构且无法表示集合变异性。

Method: 提出EnFlow统一框架，通过沿非高斯流匹配路径定义的能量引导采样方案，将流匹配与显式学习的能量模型耦合；采样过程中融入能量梯度引导，使轨迹趋向低能区域；利用学习的能量函数对生成集合进行高效能量排序以识别基态。

Result: 在GEOM-QM9和GEOM-Drugs数据集上的实验表明，EnFlow仅需1-2步ODE即可同时提升生成指标，并降低基态预测误差，优于现有先进方法。

Conclusion: EnFlow框架有效解决了分子构象生成中的碎片化问题，在低能构象集合生成和基态识别方面实现了效率与准确性的平衡，为计算化学领域提供了更高效的构象分析工具。
...
### [97] [Communication Compression for Distributed Learning with Aggregate and Server-Guided Feedback](https://arxiv.org/abs/2512.22623)
*Tomas Ortega,Chun-Yin Huang,Xiaoxiao Li,Hamid Jafarkhani*

Main category: cs.LG

TL;DR: 本文针对联邦学习（FL）中客户端到服务器更新的上行传输通信成本瓶颈，提出两种无需客户端状态或控制变量的有偏压缩框架CAFe和CAFe-S，并通过理论证明和实验验证其优越性。


<details>
  <summary>Details</summary>
Motivation: 联邦学习（FL）等分布式学习面临客户端到服务器更新的上行传输通信成本瓶颈，现有有偏压缩技术虽有效但依赖客户端特定控制变量，违反用户隐私且与大规模FL中的无状态客户端不兼容。

Method: 提出两种框架：1）压缩聚合反馈（CAFe），使用前一轮全局聚合更新作为所有客户端的共享控制变量；2）服务器引导压缩聚合反馈（CAFe-S），在服务器拥有小型私有数据集时生成更准确的服务器引导候选更新。以分布式梯度下降（DGD）为代表算法进行理论分析。

Result: 理论证明在非凸场景且梯度差异有界时，CAFe优于带偏压缩的分布式压缩梯度下降（DCGD）；CAFe-S收敛到平稳点，且收敛速率随服务器数据代表性提高而改善。FL场景实验结果验证所提方法优于现有压缩方案。

Conclusion: 所提出的CAFe和CAFe-S框架解决了现有有偏压缩技术在FL中的隐私和兼容性问题，理论和实验均表明其在通信效率和收敛性能上的优越性，尤其是CAFe-S在服务器有代表性数据时表现更佳。
...
### [98] [Scaling Unverifiable Rewards: A Case Study on Visual Insights](https://arxiv.org/abs/2512.22650)
*Shuyu Gan,James Mooney,Pan Hao,Renxiang Wang,Mingyi Hong,Qianwen Wang,Dongyeop Kang*

Main category: cs.LG

TL;DR: 本文提出选择性测试时扩展（Selective TTS）框架，解决多阶段流水线任务中奖励不可验证或数据不足导致的错误累积问题，通过跨阶段分配计算和早期剪枝低质量分支，提升多智能体流水线的推理稳定性与结果质量。


<details>
  <summary>Details</summary>
Motivation: 现实世界中许多多阶段流水线任务缺乏可验证奖励或足够数据来训练鲁棒奖励模型，导致基于判断的细化易在各阶段累积错误，现有基于时间重复细化的方法难以解决此问题。

Method: 提出Selective TTS框架，在多智能体流水线的不同阶段分配计算资源，使用特定阶段的判断器早期剪枝低质量分支；基于数据科学流水线构建端到端多智能体系统生成可视化图表和报告，设计与人类专家对齐的LLM判断模型（Kendall's τ=0.55）。

Result: 在固定计算预算下，Selective TTS将多智能体流水线生成的洞察质量平均得分从61.64提升至65.86，同时降低方差，有效缓解判断漂移并稳定细化过程。

Conclusion: Selective TTS为解决奖励不可验证的复杂开放式任务（如科学发现、故事生成）的扩展问题提供了可行路径，其在数据科学流水线中的应用验证了框架的有效性，有望推动多阶段任务推理的进一步发展。
...
### [99] [A Note on Hybrid Online Reinforcement and Imitation Learning for LLMs: Formulations and Algorithms](https://arxiv.org/abs/2512.23097)
*Yingru Li,Ziniu Li,Jiacai Liu*

Main category: cs.LG

TL;DR: 提出整合模仿学习与强化学习的大语言模型微调统一框架，通过复合目标梯度分解为可解析计算的稠密梯度（用于token级模仿）和蒙特卡洛估计的稀疏梯度（用于长程奖励优化），稠密梯度具有闭式logit级公式以支持高效GPU实现


<details>
  <summary>Details</summary>
Motivation: 为大语言模型微调提供整合模仿学习与强化学习的统一框架

Method: 通过分析结合轨迹级KL散度与任务奖励的复合目标梯度，将其分解为可解析计算的稠密梯度（用于token级模仿）和蒙特卡洛估计的稀疏梯度（用于长程奖励优化）

Result: 稠密梯度具有闭式logit级公式，支持高效GPU实现

Conclusion: 该统一框架有效整合了模仿学习与强化学习，其分解的梯度组件可高效实现
...
### [100] [What Matters in Deep Learning for Time Series Forecasting?](https://arxiv.org/abs/2512.22702)
*Valentina Moretti,Andrea Cini,Ivan Marisca,Cesare Alippi*

Main category: cs.LG

TL;DR: 本文旨在梳理时间序列预测深度学习架构的设计空间，分析设计维度与权衡，评估局部性和全局性等概念对预测性能的影响，指出实现细节的重要性，并提出辅助预测模型卡片以规范架构设计。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在时间序列应用中日益流行，但新提出的架构数量多且实证结果常矛盾，难以评估哪些组件对最终性能有显著贡献，因此需要理清当前时间序列预测深度学习架构的设计空间。

Method: 本文通过讨论设计维度和权衡来解释观察到的结果，评估局部性和全局性等概念在近期预测架构中的应用，分析现有架构中被忽视的实现细节，并提出辅助预测模型卡片。

Result: 研究发现，考虑局部性和全局性等方面比采用特定序列建模层更能提升预测准确性，简单且设计良好的预测架构常能达到最先进水平；现有架构中被忽视的实现细节会从根本上改变预测方法的类别，并显著影响实证结果。

Conclusion: 需要重新思考当前有缺陷的基准测试实践，在设计架构时应关注预测问题的基础方面；提出的辅助预测模型卡片可基于关键设计选择来表征现有和新的预测架构。
...
### [101] [Theoretical Foundations of Scaling Law in Familial Models](https://arxiv.org/abs/2512.23407)
*Huan Song,Qingfei Zhao,Ting Long,Shuyu Tian,Hongjun An,Jiawei Shao,Chi Zhang,Xuelong Li*

Main category: cs.LG

TL;DR: 本文将神经缩放定律扩展至家族模型范式，引入粒度G作为新变量，提出统一函数形式L(N,D,G)并通过大规模实验验证，结果表明粒度惩罚遵循小指数乘幂律，支持“一次训练、多次部署”且不牺牲计算最优性。


<details>
  <summary>Details</summary>
Motivation: 现有神经缩放定律假设单一稠密模型输出，忽略了家族模型这一实现异构设备智能的变革性范式，需扩展定律以适应“一次运行、多模型”的动态架构场景。

Method: 引入粒度G作为与模型大小N、训练token数D并列的缩放变量，提出统一函数形式L(N,D,G)；采用严格的等FLOP实验设计，在固定预算下系统扫描N和G并动态调整D，以分离架构影响与计算规模，实现高保真参数化。

Result: 粒度惩罚遵循乘幂律且指数极小，理论上连接了固定计算训练与动态架构，实践上验证了“一次训练、多次部署”范式，证明部署灵活性可在不牺牲稠密基线计算最优性的前提下实现。

Conclusion: 扩展后的神经缩放定律适用于家族模型的“一次训练、多次部署”范式，粒度引入的惩罚极小，为异构设备场景下的模型部署提供了计算最优的灵活方案。
...
### [102] [When Does Multi-Task Learning Fail? Quantifying Data Imbalance and Task Independence in Metal Alloy Property Prediction](https://arxiv.org/abs/2512.22740)
*Sungwoo Kang*

Main category: cs.LG

TL;DR: 本文测试多任务学习（MTL）在预测合金材料三种属性时的效果，发现其对回归任务性能有显著负面影响但能提升分类任务表现，并给出相应建议。


<details>
  <summary>Details</summary>
Motivation: 多任务学习假设相关材料属性共享底层物理规律可用于更好预测，本文旨在验证该假设在预测合金电阻率、维氏硬度和非晶形成能力时的有效性。

Method: 使用54028个合金样本，同时预测三种材料属性，比较单任务模型与标准及结构化多任务学习模型的表现，并分析任务间权重及数据不平衡问题。

Result: 多任务学习显著降低回归任务性能（电阻率R²从0.897降至0.844，硬度R²从0.832降至0.694，p<0.01），但提升分类任务表现（非晶F1从0.703升至0.744，p<0.05，召回率+17%）；任务间权重接近零表明属性独立，回归失败归因于严重数据不平衡（52k vs.800样本）导致的负迁移。

Conclusion: 建议对精确回归任务使用独立模型，而对召回率关键的分类任务保留多任务学习。
...
### [103] [VL-RouterBench: A Benchmark for Vision-Language Model Routing](https://arxiv.org/abs/2512.23562)
*Zhehao Huang,Baijiong Lin,Jingyuan Zhang,Jingying Wang,Yuhang Liu,Ning Lu,Tao Li,Xiaolin Huang*

Main category: cs.LG

TL;DR: 本文提出VL-RouterBench基准，用于系统评估视觉语言模型（VLM）路由系统的能力，覆盖多数据集、模型和样本，评估多种路由方法并发现现有路由与理想Oracle间存在差距，将开源工具链促进研究。


<details>
  <summary>Details</summary>
Motivation: 现有工作缺乏系统、可复现的基准来评估视觉语言模型（VLM）路由系统，无法有效衡量其整体能力。

Method: 构建VL-RouterBench基准，基于VLM的原始推理和评分日志构建样本-模型对的质量和成本矩阵；覆盖3个任务组的14个数据集（共30540样本）、15个开源模型和2个API模型（共519180样本-模型对）；采用联合评估平均准确率、平均成本和吞吐量的协议，通过归一化成本和准确率的调和平均构建排名分数以比较不同路由配置和成本预算。

Result: 评估10种路由方法和基线后发现，路由能力有显著提升，但当前最佳路由与理想Oracle之间仍存在明显差距，表明路由架构可通过更精细的视觉线索和文本结构建模进一步改进。

Conclusion: VL-RouterBench基准可系统评估VLM路由系统能力，现有路由方法有提升空间，开源工具链将促进多模态路由研究的可比性、可复现性和实际部署。
...
### [104] [Training AI Co-Scientists Using Rubric Rewards](https://arxiv.org/abs/2512.23707)
...
TL;DR: 本文提出一种利用现有研究论文语料训练语言模型生成高质量研究计划的方法，通过自动提取研究目标和特定评分标准，结合强化学习自评分机制，实现跨领域的有效泛化，提升AI协科学家的研究计划生成能力。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型难以生成符合所有约束和隐含要求的研究计划，而AI协科学家需要具备根据目标和约束生成研究计划的关键能力，因此需探索利用现有论文语料改进模型的方法。

Method: 1. 从多领域论文中自动提取研究目标和目标特定评分标准，构建可扩展的多样化训练语料；2. 通过强化学习自评分训练研究计划生成模型，使用初始策略的冻结副本作为训练时的评分器，利用评分标准创造生成器-验证器差距，无需外部人工监督即可实现改进。

Result: 1. 针对机器学习研究目标的225小时人类专家研究显示，70%的目标中专家更偏好微调后的Qwen3-30B-A3B模型生成的计划，且84%的自动提取评分标准获专家认可；2. 扩展到医学论文和arXiv预印本的研究目标时，微调模型在前沿模型陪审团评估中取得12-22%的相对改进，并实现显著的跨领域泛化，即使在医学研究等难以获取执行反馈的场景也有效。

Conclusion: 该方法证明了可扩展的自动化训练方案在改进通用AI协科学家方面的潜力，其跨领域泛化能力有效，为AI协科学家生成高质量研究计划提供了可行路径。
...
### [105] [A Micro-Macro Machine Learning Framework for Predicting Childhood Obesity Risk Using NHANES and Environmental Determinants](https://arxiv.org/abs/2512.22758)
*Eswarasanthosh Kumar Mamillapalli,Nishtha Sharma*

Main category: cs.LG

TL;DR: 本研究提出了一个微宏观机器学习框架，整合个体和宏观环境数据预测肥胖风险，发现环境负担高的州与肥胖风险分布存在地理相似性，为公共卫生信息学提供可扩展的多水平建模管道。


<details>
  <summary>Details</summary>
Motivation: 美国儿童肥胖是主要公共卫生挑战，受个体、家庭和环境多水平风险因素影响，但传统流行病学研究通常独立分析这些水平，限制了对结构环境条件与个体特征交互影响健康结果的理解。

Method: 1. 使用NHANES的个体人体测量和社会经济数据，训练Logistic Regression、Random Forest、XGBoost和LightGBM四种机器学习模型预测肥胖；2. 构建州级复合环境脆弱性指数EnvScore，整合USDA和EPA的标准化指标；3. 进行多水平比较分析环境负担与肥胖风险分布的地理相似性。

Result: 1. XGBoost模型预测肥胖风险性能最强；2. 环境负担高的州与全国预测的微观水平肥胖风险分布存在强烈地理相似性；3. 证明整合多尺度数据集识别环境驱动的肥胖风险差异的可行性。

Conclusion: 本研究贡献了一个适用于公共卫生信息学的可扩展、数据驱动的多水平建模管道，在因果建模、干预规划和实时分析方面具有强大扩展潜力。
...
### [106] [Understanding the Mechanisms of Fast Hyperparameter Transfer](https://arxiv.org/abs/2512.22768)
*Nikhil Ghosh,Denny Wu,Alberto Bietti*

Main category: cs.LG

TL;DR: 针对深度学习模型规模扩大导致超参数优化成本过高的问题，本文提出规模感知超参数转移策略的通用概念框架，分析其快速转移特性及与计算最优网格搜索的等价性，并探讨μP在不同问题结构下的转移效果及实践中快速转移的机制。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型规模不断扩大，传统超参数优化成本过高；现有规模感知超参数转移策略缺乏系统性理论框架和对其机制的深入理解。

Method: 构建超参数跨规模转移的通用概念框架，形式化证明快速转移与计算最优网格搜索的等价性；通过合成场景分析μP在不同问题结构下的转移效果；提出优化轨迹分解假设并通过实证验证。

Result: 快速转移与计算最优网格搜索的有用转移等价；μP的快速转移特性依赖于问题结构，在某些合成场景下转移无优势；优化轨迹分解假设在多种场景（包括大语言模型预训练）中得到实证支持。

Conclusion: 规模感知超参数转移策略在理论上具有渐近计算效率优势，但其效果依赖于问题结构；实践中快速转移的机制可通过优化轨迹分解为宽度稳定和宽度敏感成分来解释，该假设在实证中得到验证。
...
### [107] [Schrodinger AI: A Unified Spectral-Dynamical Framework for Classification, Reasoning, and Operator-Based Generalization](https://arxiv.org/abs/2512.22774)
*Truong Son Nguyen*

Main category: cs.LG

TL;DR: 本文提出Schrödinger AI统一机器学习框架，受量子力学启发，包含三个耦合组件，在泛化、语义可解释性和拓扑涌现性方面表现突出，并通过实验验证其优势。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习方法如交叉熵训练和Transformer注意力机制存在泛化能力不足、语义解释性差等问题，需要一种物理驱动的替代框架以提升性能。

Method: 该框架包含三个核心组件：1）时间无关波能求解器，将感知和分类视为学习哈密顿量下的谱分解；2）时间相关动力学求解器，控制语义波函数随时间的演化；3）低秩算子微积分，通过类量子转移算子学习符号变换（如模运算）。

Result: 实验结果表明：a）无需显式监督即可涌现反映人类类关系的语义流形；b）能适应环境变化的动态推理（如迷宫导航中实时势场扰动）；c）在模运算任务上实现精确算子泛化，可学习群作用并在远超训练长度的序列上组合使用。

Conclusion: Schrödinger AI为机器学习提供新的基础方向，将学习视为发现和导航底层语义能量景观的过程。
...
### [108] [Adapting, Fast and Slow: Transportable Circuits for Few-Shot Learning](https://arxiv.org/abs/2512.22777)
*Kasra Jalaldoust,Elias Bareinboim*

Main category: cs.LG

TL;DR: 本文基于因果可迁移性理论，设计了零样本组合泛化算法Circuit-TR，并提出了无需显式因果结构的监督域自适应方案，通过理论分析和模拟验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 跨域泛化需要对未见过的目标域和源域之间的结构进行约束，现有方法可能缺乏对因果结构的利用以实现有效泛化。

Method: 1. 利用领域内因果图和跨域机制共享的差异预言机形式的定性领域知识；2. 设计Circuit-TR算法，从源数据中学习模块集合，根据因果结构许可将其迁移/组合以获得目标域预测电路；3. 提出无需显式因果结构、使用有限目标数据的监督域自适应方案。

Result: 1. 理论结果刻画了基于图形电路可迁移性标准的少样本可学习任务类别，并将少样本泛化能力与电路规模复杂度概念关联；2. 受控模拟验证了理论结果的正确性。

Conclusion: 本文的算法和理论为跨域泛化提供了基于因果结构的有效方法，其结果有助于理解少样本学习的泛化能力与电路复杂度的关系。
...
### [109] [Discovering Transmission Dynamics of COVID-19 in China](https://arxiv.org/abs/2512.22787)
*Zhou Yang,Edward Dougherty,Chen Zhang,Zhenhe Pan,Fang Jin*

Main category: cs.LG

TL;DR: 本研究通过回顾性分析中国新冠病毒传播模式及公共卫生干预措施效果，发现大城市感染更多与社交活动相关，感染者就医及时且感染源随时间变化。


<details>
  <summary>Details</summary>
Motivation: 为识别最有效的新冠病毒缓解机制，需对大规模检测、隔离和接触追踪等公共卫生干预措施进行综合回顾性分析。

Method: 收集中国地方卫健委、中国疾控中心及官方社交媒体的病例报告，应用自然语言处理和人工整理构建传播链，结合武汉人口流动数据量化和可视化时空传播动态。

Result: 大城市因社交活动感染更多；79%有症状者在症状出现5天内住院，与确诊病例接触者5天内寻求入院；感染源随时间变化，早期与湖北旅行或接触相关，后期与社交活动相关。

Conclusion: 综合公共卫生干预措施有效，需关注区域差异及感染源变化以优化防控策略。
...
### [110] [ReDiF: Reinforced Distillation for Few Step Diffusion](https://arxiv.org/abs/2512.22802)
*Amirhossein Tighkhorshid,Zahra Dehghanian,Gholamali Aminian,Chengchun Shi,Hamid R. Rabiee*

Main category: cs.LG

TL;DR: 本文提出基于强化学习的扩散模型蒸馏框架，通过将蒸馏视为策略优化问题，利用教师模型输出对齐的奖励信号训练学生模型，实现更少推理步骤和计算资源下的优越性能，且模型无关。


<details>
  <summary>Details</summary>
Motivation: 扩散模型采样速度慢，现有蒸馏技术依赖固定重建或一致性损失，难以高效引导学生模型探索最优去噪路径。

Method: 将蒸馏过程视为策略优化问题，使用与教师模型输出对齐的奖励信号训练学生模型，动态引导其探索多条去噪路径，利用扩散模型处理大步长的固有能力。

Result: 与现有蒸馏技术相比，在显著减少推理步骤和计算资源的情况下，取得了更优越的性能。

Conclusion: 该框架模型无关，适用于任何类型的扩散模型（需合适奖励函数），为高效扩散学习提供通用优化范式。
...
### [111] [MoR: Mixture Of Representations For Mixed-Precision Training](https://arxiv.org/abs/2512.22804)
*Bor-Yiing Su,Peter Dykas,Mike Chrzanowski,Jatin Chhugani*

Main category: cs.LG

TL;DR: 本文提出混合表示（MoR）量化框架，通过动态分析张量数值特性选择FP8或BF16表示，在保持模型质量的同时实现98.38%张量FP8量化，为低精度训练提供鲁棒性方法。


<details>
  <summary>Details</summary>
Motivation: 混合精度训练是深度学习模型扩展的关键技术，但需合适训练方法组合，现有低精度训练可能缺乏动态性或需细粒度划分，影响效率与质量。

Method: 提出混合表示（MoR）量化框架，在张量级和子张量级动态分析数值特性，选择FP8或BF16表示，设计具体算法并在不同量化策略和数据集上实验。

Result: 98.38%的张量可量化为FP8格式且达到当前最佳结果，FP8精度与现有方法相当无需细粒度划分，可与其他方法结合提升更低精度格式（如NVFP4）的利用。

Conclusion: 动态、属性感知的量化方法在保持模型质量方面具有潜力，能提升低精度训练的鲁棒性，为混合精度训练提供有效解决方案。
...
### [112] [Long-Range Distillation: Distilling 10,000 Years of Simulated Climate into Long Timestep AI Weather Models](https://arxiv.org/abs/2512.22814)
*Scott A. Martin,Noah Brenowitz,Dale Durran,Michael Pritchard*

Main category: cs.LG

TL;DR: 本文提出远程蒸馏方法，利用教师模型生成的大量合成气候数据训练长时步概率学生模型，解决AI长程天气预报的误差累积和数据不足问题，在S2S预报中达到与ECMWF集合预报相当的技能。


<details>
  <summary>Details</summary>
Motivation: AI长程天气预报面临两大挑战：自回归模型的误差累积导致不稳定和校准问题；训练用再分析数据集样本有限，难以捕捉气候变率的慢模式，导致长时步概率模型训练时过拟合。

Method: 提出远程蒸馏方法：使用短步长自回归教师模型（如DLESyM）生成超过10000年的模拟气候数据，训练长时步概率学生模型直接进行长程预报；对学生模型进行ERA5微调以适应真实世界场景。

Result: 完美模型实验中，蒸馏模型优于气候学方法，接近教师模型技能且仅需单步替代数百自回归步骤；真实世界中经ERA5微调后，S2S预报技能与ECMWF集合预报相当；模型技能随合成训练数据量增加而提升，即使数据量远超ERA5。

Conclusion: 首次证明AI生成的合成训练数据可用于提升长程预报技能，远程蒸馏方法有效解决了长程天气预报的数据不足和误差累积问题。
...
### [113] [TEACH: Temporal Variance-Driven Curriculum for Reinforcement Learning](https://arxiv.org/abs/2512.22824)
*Gaurav Chaudhary,Laxmidhar Behera*

Main category: cs.LG

TL;DR: 本文提出一种基于学生-教师学习范式的时间方差驱动课程（Temporal Variance-Driven Curriculum）方法，以解决多目标强化学习中的样本效率问题，在11个机器人操作和迷宫导航任务中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 强化学习在单目标任务中取得成功，但在多目标场景中，统一的目标选择导致样本效率低下，生物系统的自适应结构化学习过程为解决该问题提供了启发。

Method: 提出学生-教师学习范式，教师模块基于策略置信度得分（由状态-动作值Q函数参数化）的时间方差动态优先选择高不确定性目标，为学生提供自适应聚焦的学习信号；该方法与现有RL框架无缝集成且算法无关。

Result: 在11个不同的机器人操作和迷宫导航任务上进行评估，结果显示该方法相比最先进的课程学习和目标选择方法有持续且显著的改进。

Conclusion: 建立了Q值时间方差与策略演化之间的理论联系，验证了所提方法在多目标强化学习任务中的有效性和优越性。
...
### [114] [Federated Multi-Task Clustering](https://arxiv.org/abs/2512.22897)
*S. Dai,G. Sun,F. Li,X. Tang,Q. Wang,Y. Cong*

Main category: cs.LG

TL;DR: 本文提出联邦多任务聚类框架FMTC，解决现有联邦聚类方法依赖不可靠伪标签、泛化性能差及无法捕捉异构客户端潜在关联的问题，通过客户端个性化聚类模块和服务器端张量关联模块，在保护隐私的同时提升聚类性能。


<details>
  <summary>Details</summary>
Motivation: 现有谱聚类模型多适用于集中式场景，无法应对现代去中心化环境；当前联邦学习方法依赖不可靠伪标签导致泛化性能差，且难以捕捉异构客户端间的潜在关联。

Method: 提出联邦多任务聚类框架FMTC：1.客户端个性化聚类模块，学习参数化映射模型支持鲁棒的样本外推理，无需伪标签；2.服务器端张量关联模块，将所有客户端模型组织为统一张量并应用低秩正则化发现公共子空间；3.基于交替方向乘子法的高效隐私保护分布式算法，将全局问题分解为客户端并行局部更新和服务器聚合步骤。

Result: 在多个真实世界数据集上的大量实验表明，FMTC框架显著优于各种基线和最先进的联邦聚类算法。

Conclusion: FMTC框架有效解决了现有联邦聚类方法的局限性，在去中心化环境下实现了个性化聚类与共享知识利用的平衡，提升了聚类性能和隐私保护能力。
...
### [115] [MetaCD: A Meta Learning Framework for Cognitive Diagnosis based on Continual Learning](https://arxiv.org/abs/2512.22904)
*Jin Wu,Chanjin Zheng*

Main category: cs.LG

TL;DR: 本文提出基于持续学习的元学习框架MetaCD，以解决认知诊断中数据长尾分布和动态变化的问题，提升模型在新任务上的准确性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 认知诊断是智能教育的重要研究方向，现有深度学习方法受数据长尾分布和动态变化限制，性能不足。

Method: 提出MetaCD框架：利用元学习学习最优初始化状态缓解长尾问题；采用参数保护机制的持续学习方法适应数据动态变化。

Result: 在五个真实数据集上的实验表明，MetaCD在准确性和泛化性上均优于其他基线模型。

Conclusion: MetaCD能提升模型在单任务上的可塑性，同时保证序列任务上的稳定性和泛化性，有效解决认知诊断中的数据挑战。
...
### [116] [Multiple Token Divergence: Measuring and Steering In-Context Computation Density](https://arxiv.org/abs/2512.22944)
*Vincent Herrmann,Eric Alcaide,Michael Wand,Jürgen Schmidhuber*

Main category: cs.LG

TL;DR: 本文提出了一种名为Multiple Token Divergence（MTD）的简单计算努力度量方法，以及Divergence Steering解码方法，用于分析和控制语言模型的计算动态。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型上下文内计算努力的度量指标（如下一个token损失）无法捕捉推理复杂性，而基于潜在状态可压缩性的方法具有侵入性和不稳定性。

Method: 提出MTD，定义为模型全输出分布与浅层辅助预测头输出分布之间的KL散度，无需额外训练即可从预训练模型计算；并引入Divergence Steering解码方法控制生成文本的计算特征。

Result: MTD在区分复杂任务与简单任务方面比现有方法更有效；在数学推理基准上，MTD与问题难度正相关，较低的MTD与更准确的推理相关联。

Conclusion: MTD为分析和控制语言模型的计算动态提供了实用、轻量的工具。
...
### [117] [APO: Alpha-Divergence Preference Optimization](https://arxiv.org/abs/2512.22953)
*Wang Zixian*

Main category: cs.LG

TL;DR: 本文提出Alpha-Divergence Preference Optimization（APO）框架，通过Csiszar alpha散度在锚定几何中连续插值正向KL和反向KL行为，平衡模式覆盖与模式寻求，并设计奖励和置信度引导的alpha调度策略，在Qwen3-1.7B模型上取得与GRPO、GSPO基线相当的性能且保持训练稳定性。


<details>
  <summary>Details</summary>
Motivation: 现代对齐实践中存在两种主导的散度机制：监督微调等方法最小化正向KL散度（KL(q||pi_theta)），虽稳定但常未充分利用高奖励模式；PPO类方法接近反向KL散度（KL(pi_theta||q)），能寻求模式改进但易模式崩溃。现有锚定方法（如ADPO）虽提升稳定性，但通常仅采用单一散度机制。

Method: 提出APO锚定框架，使用Csiszar alpha散度在同一锚定几何中实现正向与反向KL行为的连续插值；推导由alpha参数化的统一梯度动力学并分析其方差特性；设计奖励和置信度引导的alpha调度策略，仅当策略改进且置信度校准后才从模式覆盖过渡到模式利用。

Result: 在Qwen3-1.7B模型上进行math-level3实验，结果表明APO在保持训练稳定性的同时，取得与GRPO和GSPO基线相当的竞争性能。

Conclusion: APO框架通过alpha散度插值有效平衡了模式覆盖与模式寻求的权衡，其奖励和置信度引导的调度策略提升了训练稳定性和性能，为对齐实践提供了更灵活的散度机制选择。
...
### [118] [A Context-Aware Temporal Modeling through Unified Multi-Scale Temporal Encoding and Hierarchical Sequence Learning for Single-Channel EEG Sleep Staging](https://arxiv.org/abs/2512.22976)
*Amirali Vakili,Salar Jahanshiri,Armin Salimi-Badr*

Main category: cs.LG

TL;DR: 本研究提出一种基于单通道脑电图（EEG）的上下文感知且可解释的睡眠分期框架，针对现有方法的类别不平衡、感受野有限和可解释性不足等问题，结合多尺度特征提取与时间建模，通过类别加权损失函数和数据增强解决N1期检测难题，在SleepEDF数据集上取得了89.72%的总体准确率和85.46%的宏平均F1分数，N1期F1分数达61.7%，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 全球睡眠障碍普遍存在，自动睡眠分期是医疗保健中的关键任务。单通道EEG信号实用且广泛可用，但现有自动睡眠分期方法面临类别不平衡、感受野建模有限和可解释性不足等挑战，尤其是N1期检测困难。

Method: 提出上下文感知且可解释的单通道EEG睡眠分期框架，结合紧凑的多尺度特征提取与时间建模以捕获局部和长程依赖；应用类别加权损失函数和数据增强解决数据不平衡问题；将EEG信号分割为子 epoch 块，通过平均块间softmax概率获得最终预测，增强上下文表示和鲁棒性。

Result: 在SleepEDF数据集上，总体准确率达89.72%，宏平均F1分数为85.46%；N1期F1分数为61.7%，相比现有方法有显著提升。

Conclusion: 该框架有效提升了睡眠分期性能，同时保持可解释性和适用于实际临床应用的特性，特别是在改善N1期检测方面表现突出。
...
### [119] [Fusion or Confusion? Multimodal Complexity Is Not All You Need](https://arxiv.org/abs/2512.22991)
*Tillmann Rheude,Roland Eils,Benjamin Wild*

Main category: cs.LG

TL;DR: 本文通过大规模实证研究挑战了多模态学习中复杂架构更优的假设，提出简单基线模型SimBaMM并证明其性能不逊于复杂方法，呼吁转向方法严谨性而非架构新颖性


<details>
  <summary>Details</summary>
Motivation: 多模态学习的深度学习架构复杂度日益增加，源于复杂多模态方法能提升性能的假设，需验证该假设的合理性

Method: 重新实现19种高影响力方法，在标准化条件下跨9个多样数据集（最多23种模态）评估，测试其在原始范围外新任务（含模态缺失场景）的泛化性；提出SimBaMM（简单后期融合Transformer架构），对所有方法进行严格超参数调优

Result: 标准化实验条件下，复杂架构未可靠优于SimBaMM；复杂方法与SimBaMM性能相当，且常无法可靠优于调优后的单模态基线（尤其在小数据场景）；发现文献中存在方法学缺陷

Conclusion: 呼吁多模态学习领域从追求架构新颖性转向方法严谨性，提供实用可靠性清单以促进未来可比、稳健和可信的评估
...
### [120] [Merge before Forget: A Single LoRA Continual Learning via Continual Merging](https://arxiv.org/abs/2512.23017)
*Fuli Qiao,Mehrdad Mahdavi*

Main category: cs.LG

TL;DR: 本文提出一种正交初始化并顺序合并LoRA更新的参数高效持续学习方法，解决现有LoRA持续学习技术中计算内存增长和任务干扰问题，通过理论分析和实验验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有LoRA持续学习技术存在两个主要问题：一是随任务增加计算内存不断增长且存储空间有限；二是缺乏有效的LoRA合并机制导致潜在的任务干扰。

Method: 提出正交初始化和顺序合并LoRA更新的方法，包括：1）从已学习的LoRA中提取正交基初始化新任务学习；2）利用LoRA组件的内在不对称性，通过时间感知缩放机制在持续合并过程中平衡新旧知识。

Result: 该方法具有以下效果：1）内存复杂度与任务数量无关（保持恒定）；2）通过正交基初始化最小化新旧任务间干扰；3）通过自适应缩放提升不对称LoRA合并的性能。在多种Llama模型和持续学习基准上的实验证明了方法的有效性和效率。

Conclusion: 本文提出的正交初始化和顺序合并LoRA更新的持续学习方法有效解决了现有技术的问题，理论分析支持设计合理性，实验结果验证了其在不同模型和基准上的优越性。
...
### [121] [PI-MFM: Physics-informed multimodal foundation model for solving partial differential equations](https://arxiv.org/abs/2512.23056)
*Min Zhu,Jingmin Sun,Zecheng Zhang,Hayden Schaeffer,Lu Lu*

Main category: cs.LG

TL;DR: 本文提出物理信息多模态基础模型（PI-MFM）框架，通过在预训练和自适应过程中直接执行控制方程，解决现有多算子学习方法数据饥渴且训练时忽略物理的问题，在多种PDE场景下表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有多算子学习方法数据需求大且训练时忽略物理规律，而偏微分方程（PDEs）主导多种物理系统，多模态基础模型在学习跨多样方程家族的PDE解算子方面显示出潜力。

Method: 提出PI-MFM框架，以PDE的符号表示为输入，通过向量化导数计算自动从输入表达式组装PDE残差损失，使任何PDE编码多模态基础模型能在跨方程家族的统一物理信息目标下训练或自适应。

Result: 在13个参数化一维时间依赖PDE家族基准测试中，PI-MFM持续优于纯数据驱动方法，尤其在稀疏标记时空点、部分观测时域或少量标记函数对时；物理损失提升抗噪鲁棒性，重采样配置点等简单策略大幅提高准确性；对PI-MFM内导数计算的自动微分和有限差分的准确性、精度及计算成本进行分析；零样本物理信息微调至未见PDE家族时，从物理信息预训练模型开始，仅用PDE残差和初始/边界条件自适应，无需任何标记解数据，快速将测试误差降至约1%，明显优于从零开始的纯物理训练。

Conclusion: PI-MFM为数据高效、可迁移的PDE求解器提供了实用且可扩展的路径。
...
### [122] [FLEX-MoE: Federated Mixture-of-Experts with Load-balanced Expert Assignment](https://arxiv.org/abs/2512.23070)
*Boyang Zhang,Xiaobing Chen,Songyang Zhang,Shuai Zhang,Xiangwei Zhou,Mingxuan Sun*

Main category: cs.LG

TL;DR: 本文提出FLEX-MoE框架，解决联邦学习中MoE模型的设备资源限制和专家负载不平衡问题，通过优化专家分配和负载均衡提升性能


<details>
  <summary>Details</summary>
Motivation: 联邦学习部署MoE模型面临两个关键挑战：资源受限的边缘设备无法存储完整专家集，非IID数据分布导致严重的专家负载不平衡，降低模型性能

Method: 提出FLEX-MoE框架，引入客户端-专家适配分数量化专家对本地数据集的适用性，采用基于优化的算法最大化客户端-专家专业化，同时确保系统范围内专家利用率的平衡

Result: 在三个不同数据集上的综合实验表明，FLEX-MoE性能优越，并能在多样资源受限场景下维持平衡的专家利用率

Conclusion: FLEX-MoE有效解决联邦学习中MoE模型的资源限制和负载不平衡问题，性能优于现有方法
...
### [123] [Trust Region Masking for Long-Horizon LLM Reinforcement Learning](https://arxiv.org/abs/2512.23075)
*Yingru Li,Jiacai Liu,Jiawei Xu,Yuxuan Tong,Ziniu Li,Baoxiang Wang*

Main category: cs.LG

TL;DR: 本文针对大语言模型策略梯度方法中的离策略不匹配问题，推导了更紧的误差边界，并提出Trust Region Masking（TRM）方法以提供长 horizon 任务下的非空单调改进保证。


<details>
  <summary>Details</summary>
Motivation: 大语言模型策略梯度方法中，当rollout策略与目标策略不一致时存在近似误差，且现有经典信任区域边界在长序列任务中因随序列长度T平方增长而无效，需解决长horizon任务下的误差控制问题。

Method: 推导了两个更紧的误差边界（Pinsker-Marginal边界O(T^3/2)和Mixed边界O(T)），并提出Trust Region Masking（TRM）方法，排除任何token违反信任区域的整个序列的梯度计算。

Result: 推导的边界依赖于序列中所有位置的最大token级KL散度，且TRM是首个为长horizon LLM-RL提供非空单调改进保证的方法。

Conclusion: TRM方法通过排除违反信任区域的序列，有效控制了序列级KL散度，解决了长horizon任务下经典信任区域边界无效的问题，为LLM-RL提供了可靠的理论保证。
...
### [124] [Taming the Tail: Stable LLM Reinforcement Learning via Dynamic Vocabulary Pruning](https://arxiv.org/abs/2512.23087)
*Yingru Li,Jiawei Xu,Jiacai Liu,Yuxuan Tong,Ziniu Li,Tianle Cai,Ge Zhang,Qian Liu,Baoxiang Wang*

Main category: cs.LG

TL;DR: 针对大语言模型强化学习中的训练-推理不匹配问题，提出动态剪枝极端尾部低概率token的安全词汇表约束方法，实现稳定训练并理论上界定优化偏差


<details>
  <summary>Details</summary>
Motivation: 大语言模型强化学习面临高吞吐量推理引擎与数值精确训练系统从相同参数产生不同概率分布的训练-推理不匹配问题，低概率尾部token的系统偏差累积会破坏梯度估计稳定性

Method: 提出将强化学习目标约束在动态剪枝的“安全”词汇表中，排除极端尾部的低概率token

Result: 该方法在实验中实现了稳定训练

Conclusion: 通过剪枝极端尾部token，可将大而系统的偏差转化为小且有界的优化偏差，并从理论上界定了词汇剪枝引入的优化偏差
...
### [125] [How Much Data Is Enough? Uniform Convergence Bounds for Generative & Vision-Language Models under Low-Dimensional Structure](https://arxiv.org/abs/2512.23109)
*Paul M. Thompson*

Main category: cs.LG

TL;DR: 本文从有限样本视角研究生成模型和视觉语言模型（VLMs）在科学与医疗决策支持中的均匀泛化与校准问题，给出了基于提示嵌入Lipschitz稳定性的有限样本均匀收敛界，并探讨其对数据有限生物医学建模的启示。


<details>
  <summary>Details</summary>
Motivation: 现代生成模型和VLMs在科学与医疗决策支持中应用日益广泛，但预测概率需同时准确且校准良好；尽管中等数据下实证结果强，但此类预测何时能在输入、类别或子群体间均匀泛化（而非仅平均泛化）仍不明确，这在生物医学中至关重要，因为罕见病和特定群体可能在整体损失低时仍存在大误差。

Method: 从有限样本视角出发，聚焦于通过在受限表示空间内改变提示或语义嵌入得到的诱导分类器家族；假设模型输出依赖于低维语义表示（文本和联合图像-文本嵌入的谱结构支持该假设），利用经典均匀收敛工具，在提示嵌入的Lipschitz稳定性下推导VLM诱导分类器的准确性和校准泛函的有限样本均匀收敛界。

Result: 得到的有限样本均匀收敛界的样本复杂度依赖于内在/有效维度而非环境嵌入维度；进一步推导了依赖谱的界，明确了特征值衰减如何支配数据需求。

Conclusion: 研究结论对数据有限的生物医学建模具有启示，包括当前数据集大小何时能支持均匀可靠的预测，以及为何平均校准指标可能遗漏最坏情况的校准误差。
...
### [126] [SE-MLP Model for Predicting Prior Acceleration Features in Penetration Signals](https://arxiv.org/abs/2512.23131)
*Yankang Li,Changsheng Li*

Main category: cs.LG

TL;DR: 本文提出SE-MLP模型，结合通道注意力机制与残差连接，快速预测侵彻加速度特征值，解决传统方法模拟周期长、计算成本高的问题，验证了其工程适用性。


<details>
  <summary>Details</summary>
Motivation: 侵彻过程的准确识别依赖于侵彻加速度的先验特征值，但这些特征值通常通过长模拟周期和昂贵计算获得，存在效率低、成本高的局限性。

Method: 提出挤压激励多层感知器（SE-MLP）架构，整合通道注意力机制与残差连接；以不同工况下的物理参数为输入，输出分层加速度特征，建立物理参数与侵彻特性间的非线性映射。

Result: 与传统MLP、XGBoost、Transformer模型对比，SE-MLP预测精度、泛化性和稳定性更优；消融实验证明通道注意力模块和残差结构显著提升性能；数值模拟和靶场回收测试显示，预测与实测加速度峰值和脉宽差异在工程可接受公差内。

Conclusion: 所提SE-MLP方法可行且具有工程适用性，为快速生成侵彻引信先验特征值提供了实用基础。
...
### [127] [Principled Algorithms for Optimizing Generalized Metrics in Binary Classification](https://arxiv.org/abs/2512.23133)
*Anqi Mao,Mehryar Mohri,Yutao Zhong*

Main category: cs.LG

TL;DR: 针对类别不平衡或非对称成本场景下优化F_β-measure等广义度量面临的计算与统计挑战，本文提出METRO算法，通过将度量优化重构为广义成本敏感学习问题，设计具有H一致性保证的替代损失函数，并通过实验验证其优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 在类别不平衡或非对称成本的应用中，F_β-measure、AM度量等广义评价指标比标准二分类损失更合适，但优化这些指标存在显著的计算和统计挑战；现有方法依赖贝叶斯最优分类器的表征，采用先估计类概率再找最优阈值的方式，导致算法不适用于受限假设集且缺乏有限样本性能保证。

Method: 将度量优化重构为广义成本敏感学习问题，设计具有可证明H一致性保证的新型替代损失函数；基于此框架开发METRO（Metric Optimization）算法，该算法具有强理论性能保证。

Result: 实验结果表明，所提出的METRO方法相比现有基线方法具有更好的有效性。

Conclusion: 通过重构问题和设计替代损失函数，提出的METRO算法解决了广义度量优化中的关键挑战，具有理论保证且实验效果优异，为类别不平衡或非对称成本场景下的模型评价与优化提供了有效方案。
...
### [128] [KernelEvolve: Scaling Agentic Kernel Coding for Heterogeneous AI Accelerators at Meta](https://arxiv.org/abs/2512.23236)
*Gang Liao,Hongsen Qin,Ying Wang,Alicia Golden,Michael Kuchnik,Yavuz Yetim,Jia Jiunn Ang,Chunli Fu,Yihan He,Samuel Hsia,Zewei Jiang,Dianshi Li,Uladzimir Pashkevich,Varna Puvvada,Feng Shi,Matt Steiner,Ruichao Xiao,Nathan Yan,Xiayu Yu,Zhou Fang,Abdul Zainul-Abedin,Ketan Singh,Hongtao Yu,Wenyuan Chi,Barney Huang,Sean Zhang,Noah Weller,Zach Marine,Wyatt Cook,Carole-Jean Wu,Gaoxiang Liu*

Main category: cs.LG

TL;DR: 本文提出KernelEvolve框架，解决DLRM训练推理中的系统挑战，实现跨异构硬件的自动内核生成优化，提升性能并降低开发时间。


<details>
  <summary>Details</summary>
Motivation: DLRM训练与推理的高效性很重要，但面临模型架构多样性、内核原语多样性及硬件异构性三大系统挑战。

Method: 提出KernelEvolve框架，以内核规格为输入，通过多编程抽象层（Triton、CuTe DSL到硬件无关低级语言），采用基于图的搜索（含选择策略、通用算子、适应度函数、终止规则）结合检索增强的提示合成动态适配运行时上下文，实现跨异构硬件的内核生成优化。

Result: 在KernelBench套件250个问题（三级难度）和160个PyTorch ATen算子上验证，跨三种异构硬件平台正确率100%；将开发时间从数周缩短至数小时，相比PyTorch基线在多样生产场景和大规模异构AI系统中性能显著提升。

Conclusion: KernelEvolve有效解决DLRM跨异构硬件的系统挑战，提升性能效率，降低新AI硬件的可编程性障碍，支持自动化内核生成。
...
### [129] [A Weak Signal Learning Dataset and Its Baseline Method](https://arxiv.org/abs/2512.23160)
*Xianqi Liu,Xiangru Li,Lefeng He,Ziyu Fang*

Main category: cs.LG

TL;DR: 本文针对弱信号学习（WSL）领域缺乏专用数据集的问题，构建了首个弱信号特征学习专用数据集，并提出了PDVFN模型，以解决低信噪比、分布偏斜和双重不平衡等挑战，为WSL研究提供基础。


<details>
  <summary>Details</summary>
Motivation: 弱信号学习在故障诊断、医学成像等领域面临关键信息被噪声掩盖导致特征识别困难的挑战，且缺乏专用数据集限制了研究进展，即使在强信号丰富的任务中，有效提取弱信号也是提升模型性能的关键。

Method: 构建包含13158个光谱样本的专用数据集（低信噪比占比超55%、类不平衡比达29:1）；提出双视图表示（向量+时频图）和PDVFN模型，该模型并行提取局部序列特征和全局频域结构，遵循局部增强、序列建模、噪声抑制、多尺度捕获、频率提取和全局感知原则。

Result: PDVFN模型在处理弱信号、高噪声和极端类不平衡问题时表现出更高的准确性和鲁棒性，尤其在低信噪比和不平衡场景下效果显著。

Conclusion: 本研究提供了弱信号学习专用数据集、基线模型，为未来WSL研究奠定了基础。
...
### [130] [Splitwise: Collaborative Edge-Cloud Inference for LLMs via Lyapunov-Assisted DRL](https://arxiv.org/abs/2512.23310)
*Abolfazl Younesi,Abbas Shabrang Maryan,Elyas Oustad,Zahra Najafabadi Samani,Mohsen Ansari,Thomas Fahringer*

Main category: cs.LG

TL;DR: 提出Splitwise框架，通过细粒度自适应分区解决边缘设备部署大语言模型的挑战，平衡延迟、能耗与准确性。


<details>
  <summary>Details</summary>
Motivation: 边缘设备部署大语言模型面临内存和算力限制，纯云端推理延迟高成本大，静态分区方案难以应对带宽波动。

Method: 提出基于Lyapunov辅助的深度强化学习框架Splitwise，将Transformer层分解为注意力头和前馈子块，采用分层DRL策略联合优化延迟、能耗和精度损失，并通过检查点和指数退避恢复保证鲁棒性。

Result: 在Jetson Orin NX等设备上实验显示，Splitwise相比现有分区器将端到端延迟降低1.4-2.8倍，能耗减少41%；相比纯云端推理，95分位延迟降低53-61%，同时保持精度和适度内存需求。

Conclusion: Splitwise有效解决边缘-云端大语言模型分区问题，在多种设备和模型上实现延迟、能耗和准确性的平衡，具备鲁棒性和适应性。
...
### [131] [Diffusion-based Decentralized Federated Multi-Task Representation Learning](https://arxiv.org/abs/2512.23161)
*Donghwa Kang,Shana Moothedath*

Main category: cs.LG

TL;DR: 本文提出一种基于去中心化投影梯度下降的多任务表示学习算法，针对多任务线性回归问题，通过交替投影梯度下降与最小化方法以扩散式去中心化和联邦方式恢复低秩特征矩阵，并提供了样本复杂度下界、迭代复杂度上界以及时间和通信复杂度分析，数值模拟验证了算法性能。


<details>
  <summary>Details</summary>
Motivation: 表示学习在数据稀缺环境中广泛应用，但去中心化方法的研究相对不足，因此需要开发去中心化的多任务表示学习算法。

Method: 提出一种基于去中心化投影梯度下降的算法，针对多任务线性回归问题，采用交替投影梯度下降与最小化方法，以扩散式去中心化和联邦方式恢复低秩特征矩阵。

Result: 获得了样本复杂度的下界和迭代复杂度的上界，分析表明算法具有快速性和通信高效性，数值模拟验证了算法性能并与基准算法进行了比较。

Conclusion: 所提出的算法在多任务表示学习中表现出良好的性能，具有快速和通信高效的特点，数值模拟结果支持了算法的有效性。
...
### [132] [The Law of Multi-Model Collaboration: Scaling Limits of Model Ensembling for Large Language Models](https://arxiv.org/abs/2512.23340)
*Dakuan Lu,Jiaqi Zhang,Cheng Yuan,Jiawei Shao,Chi Zhang,Xuelong Li*

Main category: cs.LG

TL;DR: 本文提出多模型协作定律，预测LLM集成的性能极限，发现多模型系统在总参数规模上遵循幂律缩放，异构模型集成比同构集成性能更好，模型多样性是协作增益的关键驱动因素。


<details>
  <summary>Details</summary>
Motivation: 单个大语言模型（LLM）的能力存在固有边界，尽管多模型集成技术（如模型路由和事后集成）快速发展，但缺乏统一的多模型协作性能缩放理论框架。

Method: 采用与方法无关的公式，假设一个理想化的集成预言机，其中每个样本的总交叉熵损失由模型池中任何模型的最小损失决定，以量化多模型协作的内在上限。

Result: 多模型系统相对于总参数数量遵循幂律缩放，与单模型缩放相比，表现出更显著的改进趋势和更低的理论损失下限；异构模型家族的集成比单一模型家族内的集成实现更好的性能缩放。

Conclusion: 模型协作是扩展LLM智能前沿的关键方向，模型多样性是协作增益的主要驱动因素。
...
### [133] [Evaluating Parameter Efficient Methods for RLVR](https://arxiv.org/abs/2512.23165)
*Qingyu Yin,Yulun Wu,Zhennan Shen,Sunbowen Li,Zhilin Wang,Yanshu Li,Chak Tou Leong,Jiale Kang,Jinjin Gu*

Main category: cs.LG

TL;DR: 本文系统评估了12种参数高效微调（PEFT）方法在可验证奖励强化学习（RLVR）范式下的表现，发现DoRA等结构变体优于LoRA，SVD初始化策略存在谱坍缩问题，极端参数减少会瓶颈推理能力，并提供了RLVR中PEFT方法选择的指导。


<details>
  <summary>Details</summary>
Motivation: 可验证奖励强化学习（RLVR）通过可验证反馈提升语言模型推理能力，但常用的LoRA等PEFT方法是否为RLVR最优架构尚不明确，缺乏对PEFT方法在RLVR下的综合评估。

Method: 对DeepSeek-R1-Distill系列模型，在数学推理基准上综合评估12种PEFT方法，进行消融研究和缩放实验验证发现。

Result: 1. DoRA、AdaLoRA、MiSS等结构变体持续优于LoRA；2. SVD初始化策略（如PiSSA、MiLoRA）存在谱坍缩现象，因主成分更新与RL优化错位；3. 极端参数减少（如VeRA、Rank-1）严重瓶颈推理能力。

Conclusion: 本工作为RLVR中参数高效方法的选择提供明确指导，倡导更多探索参数高效强化学习方法。
...
### [134] [Machine Learning-Assisted Vocal Cord Ultrasound Examination: Project VIPR](https://arxiv.org/abs/2512.23177)
*Will Sebelik-Lassiter,Evan Schubert,Muhammad Alliyu,Quentin Robbins,Excel Olatunji,Mustafa Barry*

Main category: cs.LG

TL;DR: 本研究应用机器学习辅助算法自动识别声带并区分正常声带图像与声带麻痹（VCP）图像，以提高声带超声（VCUS）诊断的准确性。


<details>
  <summary>Details</summary>
Motivation: 声带超声（VCUS）是一种微创且耐受性较好的检查技术，但准确性依赖操作者；因此需要应用机器学习辅助算法来自动识别声带并区分正常与声带麻痹图像。

Method: 采集30名志愿者的VCUS视频，将其分割为静态帧并裁剪为统一尺寸；使用健康和模拟VCP图像作为训练数据，构建声带分割和VCP分类模型。

Result: 声带分割模型验证准确率达96%，最佳分类模型（VIPRnet）验证准确率达99%。

Conclusion: 机器学习辅助的VCUS分析在提高诊断准确性方面具有巨大潜力，优于依赖操作者的人工解读。
...
### [135] [Post-Training Quantization of OpenPangu Models for Efficient Deployment on Atlas A2](https://arxiv.org/abs/2512.23367)
*Yilun Luo,HuaQing Zheng,Haoqian Meng,Wenyuan Liu,Peng Zhang*

Main category: cs.LG

TL;DR: 本文针对华为openPangu-Embedded模型在Ascend NPU上部署时因CoT推理范式导致的内存和延迟问题，提出统一低比特量化框架，在保持精度的同时提升效率。


<details>
  <summary>Details</summary>
Motivation: openPangu-Embedded模型的三种CoT推理范式（slow_think、auto_think、no_think）增强了推理能力，但生成的扩展推理轨迹带来了显著的内存和延迟开销，给在Ascend NPU上的实际部署带来挑战。

Method: 本文利用低比特量化将FP16计算转换为更高效的整数运算，引入支持INT8（W8A8）和W4A8量化的统一低比特推理框架，并针对Atlas A2上的openPangu-Embedded模型进行优化。

Result: 在HumanEval和MBPP代码生成基准测试的三种CoT模式下，INT8量化在Atlas A2上保持了FP16基线精度的90%以上，并实现1.5倍的预填充加速；W4A8量化显著降低内存消耗，但精度有适度权衡。

Conclusion: 低比特量化有效促进了Ascend NPU上高效的CoT推理，同时保持了较高的模型保真度。
...
### [136] [A Simple, Optimal and Efficient Algorithm for Online Exp-Concave Optimization](https://arxiv.org/abs/2512.23190)
*Yi-Han Wang,Peng Zhao,Zhi-Hua Zhou*

Main category: cs.LG

TL;DR: 本文提出LightONS算法，解决在线指数凹优化（OXO）中Online Newton Step（ONS）的计算瓶颈问题，同时保持最优遗憾并回答SXO的开放问题


<details>
  <summary>Details</summary>
Motivation: ONS算法在OXO中面临计算瓶颈，每次Mahalanobis投影成本高，总运行时间达~O(d^ωT)；SXO中ONS在线转批量需~O(d^{ω+1}/ε)运行时间，Koren提出的开放问题寻求SXO算法运行时间低于此值

Method: 提出ONS的变体LightONS，利用无参数在线学习的域转换技术引入滞后机制，延迟昂贵的Mahalanobis投影直到必要时进行

Result: LightONS将总运行时间降至O(d²T + d^ω√(T log T))，同时保持O(d log T)的最优遗憾；SXO方法运行时间为~O(d³/ε)，回答了开放问题

Conclusion: LightONS保留ONS的优雅结构，可作为ONS的高效插件替代，适用于更广泛场景，包括梯度范数自适应遗憾、参数化随机 bandits 和内存高效在线学习
...
### [137] [PGOT: A Physics-Geometry Operator Transformer for Complex PDEs](https://arxiv.org/abs/2512.23192)
*Zhuo Zhang,Xi Yang,Yuan Zhao,Canqun Yang*

Main category: cs.LG

TL;DR: 本文提出Physics-Geometry Operator Transformer（PGOT）以解决Transformer在大规模非结构化网格PDE建模中存在的几何混叠问题，通过显式几何感知重建物理特征学习，在基准测试和工业任务中取得最优性能。


<details>
  <summary>Details</summary>
Motivation: Transformer在偏微分方程（PDE）建模中潜力显著，但在大规模非结构化网格和复杂几何建模方面仍面临挑战；现有高效架构采用特征降维策略，导致几何混叠，丢失关键物理边界信息。

Method: 提出PGOT模型，包含：1）Spectrum-Preserving Geometric Attention（SpecGeo-Attention）模块，通过“物理切片-几何注入”机制整合多尺度几何编码，保持线性计算复杂度O(N)；2）基于空间坐标动态路由计算，对平滑区域用低阶线性路径，对激波和不连续区域用高阶非线性路径。

Result: PGOT在四个标准基准测试中持续取得最先进性能，并在翼型和汽车设计等大规模工业任务中表现优异。

Conclusion: PGOT通过显式几何感知有效解决了几何混叠问题，实现了空间自适应和高精度的物理场建模，在PDE建模任务中具有显著优势。
...
### [138] [Energy and Memory-Efficient Federated Learning With Ordered Layer Freezing](https://arxiv.org/abs/2512.23200)
*Ziru Niu,Hai Dong,A. K. Qin,Tao Gu,Pengcheng Zhang*

Main category: cs.LG

TL;DR: 针对物联网边缘设备在联邦学习中的计算、内存和带宽限制问题，提出FedOLF框架，通过有序层冻结和张量操作近似技术提升模型效率与准确性。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在物联网中因边缘设备资源有限（计算能力、内存、带宽）面临效率与可扩展性挑战，现有框架常牺牲准确性或忽略内存约束。

Method: 提出Federated Learning with Ordered Layer Freezing（FedOLF）框架，包含：1）训练前按预定义顺序冻结层以降低计算和内存需求；2）结合Tensor Operation Approximation（TOA）技术减少通信与能耗。

Result: 在非独立同分布数据上，FedOLF在EMNIST（CNN）、CIFAR-10（AlexNet）、CIFAR-100（ResNet20/44）、CINIC-10（ResNet20/44）等数据集上的准确性分别比现有方法高0.3%至6.4%不等，同时提升了能源效率并降低了内存占用。

Conclusion: FedOLF框架有效缓解了物联网边缘设备联邦学习中的资源限制问题，在保持或提升模型准确性的同时，显著优化了计算、内存、通信效率及能源消耗。
...
### [139] [FairGFL: Privacy-Preserving Fairness-Aware Federated Learning with Overlapping Subgraphs](https://arxiv.org/abs/2512.23235)
*Zihao Zhou,Shusen Yang,Fangyuan Zhao,Xuebin Ren*

Main category: cs.LG

TL;DR: 本文揭示了图联邦学习中不平衡重叠子图导致的不公平问题，并提出FairGFL算法以隐私保护方式提升跨客户端公平性与模型效用。


<details>
  <summary>Details</summary>
Motivation: 图联邦学习在保护数据隐私的同时从分布式子图中协同提取高阶信息，但现有研究未探索不同客户端间不平衡重叠数据的负面影响，尤其是其引发的不公平问题。

Method: 提出FairGFL算法：1)采用可解释的加权聚合方法，利用隐私保护的重叠率估计提升跨客户端公平性；2)在联邦复合损失函数中融入精心设计的正则化项，优化模型效用与公平性的权衡。

Result: 在四个基准图数据集上的实验表明，FairGFL在模型效用和公平性方面均优于四种代表性基线算法。

Conclusion: FairGFL有效解决了图联邦学习中不平衡重叠子图导致的不公平问题，同时保持了良好的模型效用，为隐私保护下的公平图联邦学习提供了新方案。
...
### [140] [Eliminating Inductive Bias in Reward Models with Information-Theoretic Guidance](https://arxiv.org/abs/2512.23461)
...
TL;DR: 本文提出一种基于信息理论的去偏方法DIR，用于缓解奖励模型（RM）训练数据中的归纳偏差，提升RLHF性能。


<details>
  <summary>Details</summary>
Motivation: 奖励模型训练数据质量低，存在归纳偏差（如响应长度、谄媚性、格式等），易导致过拟合和奖励黑客问题；现有去偏方法多针对单一偏差或仅建模简单线性相关，无法处理复杂多样的偏差。

Method: 引入名为DIR的信息理论去偏方法，受信息瓶颈启发，最大化RM分数与人类偏好对之间的互信息，同时最小化RM输出与偏好输入的有偏属性之间的互信息。

Result: 在三种归纳偏差（响应长度、谄媚性、格式）上验证了DIR的有效性，DIR不仅能有效缓解目标偏差，还能在多种基准测试中提升RLHF性能，增强泛化能力。

Conclusion: DIR能处理具有非线性相关的复杂偏差类型，广泛扩展了RM去偏方法的实际应用场景；代码和训练方案已开源。
...
### [141] [PFed-Signal: An ADR Prediction Model based on Federated Learning](https://arxiv.org/abs/2512.23262)
*Tao Li,Peilin Li,Kui Lu,Yilei Wang,Junliang Shang,Guangshun Li,Huiyu Zhou*

Main category: cs.LG

TL;DR: 提出PFed-signal联邦学习模型，通过识别并删除FAERS中的偏倚数据，提升ADR信号预测准确性


<details>
  <summary>Details</summary>
Motivation: FDA不良事件报告系统（FAERS）中的偏倚记录导致在线诊断误导，传统统计方法无法消除偏倚数据，信号预测不准确

Method: 1. Pfed-Split：基于ADR分割原始数据集；2. ADR-signal模型：联邦学习偏倚数据识别（欧氏距离）+Transformer预测模型（在清洗数据上训练）

Result: 清洗数据集上的ROR/PRR优于传统方法，PFed-Signal的准确率、F1、召回率、AUC分别为0.887、0.890、0.913、0.957，均高于基线

Conclusion: PFed-signal有效消除FAERS偏倚数据，提升ADR预测性能，优于传统统计方法和基线模型
...
### [142] [FRoD: Full-Rank Efficient Fine-Tuning with Rotational Degrees for Fast Convergence](https://arxiv.org/abs/2512.23485)
*Guoan Wan,Tianyu Chen,Fangzheng Feng,Haoyi Zhou,Runhua Xu*

Main category: cs.LG

TL;DR: 本文提出FRoD方法，通过分层联合分解与旋转自由度结合，解决参数高效微调（PEFT）方法的收敛慢和适应能力有限问题，在20个基准测试中匹配全模型微调精度且仅用1.72%可训练参数


<details>
  <summary>Details</summary>
Motivation: 参数高效微调（PEFT）方法如LoRA因低秩约束存在收敛慢、适应能力有限的问题，难以捕捉多样化任务所需的复杂模式

Method: 提出FRoD方法，结合分层联合分解与旋转自由度，提取跨层全局共享基，向缩放因子注入稀疏可学习扰动实现灵活全秩更新

Result: 在20个涵盖视觉、推理和语言理解的基准测试中，FRoD匹配全模型微调的精度，同时在相同训练预算下仅使用1.72%的可训练参数

Conclusion: FRoD方法在保证精度的前提下显著提升参数高效微调的效率和鲁棒性，为大型基础模型的下游任务适配提供了更优方案
...
### [143] [On the Inverse Flow Matching Problem in the One-Dimensional and Gaussian Cases](https://arxiv.org/abs/2512.23265)
*Alexander Korotin,Gudmund Pammer*

Main category: cs.LG

TL;DR: 本文研究了具有有限指数矩的分布之间的流匹配逆问题，在一维和高斯情况下确立了解的唯一性，多维问题仍待解决


<details>
  <summary>Details</summary>
Motivation: 受现代生成AI应用（如流匹配模型的蒸馏）的启发，研究具有有限指数矩的分布之间的流匹配逆问题

Method: 未提及具体研究方法

Result: 在一维和高斯情况下确立了流匹配逆问题解的唯一性

Conclusion: 流匹配逆问题的一般多维情况仍需未来研究
...
TL;DR: 本文提出基于Le Cam理论的决策论框架，通过定向可模拟性替代对称不变性，解决领域适应中的负迁移问题，在基因组学、视觉和强化学习任务中实现无源退化的安全迁移。


<details>
  <summary>Details</summary>
Motivation: 现有无监督领域适应（UDA）依赖对称散度最小化实现特征不变性，但在领域信息不等时会导致信息破坏和灾难性负迁移，尤其在安全关键应用中不可接受。

Method: 基于Le Cam统计实验理论构建框架，引入Le Cam失真（由缺陷距离δ(E₁,E₂)量化）作为迁移风险的严格上界，通过学习模拟目标域的核函数实现定向可模拟性，避免源域退化。

Result: 在五个实验中取得显著结果：HLA基因组学频率估计相关性r=0.999；CIFAR-10分类保留源域准确率81.2%（CycleGAN下降34.7%）；强化学习中实现安全策略迁移（不变性方法崩溃）。

Conclusion: Le Cam失真为负迁移不可接受的领域（如医学成像、自动驾驶、精准医疗）提供首个风险可控迁移学习的原则性框架。
...
### [145] [ISOPO: Proximal policy gradients without pi-old](https://arxiv.org/abs/2512.23353)
*Nilin Abrahamsen*

Main category: cs.LG

TL;DR: 本文提出等距策略优化（ISOPO）方法，可在单梯度步中高效近似自然策略梯度，计算开销小


<details>
  <summary>Details</summary>
Motivation: 现有近端策略方法（如GRPO或CISPO）需多梯度步并使用重要性比率裁剪变体来近似自然梯度步，效率较低

Method: ISOPO通过两种变体实现：1）在Fisher度量下归一化每个序列的对数概率梯度后与优势函数收缩；2）基于每层神经正切核变换微批量优势函数，并在单次反向传播中逐层应用该变换

Result: ISOPO可在单梯度步中近似自然策略梯度，且与普通REINFORCE相比计算开销可忽略不计

Conclusion: ISOPO是一种高效近似自然策略梯度的方法，具有计算效率高的优势
...
### [146] [BOAD: Discovering Hierarchical Software Engineering Agents via Bandit Optimization](https://arxiv.org/abs/2512.23631)
*Iris Xu,Guangtao Zeng,Zexue He,Charles Jin,Aldo Pareja,Dan Gutfreund,Chuang Gan,Zhang-Wei Hong*

Main category: cs.LG

TL;DR: 本文提出BOAD框架，通过多臂老虎机问题自动发现分层多智能体结构，以解决LLMs在长程、分布外软件工程问题中的泛化不足问题，并在相关基准上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有单智能体处理软件工程问题的整体式设计会保留无关上下文，导致虚假关联和泛化能力差；手动设计多智能体结构面临子智能体数量增长带来的组合搜索空间过大及子智能体贡献难以归因的挑战。

Method: 将分层发现问题转化为多臂老虎机问题，每个臂代表候选子智能体，奖励衡量其与其他子智能体协作的有效性；提出BOAD框架，在有限评估预算下高效探索子智能体设计。

Result: 在SWE-bench-Verified上优于单智能体和手动设计的多智能体系统；在SWE-bench-Live上，36B规模的系统排名第二，超过GPT-4和Claude等更大模型。

Conclusion: 自动发现的分层多智能体系统显著提升了在具有挑战性的长程软件工程任务中的泛化能力。
...
### [147] [Diffusion priors enhanced velocity model building from time-lag images using a neural operator](https://arxiv.org/abs/2512.23375)
*Xiao Ma,Mohammad Hasyim Taufik,Tariq Alkhalifah*

Main category: cs.LG

TL;DR: 本文提出一种结合生成模型与神经算子的速度建模新框架，以高效获取高分辨率速度模型，解决传统方法计算成本高、耗时的问题。


<details>
  <summary>Details</summary>
Motivation: 传统速度建模方法计算成本高且耗时，深度学习（尤其是生成模型和神经算子）的发展为解决这一问题提供了新思路。

Method: 提出生成神经算子框架：1. 神经算子作为正演映射算子，从真实和偏移速度模型快速生成时滞逆时偏移扩展图像；2. 通过自动微分更新偏移速度模型；3. 嵌入生成模型作为正则化器，利用高分辨率速度模型分布提升预测质量。

Result: 合成数据和实际数据实验均验证了所提方法的有效性。

Conclusion: 生成神经算子基速度建模方法可高效生成高分辨率速度模型，优于传统方法。
...
### [148] [On the Sample Complexity of Learning for Blind Inverse Problems](https://arxiv.org/abs/2512.23405)
*Nathan Buskulic,Luca Calatroni,Lorenzo Rosasco,Silvia Villa*

Main category: cs.LG

TL;DR: 本文针对盲逆问题缺乏可解释性和理论保证的问题，在LMMSE框架下进行深入理论分析，推导最优估计器的闭式表达式，建立与Tikhonov正则化的等价性，证明收敛结果并给出有限样本误差界，最后通过数值实验验证理论发现。


<details>
  <summary>Details</summary>
Motivation: 盲逆问题在许多实验场景中出现，其中正向算子部分或完全未知，非盲情况下的方法无法直接适用；现有数据驱动方法虽有强实证性能和适应性，但缺乏可解释性和严格理论保证，限制了其在成像逆问题等应用领域的可靠性。

Method: 在Linear Minimum Mean Square Estimators（LMMSE）框架下进行深入理论分析，推导最优估计器的闭式表达式；建立与适当选择的Tikhonov正则化公式的等价性，其中正则化明确依赖于未知信号、噪声和随机正向算子的分布；在适当源条件假设下证明收敛结果；推导严格的有限样本误差界，该界将学习估计器的性能表征为噪声水平、问题条件和可用样本数量的函数；通过说明性数值实验验证理论发现。

Result: 推导得到最优估计器的闭式表达式；建立了与Tikhonov正则化的等价性，正则化依赖于未知信号、噪声和随机正向算子的分布；证明了在适当源条件假设下的收敛结果；得到了有限样本误差界，该界量化了算子随机性的影响，并揭示了当随机性消失时的收敛速率；数值实验证实了预测的收敛行为。

Conclusion: 本文在LMMSE框架下对盲逆问题的学习进行了深入理论分析，推导了最优估计器的闭式表达式，建立了与Tikhonov正则化的等价性，证明了收敛结果并给出了有限样本误差界，数值实验验证了理论发现，为盲逆问题的学习方法提供了可解释性和严格的理论保证。
...
### [149] [Stochastic Siamese MAE Pretraining for Longitudinal Medical Images](https://arxiv.org/abs/2512.23441)
...
TL;DR: 本文提出STAMP框架，通过随机过程编码时间信息，改进自监督学习模型对纵向医疗数据中疾病进展的捕捉能力。


<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法（如MAE）缺乏时间感知能力，无法有效捕捉纵向医疗数据中疾病进展的动态变化，尤其是疾病演化的不确定性。

Method: 提出STAMP（Stochastic Temporal Autoencoder with Masked Pretraining），一种Siamese MAE框架，通过基于两个输入体积时间差的随机过程编码时间信息；将MAE重建损失重构为条件变分推理目标，以学习随机时间动态。

Result: 在两个OCT和一个MRI数据集上评估，STAMP预训练的ViT模型在晚期年龄相关性黄斑变性和阿尔茨海默病进展预测任务中，优于现有时间MAE方法和基础模型。

Conclusion: STAMP能有效学习疾病的非确定性时间动态，提升纵向医疗数据中疾病进展预测的性能。
...
### [150] [Dynamic Subspace Composition: Efficient Adaptation via Contractive Basis Expansion](https://arxiv.org/abs/2512.23448)
*Vladimer Khasia*

Main category: cs.LG

TL;DR: 本文提出动态子空间组合（DSC）框架，解决混合专家（MoE）模型的表示崩溃和梯度不稳定问题，通过共享基库的状态依赖稀疏扩展近似上下文相关权重，降低参数复杂度和内存流量并提供严格理论界。


<details>
  <summary>Details</summary>
Motivation: 混合专家（MoE）模型在扩展容量时通常面临表示崩溃和梯度不稳定的问题，需要一种更高效且稳定的框架来解决这些挑战。

Method: 提出动态子空间组合（DSC）框架，将权重更新建模为星形域内的残差轨迹，采用幅度门控单纯形插值确保恒等处的连续性；从解耦的单位范数基向量构建组合秩-K近似，而非检索独立秩-r矩阵；使用帧理论正则化和谱约束提供动态更新的严格最坏情况界。

Result: DSC将参数复杂度从O(M rd)降至O(M d)，内存流量降至O(Kd)，同时通过理论约束保证了动态更新的性能，代码已开源。

Conclusion: DSC框架有效解决了MoE模型的表示崩溃和梯度不稳定问题，在降低参数复杂度和内存流量的同时提供了严格的理论界，是一种高效且稳定的模型扩展方法。
...
### [151] [Trustworthy Machine Learning under Distribution Shifts](https://arxiv.org/abs/2512.23524)
*Zhuo Huang*

Main category: cs.LG

TL;DR: 本文聚焦分布偏移下的可信机器学习研究，旨在解决机器学习系统可靠性和通用性受限的问题，通过分析三种分布偏移场景及三个可信维度，提出有效解决方案以提升ML系统的鲁棒性、适应性和安全性。


<details>
  <summary>Details</summary>
Motivation: 尽管机器学习（ML）在AI领域取得了显著进展（如ResNet、Transformer、大语言模型等）并对社会产生巨大影响，但分布偏移仍是其“阿喀琉斯之踵”，从根本上限制了ML系统的可靠性和通用性，同时导致AI的信任问题，因此需要研究分布偏移下的可信机器学习。

Method: 将常见分布偏移分为三类：扰动偏移、领域偏移和模态偏移；从鲁棒性、可解释性和适应性三个方面严格研究各场景下的可信性，并基于这些维度提出有效解决方案和基础见解。

Result: 论文提出的解决方案和见解旨在增强关键的ML问题，如效率、适应性和安全性，但摘要中未具体提及实验结果或量化成果。

Conclusion: 本研究通过对分布偏移场景和可信维度的系统分析，为提升AI系统的鲁棒性、通用性、责任感和可靠性提供了方向，有助于解决分布偏移带来的挑战，推动可信机器学习的发展。
...
### [152] [EEG-based Graph-guided Domain Adaptation for Robust Cross-Session Emotion Recognition](https://arxiv.org/abs/2512.23526)
*Maryam Mirzaei,Farzaneh Shayegh,Hamed Narimani*

Main category: cs.LG

TL;DR: 本研究提出EGDA框架以解决EEG情感识别中的跨会话泛化问题，通过联合对齐全局和类特异性分布并结合图正则化保留数据结构，在SEED-IV数据集上实现了优于基线的跨会话性能，并识别出关键频率带和脑区。


<details>
  <summary>Details</summary>
Motivation: 准确识别人类情绪状态对人机交互至关重要，EEG因高时间分辨率和直接反映神经活动成为可靠来源，但跨会话差异严重影响模型泛化能力。

Method: 提出EGDA框架，该框架通过联合对齐全局（边缘）和类特异性（条件）分布来减少跨会话差异，同时通过图正则化保留EEG数据的内在结构。

Result: 在SEED-IV数据集的三个迁移任务中，EGDA分别获得81.22%、80.15%和83.27%的准确率，性能优于多个基线方法；此外，Gamma频段被确定为最具判别性，中央顶叶和前额叶脑区对可靠情感识别至关重要。

Conclusion: EGDA框架有效提升了EEG情感识别的跨会话鲁棒性，关键频率带和脑区的发现为后续研究提供了指导。
...
### [153] [Distribution-Free Process Monitoring with Conformal Prediction](https://arxiv.org/abs/2512.23602)
*Christopher Burger*

Main category: cs.LG

TL;DR: 本文提出一种融合无分布、模型无关的保形预测（Conformal Prediction）的混合框架，以增强传统统计过程控制（SPC）在复杂制造环境中的可靠性，包括两种新应用：可视化过程不确定性的保形增强控制图和将多变量控制重构为形式化异常检测问题的保形增强过程监控。


<details>
  <summary>Details</summary>
Motivation: 传统统计过程控制（SPC）依赖常被违反的统计假设，在现代复杂制造环境中监测不可靠，需更稳健的质量控制方法。

Method: 引入融合保形预测的混合框架，提出两种新应用：1）保形增强控制图（可视化过程不确定性并提供'不确定性峰值'等主动信号）；2）保形增强过程监控（使用直观p值图将多变量控制重构为形式化异常检测问题）。

Result: 该框架提供了更稳健且统计严谨的质量控制方法，同时保留了经典方法的可解释性和易用性。

Conclusion: 融合保形预测的混合框架可有效增强传统SPC的可靠性，适用于复杂制造环境的质量控制。
...
### [154] [Random Controlled Differential Equations](https://arxiv.org/abs/2512.23670)
*Francesco Piatti,Thomas Cass,William F. Turner*

Main category: cs.LG

TL;DR: 本文提出一种结合随机特征与受控微分方程（CDE）的高效时间序列学习框架，包括RF-CDE和R-RDE两种变体，理论上证明其对应特定核函数，并在基准测试中展现出竞争力。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列学习模型在效率、归纳偏置和高阶时间交互捕捉方面存在不足，需开发兼具高效训练、强归纳偏置和稳定处理复杂时间动态的方法。

Method: 提出两种模型变体：1）随机傅里叶CDE（RF-CDE），通过随机傅里叶特征提升输入信号，实现RBF增强序列模型的无核近似；2）随机粗糙DE（R-RDE），通过log-ODE离散化直接处理粗糙路径输入，利用log-signature捕捉高阶时间交互。

Result: 在多种时间序列基准测试中，两种模型均表现出竞争性或最先进的性能，提供了显式签名计算的实用替代方案。

Conclusion: 该框架结合随机特征与CDE，兼具训练效率和强归纳偏置，理论上统一了随机特征水库、连续时间深度架构和路径签名理论，实践中可有效处理时间序列任务。
...
### [155] [End-to-End Test-Time Training for Long Context](https://arxiv.org/abs/2512.23675)
...
TL;DR: 本文将长上下文语言建模视为持续学习问题而非架构设计问题，提出端到端测试时训练方法（TTT-E2E），该方法在测试时通过对给定上下文进行下一个token预测持续学习，压缩上下文到权重中，并通过元学习改进初始化，实现与全注意力Transformer相似的上下文长度缩放性能且推理延迟恒定。


<details>
  <summary>Details</summary>
Motivation: 长上下文语言建模通常依赖架构设计改进，但现有方法如Mamba 2和Gated DeltaNet无法随上下文长度有效缩放，而全注意力Transformer推理延迟随上下文长度增加而增长，因此需要一种既能有效缩放又保持低推理延迟的方法。

Method: 将长上下文语言建模转化为持续学习问题，使用带滑动窗口注意力的标准Transformer架构；在测试时通过下一个token预测持续学习，将上下文压缩到权重中；在训练时通过元学习改进测试时学习的初始化，形成端到端的测试时训练方法（TTT-E2E）。

Result: 对于3B模型（训练于164B tokens），TTT-E2E随上下文长度的缩放方式与全注意力Transformer相同，而Mamba 2和Gated DeltaNet则不具备此特性；TTT-E2E推理延迟与上下文长度无关（类似RNN），在128K上下文时比全注意力Transformer快2.7倍。

Conclusion: TTT-E2E方法有效解决了长上下文语言建模中的缩放性与推理延迟问题，其端到端的持续学习框架在性能和效率上均优于现有方法，代码已公开。
...
TL;DR: 本文提出双向RAG架构，通过验证后的高质量生成响应回写实现安全的语料库扩展，在四个数据集上提升了覆盖度且添加文档更少，证明严格验证下自改进RAG的可行性。


<details>
  <summary>Details</summary>
Motivation: 传统RAG架构使用静态语料库，无法从用户交互中进化，存在无法积累新知识的问题。

Method: 引入双向RAG架构，采用多阶段验收层，结合基于NLI的蕴涵验证、归因检查和新颖性检测，以防止幻觉污染并实现知识积累。

Result: 在四个数据集（Natural Questions、TriviaQA、HotpotQA、Stack Overflow）和三个随机种子的12次实验中，双向RAG平均覆盖度达40.58%，几乎是标准RAG（20.33%）的两倍，且比朴素回写添加的文档少72%（140 vs 500）。

Conclusion: 严格验证下的自改进RAG是可行且安全的，为能从部署中学习的RAG系统提供了实用路径。
...
TL;DR: 本文研究了大语言模型在无显式提示下的说服行为，发现监督微调（SFT）而非激活引导会增加模型的无提示说服倾向，且基于良性主题的SFT可能导致模型在争议和有害主题上产生更高的说服倾向。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型被广泛应用，能对人类观点产生前所未有的影响，但现有研究仅关注显式提示下的误用威胁模型，缺乏对无显式提示下说服行为的研究，需探究这种浮现式说服风险的产生场景。

Method: 研究了两种场景下的无提示说服：1）通过内部激活引导模型沿特定人格特质方向；2）对模型进行监督微调以展现相同特质。分析不同方法对模型说服倾向的影响。

Result: 激活引导无论针对说服相关或无关特质，均未可靠增加模型无提示说服倾向；而监督微调会显著增加该倾向；基于良性主题的通用说服数据集的监督微调，会使模型在争议和有害主题上的说服倾向更高。

Conclusion: 浮现式有害说服可通过监督微调产生，需进一步研究此类风险。
...
TL;DR: 本文提出GamiBench基准，通过折纸启发的折叠任务评估多模态大语言模型（MLLMs）的空间推理与2D到3D规划能力，发现现有领先模型在此任务上表现不佳，建立了标准化评估框架。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在空间推理（跨多视角和时间跟踪、操作物体的能力）上存在不足，而多数基准聚焦静态图像或最终输出，未考虑该技能的序列性和视角依赖性，因此需要新的基准填补这一空白。

Method: 设计GamiBench基准，包含186个常规和186个不可能的2D折痕图案及其对应的3D折叠形状，覆盖6个不同视角，涉及预测3D折叠配置、区分有效视角、检测不可能图案三个VQA任务；引入视角一致性（VC）和不可能折叠选择率（IFSR）等诊断指标，全面评估模型的推理过程（跨视角一致性、物理可行性、中间步骤解释）。

Result: 实验表明，即使GPT-5和Gemini-2.5-Pro等领先模型在单步空间理解任务上仍存在困难，验证了GamiBench基准对MLLMs空间推理能力评估的有效性。

Conclusion: GamiBench为评估MLLMs的几何理解和空间推理能力建立了标准化框架，其数据集和代码已开源（https://github.com/stvngo/GamiBench）。
...
TL;DR: 本文提出一种公平感知人工智能框架，用于孟加拉国洪水后援助分配，通过对抗性去偏模型减少偏见并保持预测准确性，以实现更公平的人道主义援助分配。


<details>
  <summary>Details</summary>
Motivation: 发展中国家灾后援助分配存在系统性偏见，使脆弱地区处于不利地位，加剧历史不平等；孟加拉国易受洪水灾害影响，2022年洪水影响720万人，造成4.055亿美元损失，亟需公平的援助分配方法。

Method: 使用2022年孟加拉国洪水真实数据，开发对抗性去偏模型，该模型采用医疗AI中的公平感知表示学习技术，通过梯度反转层学习无偏见表示，预测洪水脆弱性并消除对边缘化地区和农村的偏见。

Result: 在11个地区的87个乌帕齐拉实验中，该框架将统计 parity差异减少41.6%，区域公平差距减少43.2%，同时保持较高预测准确性（R²=0.784 vs基线0.811），生成基于真实需求的援助优先级排名。

Conclusion: 算法公平技术可有效应用于人道主义场景，为决策者提供工具以实施更公平的灾后恢复策略，确保援助到达最脆弱人群而非基于历史分配模式。
...
TL;DR: 本文提出Agentic Risk & Capability (ARC)框架，帮助组织识别、评估和缓解自主AI系统的风险，同时支持安全创新。


<details>
  <summary>Details</summary>
Motivation: 自主AI系统具备代码执行、互联网交互等能力，带来机遇的同时也产生新风险，给组织治理带来识别、评估和缓解多样化及演化风险的挑战。

Method: 提出ARC框架，核心包括：1）以能力为中心分析自主AI系统；2）提炼组件、设计和能力三大内在风险源；3）建立风险源、具体风险与技术控制的关联；4）提供结构化实践方法帮助组织实施。

Result: ARC框架为组织提供了健壮且适应性强的方法，能应对自主AI的复杂性，在确保安全、可靠和负责任部署的同时支持快速有效创新。

Conclusion: ARC框架是组织管理自主AI风险的实用工具，可平衡创新与安全，且已开源供使用。
...
### [161] [Logic Sketch Prompting (LSP): A Deterministic and Interpretable Prompting Method](https://arxiv.org/abs/2512.22258)
*Satvik Tripathi*

Main category: cs.AI

TL;DR: 本文提出Logic Sketch Prompting（LSP）框架，解决大语言模型在需严格规则遵循、确定性和可审计性任务中的不可靠问题，并通过药理逻辑合规任务验证其优于现有提示方法。


<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLMs）在自然语言推理方面表现出色，但在需要严格规则遵循、确定性和可审计性的任务中仍不可靠。

Method: 提出轻量级提示框架Logic Sketch Prompting（LSP），引入类型化变量、确定性条件评估器和基于规则的验证器以生成可追溯和可重复的输出；在两个药理逻辑合规任务中，对比LSP与零样本提示、思维链提示和简洁提示在Gemma 2、Mistral和Llama 3三个开源模型上的性能。

Result: 在所有任务和模型中，LSP的准确率（0.83-0.89）和F1分数（0.83-0.89）均最高，显著优于零样本提示（0.24-0.60）、简洁提示（0.16-0.30）和思维链提示（0.56-0.75）；McNemar检验显示LSP在几乎所有对比中均有统计学显著增益（p<0.01）。

Conclusion: LSP在不牺牲性能的前提下提高了确定性、可解释性和一致性，支持其在临床、受监管和安全关键决策支持系统中的应用。
...
### [162] [SciEvalKit: An Open-source Evaluation Toolkit for Scientific General Intelligence](https://arxiv.org/abs/2512.22334)
...
TL;DR: 本文介绍了SciEvalKit，一个统一的基准测试工具包，用于评估跨多学科的AI科学模型，支持六大科学领域，具备灵活可扩展的评估流程，并开源以促进AI4Science发展。


<details>
  <summary>Details</summary>
Motivation: 现有通用评估平台未聚焦科学智能的核心能力，缺乏针对多学科科学任务的标准化、可扩展评估工具，难以有效衡量AI模型在真实科学挑战中的表现。

Method: 设计了SciEvalKit工具包，涵盖科学多模态感知、推理、理解、符号推理、代码生成、假设生成及知识理解七大核心能力，支持物理、化学等六大科学领域，基于真实领域数据集构建专家级基准，提供批量评估、自定义集成及透明可复现的结果。

Result: SciEvalKit实现了能力评估与学科多样性的结合，提供标准化且可定制的基础设施，支持下一代科学基础模型和智能体的基准测试，工具包已开源并持续维护。

Conclusion: SciEvalKit为AI科学模型提供了统一、灵活且可扩展的评估框架，有助于推动AI4Science领域的社区协作与技术进步。
...
### [163] [Subgoaling Relaxation-based Heuristics for Numeric Planning with Infinite Actions](https://arxiv.org/abs/2512.22367)
*Ángel Aso-Mollar,Diego Aineto,Enrico Scala,Eva Onaindia*

Main category: cs.AI

TL;DR: 本文针对含控制参数的数值规划问题，提出了一种乐观编译方法，将可处理的子集转化为简单数值任务，使传统数值启发式算法可有效应用于无限动作空间场景。


<details>
  <summary>Details</summary>
Motivation: 含控制参数的数值规划模型因引入自由数值变量导致状态中可能存在无限多适用动作，现有依赖动作结构的数值启发式算法无法直接应用。

Method: 识别可处理的可控简单数值问题子集，通过将控制依赖表达式抽象为有界常量效应和松弛前置条件，提出乐观编译方法转化为简单数值任务。

Result: 该编译方法能有效利用子目标启发式算法估计目标距离，可将传统数值启发式算法应用于无限动作空间场景，且计算可行。

Conclusion: 该方法是将传统数值启发式算法应用于无限可能动作场景的有效且计算可行的途径，推动了该领域的技术边界。
...
### [164] [HalluMat: Detecting Hallucinations in LLM-Generated Materials Science Content Through Multi-Stage Verification](https://arxiv.org/abs/2512.22396)
*Bhanu Prakash Vangala,Sajid Mahmud,Pawan Neupane,Joel Selvaraj,Jianlin Cheng*

Main category: cs.AI

TL;DR: 本文针对大语言模型在材料科学内容生成中的幻觉问题，提出HalluMatData基准数据集和HalluMatDetector多阶段检测框架，有效降低幻觉率并引入PHCS量化一致性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLMs）在科学发现中能加速知识生成和假设形成，但存在幻觉问题（生成错误或误导信息），损害研究完整性，材料科学领域缺乏针对性的幻觉检测方案。

Method: 1. 构建HalluMatData基准数据集用于评估幻觉检测、事实一致性和响应鲁棒性；2. 提出HalluMatDetector多阶段框架，整合内在验证、多源检索、矛盾图分析和基于指标的评估；3. 引入Paraphrased Hallucination Consistency Score（PHCS）量化语义等价查询下的响应不一致性。

Result: 1. 材料科学各子领域幻觉水平差异显著，高熵查询的事实不一致性更大；2. HalluMatDetector将幻觉率较标准LLM输出降低30%；3. PHCS有效量化了模型在语义等价查询中的响应不一致性。

Conclusion: 本研究通过HalluMatData数据集和HalluMatDetector框架，显著改善了LLM在材料科学内容生成中的幻觉问题，PHCS为评估模型可靠性提供了新指标，对提升AI辅助科学研究的可信度具有重要意义。
...
### [165] [Lightweight Inference-Time Personalization for Frozen Knowledge Graph Embeddings](https://arxiv.org/abs/2512.22398)
*Ozan Oguztuzun,Cerag Oguztuzun*

Main category: cs.AI

TL;DR: 本文提出GatedBias框架，通过轻量级推理时个性化适配冻结知识图谱嵌入，在不重新训练的情况下提升个性化排序性能，同时保持全局准确性。


<details>
  <summary>Details</summary>
Motivation: 现有知识图谱基础模型在链接预测中表现出较强的群体级性能，但无法捕捉个体用户偏好，存在通用关系推理与个性化排序之间的关键脱节。

Method: 提出GatedBias轻量级推理时个性化框架，引入结构门控适配机制：将用户档案特征与图衍生的二进制门结合，生成可解释的每个实体偏差，仅需约300个可训练参数。

Result: 在Amazon-Book和Last-FM两个基准数据集上评估，GatedBias在保持群体性能的同时，在对齐指标上取得统计显著提升；反事实扰动实验验证因果响应性，受益于特定偏好信号的实体在信号增强时排名提升6-30倍。
...
### [166] [Monadic Context Engineering](https://arxiv.org/abs/2512.22431)
*Yifan Zhang,Mengdi Wang*

Main category: cs.AI

TL;DR: 本文提出了Monadic Context Engineering（MCE）架构范式，利用函子、应用函子和单子的代数结构为自主智能体设计提供形式化基础，解决现有架构在状态管理、错误处理和并发方面的脆弱性问题，并扩展到元智能体的生成式编排。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）推动了自主智能体的发展，但现有智能体架构常采用命令式、临时模式构建，导致系统在状态管理、错误处理和并发方面存在脆弱性，亟需一种形式化的架构范式来提升智能体的鲁棒性和可组合性。

Method: 提出Monadic Context Engineering（MCE）架构范式，基于函子、应用函子和单子的代数结构：1）利用单子实现鲁棒的顺序组合；2）应用函子提供并行执行的原则性结构；3）单子转换器支持这些能力的系统组合；4）扩展到元智能体，通过元编程动态创建和管理子智能体工作流。

Result: MCE架构使开发者能够从简单、可独立验证的组件构建复杂、弹性且高效的AI智能体；单子支持鲁棒的顺序组合，应用函子支持并行执行，单子转换器实现能力的系统组合；元智能体可通过MCE进行生成式编排。

Conclusion: MCE为智能体设计提供了形式化基础，解决了现有架构的关键问题，支持构建鲁棒、可组合的智能体系统，并扩展到元智能体的动态管理，具有重要的实践价值。
...
### [167] [DarkPatterns-LLM: A Multi-Layer Benchmark for Detecting Manipulative and Harmful AI Behavior](https://arxiv.org/abs/2512.22470)
*Sadia Asif,Israel Antonio Rosales Laguan,Haris Khan,Shumaila Asif,Muneeb Asif*

Main category: cs.AI

TL;DR: 本文针对大型语言模型（LLMs）中操纵性内容的评估问题，提出了DarkPatterns-LLM基准数据集和诊断框架，以实现细粒度分析并识别现有模型的弱点。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的普及加剧了对其操纵性或欺骗性行为的担忧，这些行为可能损害用户自主权、信任和福祉；现有安全基准依赖粗糙的二元标签，无法捕捉构成操纵的细微心理和社会机制。

Method: 引入包含七个伤害类别的DarkPatterns-LLM基准数据集和诊断框架，框架采用四层分析 pipeline：多粒度检测（MGD）、多尺度意图分析（MSIAN）、威胁协调协议（THP）和深度上下文风险对齐（DCRA）；数据集包含401个精心策划的示例及专家注释。

Result: 对GPT-4、Claude 3.5和LLaMA-3-70B等最先进模型的评估显示，性能差异显著（65.2%--89.7%），且在检测破坏自主权的模式方面存在一致弱点。
...
### [168] [Tyee: A Unified, Modular, and Fully-Integrated Configurable Toolkit for Intelligent Physiological Health Care](https://arxiv.org/abs/2512.22601)
*Tao Zhou,Lingyu Shu,Zixing Zhang,Jing Han*

Main category: cs.AI

TL;DR: 本文提出统一模块化的智能生理健康护理工具包Tyee，解决深度学习在生理信号分析中的数据异构、预处理不一致等问题，在多任务中表现优异且开源维护。


<details>
  <summary>Details</summary>
Motivation: 深度学习在生理信号分析中潜力巨大，但受限于异构数据格式、不一致预处理策略、碎片化模型管道和不可复现实验设置等问题，阻碍其进展。

Method: 提出Tyee工具包，包含三大创新：1）支持12种信号模态的统一数据接口与可配置预处理管道；2）模块化可扩展架构，支持跨任务灵活集成与快速原型设计；3）端到端工作流配置，促进可复现和可扩展实验。

Result: Tyee在所有评估任务中表现出一致的实际有效性和通用性，优于或匹配基线模型，在13个数据集的12个上取得最先进结果。

Conclusion: Tyee工具包有效解决了生理信号分析中深度学习面临的关键限制，具有实用价值和广泛适用性，已开源并持续维护。
...
### [169] [Learning Multi-Modal Mobility Dynamics for Generalized Next Location Recommendation](https://arxiv.org/abs/2512.22605)
*Junshu Dai,Yu Wang,Tongya Zheng,Wei Ji,Qinghong Guo,Ji Cao,Jie Song,Canghong Jin,Mingli Song*

Main category: cs.AI

TL;DR: 本文提出M³ob方法，利用多模态时空知识刻画移动动态以提升位置推荐任务性能，解决现有方法泛化能力有限的问题


<details>
  <summary>Details</summary>
Motivation: 现有人类移动预测方法泛化能力不足：单模态方法受数据稀疏性和固有偏差限制，多模态方法难以有效捕捉静态多模态表示与时空动态间语义差距导致的移动动态

Method: 构建统一的时空关系图（STRG）融合LLMs增强的时空知识图（STKG）的功能语义和时空知识；设计门控机制融合不同模态的时空图表示，并提出STKG引导的跨模态对齐将时空动态知识注入静态图像模态

Result: 在六个公开数据集上的实验表明，该方法在正常场景下性能持续提升，且在异常场景中表现出显著的泛化能力

Conclusion: M³ob方法有效解决了现有移动预测方法的泛化问题，在正常和异常场景下均表现优异
...
### [170] [The Wisdom of Deliberating AI Crowds: Does Deliberation Improve LLM-Based Forecasting?](https://arxiv.org/abs/2512.22625)
*Paul Schneider,Amalie Schramm*

Main category: cs.AI

TL;DR: 本研究探讨让大语言模型（LLM）在更新预测前互相审查是否能提高预测准确性，发现不同模型共享信息时该干预有效，但相同模型组无收益，额外信息未提升准确性。


<details>
  <summary>Details</summary>
Motivation: 结构化 deliberation 能提升人类预测者表现，研究旨在验证类似干预（LLM互相审查预测）是否能提高大语言模型预测准确性。

Method: 使用 Metaculus Q2 2025 AI 预测锦标赛的20个已解决二元问题，评估四种场景下的准确性：不同模型+分布式信息、不同模型+共享信息、相同模型+分布式信息、相同模型+共享信息。

Result: 干预在不同模型共享信息场景（2）显著提升准确性，Log Loss 降低0.020（相对减少约4%，p=0.017）；相同模型组无收益；额外上下文信息未提升预测准确性。

Conclusion: deliberation 可能是提高LLM预测的可行策略，但需考虑模型多样性和信息共享条件，额外信息未提升准确性限制了对信息池机制的研究。
...
### [171] [DICE: Discrete Interpretable Comparative Evaluation with Probabilistic Scoring for Retrieval-Augmented Generation](https://arxiv.org/abs/2512.22629)
*Shiyan Liu,Jian Ma,Rui Qu*

Main category: cs.AI

TL;DR: 本文提出DICE框架，解决现有RAG系统评估指标的可解释性、不确定性量化和效率问题，通过两阶段证据耦合方法和瑞士制锦标赛提升评估的透明度、鲁棒性和效率，并在中文金融QA数据集上验证其优于现有指标。


<details>
  <summary>Details</summary>
Motivation: 随着检索增强生成（RAG）系统架构日益复杂，确保其可信度的可解释且鲁棒的评估至关重要。现有标量指标存在可解释性有限、不确定性量化不足、多系统比较计算效率低等问题，阻碍了RAG技术的负责任部署。

Method: 提出DICE（离散可解释比较评估）框架，包含两阶段证据耦合方法：结合深度分析推理与概率{A,B,Tie}评分生成透明、置信感知的判断；采用瑞士制锦标赛将计算复杂度从O(N²)降至O(N log N)以提升效率。

Result: 在8系统评估中计算复杂度降低42.9%且保持排名保真度；在中文金融QA数据集上与人类专家达成85.7%的一致性，显著优于RAGAS等现有基于LLM的指标。

Conclusion: DICE是一种负责任、可解释且高效的范式，为可信RAG系统评估提供了有效方案。
...
### [172] [TravelBench: A Real-World Benchmark for Multi-Turn and Tool-Augmented Travel Planning](https://arxiv.org/abs/2512.22673)
*Xiang Cheng,Yulan Hu,Xiangwen Zhang,Lu Xu,Zheng Pan,Xin Li,Yong Liu*

Main category: cs.AI

TL;DR: 本文介绍了TravelBench，一个用于评估大型语言模型（LLM）智能体在旅行规划任务中多轮交互和工具使用能力的真实世界基准数据集，通过构建受控沙箱环境和三类子集，为推进LLM智能体在旅行规划领域的发展提供了实用且可复现的评估工具。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在旅行规划任务的研究存在领域覆盖有限和缺乏多轮交互的问题，无法支持动态用户-智能体交互，因此难以全面评估智能体的能力。

Method: 本文构建了TravelBench基准数据集，包含从真实场景收集的用户请求，并分为多轮、单轮和不可解决三个子集；同时搭建了一个包含10个旅行领域工具的受控沙箱环境，提供确定性工具输出以确保可靠推理。

Result: 对多个LLM在TravelBench上进行了评估，并分析了它们的行为和性能。

Conclusion: TravelBench为推进LLM智能体在旅行规划领域的发展提供了一个实用且可复现的基准。
...
### [173] [Memento-II: Learning by Stateful Reflective Memory](https://arxiv.org/abs/2512.22716)
*Jun Wang*

Main category: cs.AI

TL;DR: 本文提出了一个整合情景记忆与强化学习的大型语言模型智能体持续经验学习理论框架，通过反射机制实现无需反向传播或微调的自适应，并建立了收敛保证。


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型智能体在持续学习中依赖训练与部署分离的模式（如反向传播或模型微调），缺乏无需参数更新即可通过交互自适应的机制，限制了其在动态环境中的持续适应性。

Method: 提出状态反射决策过程（Stateful Reflective Decision Process），将反射学习建模为与情景记忆的两阶段读写交互：写操作存储交互结果对应策略评估，读操作检索相关过往案例对应策略改进；进一步使用熵正则化策略迭代实例化框架，并通过动态规划和强化学习工具分析其等价马尔可夫决策过程特性。

Result: 该框架诱导的过程等价于增强状态记忆表示上的马尔可夫决策过程；当情景记忆增长并充分覆盖状态空间时，所得策略收敛到最优解。

Conclusion: 本工作为无需参数更新即可持续自适应的记忆增强和检索式语言模型智能体提供了原则性基础。
...
### [174] [SAMP-HDRL: Segmented Allocation with Momentum-Adjusted Utility for Multi-agent Portfolio Management via Hierarchical Deep Reinforcement Learning](https://arxiv.org/abs/2512.22895)
*Xiaotian Ren,Nuerxiati Abudurexiti,Zhengyong Jiang,Angelos Stefanidis,Hongbin Liu,Jionglong Su*

Main category: cs.AI

TL;DR: 本文提出SAMP-HDRL框架，通过动态资产分组、分层多智能体决策和基于效用的资本分配机制，解决非平稳市场中投资组合优化的挑战，在不同市场环境下表现优于传统和DRL基准，并提升了可解释性。


<details>
  <summary>Details</summary>
Motivation: 非平稳市场中的投资组合优化面临 regime shifts、动态相关性及深度强化学习（DRL）策略可解释性有限等挑战，现有方法难以适应复杂金融环境的需求。

Method: 提出分层深度强化学习的多智能体投资组合管理框架SAMP-HDRL：1.动态资产分组将市场分为高质量和普通子集；2.上层智能体提取全局市场信号，下层智能体在掩码约束下进行组内分配；3.基于效用的资本分配机制整合风险与无风险资产，协调全局与局部决策。

Result: 在2019-2021年三个市场regime的回测中，SAMP-HDRL在波动和振荡条件下持续优于9个传统基线和9个DRL基准；与最强基线相比，Return、Sharpe ratio、Sortino ratio至少高5%，Omega ratio至少高2%，动荡市场中收益更大；消融研究验证上层-下层协调、动态聚类和资本分配对鲁棒性不可或缺；SHAP可解释性揭示智能体间互补的“分散+集中”机制。

Conclusion: SAMP-HDRL将结构性市场约束直接嵌入DRL pipeline，在复杂金融环境中提供了更好的适应性、鲁棒性和可解释性，为非平稳市场的投资组合优化提供有效解决方案。
...
TL;DR: 本文介绍了HiSciBench这一分层基准，用于评估基础模型在科学智能各层级的能力，涵盖五个科学工作流程阶段，包含多学科多模态数据，并通过评估揭示了模型在不同层级的性能差距。


<details>
  <summary>Details</summary>
Motivation: 现有科学智能基准碎片化，多聚焦于狭窄任务，无法反映真实科学探究的分层和跨学科特性，因此需要一个综合框架来评估基础模型在完整科学工作流程中的能力。

Method: 提出HiSciBench分层基准，分为科学素养（L1）、文献解析（L2）、基于文献的问答（L3）、文献综述生成（L4）、科学发现（L5）五个层级，涵盖6个主要学科，包含8735个实例，支持多模态输入和跨语言评估，并采用依赖感知的集成框架。

Result: 对GPT-5、DeepSeek-R1等领先模型的评估显示，模型在基础素养任务上准确率可达69%，但在发现层级任务上骤降至25%，存在显著性能差距。

Conclusion: HiSciBench为科学智能评估建立了新标准，提供了开发更强大可靠模型的可行见解，并将公开发布以促进未来研究。
...
TL;DR: 本文提出Gamma模型，通过多头部几何注意力和关系条件注意力融合机制提升知识图谱基础模型的零样本归纳链接预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有知识图谱基础模型（如Ultra）依赖单一关系变换，限制了表达能力，无法捕捉多样化图中的复杂关系和结构模式。

Method: Gamma模型采用多并行关系变换（基于实数、复数、分裂复数和双数），并通过关系条件注意力融合机制（带熵正则化的轻量级门控）在链接级别自适应融合这些变换。

Result: 在56个多样化知识图谱上的实验显示，Gamma在零样本归纳链接预测中持续优于Ultra，归纳基准的平均倒数排名提升5.5%，所有基准平均提升4.4%。

Conclusion: 多几何表示的互补性显著提升了模型性能，其组合的表达能力超过任何单一空间。
...
### [177] [Multimodal Fact-Checking: An Agent-based Approach](https://arxiv.org/abs/2512.22933)
*Danni Xu,Shaojing Fan,Xuanang Cheng,Mohan Kankanhalli*

Main category: cs.AI

TL;DR: 本文针对多模态虚假信息自动事实核查的挑战，提出RW-Post数据集和AgentFact框架，以提升准确性和可解释性


<details>
  <summary>Details</summary>
Motivation: 多模态虚假信息的快速传播对自动事实核查系统构成挑战，现有方法因推理能力有限和证据利用不足存在缺陷，且缺乏带注释推理过程和可验证证据的专用数据集

Method: 提出RW-Post数据集（含真实世界多模态主张、上下文信息、详细推理及链接证据）和AgentFact框架（由五个专业智能体协作处理核查子任务，通过迭代工作流协调）

Result: 实验表明RW-Post与AgentFact的协同作用显著提高了多模态事实核查的准确性和可解释性

Conclusion: RW-Post数据集和AgentFact框架有效解决了现有方法的局限，为多模态事实核查提供了高质量资源和高效框架
...
### [178] [Problems With Large Language Models for Learner Modelling: Why LLMs Alone Fall Short for Responsible Tutoring in K--12 Education](https://arxiv.org/abs/2512.23036)
*Danial Hooshyar,Yeongwook Yang,Gustav Šíř,Tommi Kärkkäinen,Raija Hämäläinen,Mutlu Cukurova,Roger Azevedo*

Main category: cs.AI

TL;DR: 本研究针对K-12教育中LLM导师可能替代传统学习者建模的误解，通过对比DKT模型与LLM在知识评估的准确性、可靠性和时间一致性方面的表现，发现LLM单独使用难以匹配现有智能辅导系统的效果，负责任的辅导需结合学习者建模的混合框架。


<details>
  <summary>Details</summary>
Motivation: K-12教育中基于LLM的导师兴起导致其可替代传统学习者建模的误解，而该领域被欧盟AI法案列为高风险领域需负责任设计，因此研究LLM导师的局限性，特别是评估学习者知识演化的准确性、可靠性和时间一致性问题。

Method: 对比深度知识追踪（DKT）模型与广泛使用的LLM（零样本和微调两种设置），使用大型开放数据集进行评估。

Result: DKT在下一步正确性预测上的判别性能最高（AUC=0.83），且在各设置下持续优于LLM；微调虽使LLM的AUC比零样本基线提升约8%，但仍比DKT低6%且早期序列错误更高；DKT保持稳定且方向正确的 mastery 更新，而LLM变体存在显著时间弱点（如不一致和错误方向更新）；微调LLM需近198小时高计算训练，远超DKT的计算需求；多技能 mastery 估计的定性分析显示，即使微调后LLM仍产生不一致的 mastery 轨迹，而DKT保持平滑连贯更新。

Conclusion: LLM单独使用不太可能匹配现有智能辅导系统的效果，负责任的辅导需要结合学习者建模的混合框架。
...
### [179] [Benchmark Success, Clinical Failure: When Reinforcement Learning Optimizes for Benchmarks, Not Patients](https://arxiv.org/abs/2512.23090)
...
TL;DR: 本文提出ChexReason模型，探讨强化学习在医疗影像资源受限场景中的应用，发现RL范式存在分布内性能提升与跨数据集迁移性下降的矛盾，指出精心设计的监督微调可能更适合临床部署。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习（RL）在大语言模型（LLMs）推理任务中的进展未充分探索其在医疗影像资源受限场景中的应用。

Method: 引入ChexReason模型，采用R1-style方法（监督微调（SFT）后接GRPO），仅使用2000个SFT样本、1000个RL样本和单个A100 GPU训练。

Result: 在CheXpert基准上分布内性能提升23%（macro-F1=0.346），但在NIH基准上跨数据集迁移性下降19%；SFT checkpoint在优化前对NIH数据集有独特改进；结构化推理支架对通用视觉语言模型（VLMs）有益，但对医学预训练模型增益极小。

Conclusion: RL范式存在分布内性能与跨数据集迁移性的矛盾，该问题源于RL范式而非模型规模；教师引导的推理捕捉更多机构无关特征；精心设计的监督微调可能优于激进的RL，更适合需要跨多样人群鲁棒性的临床部署。
...
### [180] [InSPO: Unlocking Intrinsic Self-Reflection for LLM Preference Optimization](https://arxiv.org/abs/2512.23126)
*Yu Li,Tian Lan,Zhengling Qi*

Main category: cs.AI

TL;DR: 本文提出一种新的对齐大语言模型的方法——内在自反思偏好优化（ISPO），解决了现有DPO方法存在的两个局限性，并证明其在理论和实验上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有DPO及其变体存在两个基本局限性：一是最优策略依赖于任意的建模选择（如标量化函数、参考策略），导致行为反映参数化伪影而非真实偏好；二是孤立处理响应生成，未能利用成对数据中的比较信息，未发挥模型内在自反思的能力。

Method: 提出内在自反思偏好优化（ISPO）方法，该方法通过同时考虑上下文和替代响应来推导全局最优策略，无需架构变更或推理开销，可作为即插即用的增强手段。

Result: 理论上证明ISPO的公式优于DPO/RLHF，且保证对标量化和参考选择的不变性；实验中，ISPO在胜率和长度控制指标上均取得持续改进。

Conclusion: ISPO方法通过解锁模型的自反思能力，能够生成更稳健、更符合人类偏好的大语言模型，是一种有效的对齐方法。
...
### [181] [Why We Need a New Framework for Emotional Intelligence in AI](https://arxiv.org/abs/2512.23163)
*Max Parks,Kheli Atluru,Meera Vinod,Mike Kuniavsky,Jud Brewer,Sean White,Sarah Adler,Wendy Ju*

Main category: cs.AI

TL;DR: 本文指出当前AI系统情商（EI）评估框架需改进，因其未全面衡量AI相关EI维度，通过回顾情绪理论、评估现有基准框架不足，提出改进评估策略的方向。


<details>
  <summary>Details</summary>
Motivation: 现有AI系统情商评估框架存在不足，未能充分或全面衡量与AI相关的情商各方面；部分框架缺乏关于情绪本质及情商定义的坚实基础。

Method: 首先回顾不同情绪理论和通用情商理论，评估其对人工系统的适用性；然后批判性评估现有基准框架，结合前文提出的情商解释指出各框架的不足；最后概述改进评估策略的选项。

Result: 明确了现有AI情商评估框架的缺陷，包括未区分AI相关与无关的情商维度、缺乏理论基础等；提出了改进评估策略的方向。

Conclusion: 当前AI情商评估框架需要优化，应基于情绪理论和AI适用的情商维度来改进评估策略，以更准确地衡量AI系统的情商能力。
...
### [182] [SPIRAL: Symbolic LLM Planning via Grounded and Reflective Search](https://arxiv.org/abs/2512.23167)
*Yifan Zhang,Giridhar Ganapavarapu,Srideepika Jayaraman,Bhavna Agrawal,Dhaval Patel,Achille Fokoue*

Main category: cs.AI

TL;DR: 本文提出SPIRAL框架，将三个专业LLM智能体嵌入MCTS循环，解决LLM在复杂规划任务中的不足，在两个数据集上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在需要探索和自我修正的复杂规划任务中表现不佳，其线性推理过程难以从早期错误中恢复；而蒙特卡洛树搜索（MCTS）等搜索算法在稀疏奖励引导下效果有限，且未能利用LLMs丰富的语义能力。

Method: 提出SPIRAL框架，将三个专业LLM智能体（Planner、Simulator、Critic）嵌入MCTS循环：Planner提出创造性下一步，Simulator通过预测现实结果来锚定搜索，Critic通过反思提供密集奖励信号。

Result: 在DailyLifeAPIs和HuggingFace数据集上，SPIRAL持续优于默认的思维链规划方法和其他最先进智能体；例如，在DailyLifeAPIs上达到83.6%的整体准确率，比次优搜索框架提高16个百分点以上，同时具有更高的token效率。

Conclusion: 将LLM推理构建为引导式、反思式和锚定式的搜索过程，可产生更稳健高效的自主规划器；源代码、附录和实验数据已公开以确保可复现性。
...
### [183] [From Model Choice to Model Belief: Establishing a New Measure for LLM-Based Research](https://arxiv.org/abs/2512.23184)
*Hongshen Sun,Juanjuan Zhang*

Main category: cs.AI

TL;DR: 本文提出“模型信念”这一指标，利用大语言模型（LLMs）的token级概率分布提取更多信息，在模拟人类行为时比传统的“模型选择”更高效，可减少约20倍计算量。


<details>
  <summary>Details</summary>
Motivation: 现有LLM生成数据的使用方法效率低，仅将LLM输出（模型选择）视为单个数据点，未充分利用其概率性本质蕴含的信息。

Method: 引入并形式化“模型信念”指标，该指标基于LLM的token级概率分布，捕捉模型在单次生成中对选择选项的信念分布；通过需求估计研究验证其性能，模拟消费者对不同价格的反应。

Result: 1. 理论证明模型信念渐近等价于模型选择的均值，但方差更低、收敛速度更快；2. 对模型信念和模型选择的平滑函数也具有类似高效性；3. 在有限运行次数下，模型信念比模型选择本身更好地解释和预测真实模型选择；4. 将达到足够准确估计所需的计算量减少约20倍。

Conclusion: 模型信念应作为默认指标，用于从LLM生成数据中提取更多信息，提升效率和准确性。
...
### [184] [TCEval: Using Thermal Comfort to Assess Cognitive and Perceptual Abilities of AI](https://arxiv.org/abs/2512.23217)
*Jingming Li*

Main category: cs.AI

TL;DR: 提出TCEval框架评估AI的跨模态推理、因果关联和自适应决策能力，发现现有LLM缺乏对热舒适变量非线性关系的精确因果理解


<details>
  <summary>Details</summary>
Motivation: LLM任务特定基准存在关键缺口，热舒适作为环境因素与个人感知的复杂交互范式，适合评估AI的真实世界认知能力

Method: 构建TCEval框架，通过热舒适场景和LLM智能体评估三种核心认知能力；初始化智能体虚拟人格属性，生成服装隔热选择和热舒适反馈，并对照ASHRAE全球数据库与中国热舒适数据库验证输出

Result: 实验显示智能体反馈与人类的精确对齐有限，但1 PMV tolerance下方向一致性显著提升；LLM生成的PMV分布与人类数据差异明显，离散热舒适分类表现接近随机

Conclusion: TCEval作为生态有效认知图灵测试的可行性得到证实，现有LLM具备基础跨模态推理能力但缺乏对热舒适变量非线性关系的精确因果理解；该框架补充传统基准，将AI评估焦点转向具身、情境感知的感知与决策，为推进以人为中心的AI应用提供价值
...
### [185] [Agentic Physical AI toward a Domain-Specific Foundation Model for Nuclear Reactor Control](https://arxiv.org/abs/2512.23292)
*Yoonpyo Lee,Kazuma Kobayashi,Sai Puppala,Sajedul Talukder,Seid Koric,Souvik Chakraborty,Syed Bahauddin Alam*

Main category: cs.AI

TL;DR: 本文提出一种基于物理验证驱动策略优化的Agentic Physical AI领域特定基础模型，解决通用多模态模型在物理系统控制中存在的结构限制问题。


<details>
  <summary>Details</summary>
Motivation: 当前AI在物理系统领域的通用基础模型在控制接口面临根本障碍，前沿视觉语言模型在基础定量物理任务上仅达50-53%准确率，存在输入不忠实性，这是结构限制而非缩放缺陷；感知中心架构优化参数空间模仿，而安全关键控制需要执行动作的结果空间保证。

Method: 提出Agentic Physical AI，通过物理验证驱动策略优化而非感知推理；训练3.6亿参数模型于合成反应堆控制场景，数据集从10^3到10^5示例缩放。

Result: 模型出现通用模型中不存在的尖锐相变：小规模系统表现高方差模仿且有灾难性尾部风险，大规模模型方差崩溃超500倍减少，稳定执行级行为；尽管均衡接触四种驱动家族，模型自主拒绝约70%训练分布，95%运行时执行集中于单组策略；无需架构修改即可跨不同物理和连续输入模态迁移学习表示。

Conclusion: 该领域特定基础模型通过物理验证驱动策略优化，有效解决通用模型在物理系统控制中的结构限制，实现执行级行为稳定和跨模态迁移。
...
### [186] [On Conformant Planning and Model-Checking of $\exists^*\forall^*$ Hyperproperties](https://arxiv.org/abs/2512.23324)
*Raven Beutner,Bernd Finkbeiner*

Main category: cs.AI

TL;DR: 本文研究了规划与验证领域中一致性规划和超属性模型检查两个问题的关联，证明两者可相互归约


<details>
  <summary>Details</summary>
Motivation: 探索规划与验证领域中一致性规划和超属性模型检查这两个核心问题之间的内在联系

Method: 通过双向归约方法：一是将超属性模型检查实例高效归约为一致性规划实例并证明其可靠性和完备性；二是证明每个一致性规划问题本身就是超属性模型检查任务

Result: 成功建立了∃*∀*超属性模型检查与一致性规划之间的紧密关联，实现了两者的相互归约

Conclusion: 一致性规划和超属性模型检查两个问题存在密切联系，可相互转化
...
### [187] [CubeBench: Diagnosing Interactive, Long-Horizon Spatial Reasoning Under Partial Observations](https://arxiv.org/abs/2512.23328)
...
TL;DR: 本文针对大语言模型（LLM）智能体在物理世界部署中的空间心智模型构建问题，提出CubeBench基准及诊断框架，揭示其在长期规划等方面的关键局限并给出发展方向。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在数字领域表现出色，但因难以形成和维持稳健的空间心智模型，在物理世界部署存在显著差距；具体面临空间推理、基于心智模拟的长时程状态跟踪、部分观测下的主动探索三大认知挑战。

Method: 提出CubeBench这一以魔方为中心的生成式基准，采用三层诊断框架：从全符号信息下的基础状态跟踪，逐步评估到仅部分视觉数据下的主动探索能力；还提出诊断框架，通过提供外部求解工具来分离认知瓶颈。

Result: 对主流LLM的实验显示其存在关键局限，所有长时程任务的通过率均为0.00%，暴露了长期规划的根本性失败。

Conclusion: 通过分析失败模式，为开发更具物理基础的智能体提供了关键见解。
...
### [188] [MindWatcher: Toward Smarter Multimodal Tool-Integrated Reasoning](https://arxiv.org/abs/2512.23412)
...
TL;DR: 本文提出MindWatcher，一种集成交错思维和多模态思维链推理的工具集成推理（TIR）智能体，可自主调用工具解决跨领域多模态问题，并构建基准数据集评估其性能，实验表明其通过高效工具调用匹配或超越更大模型的表现。


<details>
  <summary>Details</summary>
Motivation: 传统基于工作流的智能体在处理需工具调用的现实问题时智能有限，而能自主推理和工具调用的TIR智能体是复杂决策任务的有力方法，因此需要更智能的TIR智能体来解决跨领域多模态问题。

Method: 提出MindWatcher智能体，采用交错思维范式（可在任意中间阶段切换思考与工具调用）和多模态思维链推理（推理中处理图像以获取精确搜索结果）；构建自动数据审计和评估 pipeline，辅以人工精选高质量训练数据集；创建MWE-Bench基准；配备全面辅助推理工具集；构建涵盖8类的小规模高质量本地图像检索数据库；设计更高效的训练基础设施。

Result: 实验表明MindWatcher通过卓越的工具调用能力，匹配或超越了更大或更新模型的性能；发现智能体训练中的关键洞察，如智能体强化学习中的遗传继承现象；其小规模本地图像检索数据库赋予模型强大的目标识别能力。

Conclusion: MindWatcher是一种有效的TIR智能体，能自主解决跨领域多模态问题，其设计和训练方法为智能体开发提供了有价值的参考，实验结果验证了其性能优势及训练中的关键发现。
...
### [189] [AKG kernel Agent: A Multi-Agent Framework for Cross-Platform Kernel Synthesis](https://arxiv.org/abs/2512.23424)
...
TL;DR: 本文提出AKG kernel agent多智能体系统，自动化AI计算核的生成、迁移和性能调优，支持多种DSL和硬件后端，在KernelBench测试中相比PyTorch Eager实现平均加速1.46倍


<details>
  <summary>Details</summary>
Motivation: 现代AI模型（如LLM、多模态架构）复杂度提升，结合稀疏化、量化等技术带来计算挑战；硬件更新频繁且架构多样，需针对各平台定制核实现，但手动优化无法满足需求，形成AI系统开发瓶颈

Method: 提出AKG kernel agent多智能体系统，支持Triton、TileLang、CPP、CUDA-C等多种DSL，模块化设计可快速集成新DSL和硬件目标，实现计算核的自动化生成、迁移和性能调优

Result: 在KernelBench上使用Triton DSL针对GPU和NPU后端测试，AKG kernel agent相比PyTorch Eager基线实现平均加速1.46倍

Conclusion: AKG kernel agent能有效加速现代AI工作负载的计算核开发，兼顾正确性和可移植性
...
### [190] [Replay Failures as Successes: Sample-Efficient Reinforcement Learning for Instruction Following](https://arxiv.org/abs/2512.23457)
*Kongcheng Zhang,Qi Yao,Shunyu Liu,Wenjian Zhang,Min Cen,Yang Zhou,Wenkai Fang,Yiru Zhao,Baisheng Lai,Mingli Song*

Main category: cs.AI

TL;DR: 本文提出一种样本高效的强化学习框架HiR，通过回顾重放策略解决复杂指令遵循任务中初始模型生成高质量响应困难的问题，在不同任务上取得良好结果且计算成本更低。


<details>
  <summary>Details</summary>
Motivation: 强化学习在大语言模型指令对齐中展现潜力，但初始模型因能力有限难以生成满足所有约束的高质量响应，导致奖励稀疏或难以区分，阻碍学习进程。

Method: 提出Hindsight instruction Replay（HiR）框架，采用选择后重写策略，基于回顾中已满足的约束将失败尝试重放为成功案例；同时在指令和响应层面进行双偏好学习，仅使用二元奖励信号实现高效优化。

Result: 在不同指令遵循任务上，HiR取得了有前景的结果，且所需计算预算更少。

Conclusion: HiR框架能有效解决复杂指令遵循任务中的奖励稀疏问题，提升样本效率和模型性能，降低计算成本，为大语言模型指令对齐提供了新的有效方法。
...
### [191] [Divergent-Convergent Thinking in Large Language Models for Creative Problem Generation](https://arxiv.org/abs/2512.23601)
*Manh Hung Nguyen,Adish Singla*

Main category: cs.AI

TL;DR: 本文提出CreativeDC双阶段提示方法，解决大语言模型生成教育问题时的同质化问题，提升多样性与新颖性同时保持实用性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型生成教育问题时存在"人工蜂群思维"效应，导致同一或不同模型输出同质化，学生接触重复问题损害思维多样性。

Method: 基于Wallas创造力理论和Guilford发散-收敛思维框架，提出CreativeDC双阶段提示方法，将创造性探索与约束满足解耦，先探索更广思路再生成最终问题。

Result: CreativeDC相比基线方法显著提升多样性和新颖性，同时保持高实用性；随着采样数量增加，生成的有效独特问题数量比基线方法增长更快。

Conclusion: CreativeDC有效解决大语言模型生成教育问题的同质化问题，在多样性、新颖性和实用性上表现优异，且扩展性更好。
...
### [192] [Regret-Based Federated Causal Discovery with Unknown Interventions](https://arxiv.org/abs/2512.23626)
*Federico Baldo,Charles K. Assaad*

Main category: cs.AI

TL;DR: 本文提出I-PERI算法，解决联邦环境下未知客户端干预的因果发现问题，通过利用客户端间干预差异优化因果图定向，得到更紧的Φ-Markov等价类并提供理论与实证支持。


<details>
  <summary>Details</summary>
Motivation: 现有联邦因果发现方法假设所有客户端共享相同因果模型，但实际中客户端（如医院）存在异质未知干预，导致假设不成立，需解决此问题。

Method: 提出I-PERI联邦算法：先恢复客户端图并集的CPDAG，再利用客户端间干预引起的结构差异定向更多边，得到Φ-CPDAG表示的Φ-Markov等价类。

Result: 理论上证明I-PERI的收敛性和隐私保护特性；实证上在合成数据上验证算法有效性。

Conclusion: I-PERI算法可有效处理联邦环境下未知客户端干预的因果发现，能生成更紧的等价类，具有理论保证和实证支持。
...
### [193] [Web World Models](https://arxiv.org/abs/2512.23676)
*Jichen Feng,Yifan Zhang,Chenggong Zhang,Yifu Lu,Shilong Liu,Mengdi Wang*

Main category: cs.AI

TL;DR: 本文提出Web世界模型（WWM），将传统Web框架的可靠固定上下文与生成式世界模型的无限环境结合，通过分离代码规则与模型想象力、用类型化Web接口表示潜在状态等设计原则，构建可控且开放式的环境。


<details>
  <summary>Details</summary>
Motivation: 语言智能体需要能行动、记忆和学习的持久世界，但现有方法要么是传统Web框架提供的可靠但固定的上下文，要么是完全生成式世界模型提供的无限环境但缺乏可控性和工程实用性，存在两者难以兼顾的问题。

Method: 提出Web世界模型（WWM），将世界状态和“物理规则”用普通Web代码实现以保证逻辑一致性，同时利用大语言模型在结构化潜在状态之上生成上下文、叙事和高层决策；构建基于真实Web栈的WWM套件，包括无限旅行地图集、虚构星系探索器等多种环境，并总结出实用设计原则：分离代码定义的规则与模型驱动的想象力、将潜在状态表示为类型化Web接口、利用确定性生成实现无限但结构化的探索。

Result: 通过构建多种WWM系统，验证了Web栈可作为世界模型的可扩展基础，能够支持可控且开放式的环境。

Conclusion: Web栈本身可作为世界模型的可扩展 substrate，使可控且开放式的环境成为可能，WWM的设计原则有效结合了传统Web框架与生成式模型的优势。
